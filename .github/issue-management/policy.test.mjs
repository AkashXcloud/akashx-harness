...
test('removes reserved labels from Issues before validation', async (t) => {
  ...
  assert.deepEqual(requests, [
    {
      url: 'https://api.github.com/repos/akx-harness/akx-harness/issues/42/labels/kind%2Fbug-fix',
      method: 'DELETE',
    },
    {
      url: 'https://api.github.com/repos/akx-harness/akx-harness/issues/42/labels/bug-fix',
      method: 'DELETE',
    },
  ])
})
...
test('deletes a stale audit comment after repairing its only violation', async (t) => {
  ...
  assert.deepEqual(
    ...
    [
      { path: '/repos/akx-harness/akx-harness/issues/42', method: 'GET' },
      { path: '/graphql', method: 'POST' },
      {
        path: '/repos/akx-harness/akx-harness/issues/42/labels/kind%2Fbug-fix',
        method: 'DELETE',
      },
      {
        path: '/repos/akx-harness/akx-harness/issues/42/comments?per_page=100',
        method: 'GET',
      },
      {
        path: '/repos/akx-harness/akx-harness/issues/comments/99',
        method: 'DELETE',
      },
    ],
  )
...
test('separates resolving and informational references', () => {
  assert.deepEqual(
    parseReferences({
      body: 'Fixes #12\nRelated to #4\nRefs akashxharness/akx-test#7',
      repository: 'akashxharness/akx-test',
    }),
...
test('reads Priority and Status from Project custom fields', async (t) => {
  ...
  assert.deepEqual(urls, [
    'https://api.github.com/repos/akx-harness/akx-harness/issues/42',
