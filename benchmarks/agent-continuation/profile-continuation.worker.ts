/** End-to-end SDK continuation through built akx sdk-minimal with an explicitly mounted file editor. */
...
async function run(root: string): Promise<ProfileReport> {
  ...
  await writeFile(patch, [
    ...
    '- insert:',
    '    - id: fs-local', "      name: '@akashx/akx-fs-local'",
    '    - id: str-replace-editor', "      name: '@akashx/akx-tool-str-replace-editor'",
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    AKX_AGENTS_HOME: join(home, 'agents'),
  }
  const harness = new AkashXHarness({
    akxBin: join(import.meta.dirname, '..', '..', '..', 'apps', 'cli', 'lib', 'bin.js'),
    profile: 'sdk-minimal', akxHome: home, processCwd: cwd, cwd,
...
assertBuiltBenchmarkRuntime(import.meta.url, { '@akashx/akx-sdk-client': import.meta.resolve('@akashx/akx-sdk-client') })
