...
def _host_platform_tag() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"unsupported akx-harness-runtime-bin build platform: {key}") from exc
...
class RuntimeBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, object]) -> None:
        ...
        if self.target_name == "sdist":
            raise RuntimeError(
                "akx-harness-runtime-bin is wheel-only; build and publish platform wheels only."
            )

        platform_tag = os.environ.get("AKX_RUNTIME_PLATFORM_TAG") or _host_platform_tag()
        ...
        if len(matches) != 1:
            ...
            raise RuntimeError(
                f"unsupported AKX_RUNTIME_PLATFORM_TAG {platform_tag!r}; expected one of {supported}"
            )
        expected_executable = matches[0][1]
        runtime_dir = Path(self.root) / "src" / "akx_harness_runtime" / "runtime"
        runtime_files = sorted(
            runtime_dir.glob("akx-harness-sdk-runtime-*") if runtime_dir.is_dir() else []
        )
        expected_files = (
