"""Drive the repo-source akx SDK profile through the SDK and a keyless mock SSE server.
...
def run_smoke(repo_root: Path, keep_sessions: bool) -> None:
    akx_home = Path(tempfile.mkdtemp(prefix="akx-sdk-smoke-home-"))
    session_root = akx_home / "sessions"
    ...
    if keep_sessions:
        print(f"kept_akx_home={akx_home}")
    else:
        shutil.rmtree(akx_home)
        print("removed temporary akx home")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to the akx-harness checkout.",
    )
