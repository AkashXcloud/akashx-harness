...
from akx_harness import AkashXHarness, HarnessClient, HarnessConfig, Notification, RunResult, SdkProtocolError
from akx_harness.errors import JsonRpcError
...
json.dump({
    "AKASHX_API_KEY": os.environ.get("AKASHX_API_KEY"),
    "AKASHX_BASE_URL": os.environ.get("AKASHX_BASE_URL"),
    "AKX_CWD": os.environ.get("AKX_CWD"),
    "AKX_SESSION_ROOT": os.environ.get("AKX_SESSION_ROOT"),
    "AKX_CORDIS_CONFIG": os.environ.get("AKX_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    with AkashXHarness(
        ...
        env={
            ...
            "AKASHX_API_KEY": "env-key",
            "AKASHX_BASE_URL": "http://127.0.0.1:4321",
        },
    ) as harness:
    ...
    assert dumped_env["AKASHX_API_KEY"] == "env-key"
    assert dumped_env["AKASHX_BASE_URL"] == "http://127.0.0.1:4321"
    assert dumped_env["AKX_CWD"] is None
    assert dumped_env["AKX_SESSION_ROOT"] is None
    assert dumped_env["AKX_CORDIS_CONFIG"] is None
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("AKX_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
""".strip()
    ...
    with AkashXHarness(_launch_args=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
for line in sys.stdin:
    ...
    method = msg.get("method")
    if method == "initialize":
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-akx"}}}), flush=True)
    elif method == "session/prompt":
        params = msg.get("params") or {}
...
""".strip()
    ...
    with HarnessClient(_launch_args=(sys.executable, str(script))) as client:
        init = client.initialize(provider="akashx-official", cwd="/workspace", model="dsagent")
        assert init.serverInfo.name == "fake-akx"
...
""".strip()
    ...
    with HarnessClient(_launch_args=(sys.executable, str(script))) as client:
        client.initialize(provider="akashx-official", cwd="/workspace", model="dsagent")
        with (
...
""".strip()
    ...
    with pytest.raises(JsonRpcError, match="bad initialize") as excinfo:
        client.initialize(provider="akashx-official", cwd=".", model="dsagent")
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from akx_harness import AkashXHarnessConfig, Session
    ...
    assert "profile" not in inspect.signature(AkashXHarness.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in AkashXHarnessConfig.__dataclass_fields__
    assert "max_tokens" in AkashXHarnessConfig.__dataclass_fields__
    assert "reasoning_effort" in AkashXHarnessConfig.__dataclass_fields__
    ...
    assert {"akx_bin", "profile", "patches", "akx_home"} <= set(
        AkashXHarnessConfig.__dataclass_fields__
    )
    assert {"akx_bin", "profile", "patches", "akx_home"} <= set(
        HarnessConfig.__dataclass_fields__
    )
    assert "initialize_timeout_seconds" in AkashXHarnessConfig.__dataclass_fields__
    assert "initialize_timeout_seconds" in HarnessConfig.__dataclass_fields__
    assert AkashXHarnessConfig().initialize_timeout_seconds == 30.0
    ...
    for removed in ("cordis", "session_root", "runtime_bin", "bridge_bin", "launch_args_override"):
        assert removed not in AkashXHarnessConfig.__dataclass_fields__
...
def _install_fake_bundled_akx(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Install a fake runtime package that records akx argv and serves lifecycle calls."""
    runtime = tmp_path / "akx.py"
    runtime.write_text(
        """
import json
import os
...
json.dump({
    "argv": sys.argv[1:],
    "AKX_HOME": os.environ.get("AKX_HOME"),
    "AKX_CORDIS_CONFIG": os.environ.get("AKX_CORDIS_CONFIG"),
...
""".strip()
    ...
    module_dir = tmp_path / "akx_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "akx_harness_runtime", raising=False)


def test_client_default_launch_uses_bundled_akx_sdk_profile_and_explicit_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    env_dump = tmp_path / "env.json"
    home = tmp_path / "home"
    ...
    patch.write_text("[]\n")
    _install_fake_bundled_akx(tmp_path, monkeypatch)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("AKX_HOME", str(tmp_path / "ambient-home"))
    monkeypatch.delenv("AKX_CORDIS_CONFIG", raising=False)

    with HarnessClient(HarnessConfig(
        profile="sdk",
        patches=("sdk.patch.yml",),
        akx_home=str(home),
        env={"ENV_DUMP": str(env_dump), "AKX_HOME": str(tmp_path / "env-home")},
    )) as client:
        init = client.initialize(provider="akashx-official", cwd="/workspace", model="akashx-v4-pro")

    assert init.serverInfo.name == "bundled-runtime"
    assert json.loads(env_dump.read_text()) == {
        "argv": ["--profile", "sdk", "--patch", str(patch)],
        "AKX_HOME": str(home),
        "AKX_CORDIS_CONFIG": None,
    }


def test_client_accepts_explicit_environment_akx_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    env_dump = tmp_path / "env.json"
    ...
    with HarnessClient(
        HarnessConfig(profile="custom", env={"ENV_DUMP": str(env_dump), "AKX_HOME": str(home)})
    ) as client:
        client.initialize(provider="akashx-official", cwd="/workspace", model="akashx-v4-pro")
...
def test_client_rejects_an_implicit_default_akx_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _install_fake_bundled_akx(tmp_path, monkeypatch)
    monkeypatch.delenv("AKX_HOME", raising=False)

    with pytest.raises(ValueError, match="explicit akx_home or non-empty AKX_HOME"):
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install akx-harness-runtime-bin"):
        HarnessClient(HarnessConfig(akx_home="/explicit/home")).start()
