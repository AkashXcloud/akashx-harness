"""Keyless boot tests for the production exe and development akx carrier.
...
from akx_harness import AkashXHarness, HarnessClient, HarnessConfig
from akx_harness.errors import JsonRpcError, TransportClosedError
from akx_harness_runtime import RUNTIME_MODE_ENV_VAR, resolve_bundled_launch_args
...
def test_bundled_runtime_boots_the_sdk_profile(
    ...
    with _client(tmp_path, mode, monkeypatch) as client:
        init = client.initialize(provider="akashx-official", cwd=str(tmp_path), model="akashx-v4-pro")

    assert init.serverInfo is not None
    assert init.serverInfo.name == "akx-harness-sdk-runtime"
    profile = json.loads((tmp_path / "home" / "profiles" / "sdk" / "package.json").read_text())
    assert profile["akx"]["profile"]["bundles"] == [
        "@akashx/akx-base",
        "@akashx/akx-sdk-app",
    ]
...
def test_python_sdk_applies_an_ordered_profile_patch(
    ...
    harness = AkashXHarness(
        ...
        akx_home=str(tmp_path / "home"),
        patches=(str(patch),),
        env={"AKX_PERMISSION_MODE": "danger-full-access"},
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(
    ...
    patch.write_text(json.dumps([{
        "insert": [{"id": "missing", "name": "@akashx/akx-does-not-exist"}],
    }]))
    ...
    try:
        with pytest.raises((JsonRpcError, TransportClosedError, TimeoutError)) as excinfo:
            client.initialize(provider="akashx-official", cwd=str(tmp_path), model="akashx-v4-pro")
    finally:
        client.close()

    assert "@akashx/akx-does-not-exist" in str(excinfo.value)
