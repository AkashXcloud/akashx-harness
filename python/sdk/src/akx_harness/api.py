...
class AkashXHarnessConfig:
    ...
    AKASHX_API_KEY and AKASHX_BASE_URL settings keep working. Use ``env`` to
    intentionally override or inject variables for a subprocess.
    """

    provider: str = "akashx-official"
    model: str = "akashx-v4-flash"
...
class AkashXHarness:
    """Reusable synchronous SDK for running AkashX Harness agent turns.
    ...
    def __init__(
        self,
        config: AkashXHarnessConfig | None = None,
        *,
        _launch_args: tuple[str, ...] | None = None,
        **kwargs: object,
    ) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either AkashXHarnessConfig or keyword options, not both")
        self.config = config or AkashXHarnessConfig(**kwargs)
        ...
        if self.config.base_url is not None:
            env["AKASHX_BASE_URL"] = self.config.base_url
        if self.config.api_key is not None:
            env["AKASHX_API_KEY"] = self.config.api_key

        self._client = HarnessClient(
            HarnessConfig(
                akx_bin=self.config.akx_bin,
                profile=self.config.profile,
                patches=self.config.patches,
                akx_home=self.config.akx_home,
                cwd=runtime_cwd,
                env=env,
    ...
    def __enter__(self) -> "AkashXHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: AkashXHarness, session_id: str) -> None:
