 ...
 * the real `akx --profile sdk` runtime through
 * `@akashx/akx-sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `AKX_SNAPSHOT=record`
 * re-records against the live API; `AKX_SNAPSHOT=refresh` replays committed
...
const mode = process.env.AKX_SNAPSHOT ?? 'replay'
...
const RUNTIME_WORKSPACE_ENTRIES = [
  ...
  '.child-akx',
  '.akx',
  '.akx-sdk-background-release',
  '.replay-fixtures',
  '.snapshot-patches',
] as const
const akxSdkDiagnosticChildPatch = fileURLToPath(new URL(
  './subagent-akx-sdk-diagnostic/child.cordis.yml',
  import.meta.url,
))
const akxSdkChildConfig = fileURLToPath(new URL(
  '../../packages/subagent/subagent-akx-sdk/tests/fixtures/loader/child.patch.yml',
  import.meta.url,
))
...
interface SdkAssertions {
  ...
  /** A separate AKX SDK child whose persisted session joins the evidence. */
  akxSdkChild?: {
...
const SDK_ASSERTIONS: Readonly<Record<string, SdkAssertions>> = {
  ...
  'subagent-continuable': {
    environment: { AKX_SNAPSHOT_HUMAN_STEER: '1' },
  },
  'subagent-akx-sdk-diagnostic': {
    environment: { AKX_TEST_CHILD_PATCH: akxSdkDiagnosticChildPatch },
  },
  'persistent-tools': {
    environment: { AKX_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
    ...
    runtimeContext: {
      includes: ['Current AKX file policy: danger-full-access', 'Approval prompts are disabled in this session'],
      excludes: ['workspace-write'],
    },
  },
  'subagent-akx-sdk-dynamic-route': {
    environment: { AKX_TEST_PARENT_PROVIDER: 'akashx-official' },
    akxSdkChild: {
      config: akxSdkChildConfig,
      agentConfig: {
        provider: 'mock',
        model: 'mock-routed',
        reasoningEffort: 'max',
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@akashx/akx-system-prompt') return []
...
/** One SDK-controlled recorded scenario against a fresh `akx --profile sdk` subprocess. */
async function runScenario(scenario: CorpusScenario): Promise<{
  ...
  const akxHome = join(cwd, '.akx')
  const sessionsRoot = join(akxHome, 'sessions')
  ...
  if (assertions.akxSdkChild !== undefined) {
    const childHome = join(cwd, '.child-akx')
    const childPatch = materializeProfilePatch(assertions.akxSdkChild.config, cwd, patchRoot, patches.length)
    await mkdir(childHome, { recursive: true })
    childSessionsRoot = join(childHome, 'sessions')
    childEnvironment = {
      AKX_TEST_CHILD_PATCHES: JSON.stringify([childPatch]),
      AKX_TEST_CHILD_HOME: childHome,
    }
  }
  ...
  const env: Record<string, string> = {
    ...
    AKX_SNAPSHOT: mode,
    AKX_SNAPSHOT_PROVIDER: route.provider,
    AKX_SNAPSHOT_MODEL: route.model,
    AKX_TELEMETRY_DISABLED: '1',
    AKX_AGENTS_HOME: join(cwd, '.agents'),
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      AKX_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { AKX_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
    ...!recording && scenario.manifest.replay?.override === true
      ? { AKX_SNAPSHOT_OVERRIDE: join(scenario.dir, 'replay.override.json') }
      : {},
  ...
  const harness = new AkashXHarness({
    profile: 'sdk',
    patches,
    akxHome,
    processCwd: cwd,
  ...
  try {
    ...
    try {
      ...
      for (const action of turnActions(primaryFixture)) {
        ...
        results.push(result)
        if (scenario.manifest.environment?.AKX_SNAPSHOT_FEEDBACK === '1') {
...
function orderLogs(logs: PersistedLog[], expectedCount: number, separateAkxSdkChild: boolean): PersistedLog[] {
  if (separateAkxSdkChild) {
...
async function verifyHeaders(
  ...
  ordered: readonly PersistedLog[],
  ctx: NormalizeContext,
  akxSdkChildConfig?: Readonly<Record<string, unknown>>,
): Promise<void> {
  ...
  for (const [logIndex, log] of ordered.entries()) {
    ...
    for (const [index, header] of headers.entries()) {
      ...
      const configured = logIndex === 1 && akxSdkChildConfig !== undefined
        ? { ...base as JsonObject, config: akxSdkChildConfig }
        : base
...
describe('TypeScript SDK snapshots over the jsonrpc runtime', () => {
  for (const scenario of sdkScenarios) {
    const scenarioTest = recording
      ...
      ? it.skip
      : it
    scenarioTest(`${mode}s ${scenario.name} through akx --profile sdk`, async () => {
      ...
      const ordered = orderLogs(
        ...
        assertions.akxSdkChild !== undefined,
      )
      ...
      await verifyHeaders(scenario, ordered, actualContext, assertions.akxSdkChild?.agentConfig)
      ...
      if (ordered.length > 1 && assertions.akxSdkChild === undefined) {
