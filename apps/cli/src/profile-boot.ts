/**
 * Shared profile boot for every `akx` surface: resolve the profile, stack its
 * patch layers (bundle layers in `akx.profile.bundles` order, the profile's
 ...
 * @module @akashx/akx/profile-boot
 */

import { existsSync, mkdirSync, rmSync, writeFileSync } from 'node:fs'
import { dirname, join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'
import { FiberState, type Context } from '@akashx/cordis'
...
import { provideCmdline, type AppReady } from '@akashx/akx-cmdline'
...
/**
 * The home-level user patch layer (`$AKX_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$AKX_HOME` may be set by the test or launcher after import.
 * @returns the absolute patch-file path.
 */
export function homePatchPath(): string {
  return join(resolveAkxHome(), PROFILE_PATCH_FILENAME)
}

/** Absolute path of this akx installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the AKX_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# akx profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's akx.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `AKX_TELEMETRY_DISABLED` value (`undefined` when unset).
...
export function prepareProfile(name: string, userLayer = true, fromDefaultProfile?: string, binName = NAME): Profile {
  if (fromDefaultProfile !== undefined) initializeProfileFromDefault(name, fromDefaultProfile, resolveAkxHome(), binName)
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$AKX_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `akx.profile.bundles` order (a base-backed profile gets the base bundle's
 * platform-gated shell rows), the profile's user layer, the home-level user
 * layer (`$AKX_HOME/cordis.patch.yml` — machine-local preferences that apply
...
async function composeProfile(
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.AKX_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(binName, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(AKX_LAUNCH_ENVIRONMENT_KEY, options.environment)
  ...
  if (composed.profile.patchReload === 'live'
    ...
    try {
      // Config-only HMR for the live profile patch layer: akx-base disables
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@akashx/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@akashx/cordis-plugin-hmr', config: { root: [] } })
      }
