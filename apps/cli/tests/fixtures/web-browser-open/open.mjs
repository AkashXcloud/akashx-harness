...
export default async function open(url) {
  ...
  console.log(`akx browser-open: ${JSON.stringify({
    url,
    status: response.status,
    bootManifest: html.includes('__AKX_BOOT__'),
    apiKeyPresent: process.env.AKASHX_API_KEY !== undefined,
    akxHomePresent: process.env.AKX_HOME !== undefined,
  })}`)
  ...
  const launcher = spawn(process.execPath, [
    '--eval', handoffProbe,
    '--', join(process.cwd(), `.akx-browser-open-${process.ppid}`), String(process.pid),
  ], { stdio: 'ignore' })
  launcher.unref()
  return launcher
}
