 ...
 * the REAL shipped bundle layers (akx-base + akx-web-app resolved from the
...
import { evaluate } from '@akashx/cordis-plugin-loader'
import { SHIPPED_PRESET_ROOT } from '@akashx/akx-agent-presets'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@akashx/akx-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL akx-base/akx-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'akx-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@akashx/akx-base', '@akashx/akx-web-app'])
    const profile = loadProfile('akx', 'web', anchor, home)
    ...
    for (const name of ['@akashx/akx-pwsh-sandbox', '@akashx/akx-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'akx-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@akashx/akx-base'])
    const profile = loadProfile('akx', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
...
describe('shipped agent presets gate both shell tools by platform', () => {
  ...
  it.each(['standard', 'ptc', '@akashx/cordis'])('preset %s gates its shell tool rows by platform', (preset) => {
