...
import { parseAkxArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseAkxArgs(argv, '1.2.3')
...
describe('parseAkxArgs', () => {
  ...
  it('accepts the AkashX launcher name for the same profile grammar', () => {
    expect(parseAkxArgs(['--profile', 'headless', 'run', 'the', 'tests'], '1.2.3', 'akashx'))
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@akashx/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@akashx/cordis'] })
