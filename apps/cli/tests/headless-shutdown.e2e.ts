...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@akashx/akx-loader-smoke'
...
markers = [b"akx-test: never-dispose ready", b"akx-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'akx-headless-shutdown-'))
  try {
    const home = join(cwd, '.akx')
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      name: 'akx-profile-headless',
      private: true,
      dependencies: {},
      akx: { profile: { bundles: ['@akashx/akx-base', '@akashx/akx-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        AKASHX_API_KEY: 'keyless-shutdown-no-call',
        AKX_TELEMETRY_DISABLED: '1',
        AKX_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`akx headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`akx headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('akx: observing at ')
    expect(output).toContain('akx-test: never-dispose ready')
    expect(output).toContain('akx-test: never-dispose started')
