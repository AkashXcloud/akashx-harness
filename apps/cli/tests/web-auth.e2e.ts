/** Real `akx web` authentication against a temporary Harness home. */
...
const AKX_SOURCE_BIN = join(REPO_ROOT, 'apps/cli/src/bin.ts')
...
function cleanEnvironment(root: string, akxHome: string): NodeJS.ProcessEnv {
...
async function startWeb(root: string, akxHome: string, port: number): Promise<RunningWeb> {
  const child = spawn(process.execPath, [
    ...
    cwd: root,
    env: cleanEnvironment(root, akxHome),
  ...
  const launchUrl = await new Promise<string>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      fail(new Error(`akx web did not become ready:\n${redact(output)}`))
    }, 90_000)
    ...
    child.once('exit', (code) => {
      fail(new Error(`akx web exited before readiness (${String(code)}):\n${redact(output)}`))
    })
  })
...
describe('akx web authentication through the real CLI', () => {
  it('rejects a forged loopback Host and preserves the browser cookie across restart', { timeout: 180_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'akx-web-auth-real-cli-'))
    const akxHome = join(root, '.akx')
    const port = await freePort()
    let first: RunningWeb | undefined
    let second: RunningWeb | undefined
    try {
      first = await startWeb(root, akxHome, port)
      ...
      await stopWeb(first)
      first = undefined
      second = await startWeb(root, akxHome, port)
      ...
      const credentialMode = (await stat(join(akxHome, '.credentials.yaml'))).mode & 0o777
