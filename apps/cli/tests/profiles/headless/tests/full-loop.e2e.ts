...
describe.skipIf(!process.env.AKASHX_API_KEY)('full loop: real model + real bash tool', () => {
  it('runs a bash command on request and reports its output', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'akx-full-loop-e2e-'))
    ctx = await codingHarness(workdir, { personaPrefix: SYSTEM_PROMPT })
    const agent = await ctx.agentLoop.create(SessionId('e2e-loop'), { provider: 'akashx-official', model: 'akashx-v4-flash' })
