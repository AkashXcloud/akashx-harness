...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, runLoaderSmoke } from '@akashx/akx-loader-smoke'
import {
  decompressZstdFrame,
  scanZstdFrames,
} from '@akashx/akx-session-persistence-jsonl/src/zstd.ts'
...
const akxBinScript = fileURLToPath(new URL('../../../../src/bin.ts', import.meta.url))
const tsconfigPath = fileURLToPath(new URL('../../../../../../tsconfig.json', import.meta.url))
const reasoningConfigPath = fileURLToPath(new URL('./fixtures/cli.patch.yml', import.meta.url))
const akashxDefaultsConfigPath = fileURLToPath(new URL('./fixtures/akashx-defaults.patch.yml', import.meta.url))
...
const refreshing = process.env.AKX_SNAPSHOT === 'refresh'
...
interface AkashXDefaultsServer {
  readonly url: string
...
/** Serve one deterministic AkashX-compatible response while retaining its request body. */
async function akashxDefaultsServer(options: { waitForTitleRequest?: boolean } = {}): Promise<AkashXDefaultsServer> {
  ...
  if (address === null || typeof address === 'string') throw new Error('AkashX defaults snapshot server has no port')
  return {
...
describe('headless stream-json snapshots', () => {
  ...
  it('runs one task through the product headless profile command', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      inspect: async (cwd) => {
        const logs = await persistedLogs(cwd, join(cwd, '.akx', 'sessions'))
  ...
  it('logs actionable missing-credential guidance through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tsconfigPath,
      env: {
        // First-run posture: no key in the environment, none under ./.akx.
        AKASHX_API_KEY: '',
        AKASHX_BASE_URL: '',
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
    ...
    expect(normalized).toContain(
      'store AKASHX_API_KEY through the credentials service (the web Models page writes it),',
    )
    expect(normalized).toContain('or export AKASHX_API_KEY in the launching environment')
  ...
  it('logs actionable invalid-credential guidance through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        AKASHX_API_KEY: 'sk-\u{1F600}pasted-from-a-chat-window',
        AKASHX_BASE_URL: '',
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
    ...
    // page at all.
    expect(normalized).toContain('the API key resolved from AKASHX_API_KEY contains characters')
  ...
  it('keeps provider comments alive and sends AkashX defaults through the one-shot app', async () => {
    const server = await akashxDefaultsServer()
    try {
      const result = await runLoaderSmoke({
        label: 'AkashX adapter defaults headless stream-json snapshot',
        tempDirPrefix: 'headless-snapshot-akashx-defaults-',
        binScript,
        libBinScript: binScript,
        configPath: akashxDefaultsConfigPath,
        binArgs: [
          akashxDefaultsConfigPath,
        ...
        env: {
          ...
          AKASHX_API_KEY: 'snapshot-key',
          AKX_SNAPSHOT_BASE_URL: server.url,
          NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
        },
      })
  ...
  it('keeps the compatibility stream open until the title request arrives', async () => {
    const server = await akashxDefaultsServer({ waitForTitleRequest: true })
    try {
  ...
  it('sends pi-ai AkashX compatibility through the one-shot app', async () => {
    const server = await akashxDefaultsServer({ waitForTitleRequest: true })
    try {
      const result = await runLoaderSmoke({
        label: 'pi-ai AkashX compatibility headless stream-json snapshot',
  ...
  it('replays persisted goal tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        AKX_SNAPSHOT: 'replay',
        AKX_SNAPSHOT_FILE: join(goalScenarioDir, 'session.jsonl'),
        AKX_SNAPSHOT_OVERRIDE: join(goalScenarioDir, 'replay.override.json'),
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('delivers a continuable child result without parent polling', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        AKX_SNAPSHOT_FILE: parentReplay,
        AKX_SNAPSHOT_OVERRIDE: parentOverride,
        AKX_SNAPSHOT_CHILD_FILES: childReplay,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
