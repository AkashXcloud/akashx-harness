...
describe.skipIf(!process.env.AKASHX_API_KEY)('coding task: fix a failing test via bash', () => {
  it('repairs add.js so node add.test.js passes', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'akx-coding-task-'))
    ...
    const agent = await ctx.agentLoop.create(SessionId('e2e-task'), { provider: 'akashx-official', model: 'akashx-v4-flash' })
