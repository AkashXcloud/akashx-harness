...
describe.skipIf(!process.env.AKASHX_API_KEY)('todo_write: real model records a plan', () => {
  it('appends a todo/write event with the model-produced task list', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'akx-todo-write-e2e-'))
    ctx = await codingHarness(workdir, { personaPrefix: TODO_SYSTEM_PROMPT })
    const agent = await ctx.agentLoop.create(SessionId('e2e-todo'), { provider: 'akashx-official', model: 'akashx-v4-flash' })
