...
import { createMessage, createUserMessage } from '@akashx/akx-llm'
import {
...
import { renderWorkspaceContext } from '@akashx/akx-agent-instructions'
import { resolveConfig, workspaceBaselineIdentity } from '@akashx/akx-agent-instructions/src/config.ts'
...
describe('agent-instructions resume snapshot', () => {
  it('appends an offline replacement without duplicating the visible baseline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'akx-workspace-context-resume-',
      binScript,
      ...
      tsconfigPath,
      env: {
        AKX_SNAPSHOT_FILE: replayFixture,
  ...
  it('supersedes an incompatible baseline when precedence changed offline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'akx-workspace-context-precedence-',
      binScript,
      libBinScript: binScript,
      configPath,
      binArgs: [configPath, 'Acknowledge the current workspace instruction.'],
