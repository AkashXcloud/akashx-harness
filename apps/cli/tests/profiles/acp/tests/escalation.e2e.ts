...
import { bwrapProfileArgs } from '@akashx/akx-sandbox-local/src/profiles.ts'
import { cleanupAcpExampleTest } from './cleanup.ts'

/**
 * The default ACP composition (`cordis.yml`) end to end.
 *
 * Keyless smoke: boot the real profile patch through `akx --profile acp` as
 ...
 *
 * With-key escalation flow (self-skips without AKASHX_API_KEY or a usable
...
function launchExampleAcpAgent(
  ...
  const launched = launchAcpTestAgent({
    ...
    env: {
      AKASHX_API_KEY: process.env.AKASHX_API_KEY ?? 'sk-dummy-for-boot',
      AKX_PERMISSION_MODE: sandboxMode,
    },
...
describe.skipIf(!process.env.AKASHX_API_KEY || !hasRunner)('default sandbox composition e2e: the live approval loop', () => {
