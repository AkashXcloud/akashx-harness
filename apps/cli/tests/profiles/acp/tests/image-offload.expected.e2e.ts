...
it('pins native AkashX Files offload and inline fallback in assembled requests', async () => {
  ...
  try {
    const result = await runScenario(input, {
      ...
      env: {
        AKX_SNAPSHOT_API_KEY: 'snapshot-key',
        AKX_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })
    ...
    const fallback = await runScenario(input, {
      ...
      workspaceDir: READ_IMAGE_WORKSPACE,
      env: {
        AKX_SNAPSHOT_API_KEY: 'snapshot-fallback-key',
        AKX_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })
