...
describe('Python SDK akx profile keyless smoke', () => {
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), 'akx-python-sdk-runtime-smoke-'))
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        ...
        AKASHX_API_KEY: 'keyless-smoke-no-call',
        AKASHX_BASE_URL: `http://127.0.0.1:${address.port}`,
        ...(envValue === undefined ? {} : { AKX_MAX_TOKENS_AS_SUCCESS: envValue }),
      },
      timeout: 35_000,
      killSignal: 'SIGKILL',
      reject: false,
    ...
    try {
      ...
      const sessionsRoot = join(root, '.akx', 'sessions')
  ...
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), 'akx-python-sdk-minimal-'))
    const editorPatch = join(root, 'editor.patch.yml')
    if (editorEnabled) {
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        AKX_HOME: join(root, '.akx'),
        AKX_SYSTEM_PROMPT: 'Minimal allowlist prompt.',
        AKASHX_API_KEY: 'keyless-smoke-no-call',
        AKASHX_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
      timeout: 35_000,
    ...
    try {
      child.stdin.write(`${JSON.stringify({
        ...
        id: 1,
        method: 'initialize',
        params: { cwd: root, provider: 'akashx-official', model: 'akashx-v4-pro' },
      })}\n`)
      ...
      const profile = JSON.parse(
        await readFile(join(root, '.akx', 'profiles', 'sdk-minimal', 'package.json'), 'utf8'),
      ) as { akx?: { profile?: { bundles?: string[]; patchReload?: string } } }
      expect(profile.akx?.profile).toEqual({
  ...
  it('rejects an invalid max-token success env value', async () => {
    const root = await mkdtemp(join(tmpdir(), 'akx-python-sdk-runtime-invalid-'))
    try {
      const { exitCode, stdout, stderr } = await execa(process.execPath, [
        ...
        env: {
          AKX_HOME: join(root, '.akx'),
          AKASHX_API_KEY: 'keyless-smoke-no-call',
          AKX_MAX_TOKENS_AS_SUCCESS: 'sometimes',
        },
        stdin: 'ignore',
        timeout: 25_000,
        killSignal: 'SIGKILL',
      ...
      expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (@akashx/akx-sdk-jsonrpc-server)')
