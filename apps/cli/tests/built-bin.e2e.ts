...
import { startMockLlmServer } from '@akashx/akx-llm-mock-server'
...
// The akx built bin cold-starts slowly on the contended self-hosted Windows pool; the
...
const akxBin = join(repoRoot, 'apps/cli/lib/bin.js')
...
async function runBuiltBin(
  ...
  const result = await execa(process.execPath, [akxBin, ...args], {
    input: '',
  ...
  if (result.timedOut) {
    throw new Error(`akx built bin did not exit within ${SPAWN_TIMEOUT_MS / 1_000}s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
  }
...
async function waitForFile(file: string): Promise<void> {
  ...
  while (!existsSync(file)) {
    if (Date.now() >= deadline) throw new Error(`akx profile lifecycle marker did not appear: ${file}`)
 ...
 * akx.profile.bundles, no akx-base — proving out-of-box composition machinery without
 * booting the entire product tree.
 */
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  const home = mkdtempSync(join(tmpdir(), 'akx-profile-lifecycle-'))
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.AKX_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'akx-profile-lifecycle',
    private: true,
    dependencies: {},
    akx: { profile: { bundles: ['akx-lifecycle-bundle'] } },
  }, undefined, 2))
  // Hand-place the "installed" bundle where profile resolution finds it.
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  const linkTarget = join(profileDir, 'node_modules', 'akx-lifecycle-bundle')
  mkdirSync(join(profileDir, 'node_modules'), { recursive: true })
  try {
...
function startProfileLifecycle(fixture: ProfileLifecycleFixture, args: readonly string[] = []) {
  return execa(process.execPath, [akxBin, '--profile', 'lifecycle', ...args], {
    cwd: fixture.home,
    input: '',
    timeout: SPAWN_TIMEOUT_MS,
    killSignal: 'SIGKILL',
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'akx-profile-environment-probe',
    private: true,
    dependencies: {},
    akx: { profile: { bundles: ['@akashx/akx-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@akashx/akx-cmdline` and `commander` through the profile module
 * fallback, exactly as an installed out-of-tree bundle does.
 */
function createStartupFixture(): StartupFixture {
  const home = mkdtempSync(join(tmpdir(), 'akx-profile-startup-'))
  ...
  const bundleDir = join(profileDir, 'node_modules', 'akx-startup-bundle')
  mkdirSync(bundleDir, { recursive: true })
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.AKX_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
  ...
  writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
    name: 'akx-startup-bundle',
    version: '0.0.0',
    type: 'module',
    akx: { bundle: { patch: './cordis.patch.yml' } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'akx-profile-startup',
    private: true,
    dependencies: {},
    akx: { profile: { bundles: ['akx-startup-bundle'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  return {
    home,
...
function startStartupProfile(fixture: StartupFixture, args: readonly string[]) {
  return execa(process.execPath, [akxBin, '--profile', 'startup', ...args], {
    cwd: fixture.home,
    input: '',
    reject: false,
    timeout: SPAWN_TIMEOUT_MS,
...
describe.skipIf(!existsSync(akxBin))('akx BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('akx --profile web')
    expect(help.stdout).toContain('akx plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    const home = mkdtempSync(join(tmpdir(), 'akx-app-help-'))
    try {
      const web = await runBuiltBin(['--profile', 'web', '--help'], {
        AKX_HOME: home,
        AKX_TELEMETRY_DISABLED: '1',
      })
      expect(web.code).toBe(0)
      expect(web.stderr).toBe('')
      expect(web.stdout).toContain('Usage: akx --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('akx web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('akx web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: akx --profile headless')
      ...
      expect(sdkHelp.stdout).toContain('Usage: akx --profile sdk')
      ...
      expect(acpHelp.stdout).toContain('Usage: akx --profile acp')
  ...
  it('reports SDK startup failure when stdin reaches EOF first', async () => {
    const home = mkdtempSync(join(tmpdir(), 'akx-built-sdk-startup-failure-'))
    const patch = join(home, 'broken-sdk.cordis.yml')
    writeFileSync(patch, [
      '- insert:',
      '    - id: missing-sdk-startup-plugin',
      '      name: "@akashx/akx-missing-sdk-startup-plugin"',
      '',
    ].join('\n'))
    try {
      const result = await runBuiltBin(['--profile', 'sdk', '--patch', patch], {
        AKX_HOME: home,
        AKX_TELEMETRY_DISABLED: '1',
        AKASHX_API_KEY: 'built-sdk-startup-failure-no-call',
      }, home)
      expect(result.code).toBe(1)
      expect(result.stdout).toBe('')
      expect(result.stderr).toContain('plugin tree failed to load')
      expect(result.stderr).toContain('@akashx/akx-missing-sdk-startup-plugin')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  ...
  it('serves the SDK protocol with an absolute-path overlay plugin and exits after shutdown', async () => {
    const home = mkdtempSync(join(tmpdir(), 'akx-built-sdk-'))
    ...
    const child = execa(process.execPath, [akxBin, '--profile', 'sdk', '--patch', patch], {
      cwd: home,
      reject: false,
      timeout: SPAWN_TIMEOUT_MS,
      killSignal: 'SIGKILL',
      env: {
        ...
        AKX_HOME: home,
        AKX_TELEMETRY_DISABLED: '1',
        AKASHX_API_KEY: 'built-sdk-profile-no-call',
      },
      extendEnv: false,
    })
    ...
    try {
      child.stdin.write(`${JSON.stringify({
        ...
        id: 1,
        method: 'initialize',
        params: { cwd: home, provider: 'akashx-official', model: 'akashx-v4-flash' },
      })}\n`)
      const initialized = await response(1)
      expect(initialized, `${JSON.stringify(initialized)}\n${stderr}`).toMatchObject({
        jsonrpc: '2.0',
        id: 1,
        result: { serverInfo: { name: 'akx-harness-sdk-runtime' } },
      })
  ...
  it('runs a mock-backed ACP turn through the acp profile and exits on disconnect', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'akx-built-acp-'))
    const child = execa(process.execPath, [akxBin, '--profile', 'acp'], {
      cwd: home,
      reject: false,
      timeout: SPAWN_TIMEOUT_MS,
      killSignal: 'SIGKILL',
      env: {
        ...
        AKX_HOME: home,
        AKX_TELEMETRY_DISABLED: '1',
        AKASHX_API_KEY: apiKey,
        AKASHX_BASE_URL: server.baseURL,
        AKX_PERMISSION_MODE: 'danger-full-access',
      },
      extendEnv: false,
    })
    const rawOut: string[] = []
    ...
    const clientApp = createAcpClientApp({ name: 'akx-built-acp-profile' })
    ...
    try {
      ...
      expect(initialized.agentInfo).toMatchObject({ name: 'akx-harness-acp' })
  ...
  it('runs the headless profile through its app-owned task positional', async () => {
    const apiKey = 'built-akx-headless-key'
    const server = await startMockLlmServer({
    ...
    const home = mkdtempSync(join(tmpdir(), 'akx-built-headless-'))
    try {
      ...
      expect(result.stderr).toBe('akx: reasoning:\nInspecting the published entry.')
  ...
  it('does not load a project environment for --version', async () => {
    const project = mkdtempSync(join(tmpdir(), 'akx-version-project-'))
  ...
  it.skipIf(process.platform === 'win32')('runs through an installed-style symlink', async () => {
    const installation = mkdtempSync(join(tmpdir(), 'akx-bin-link-'))
    const installedBin = join(installation, 'akx')
    symlinkSync(akxBin, installedBin)
    try {
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    const home = mkdtempSync(join(tmpdir(), 'akx-missing-profile-'))
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { AKX_HOME: home })
      expect(result.code).toBe(1)
      expect(result.stderr).toContain('profile "nope" does not exist')
      expect(result.stderr).toContain('akx plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS + 30_000)
  ...
  it('creates a custom profile from the shipped web template before booting it', async () => {
    const home = mkdtempSync(join(tmpdir(), 'akx-from-default-profile-'))
    try {
      const created = await runBuiltBin(
        ['--profile', 'rescue', '--from-default-profile', 'web', '--help'],
        { AKX_HOME: home, AKX_TELEMETRY_DISABLED: '1' },
      )
      expect(created.code).toBe(0)
      expect(created.stderr).toBe('')
      expect(created.stdout).toContain('Usage: akx --profile web')

      const dir = join(home, 'profiles', 'rescue')
      const manifest = JSON.parse(readFileSync(join(dir, 'package.json'), 'utf8')) as {
        dependencies: Record<string, string>
        akx: { profile: { bundles: string[]; patchReload: string } }
      }
      expect(manifest.dependencies).toEqual({})
      expect(manifest.akx.profile).toEqual({
        bundles: ['@akashx/akx-base', '@akashx/akx-web-app'],
        patchReload: 'live',
      })
      ...
      expect(reopened.stdout).toContain('Usage: akx --profile web')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS * 3 + 30_000)
  ...
  it('keeps a newly created profile when application boot rejects its arguments', async () => {
    const home = mkdtempSync(join(tmpdir(), 'akx-from-default-profile-failed-boot-'))
    try {
      ...
      expect(retried.stdout).toContain('Usage: akx --profile web')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  ...
  it('uses the launching endpoint and managed credential through the published entry', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'akx-home-environment-'))
    const project = mkdtempSync(join(tmpdir(), 'akx-home-project-'))
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    // refresh drain: akx exited 13 with no diagnostic instead of settling
    // ([vendor/README.md](../../../vendor/README.md)).
    const home = mkdtempSync(join(tmpdir(), 'akx-invalid-patch-'))
    try {
      const result = await runBuiltBin(['--profile', 'web', '--patch', invalidProvider], {
        AKX_HOME: home,
        AKASHX_API_KEY: 'keyless-invalid-config',
        AKX_TELEMETRY_DISABLED: '1',
      })
      expect(result.code).toBe(1)
      expect(result.stdout).toBe('')
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($AKX_HOME/cordis.patch.yml) is live too
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `akx plugin --profile x add .` from a plugin checkout must install THAT
    // checkout — pnpm's cwd is the profile directory, so an un-anchored `.`
    // would self-link the profile.
    const home = mkdtempSync(join(tmpdir(), 'akx-plugin-anchor-'))
    const checkout = mkdtempSync(join(tmpdir(), 'akx-plugin-checkout-'))
    try {
      ...
      const result = await execa(process.execPath, [akxBin, 'plugin', '--profile', 'anchor', 'add', '.'], {
        cwd: checkout,
        input: '',
        timeout: SPAWN_TIMEOUT_MS,
        killSignal: 'SIGKILL',
      ...
      const manifest = JSON.parse(readFileSync(join(home, 'profiles', 'anchor', 'package.json'), 'utf8')) as {
        dependencies: Record<string, string>
        akx: { profile: { bundles: string[] } }
      }
      expect(Object.keys(manifest.dependencies)).toEqual(['anchored-bundle'])
      expect(manifest.akx.profile.bundles).toContain('anchored-bundle')
      ...
      expect(afterRemove.akx.profile.bundles).not.toContain('anchored-bundle')
    } finally {
      rmSync(home, { recursive: true, force: true })
  ...
  it('activates a dependency that gained akx.bundle in a later update', async () => {
    ...
    // version declares akx.bundle. Simulated without a registry: hand-place
    // the installed package, flip its manifest, and run a benign pnpm verb.
    const home = mkdtempSync(join(tmpdir(), 'akx-plugin-update-'))
    try {
      ...
      writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
      // v1: no akx manifest — a plain dependency.
      writeFileSync(join(installed, 'package.json'), JSON.stringify({ name: 'late-bundle', version: '1.0.0' }))
      const first = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { AKX_HOME: home })
      expect(first.code).toBe(0)
      let manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { akx: { profile: { bundles: string[] } } }
      expect(manifest.akx.profile.bundles).toEqual(['@akashx/akx-base'])
      // v2: the installed package now declares akx.bundle (an update landed).
      writeFileSync(join(installed, 'package.json'), JSON.stringify({
        name: 'late-bundle', version: '2.0.0', akx: { bundle: { patch: './cordis.patch.yml' } },
      }))
      writeFileSync(join(installed, 'cordis.patch.yml'), '[]\n')
      const second = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { AKX_HOME: home })
      expect(second.code).toBe(0)
      manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { akx: { profile: { bundles: string[] } } }
      expect(manifest.akx.profile.bundles).toEqual(['@akashx/akx-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS * 2 + 30_000)
  ...
  describe('config dump', () => {
    let home: string
    beforeEach(() => { home = mkdtempSync(join(tmpdir(), 'akx-dump-bin-')) })
    afterEach(() => { rmSync(home, { recursive: true, force: true }) })

    it('prints the web profile bundle layers without a user layer', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { AKX_HOME: home })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@akashx/akx-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @akashx/akx-base')
      expect(stdout).toContain("name: '@akashx/akx-host-webserver'")
    ...
    it('creates a custom profile from a shipped template before printing it', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain('# == @akashx/akx-web-app')
      expect(existsSync(join(home, 'profiles', 'rescue', 'package.json'))).toBe(true)
    }, SPAWN_TIMEOUT_MS + 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@akashx/akx-headless'")
      expect(stdout).not.toMatch(/name: '@akashx\/akx-host-/)
      expect(stdout).not.toContain("name: '@akashx/akx-web-app'")
      expect(stdout).not.toMatch(/name: '@akashx\/akx-client-/)
    }, SPAWN_TIMEOUT_MS + 30_000)

    it('prints the exact standalone sdk-minimal tree without akx-base', async () => {
      const { stdout, code, stderr } = await runBuiltBin(
      ...
      expect(stdout).toContain('# == @akashx/akx-sdk-minimal')
      expect(stdout).not.toContain('@akashx/akx-base')
      expect(stdout).not.toContain('@akashx/akx-web-app')
    }, SPAWN_TIMEOUT_MS * 2 + 30_000)

    it('composes the profile user layer and a --patch overlay in order', async () => {
      // Auto-init the web profile first, then write its user layer.
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { AKX_HOME: home })
