...
import { bootInjections, orderByModuleGraph } from '@akashx/akx-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry, WebBootGraph } from '@akashx/akx-client-modules/client'
import { AppWebEntry } from '@akashx/akx-client-web'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@akashx/akx-app-boot')).href) as unknown as BootComposition
...
function resolveClientExport(packagePath: string, pkg: ClientPackageManifest): string {
  ...
  if (relative === undefined) {
    throw new Error(`assembled boot: ${pkg.name ?? packagePath} declares akx.client without a ./client export`)
  }
...
/** Derive the assembled browser graph from the same bundle patches and package declarations as `akx web`. */
function loadAssembledPlugins(): readonly AssembledPlugin[] {
  ...
  for (const entry of entries) {
    ...
    const declaration = pkg.akx?.client
...
const BOOTSTRAP_IDS = ['@akashx/akx-client-modules'] as const
...
export function mountAssembledApp(search = '?fixture', options: AssembledBootOptions = {}): void {
  ...
  win.__AKX_BOOT__ = graph
  const [facadeRow] = bootInjections(win.__AKX_BOOT__)
 ...
 * the snapshot gate's `AKX_SNAPSHOT` mode (`record` re-runs the scenarios from
 * scratch, `refresh` re-derives the expected text from the existing ones).
 */
export const REFRESHING_GOLDEN = process.env.AKX_SNAPSHOT === 'record' || process.env.AKX_SNAPSHOT === 'refresh'
