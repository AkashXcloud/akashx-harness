// Real-host smoke: spawn `akx web` with a real key, walk the full flow
// list in a real chromium, screenshot every screen into .artifacts/ for the
// figma comparison pass. Self-skips without AKASHX_API_KEY (repo e2e
...
function authenticatedWeb(launchUrl: string): Promise<{ origin: string; cookie: string }> {
  ...
  const exchange = (async () => {
    ...
    if (response.status !== 303 || setCookie === null) {
      throw new Error(`akx web authentication returned HTTP ${String(response.status)}`)
    }
    return {
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`akx web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /akx web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`akx web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `akx web` serves every production manifest plugin; until every UI
...
describe('akx web keyless CLI smoke', () => {
  it('serves a usable app from two immutable plugin batches', async () => {
    requireDist()
    const sessionsDir = mkdtempSync(join(tmpdir(), 'akx-web-keyless-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          AKASHX_API_KEY: 'keyless-web-no-call',
          AKX_HOME: join(sessionsDir, '.akx'),
          AKX_AGENTS_HOME: join(sessionsDir, '.agents'),
    ...
    try {
      ...
      expect(batchPaths).toContainEqual(expect.stringMatching(
        /^\/plugins\/\?\?@akashx\/akx-client-modules\/client\.js&rev=[a-f\d]{12}$/,
      ))
  ...
  it('routes web runtime context and workspace instructions through the real CLI request', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'akx-web-workspace-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          AKASHX_API_KEY: 'keyless-web-workspace',
          AKASHX_BASE_URL: `http://127.0.0.1:${address.port}`,
          AKX_HOME: join(workspace, '.akx'),
          AKX_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
      },
  ...
  it('retries a partial transport failure through the shipped Web composition', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'akx-web-retry-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          AKASHX_API_KEY: 'keyless-web-retry',
          AKASHX_BASE_URL: `http://127.0.0.1:${address.port}`,
          AKX_HOME: join(workspace, '.akx'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
  ...
  it('AKX_TOOLS_MODE=ptc collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'akx-web-ptc-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          AKASHX_API_KEY: 'keyless-web-ptc',
          AKASHX_BASE_URL: `http://127.0.0.1:${address.port}`,
          AKX_TOOLS_MODE: 'ptc',
          AKX_HOME: join(workspace, '.akx'),
          AKX_AGENTS_HOME: join(workspace, '.agents'),
...
describe.skipIf(!process.env.AKASHX_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    requireDist()
    sessionsDir = mkdtempSync(join(tmpdir(), 'akx-web-w5-'))
    const port = await probeFreePort()
    // tsx boot mirrors the runtime half of the root akx script. Isolate
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $AKX_HOME with the
    // AkashX credential inherited from the environment, so no onboarding
