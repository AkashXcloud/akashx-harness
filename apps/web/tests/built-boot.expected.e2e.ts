...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('AKX_CLIENT_BUILD_PROFILE') === 'official') {
    expect(document.querySelector('svg[viewBox="26 0 156 24"]')).not.toBeNull()
    expect(screen.queryByText('AKX Local Build')).toBeNull()
  } else {
    expect(document.querySelector('svg[viewBox="0 0 23.16 17.04"]')).not.toBeNull()
    const version = clientBuildValue('AKX_CLIENT_VERSION')
    if (version === undefined) throw new Error('default client build record must carry AKX_CLIENT_VERSION')
    const commit = clientBuildValue('AKX_CLIENT_COMMIT_HASH')
    const buildVersion = version
      + (commit === undefined ? '' : `-${commit}`)
      + (clientBuildValue('AKX_CLIENT_GIT_DIRTY') === 'true' ? '-dirty' : '')
    screen.getByText('AKX Local Build')
  ...
  for (const plugin of ['@akashx/akx-client-ui-layout', '@akashx/akx-client-ui-sidebar', '@akashx/akx-client-ui-conversation', '@akashx/akx-client-ui-tool']) {
...
it('boots without ui-chat and does not select another conversation view implicitly', async () => {
  mountAssembledApp('?fixture', { exclude: ['@akashx/akx-client-ui-chat'] })

  const tree = await screen.findByRole('tree', { name: 'Sessions' }, { timeout: 10_000 })
  const boot = Reflect.get(window, '__AKX_BOOT__') as { entries: Array<{ id: string }> } | undefined
  expect(boot?.entries.some(entry => entry.id === '@akashx/akx-client-ui-chat')).toBe(false)
