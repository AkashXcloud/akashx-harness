...
import type { ReplayEntry } from '@akashx/akx-llm-replay'
import { sessionFixtureName } from '@akashx/akx-session-snapshot'
...
describe('web e2e: Trajectory virtualization over tail-paged history', () => {
  ...
  beforeAll(async () => {
    replayDir = await mkdtemp(join(tmpdir(), 'akx-trajectory-virtualization-'))
