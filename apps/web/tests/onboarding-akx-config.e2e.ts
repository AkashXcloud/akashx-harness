// Keyless browser e2e: the shipped AkashX adapter stays mounted while its
...
const SNAPSHOT_DIR = fileURLToPath(new URL('./expected/onboarding-akashx-config', import.meta.url))
...
describe.skipIf(MODE === 'record')('web e2e: first-run AkashX credential setup', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-akashx-config'))
    ...
    const secret = `akx_onboarding_${randomBytes(12).toString('hex')}`
    ...
    expect(stored.includes(`AKASHX_API_KEY: ${secret}`)).toBe(true)
    ...
    const deepSeekRow = settings.getByText('AkashX', { exact: true }).first()
  ...
  it('configures arbitrary AkashX models and prompts after the selected model is removed', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-akashx-models'))
    ...
    await settings.waitFor({ timeout: 10_000 })
    await settings.getByRole('button', { name: '模型' }).click()
    const deepSeek = settings.getByText('AkashX', { exact: true }).first()
    ...
    expect(await settings.getByLabel('模型 ID 1').inputValue()).toBe('akashx-flash')
    expect(await settings.getByLabel('显示名称 1').inputValue()).toBe('AkashX-V41-Flash')
    expect(await settings.getByLabel('模型 ID 2').inputValue()).toBe('akashx-v4-flash')
    expect(await settings.getByLabel('模型 ID 3').inputValue()).toBe('akashx-v4-pro')
    expect(await settings.getByLabel('模型 ID 4').inputValue()).toBe('akashx-v4-flash-vision-exp')
    ...
    expect(savedDefaults).toContain('id: akashx-flash')
    ...
    await expect(scaffold.ctx.llm.resolveModelInfo('akashx-official', 'akashx-flash')).resolves.toMatchObject({
    ...
    expect(document).not.toContain('id: akashx-flash')
