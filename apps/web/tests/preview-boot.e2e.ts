...
} from '@akashx/akx-experimental-webworker-packer'
import {
...
function requireVfsAssets(): PreviewAssets {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'akx-preview-boot-'))
...
async function bootPreview(origin: string, browser: Browser): Promise<void> {
  ...
  try {
    ...
    const exercised = await page.evaluate(async ({ seededSessionId, seededSessionTitle }) => {
      ...
      const transport = (globalThis as typeof globalThis & { __AKX_TRANSPORT__?: PreviewTransport }).__AKX_TRANSPORT__
      ...
      await createDirectory('/akx/workspace/.agents/skills', 'runtime-created')
  ...
  try {
    ...
    const sessionCount = await page.evaluate(async () => {
      const transport = (globalThis as typeof globalThis & {
        __AKX_TRANSPORT__?: { fetch(input: string, init: RequestInit): Promise<Response> }
      }).__AKX_TRANSPORT__
