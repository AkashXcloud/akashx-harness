...
import type {} from '@akashx/akx-webhook'
import {
...
const TITLE = 'Review akx-harness/akx-harness#314'
...
describe.skipIf(MODE === 'record')('web e2e: GitHub ready-for-review', () => {
  ...
  beforeAll(async () => {
    previousPort = process.env.AKX_GITHUB_WEBHOOK_PORT
    previousSecret = process.env.AKX_GITHUB_WEBHOOK_SECRET
    const port = await freePort()
    process.env.AKX_GITHUB_WEBHOOK_PORT = String(port)
    process.env.AKX_GITHUB_WEBHOOK_SECRET = SECRET
    ...
    await page.addInitScript(() => { localStorage.setItem('akx.locale', 'en') })
  ...
  afterAll(async () => {
    ...
    if (previousPort === undefined) Reflect.deleteProperty(process.env, 'AKX_GITHUB_WEBHOOK_PORT')
    else process.env.AKX_GITHUB_WEBHOOK_PORT = previousPort
    if (previousSecret === undefined) Reflect.deleteProperty(process.env, 'AKX_GITHUB_WEBHOOK_SECRET')
    else process.env.AKX_GITHUB_WEBHOOK_SECRET = previousSecret
  })
  ...
  it('isolates ingress and creates a browsable Workspace Session', async () => {
    ...
    const payload = {
      ...
      pull_request: {
        ...
        html_url: 'https://github.com/akx-harness/akx-harness/pull/314',
        draft: false,
        user: { login: 'octocat' },
