...
// Boots the REAL web composition — the akx-base and akx-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $AKX_SNAPSHOT:
// replay (default, keyless: normally disables the llm-akx row and
// inserts akx-llm-replay in providers mode), record (real adapter + key,
...
//
// Composition divergences from `akx web`, all deliberate, all via include
...
// built dist; ordinary keyless modes disable llm-akx and fill the open
...
import { AKX_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@akashx/akx-launch-environment'
import Loader from '@akashx/cordis-plugin-loader'
import Include, { type PatchOptions } from '@akashx/cordis-plugin-include'
import Group from '@akashx/cordis-plugin-group'
import {
...
} from '@akashx/akx-session-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
...
import { akxHomePath } from '@akashx/akx-home-paths'
import { LlmAdapter } from '@akashx/akx-llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, RetryPolicyConfig, StreamChunk,
} from '@akashx/akx-llm'
import type { ReplayHandle, ReplayProviderConfig } from '@akashx/akx-llm-replay'
import {
  installLlmReplay,
  parseSessionLog,
  prepareSessionSnapshotFixtureForComparison,
} from '@akashx/akx-llm-replay'
import type { SessionFormatEvent } from '@akashx/akx-session-format'
import { sessionFormatCatalog } from '@akashx/akx-session-format-catalog'
import {
...
// } from '@akashx/akx-client-ui-settings-models'
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'AkashX Harness 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 AkashX Harness 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入 AKX 插件生态。',
    continueLabel: '继续',
  },
} as const

/** Snapshot mode for the lane, from $AKX_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.AKX_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`AKX_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}
...
/** The shipped composition under test: the akx-base and akx-web-app bundle patches over the empty profile root. */
...
// mode, never catch-all: with llm-akx disabled no adapter exists, so a
...
const REPLAY_PROVIDERS = [{
  ...
  name: 'AkashX',
  models: [
    { id: 'akashx-v4-flash', name: 'AkashX-V4-Flash', contextWindow: 128_000 },
    {
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($AKX_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted akx-llm-replay row
   ...
   * fails loud with NO_ADAPTER (llm-akx is disabled and no replay row
  ...
  cordisTools?: boolean
  /**
   * Keep the shipped AkashX adapter mounted while masking the process
   * environment's AKASHX_API_KEY for this scaffold lifetime. This is the
  ...
  /**
   * Patch the shipped AkashX search row to a deterministic endpoint and
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  if (mode === 'record') {
    ...
    if (process.env.AKASHX_API_KEY === undefined || process.env.AKASHX_API_KEY.length === 0) {
      throw new Error('web e2e record mode needs AKASHX_API_KEY (env or repo-root .env)')
    }
  }
  ...
  const maskAkashXCredential = mode !== 'record' && options.deepSeekMissingCredential === true
  const originalAkashXCredential = process.env.AKASHX_API_KEY
  let credentialEnvironmentRestored = false
  const restoreCredentialEnvironment = (): void => {
    if (credentialEnvironmentRestored || !maskAkashXCredential) return
    credentialEnvironmentRestored = true
    if (originalAkashXCredential === undefined) {
      Reflect.deleteProperty(process.env, 'AKASHX_API_KEY')
    } else {
      process.env.AKASHX_API_KEY = originalAkashXCredential
    }
  }
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'akx-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $AKX_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.akx document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.akx-home')
  ...
  // session is created. Without this a developer's real ~/.akx/skills silently
  // enters replay requests and goldens while CI sees none. `AKX_HOME` follows
  ...
  const skillRootEnvironment = {
    AKX_HOME: harnessHome,
    AKX_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    AKX_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'akx-web-e2e-sessions-'))
  } catch (error) {
  ...
  if (maskAkashXCredential) Reflect.deleteProperty(process.env, 'AKASHX_API_KEY')

  // The include patch set — the same layer stack the profile boot composes
  // (bundle patches in akx.profile.bundles order), applied over the SAME empty root (a
  ...
  const patches: PatchOptions[] = [
    ...
    ...mode === 'record' || options.deepSeekMissingCredential === true
      ? []
      : [{ id: 'agent-default-model', config: { provider: 'akashx-official', model: 'akashx-v4-flash' } }],
    ...extraOverlayPatches,
    // The roster's shipped presets are the plugin's own, bundled inside
    // `akx-agent-presets` and prepended by it. Pin only the machine-local
    // root away: a developer's own `~/.akx/.agent-presets` must not be able
    // to change a golden.
    {
      id: 'agent-presets',
      config: {
    ...
    // storage-json's yml root is anchored to the real $AKX_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.akx-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.akx, ~/.agents, and a bundled-root env setting
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        akxHome: join(workspaceCwd, '.akx-home'),
    ...
    // to the production OTLP endpoint (or whatever AKX_TELEMETRY_OTLP_URL
    ...
    // (apps/web IS @akashx/akx-web-frontend); native browser opening and the
    ...
    { id: 'settings', config: { akxHome: harnessHome } },
    { id: 'credentials', config: { akxHome: harnessHome } },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@akashx/akx-tool-cordis' },
      ] }]
      : [],
  ...
  if (options.openInAppEnvironment !== undefined) ctx.provide(AKX_LAUNCH_ENVIRONMENT_KEY, options.openInAppEnvironment)
  ...
  try {
    ...
    ctx.provide('akxHomePath', akxHomePath)
    ...
    // disable llm-akx; the first-run lane keeps it mounted but has no
...
export function normalizeWebSessionVolatiles(log: string, workspaceCwd?: string): string {
  ...
  return log.split(/\r?\n/).map((line) => {
    ...
    if (record.type === 'web/akashx-search-llm-request' && typeof record.data?.endpoint === 'string') {
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run AKX_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
