...
// in the temp workspace) all run; the model adapter is akx-llm-replay (keyless)
...
// Record: AKX_SNAPSHOT=record writes session.v3.jsonl, then a keyless
// AKX_SNAPSHOT=refresh regenerates ui.expected.md.
...
import { ToolCallId, expandAssistantStream } from '@akashx/akx-llm'
...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  it.skipIf(MODE === 'record')('matches the conversation aria golden with stable anchors', async () => {
    ...
    await page.getByRole('button', {
      name: 'Select model, current AkashX-V4-Flash',
  ...
  it.skipIf(MODE === 'record')('renders the system prompt disclosure inside the expanded Turn process', async () => {
    ...
    expect(await opaque.textContent()).toContain('You are an AI agent powered by AkashX Harness.')
