...
import type { RetryPolicyConfig } from '@akashx/akx-llm'
import { deriveReplayScript, parseSessionLog } from '@akashx/akx-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'akx-web-e2e-sidecar-'))
