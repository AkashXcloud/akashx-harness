...
function isDesktopHostEvent(message: unknown): message is DesktopHostEvent {
  ...
  switch (candidate.type) {
    case 'ready':
      return candidate.protocolVersion === DESKTOP_HOST_PROTOCOL_VERSION && typeof candidate.akxVersion === 'string'
    case 'fatal':
...
export class DesktopHostProcess {
  ...
  async start(): Promise<DesktopHostReady> {
    ...
    const entry = join(this.runtimeDir, 'node_modules', '@akashx', 'akx-desktop-host', 'lib', 'index.js')
    const child = spawn(this.node, [
      ...
      env: Object.fromEntries(Object.entries(this.environment).filter(([name]) => (
        name !== 'NODE_OPTIONS' && name !== 'NODE_PATH' && !/^AKX_DESKTOP_/u.test(name) && !/^(?:npm|pnpm|corepack)_/iu.test(name)
      ))),
    ...
    if (!(requestPipe instanceof Writable) || !(responsePipe instanceof Readable)) {
      child.kill('SIGTERM')
      throw new Error('akx desktop host did not expose the required byte pipes and IPC channel')
    }
    this.child = child
    ...
    responsePipe.once('end', () => {
      try {
        ...
        this.fail(new Error('akx desktop host response pipe ended'))
      } catch (error) {
        this.fail(errorOf(error, 'akx desktop host response pipe failed'))
      }
    })
    ...
    child.on('message', (message: unknown) => {
      if (!isDesktopHostEvent(message)) {
        this.fail(new Error('akx desktop host sent an invalid IPC event'))
        child.kill('SIGTERM')
        return
      }
    ...
    this.exitPromise = new Promise<void>((resolve) => {
      child.once('close', (code) => {
        ...
        if (code !== 0 && code !== null) this.fail(new Error(`akx desktop host exited with ${String(code)}${suffix}`))
        else this.fail(new Error(`akx desktop host stopped${suffix}`))
        resolve()
      })
    })
  ...
  /** Forward one `akx-app://app` request to the child without buffering its body. */
  async fetch(request: Request): Promise<Response> {
    await this.start()
    const child = this.child
    if (child === undefined || !child.connected || this.requestPipe === undefined) {
      throw new Error('akx desktop host is unavailable')
    }
    if (this.nextStreamId > 0xffff_ffff) throw new Error('akx desktop host exhausted its request stream ids')
    ...
    return new Promise<Response>((resolve, reject) => {
      ...
      const abort = (): void => {
        ...
        this.enqueueRequestFrame(encodeDesktopRequestCancel(streamId)).catch((pipeError: unknown) => {
          this.fail(errorOf(pipeError, 'akx desktop request pipe failed'))
        })
      ...
      this.pumpRequest(streamId, request, hasBody).catch((error: unknown) => {
        this.failPending(streamId, errorOf(error, 'akx desktop request upload failed'))
      })
    })
  }
  ...
  async stop(): Promise<void> {
    ...
    if (!await exitsWithin(exited, 5_000)) {
      child.kill('SIGKILL')
      if (!await exitsWithin(exited, 5_000)) {
        throw new Error('akx desktop host did not exit after SIGKILL')
      }
    }
  ...
  private async pumpRequest(streamId: number, request: Request, hasBody: boolean): Promise<void> {
    ...
    if (body === null) throw new Error('akx desktop request body disappeared before upload')
  ...
  private enqueueRequestFrame(frame: Buffer): Promise<void> {
    const write = this.requestWriteTail.then(async () => {
      ...
      if (pipe === undefined || pipe.destroyed) throw new Error('akx desktop host request pipe is unavailable')
  ...
  private send(message: DesktopHostCommand): void {
    const child = this.child
    if (child === undefined || !child.connected) throw new Error('akx desktop host IPC is unavailable')
  ...
  private handleResponseFrame(frame: DesktopHostResponseFrame): void {
    ...
    if (pending === undefined) {
      if (frame.streamId >= this.nextStreamId) {
        throw new Error(`akx desktop host responded for unknown stream ${String(frame.streamId)}`)
      }
      return
    }
    switch (frame.type) {
      case 'start': {
        if (pending.responseStarted) throw new Error(`akx desktop host started stream ${String(frame.streamId)} twice`)
      ...
      case 'data': {
        ...
        if (!pending.responseStarted || controller === undefined) {
          throw new Error(`akx desktop host sent body data before a body start for stream ${String(frame.streamId)}`)
        }
      ...
      case 'end':
        if (!pending.responseStarted) {
          throw new Error(`akx desktop host ended stream ${String(frame.streamId)} before its response start`)
        }
  ...
  private cancelResponse(streamId: number, reason: unknown): void {
    ...
    this.enqueueRequestFrame(encodeDesktopRequestCancel(streamId)).catch((error: unknown) => {
      this.fail(errorOf(error, 'akx desktop request pipe failed'))
    })
    this.finishPending(streamId, false)
  }
