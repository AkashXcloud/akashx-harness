/** Signed local npm package set that supplies the Desktop-owned akx runtime and private Host. */
...
/** Private package installed beside akx to boot the Desktop Host process. */
export const DESKTOP_HOST_PACKAGE = '@akashx/akx-desktop-host'
...
/** Complete union of the first-party package closures rooted at akx and its private Desktop Host. */
...
const RELEASE_PACKAGES = [AKX_PACKAGE, DESKTOP_HOST_PACKAGE] as const
 ...
 * @param expectedReleaseVersion - Required akx and Desktop Host version when validating one release.
...
/** Return the local direct dependency spec for the akx package. */
export function desktopAkxPackageSpec(packageSet: DesktopCorePackageSet): string {
  const record = packageSet.packages.find(entry => entry.name === AKX_PACKAGE)
  if (record === undefined) throw new Error(`desktop package set: missing ${AKX_PACKAGE}`)
 ...
 * @param expectedReleaseVersion - Exact akx and Desktop Host version bound to Electron.
