...
import { DESKTOP_IPC, type AkxDesktopApi, type DesktopUpdateState } from './ipc.ts'
import type { DesktopBackendState } from './backend-controller.ts'

const api: AkxDesktopApi = {
  protocolVersion: 1,
  locale: () => ipcRenderer.invoke(DESKTOP_IPC.localeGet) as Promise<ReturnType<AkxDesktopApi['locale']> extends Promise<infer T> ? T : never>,
  plugins: {
    list: () => ipcRenderer.invoke(DESKTOP_IPC.pluginsList) as Promise<ReturnType<AkxDesktopApi['plugins']['list']> extends Promise<infer T> ? T : never>,
  ...
  backend: {
    status: () => ipcRenderer.invoke(DESKTOP_IPC.backendStatus) as ReturnType<AkxDesktopApi['backend']['status']>,
...
contextBridge.exposeInMainWorld('akxDesktop', api)
