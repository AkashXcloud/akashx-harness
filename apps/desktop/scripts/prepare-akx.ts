...
const AKX_OUTPUT_ROOT = BUILD_PATHS.akx
const BUILD_ROOT = mkdtempSync(join(tmpdir(), 'akx-desktop-runtime-'))
const STORE_ROOT = join(BUILD_ROOT, 'store')
const RUNTIME_ROOT = BUILD_PATHS.runtime
const PNPM_BUILD_STATE = BUILD_PATHS.akxPnpm
...
function desktopRelease(): DesktopRelease {
  ...
  const akxVersion = manifestVersion(resolve(APP_ROOT, '..', '..', 'package.json'), 'root akx package')
  if (version !== akxVersion) {
    throw new Error(`desktop runtime: Electron ${version} must bind the same version of @akashx/akx, found ${akxVersion}`)
  }
...
async function main(): Promise<void> {
  rmSync(AKX_OUTPUT_ROOT, { recursive: true, force: true })
  ...
  try {
    ...
    mkdirSync(AKX_OUTPUT_ROOT, { recursive: true })
    cpSync(modules, join(AKX_OUTPUT_ROOT, 'node_modules'), {
      recursive: true, dereference: true,
      filter: source => desktopRuntimeFileExclusion(relative(modules, source), target) === undefined,
    })
    writeFileSync(join(AKX_OUTPUT_ROOT, 'package.json'), `${JSON.stringify({
      name: '@akashx/akx-desktop-runtime', private: true, version: release.version, type: 'module',
    ...
    for (const file of DESKTOP_HOST_RUNTIME_FILES) {
      if (!existsSync(join(AKX_OUTPUT_ROOT, 'node_modules', DESKTOP_HOST_PACKAGE, file))) {
        throw new Error(`desktop runtime: missing private Host file ${file}`)
      }
    }
    if (process.platform === 'darwin') {
      await signMacOSRuntime(AKX_OUTPUT_ROOT, resolveDesktopAppId(process.env), resolveMacOSSigningEnvironment(process.env))
    }
    writeDesktopRuntime(AKX_OUTPUT_ROOT, release, packageSet.packages.map(entry => entry.name), target)
    const descriptor = await verifyDesktopRuntime(AKX_OUTPUT_ROOT, release.version, target)
    await new Promise<void>((accept, reject) => {
      execFile(NODE, [join(APP_ROOT, 'tests/fixtures/runtime-payload-smoke.mjs'), AKX_OUTPUT_ROOT],
    ...
    await smokeDesktopRuntime(AKX_OUTPUT_ROOT, NODE, descriptor)
    await verifyDesktopRuntime(AKX_OUTPUT_ROOT, release.version, target)
  } catch (error) {
    rmSync(AKX_OUTPUT_ROOT, { recursive: true, force: true })
    throw error
  } finally {
