...
/**
 * Validate the completed package record, akx version, update metadata, hashes, and target files.
...
export async function createDesktopUploadPlan(
  ...
  const akxVersion = await manifestVersion(join(repositoryRoot, 'package.json'), 'akx package')
  const desktopVersion = await manifestVersion(join(appRoot, 'package.json'), 'desktop package')
  if (akxVersion !== desktopVersion) {
    throw new Error(`desktop upload: desktop version ${desktopVersion} does not match current akx version ${akxVersion}`)
  }
  ...
  if (buildRecord.schemaVersion !== 1
    ...
    || buildRecord.version !== akxVersion
    || buildRecord.environment !== update.environment
    || buildRecord.publicUrl !== update.publicUrl) {
    throw new Error(`desktop upload: ${targetName} package completion record does not match akx ${akxVersion} and ${update.environment} update destination`)
  }

  const metadataFilename = desktopUpdateMetadataFilename(akxVersion, target.platform)
  ...
  if (metadataVersion !== akxVersion) {
    throw new Error(`desktop upload: ${metadataFilename} version ${metadataVersion} does not match current akx version ${akxVersion}`)
  }
  ...
  const base = `akx-harness-${akxVersion}-${target.os}-${target.arch}`
