 ...
 * @param root - Materialized akx resources.
...
export async function smokeDesktopRuntime(root: string, node: string, runtime: DesktopRuntimeDescriptor): Promise<void> {
  const home = mkdtempSync(join(tmpdir(), 'akx-desktop-smoke-'))
  const profile = join(home, 'profiles', 'desktop')
  const host = new DesktopHostProcess(node, root, profile, undefined, { ...process.env, AKX_HOME: home })
  try {
    createPluginProfile(profile)
    const pluginName = 'desktop-runtime-smoke-plugin'
    const plugin = join(profile, 'node_modules', pluginName)
    mkdirSync(plugin, { recursive: true })
    const cordis = runtime.sharedPackages.find(entry => entry.name === '@akashx/cordis')
    ...
    writeFileSync(join(plugin, 'package.json'), JSON.stringify({
      ...
      peerDependencies: { '@akashx/cordis': cordis.version }, akx: { bundle: { patch: './bundle.yml' } },
    }))
...
`)
    ...
    manifest.akx.profile.bundles.push(pluginName)
    ...
    if (ready.akxVersion !== runtime.release.version) throw new Error('desktop runtime: Host reported another akx release')
    const response = await host.fetch(new Request('akx-app://app/'))
