 ...
 * @module @akashx/akx-desktop-host
 */
...
import { AKX_LAUNCH_ENVIRONMENT_KEY } from '@akashx/akx-launch-environment'
import type {} from '@akashx/akx-api-gateway'
import type { ConnectionFetchHandler } from '@akashx/akx-client-connection'
import type {} from '@akashx/akx-client-modules'
import { renderIndexInjections, type IndexInjection } from '@akashx/akx-host-webserver'
import {
...
/** One request forwarded from Electron's `akx-app://` handler. */
...
export interface DesktopHostController {
  /** Installed akx version carried by this host. */
  readonly akxVersion: string
...
const DESKTOP_STREAM_PATH = '/.akx/remote-stream'

const DESKTOP_TRANSPORT_SCRIPT = `globalThis.__AKX_TRANSPORT__={
  ownsHost:true,
...
function readManifest(path: string): PackageManifest {
  ...
  if (!isRecord(value)) throw new Error(`akx desktop: ${path} must contain a package manifest`)
  return {
...
function packageManifestPath(projectDir: string, packageName: string): string {
  ...
  if (!existsSync(path)) throw new Error(`akx desktop: installed package ${JSON.stringify(packageName)} has no manifest`)
  return path
}
...
function desktopPatches(runtimeDir: string, projectDir: string, allowLinkedPackages: boolean): PatchOptions[] {
  const akxRoot = dirname(packageManifestPath(runtimeDir, '@akashx/akx'))
  const profile = loadProfileDirectory('akx desktop', projectDir, join(akxRoot, 'package.json'))
  for (const layer of profile.layers) {
    if (!allowLinkedPackages && !isProjectPath(projectDir, layer.packageDir) && !isProjectPath(runtimeDir, layer.packageDir)) {
      throw new Error(`akx desktop: profile bundle ${JSON.stringify(layer.packageName)} resolved outside the Desktop runtime and profile`)
    }
  }
  const layers = [
    ...profile.layers.map(layer => layer.patches),
    profile.patches,
    loadOverlayPatches('akx desktop', DESKTOP_PATCH),
  ]
  ...
  if (agentPresets !== undefined) {
    layers.push([{
      ...
      config: {
        ...
        roots: [{ path: join(akxRoot, 'config', 'agent-presets'), trust: 'system' }],
      },
    }])
  }
  return layers.flat()
...
function akxVersion(runtimeDir: string): string {
  const manifest = readManifest(packageManifestPath(runtimeDir, '@akashx/akx'))
  if (typeof manifest.version !== 'string') throw new Error('akx desktop: installed akx manifest has no version')
...
function assetHandler(ctx: Context, runtimeDir: string): ConnectionFetchHandler {
  ...
  const distIndex = require.resolve('@akashx/akx-web-frontend/dist/index.html')
 ...
 * @param runtimeDir - immutable akx packages supplied by the Electron application.
...
export async function runDesktopHost(
  ...
  const environment = loadLayeredEnv('akx desktop')
  let current: Context | undefined
  const ctx = await boot('akx desktop', rootConfig, structuredClone(desktopPatches(
    resolve(runtimeDir),
    absoluteProject,
    options.allowLinkedPackages === true,
  )), (hostCtx) => {
    current = hostCtx
    hostCtx.provide(AKX_LAUNCH_ENVIRONMENT_KEY, environment)
  ...
  if (connection === undefined || clientModules === undefined || gateway === undefined) {
    ...
    throw new Error('akx desktop: composition did not provide connection, typertGateway, and clientModules')
  }
  ...
  return {
    akxVersion: akxVersion(resolve(runtimeDir)),
    cancel(streamId) {
      requests.get(streamId)?.abort()
    },
    async fetch(command, body) {
      if (disposing !== undefined) throw new Error('akx desktop: host is disposing')
...
async function main(): Promise<void> {
  ...
  if (runtimeDir === undefined || projectDir === undefined || process.send === undefined) {
    throw new Error('akx desktop: expected runtime and profile directories, byte pipes, and a Node IPC channel')
  }
  const option = process.argv[4]
  if (option !== undefined && option !== '--allow-linked-profile') {
    throw new Error(`akx desktop: unsupported internal option ${JSON.stringify(option)}`)
  }
  ...
  const writeResponse = (frame: Buffer): Promise<void> => {
    const write = responseWriteTail.then(async () => {
      if (responsePipe.destroyed) throw new Error('akx desktop: Electron response pipe is unavailable')
  ...
  send({
    ...
    akxVersion: controller.akxVersion,
  })
  ...
  const stop = (exitCode = 0): Promise<void> => {
    ...
    stopping ??= (async () => {
      ...
      const stopped = new Error('akx desktop: Host is stopping')
  ...
  const beginRequest = (frame: Extract<DesktopHostRequestFrame, { type: 'start' }>): void => {
    if (frame.streamId <= lastStreamId) {
      throw new Error(`akx desktop: Electron reused or reordered request stream ${String(frame.streamId)}`)
    }
    ...
    void run.catch(failTransport).finally(() => {
      ...
      openBody.error(new Error('akx desktop: response completed before the request body ended'))
      requestBodies.delete(frame.streamId)
      blockedRequests.delete(frame.streamId)
    ...
    switch (frame.type) {
      ...
      case 'data': {
        ...
        if (body === undefined) {
          ...
          throw new Error(`akx desktop: Electron sent body data for inactive stream ${String(frame.streamId)}`)
        }
      ...
      case 'end': {
        ...
        if (body === undefined) {
          ...
          throw new Error(`akx desktop: Electron ended inactive body stream ${String(frame.streamId)}`)
        }
        body.close()
        requestBodies.delete(frame.streamId)
        blockedRequests.delete(frame.streamId)
      ...
      case 'cancel': {
        if (frame.streamId > lastStreamId) {
          throw new Error(`akx desktop: Electron canceled unknown stream ${String(frame.streamId)}`)
        }
        const body = requestBodies.get(frame.streamId)
        body?.error(new Error('akx desktop: Electron canceled the request'))
        requestBodies.delete(frame.streamId)
        blockedRequests.delete(frame.streamId)
  ...
  requestPipe.once('end', () => {
    ...
    try {
      decoder.finish()
      failTransport(new Error('akx desktop: Electron request pipe ended'))
    } catch (error) {
      failTransport(error)
    }
  })
  ...
  process.on('message', (message: unknown) => {
    if (!isDesktopHostCommand(message)) {
      send({ type: 'fatal', message: 'akx desktop: invalid Electron IPC command' })
      void stop(1)
      return
    }
    void stop()
...
if (import.meta.main) {
  main().catch((error: unknown) => {
    ...
    else process.stderr.write(`akx desktop: ${message}\n`)
    process.exitCode = 1
  })
}
