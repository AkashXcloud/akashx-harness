...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/akashx-ai/akx-harness' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/akashx-ai/akx-harness/edit/master/${editSource}`
    },
    text: '在 GitHub 上编辑此页',
  },
}
...
/**
 * The AkashX wordmark, inlined so its `currentColor` fills follow the active
...
const wordmark = readFileSync(resolve(import.meta.dirname, '../public/wordmark.svg'), 'utf8')
  .trim()
  .replace('<svg ', '<svg class="akx-wordmark" ')
 ...
 */
const siteStyle = `
.akx-lockup { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }
.akx-wordmark { display: block; height: 22px; width: auto; color: var(--vp-c-text-1); }
.akx-tag {
  display: inline-flex;
  align-items: center;
...
/**
 * Navigation-bar title: the AkashX wordmark and the release-stage tag.
...
function siteTitle(previewTag: string): string {
  return `<span class="akx-lockup">${wordmark}<span class="akx-tag">${previewTag}</span></span>`
}
...
export default withMermaid({
  ...
  vite: {
    ...
    plugins: [
      {
        name: 'akx-harness-doc-projector',
