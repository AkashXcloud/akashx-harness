/** Package-owned invariants for AkashX session-log acceptance watermarks. */
...
const PACKAGE_NAME = '@akashx/akx-session-log-akx'

/** Cordis companion plugin name. */
export const name = 'session-log-akx-invariant'
/** Service required before the companion can reserve package ownership. */
export const inject = ['invariants']

/** Validate one acceptance watermark against its containing event and session. */
function validateDeliveryAccepted(session: Session, event: SessionEvent<'session-log-akx/delivery-accepted'>, fail: InvariantFailure): void {
  ...
  if (!Number.isSafeInteger(acceptedFormatVersion)
    ...
    fail(
      'session-log-akx/delivery-accepted sessionFormatVersion must be a non-negative safe integer'
  ...
  if (sessionId !== session.id && !inherited) {
    fail('a non-inherited session-log-akx/delivery-accepted event must name its containing session')
  }
  let acceptedSeq: ReturnType<typeof SessionSeq>
  try {
    acceptedSeq = SessionSeq(throughSeq)
  } catch {
    fail(`session-log-akx/delivery-accepted throughSeq must identify an earlier event, got ${throughSeq} at seq ${event.seq}`)
  }
...
function validateSession(session: Session, fail: InvariantFailure): void {
  ...
  for (const event of session.snapshotEvents()) {
    if (event.type === 'session-log-akx/delivery-accepted') validateDeliveryAccepted(session, event, fail)
  }
}
