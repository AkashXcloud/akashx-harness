...
import MessageFeedback from '@akashx/akx-message-feedback'
import { recordFeedback } from '@akashx/akx-command-feedback'
import LlmRuntime, { createAssistantMessage, createUserMessage } from '@akashx/akx-llm'
import * as LlmAkashX from '@akashx/akx-llm-akx'
import AkashXLlmApiExtensions from '@akashx/akx-llm-api-extensions'
import { startMockLlmServer, type MockLlmServer } from '@akashx/akx-llm-mock-server'
import * as SessionLogAkashX from '../src/index.ts'
import type { AkashXSessionLogExtension } from '../src/types.ts'
...
it('uploads freeform feedback and message put/edit/delete through the unchanged provider route', async () => {
  root = await mkdtemp(join(tmpdir(), 'akx-feedback-upload-'))
  vi.stubEnv('AKX_HOME', root)
  vi.stubEnv('AKASHX_API_KEY', 'feedback-test-key')
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@akashx/akx-message-feedback', MessageFeedback],
    ['@akashx/akx-llm', LlmRuntime],
    ['@akashx/akx-llm-akx', LlmAkashX],
    ['@akashx/akx-llm-api-extensions', AkashXLlmApiExtensions],
  ...
  try {
    ...
    const assistant = createAssistantMessage({ content: [{ type: 'text', text: 'Answer' }], source: { provider: 'akashx-official', model: 'akashx-v4-flash' } })
    ...
    const request = async () => {
      const chunks = []
      for await (const chunk of ctx!.llm.stream({ provider: 'akashx-official', model: 'akashx-v4-flash', sessionId: session.id, messages: session.deriveMessages() })) chunks.push(chunk)
      return chunks.at(-1)
    }
    ...
    const first = (server.requests[0]!.body as { akx_session_log: AkashXSessionLogExtension }).akx_session_log
    const retry = (server.requests[1]!.body as { akx_session_log: AkashXSessionLogExtension }).akx_session_log
    ...
    expect(SessionLogAkashX.acceptedThrough(session)).toBe(first.throughSeq)
    ...
    expect(await request()).toMatchObject({ type: 'finish', reason: { kind: 'stop' } })
    const suffix = (server.requests[2]!.body as { akx_session_log: AkashXSessionLogExtension }).akx_session_log
    ...
    expect(SessionLogAkashX.acceptedThrough(session)).toBe(suffix.throughSeq)
    ...
    for (const wire of server.requests) {
      ...
      expect(wire.body).not.toHaveProperty('akx_feedback')
      expect(wire.body).toMatchObject({ model: 'akashx-v4-flash', messages: [
