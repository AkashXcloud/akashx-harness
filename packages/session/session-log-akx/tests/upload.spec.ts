...
import { createAssistantMessage, createSystemMessage, createUserMessage } from '@akashx/akx-llm'
import type { JsonValue } from '@akashx/akx-util-values'
import * as SessionLogAkashX from '../src/index.ts'
import type { AkashXSessionLogExtension, AkashXSessionLogWireEvent, AkashXSessionLogWireSurfaceOp } from '../src/types.ts'
...
async function harness(
  ...
  const upload = ctx.plugin(SessionLogAkashX, { enabled: true })
  await upload
...
describe('incremental AkashX session-log upload', () => {
  it('publishes raw numeric sequence fields on its external wire DTO', () => {
    expectTypeOf<AkashXSessionLogExtension['sessionFormatVersion']>().toEqualTypeOf<number>()
    expectTypeOf<AkashXSessionLogExtension['afterSeq']>().toEqualTypeOf<number>()
    expectTypeOf<AkashXSessionLogExtension['throughSeq']>().toEqualTypeOf<number>()
    expectTypeOf<AkashXSessionLogExtension['events'][number]['seq']>().toEqualTypeOf<number>()
    expectTypeOf<AkashXSessionLogExtension['events'][number]['data']>().toEqualTypeOf<JsonValue>()
    expectTypeOf<AkashXSessionLogExtension['session']['seedLength']>()
      .toEqualTypeOf<number | undefined>()
  })

  it('requires surface placement and restricts sources to non-assistant surface wire events', () => {
    type System = Extract<AkashXSessionLogWireEvent, { type: 'system/message' }>
    type Assistant = Extract<AkashXSessionLogWireEvent, { type: 'assistant/message' }>
    type User = Extract<AkashXSessionLogWireEvent, { type: 'user/message' }>
    type Tool = Extract<AkashXSessionLogWireEvent, { type: 'tool/result' }>
    type LogOnly = Extract<AkashXSessionLogWireEvent, { type: 'turn/start' }>
    type Replace = Exclude<AkashXSessionLogWireSurfaceOp, 'append'>
    expectTypeOf<System['surfaceOp']>().toEqualTypeOf<AkashXSessionLogWireSurfaceOp>()
    expectTypeOf<System['sourceEventSeqs']>().toEqualTypeOf<readonly number[] | undefined>()
    expectTypeOf<Assistant['surfaceOp']>().toEqualTypeOf<AkashXSessionLogWireSurfaceOp>()
    expectTypeOf<User['surfaceOp']>().toEqualTypeOf<AkashXSessionLogWireSurfaceOp>()
    expectTypeOf<Tool['surfaceOp']>().toEqualTypeOf<AkashXSessionLogWireSurfaceOp>()
  ...
  it('uploads assistant provenance only through its embedded stream', async () => {
    ...
    const prepared = await ctx.akashxLlmApiExtensions.prepare({
      body: body(), signal: SIGNAL, sessionId: session.id,
    })
    expect(prepared.fields.akx_session_log?.events).toEqual([{
      type: assistant.type,
  ...
  it('uploads system append and replacement placement with unchanged data and provenance', async () => {
    ...
    const prepared = await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    expect(prepared.fields.akx_session_log?.events).toEqual(session.snapshotEvents())
  })
  ...
  it.each(['extension/event', 'tool/code-dispatch', 'tool/code-dispatch-start'])('uploads opaque ignorable %s without interpreting its metadata', async (type) => {
    for (const metadata of [
      ...
      const prepared = await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
      expect(prepared.fields.akx_session_log?.events[0]).toStrictEqual(event)
  ...
  it('does not contribute the session log under its default configuration', async () => {
    ...
    await ctx.plugin(SessionStore)
    await ctx.plugin(AkashXLlmApiExtensionRegistry)
    await ctx.plugin(SessionLogAkashX)
    ...
    expect(prepared.fields).not.toHaveProperty('akx_session_log')
  })
  ...
  it('uploads the full first prefix, records acceptance, then sends only the appended suffix', async () => {
    ...
    const first = await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    const firstPayload = first.fields.akx_session_log
    ...
    await first.accept()
    expect(SessionLogAkashX.acceptedThrough(session)).toBe(1)
    ...
    const second = await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    expect(second.fields.akx_session_log).toMatchObject({ afterSeq: 1, throughSeq: 3 })
    expect(second.fields.akx_session_log?.events).toHaveLength(2)
    expect(second.fields.akx_session_log?.events[0]).toMatchObject({
  ...
  it('reconstructs a persisted cursor and ignores an inherited parent watermark in a fork', async () => {
    ...
    const prepared = await first.ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: first.session.id })
    await prepared.accept()
    const seed = first.session.snapshotEvents()

    const resumed = await harness('parent', seed)
    expect(SessionLogAkashX.acceptedThrough(resumed.session)).toBe(0)
    const resumedPayload = await resumed.ctx.akashxLlmApiExtensions.prepare({
      body: body(), signal: SIGNAL, sessionId: resumed.session.id,
    })
    expect(resumedPayload.fields.akx_session_log?.afterSeq).toBe(0)

    const fork = await harness('child', seed, {
      inheritedEventCount: SessionLogOffset(seed.length),
      meta: { parentSession: first.session.id, isSeeded: true },
    })
    expect(SessionLogAkashX.acceptedThrough(fork.session)).toBe(-1)
    const forkPayload = await fork.ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: fork.session.id })
    expect(forkPayload.fields.akx_session_log).toMatchObject({ afterSeq: -1, throughSeq: fork.session.seq - 1 })
  })
  ...
  it('takes the maximum watermark when concurrent acceptances settle out of order', async () => {
    ...
    session.append('turn/start', { turn: 1 })
    const earlier = await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    session.append('step/start', { turn: 1, step: 1 })
    const later = await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
  ...
  it('folds only events appended after the cached acceptance scan', () => {
    ...
    expect(SessionLogAkashX.acceptedThrough(session)).toBe(0)
    expect(reads).toBe(2)
    reads = 0
    expect(SessionLogAkashX.acceptedThrough(session)).toBe(0)
    expect(reads).toBe(0)
    ...
    expect(SessionLogAkashX.acceptedThrough(session)).toBe(2)
    expect(reads).toBe(2)
  })
  ...
  it('rejects a missing event below the captured Session length', () => {
    ...
    expect(() => SessionLogAkashX.acceptedThrough(session))
      .toThrow('session-log-akx: missing event 0 below captured length 1')
  })
  ...
  it('ignores another format generation before interpreting its frozen sequence', () => {
    ...
    expect(SessionLogAkashX.acceptedThrough(migrated)).toBe(2)
  })
  ...
  it.each([-1, -0, 0.5])('rejects malformed acceptance format version %s', (sessionFormatVersion) => {
    ...
    expect(() => SessionLogAkashX.acceptedThrough(session)).toThrow(/malformed acceptance format version/)
  })
  ...
  it('omits the field for direct or stale requests and uploads the prior acceptance marker next', async () => {
    ...
    await expect(ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL }))
      .resolves.toMatchObject({ fields: {} })
    await expect(ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: 'missing' }))
      .resolves.toMatchObject({ fields: {} })
    await expect(ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id }))
      .resolves.toMatchObject({ fields: {} })
    session.append('turn/start', { turn: 1 })
    const first = await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    await first.accept()
    const current = await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    expect(current.fields.akx_session_log).toMatchObject({
      afterSeq: 0,
      throughSeq: 1,
      events: [{ type: 'session-log-akx/delivery-accepted' }],
    })
  })
  ...
  it('contributes complete events without reading request messages', async () => {
    ...
    session.append('turn/start', { turn: 1 })
    const prepared = await ctx.akashxLlmApiExtensions.prepare({ body: {}, signal: SIGNAL, sessionId: session.id })
    expect(prepared.fields.akx_session_log?.events).toEqual(session.snapshotEvents())
  })
  ...
  it('translates logical brands and isSeeded into the raw upload DTO', async () => {
    ...
    const wire = JSON.parse(JSON.stringify(prepared.fields.akx_session_log)) as Record<string, unknown>
  ...
  it('translates ignorable and surface event envelopes to raw wire values', async () => {
    ...
    const events = prepared.fields.akx_session_log?.events ?? []
  ...
  it('fails closed on a malformed persisted acceptance watermark', async () => {
    ...
    expect(() => SessionLogAkashX.acceptedThrough(session)).toThrow(/malformed acceptance watermark/)
  })
  ...
  it('withdraws its request field when the contributing plugin reloads', async () => {
    ...
    session.append('turn/start', { turn: 1 })
    expect((await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })).fields)
      .toHaveProperty('akx_session_log')
    await disposeUpload()
    expect((await ctx.akashxLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })).fields)
      .not.toHaveProperty('akx_session_log')
  })
})
