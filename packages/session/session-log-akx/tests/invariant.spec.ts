...
import InvariantRegistry, { InvariantError } from '@akashx/akx-invariants'
import SessionStore, {
...
describe('AkashX session-log acceptance invariant', () => {
  it('accepts a watermark naming an earlier event in its containing Session', async () => {
    ...
    expect(() => session.append('session-log-akx/delivery-accepted', {
  ...
  it.each([-1, 0.5])('rejects malformed acceptance format version %s', async (sessionFormatVersion) => {
    ...
    expect(() => session.append('session-log-akx/delivery-accepted', {
      ...
      code: 'INVARIANT',
      packageName: '@akashx/akx-session-log-akx',
    }))
  })
  ...
  it('rejects negative-zero format versions restored across the owned invariant boundary', async () => {
    ...
    const session = {
      ...
      snapshotEvents: () => [{
        type: 'session-log-akx/delivery-accepted' as const,
        seq: SessionSeq(0),
        time: 1,
  ...
  it('rejects current-generation mismatches and leaves v0 watermarks inert', async () => {
    ...
    expect(() => wrongId.append('session-log-akx/delivery-accepted', {
    ...
    expect(() => wrongSeq.append('session-log-akx/delivery-accepted', {
    ...
    expect(() => invalidSeq.append('session-log-akx/delivery-accepted', {
