...
import { SessionFormatUnsupportedMigrationError } from '@akashx/akx-session-format'
import {
...
describe('released v1 whole-artifact relationships', () => {
  ...
  it('enforces title source cardinality and exact auxiliary framing', () => {
    ...
    expect(() => decode([...prefix, {
      ...
      data: {
        ...
        messages: [user('unrelated', { kind: 'plugin', plugin: 'akx-session-title-llm' })], maxTokens: 1,
      },
  ...
  it('validates own delivery ids while preserving inherited ancestor markers', () => {
    const marker = (sessionId: string, version: number | undefined, seq: number) => ({
      type: 'session-log-akx/delivery-accepted', seq, time: seq + 1,
    ...
    const inertV0 = {
      type: 'session-log-akx/delivery-accepted', seq: 1, time: 2,
