...
import { SessionSeq, SessionId } from '@akashx/akx-session'
...
async function freshRoot(prefix = 'akx-jsonl-zstd-'): Promise<string> {
...
runPersistenceContract('jsonl-zstd', async () => {
  const root = await mkdtemp(join(tmpdir(), 'akx-jsonl-zstd-contract-'))
...
describe('JsonlSessionPersistence: encoding selection', () => {
  it('rejects roots owned by the opposite encoding in both directions', async () => {
    const rawRoot = await freshRoot('akx-jsonl-raw-mismatch-')
    ...
    const zstdRoot = await freshRoot('akx-jsonl-zstd-mismatch-')
