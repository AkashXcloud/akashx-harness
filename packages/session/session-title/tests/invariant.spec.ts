...
import * as SessionTitleInvariantCompanion from '@akashx/akx-session-title/invariant'
import InvariantRegistry, { InvariantError } from '@akashx/akx-invariants'
import SessionStore, { SessionId, SessionSeq } from '@akashx/akx-session'
...
describe('session-title source invariant', () => {
  ...
  it('rejects a citation-free automatic title and a user rename that cites messages', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@akashx/akx-session-title',
    }))
    expect(() => {
