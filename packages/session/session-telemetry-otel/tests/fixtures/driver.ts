...
import { resolveConfigPath } from '@akashx/akx-app-boot'
import { recordFeedback } from '@akashx/akx-command-feedback'
import { runFixtureTurn } from '@akashx/akx-loader-smoke'
...
process.env.AKX_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
process.env.AKX_TELEMETRY_OTLP_URL = process.env.AKX_TELEMETRY_E2E_URL
process.env.AKX_TELEMETRY_MODE = process.env.AKX_TELEMETRY_E2E_MODE ?? 'FEEDBACK_ONLY'
...
try {
  ...
  try {
    ...
    if (process.env.AKX_TELEMETRY_E2E_FEEDBACK !== 'none') {
