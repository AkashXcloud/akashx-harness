...
import { SessionFormatError, SessionFormatUnsupportedMigrationError, defineSessionFormatMigration, sessionFormatCount } from '@akashx/akx-session-format'
import type { SessionFormatEvent, SessionFormatEventRun, SessionFormatJsonObject, SessionFormatJsonValue, SessionFormatMigrationContext, SessionFormatMigrationStage, SessionFormatMigrationStageInput } from '@akashx/akx-session-format'
import { assertReleasedV2Header } from '@akashx/akx-session-format-v1-to-v2'
...
export const sessionFormatV2ToV3 = defineSessionFormatMigration({
  name: '@akashx/akx-session-format-v2-to-v3',
  fromVersion: 2,
  toVersion: 3,
...
class ReleasedV2ToV3Stage implements SessionFormatMigrationStage {
  ...
  transformEvent(event: SessionFormatEvent, context: SessionFormatMigrationContext): void {
    ...
    if (event.type === 'session-log-akx/delivery-accepted') {
  ...
  private emitSystem(prompt: string, anchor: SessionFormatEvent, context: SessionFormatMigrationContext): void {
    ...
    context.emitEvent(canonicalizeTransformedEvent({
      ...
      data: {
        ...
        message: {
          id,
          role: 'system', source: { kind: 'plugin', plugin: '@akashx/akx-system-prompt' },
