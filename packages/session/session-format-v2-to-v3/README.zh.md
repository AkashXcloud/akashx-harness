...
合成消息携带开放步骤的 `turn` 和 `step`、锚点事件的 `time`、角色 `system`，以及来源 `{ kind: 'plugin', plugin: '@akashx/akx-system-prompt' }`。空提示词使用 `content: []`；其他提示词使用包含精确字符串的单个文本块。首次追加没有溯源；每次替换的两个端点及唯一源引用都使用前一头节点的目标序号。空头节点保持受保护，但不产生模型消息。
...
V2 `session-log-akx/delivery-accepted` 若携带 `data.sessionFormatVersion === 3`，就会被拒绝，而非提升为 V3 上传水位。其他代的标记保留其载荷，包括缺失代次和非目标的未来代次。V2 代标记必须具有有效且更早的 `throughSeq`；若它指向另一个会话，则仅允许出现在具有 `parentSession` 的会话的继承前缀中。本地的外部会话标记或没有父会话元数据的外部会话标记会被拒绝。标记的信封序号正常变化；其捕获的接收坐标不变。
