...
import { createSessionFormatCatalog, SessionFormatEventCollector } from '@akashx/akx-session-format'
import type { SessionFormatArtifact, SessionFormatEvent, SessionFormatHeader, SessionFormatJsonObject } from '@akashx/akx-session-format'
...
import { sessionFormatV1ToV2 } from '@akashx/akx-session-format-v1-to-v2'
...
describe('streaming V2 system prompt migration', () => {
  ...
  it.each([undefined, 0, 1, 2])('preserves delivery generation coordinates (%s) and validates ownership before promotion', (version) => {
    const marker = event('session-log-akx/delivery-accepted', { sessionId: header.id, throughSeq: 1, ...(version === undefined ? {} : { sessionFormatVersion: version }) })
  ...
  it('derives unknown seeded cuts after insertion and isolates simultaneous stages', () => {
    ...
    const events = dense([...opening(), event('request/header', request('seed')), event('session-log-akx/delivery-accepted', { sessionId: 'parent', throughSeq: 1, sessionFormatVersion: 2 }), event('session/end-seed', { inherited: true })])
  ...
  it('rejects invalid references, future generation claims, and unclassified message content', () => {
    ...
    expect(() => migrate([...opening(), event('session-log-akx/delivery-accepted', { sessionId: header.id, throughSeq: 1, sessionFormatVersion: 3 })])).toThrow(/format v3|between/)
