...
import type { SaveTextSpill } from '@akashx/akx-spill'
import LocalSpillStore, {
  ...
  sessionDir,
  sweepSpillRoots,
} from '@akashx/akx-spill-local'
import type { SweepRoot } from '@akashx/akx-spill-local'
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'akx-spill-test-'))
})
afterEach(() => {
...
describe('LocalSpillStore service', () => {
  ...
  it('defaults cleanupPeriodDays to 30', async () => {
    ...
    const emptyBase = mkdtempSync(join(tmpdir(), 'akx-empty-'))
...
describe('startup cleanup sweep', () => {
  ...
  it('prunes an emptied DISCOVERED default root but never the active root', async () => {
    ...
    const prior = mkdtempSync(join(tmpdir(), 'akx-spill-'))
  ...
  it('covers the configured root AND discovered default roots (real gatherRoots)', async () => {
    ...
    const fakeTmp = mkdtempSync(join(tmpdir(), 'akx-faketmp-'))
...
describe('discoverDefaultRoots', () => {
  it('returns only real akx-spill-* directories, excluding symlinks and non-matches', async () => {
    const base = mkdtempSync(join(tmpdir(), 'akx-disc-'))
    try {
      // A real backend-shaped root (akx-spill-<6>) via mkdtemp — the only match.
