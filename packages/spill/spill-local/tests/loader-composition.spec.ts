...
import LocalSpillStore, { sessionDir } from '@akashx/akx-spill-local'
...
describe('spill-local real Loader composition through cordis.yml', () => {
  it('loads cleanupPeriodDays and prunes only expired session contents', async () => {
    root = await mkdtemp(join(tmpdir(), 'akx-spill-loader-'))
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier !== '@akashx/akx-spill-local') throw new Error(`unexpected Loader import: ${specifier}`)
