...
/**
 * A backend-generated default root name: `akx-spill-` plus the 6-character
 * suffix `mkdtemp` appends. Discovery matches this
 * EXACT shape, not the bare prefix, so an unrelated `akx-spill-test-*` fixture
 * or a foreign tool's differently-shaped `akx-spill-…` directory is never
...
export interface SweepRoot {
   ...
   * pruned. Set for DISCOVERED prior-default `akx-spill-*` roots (one per past
...
/**
 * Discover prior default spill roots: the `akx-spill-<6 chars>` directories
 ...
 * prefix, so an unrelated `akx-spill-test-*` fixture or a foreign
