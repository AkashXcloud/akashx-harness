 ...
 *
 * `@akashx/akx-tools`' `ToolExecution` satisfies this shape, so the policy
 * reads `exec` straight through without importing `akx-tools` or `akx-agent`.
 * Only the session HEADER id is read — the same identity every other subsystem
 * keys off (see `akx-tool-bash`'s owner derivation).
 *
 * @module @akashx/akx-spill-policy/types
 */
