...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@akashx/akx-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@akashx/akx-hook-protocol'
import type { RunHookOptions } from '@akashx/akx-hook-protocol'
 ...
 * executor (akx-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what akx-bash-local's resolve does for the fields runHook sets.
      return {
