...
import ToolRegistry, { defineContentToolFixture } from '@akashx/akx-tools'
import { FILE_REFERENCE_PROMPT } from '@akashx/akx-file-reference'
...
async function stubAgent(
  ...
  const root = await mkdtemp(join(tmpdir(), 'akx-file-reference-service-'))
  roots.push(root)
