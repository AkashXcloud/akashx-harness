...
async function harness(): Promise<{ ctx: Context; agent: Agent }> {
  workdir = await mkdtemp(join(tmpdir(), 'akx-workspace-context-e2e-'))
  ...
  await ctx.plugin(LlmAkashX, { models: [{ id: 'akashx-v4-flash' }] })
...
describe.skipIf(!process.env.AKASHX_API_KEY)('workspace context e2e: real model sees AGENTS.md baseline', () => {
