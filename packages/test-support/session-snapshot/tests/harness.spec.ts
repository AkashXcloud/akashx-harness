...
it('keeps scenario-owned snapshot spill root length stable across platforms', () => {
  ...
  expect(posix).toMatch(/^\/tmp\/akx-acp-snap-[0-9a-f]{9}$/)
  expect(windows).toMatch(/^\/t\/akx-acp-snap-[0-9a-f]{9}$/)
...
describe('runScenario', () => {
  ...
  it('builds akx profile argv and rebases relative modules in live and replay patches', async () => {
    ...
    const live = launchAcpTestAgent({
      ...
      env: { AKX_SNAPSHOT: 'record', AKX_SNAPSHOT_FILE: fixtureFile },
    })
    await live.spawned
    await live.close()
    const materializedRoot = join(dir, '.akx-profile-patches')
    ...
    expect(await realpath(join(dir, '.akx', 'profiles', 'node_modules', 'example-package')))
      .toBe(await realpath(packageDir))
    expect(await realpath(join(dir, '.akx', 'profiles', 'node_modules', '@fixture', 'example-package')))
    ...
    const replay = launchAcpTestAgent({
      ...
      cwd: dir,
      configPath: selectedPatch,
      env: { AKX_SNAPSHOT: 'replay', AKX_SNAPSHOT_FILE: fixtureFile },
    })
    ...
    const conflictLink = join(dir, '.akx', 'profiles', 'node_modules', 'conflict-package')
    await Promise.all([
  ...
  it('waits for inherited stdio and buffered ACP parsing after the parent exits', { timeout: 20_000 }, async () => {
    ...
    const launched = launchAcpTestAgent({
      agent: AGENT,
      cwd: dir,
      env: { AKX_SNAPSHOT_FILE: fixtureFile },
    })
    await launched.client.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} })
  ...
  it('preserves launch-resolution errors when no child process exists', async () => {
    const { dir, fixtureFile } = await scenario({})
    vi.stubEnv('AKX_EXAMPLE_MODE', 'lib')
    try {
      await expect(runScenario(
        { steps: [] },
        {
