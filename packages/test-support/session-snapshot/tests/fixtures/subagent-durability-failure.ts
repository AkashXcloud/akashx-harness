...
import type { SubagentPromptRequestId } from '@akashx/akx-subagent'
 ...
 *  - Under `AKX_SNAPSHOT_HUMAN_STEER`, a browser-authored prompt steers the
 ...
 *    last flush.
 *  - Under `AKX_SUBAGENT_PUBLISHED_FAILURE`, a one-shot child's first
...
export function apply(ctx: Context): void {
  ...
  const publishedFailure = process.env.AKX_SUBAGENT_PUBLISHED_FAILURE === '1'
  const humanSteer = process.env.AKX_SNAPSHOT_HUMAN_STEER === '1'
