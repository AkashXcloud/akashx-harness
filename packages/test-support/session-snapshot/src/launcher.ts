 ...
 *
 * @module @akashx/akx-session-snapshot/launcher
 */
...
import { entryListSchema, type PatchOptions } from '@akashx/cordis-plugin-include'
...
export interface AgentUnderTest {
  ...
  configPath: string
  /** Named akx profile; omitted only for test-only fake bins with their own config grammar. */
  profile?: string
...
export function launchAcpTestAgent(options: AcpTestLaunchOptions): LaunchedAcpTestAgent {
  ...
  const launch = resolveExampleLaunch({
    ...
    configArgs: agent.profile === undefined
      ...
      : profileArgs(agent.profile, agent.configPath, selectedConfig, options.env?.AKX_SNAPSHOT, cwd),
    tsconfigPath: agent.tsconfigPath,
    ...agent.profile === undefined ? {} : { sourceImport: 'tsx/esm' },
    env: {
      ...options.env,
      AKX_HOME: join(cwd, '.akx'),
  ...
  const clientApp = createAcpClientApp({ name: 'akx-harness-acp-test-client' })
...
/** Build one akx profile invocation from the base and optional scenario patches. */
function profileArgs(
  ...
  const materializedRoot = join(cwd, '.akx-profile-patches')
 ...
 * profile fallback. This mirrors `akx plugin` while retaining the bare entry
...
function linkProfilePackage(source: string, cwd: string, packageName: string): void {
  ...
  // The package may instead belong to the akx installation; profile boot heals those links.
  if (packageDir === undefined) return
  const link = join(cwd, '.akx', 'profiles', 'node_modules', packageName)
