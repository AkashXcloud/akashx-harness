 ...
 * @module @akashx/akx-session-snapshot/normalize
 */

import {
  decodeSeqRanges,
} from '@akashx/akx-session'
import { prepareSessionSnapshotFixtureForComparison } from '@akashx/akx-llm-replay'
...
const SNAPSHOT_SPILL_PATH_RE = new RegExp(
  String.raw`(?:[A-Za-z]:)?[\\/]+(?:tmp|t)[\\/]+(?:akx-acp-snap-[0-9a-f]{9}|akx-acp-snapshot-spill)[\\/]+session-[0-9a-f]{12}[\\/]+[0-9a-f]{12}-([A-Za-z0-9._~-]+?)`
  + String.raw`(?=\. Use read with offset/limit|[\s)"]|\\+"|$)`,
  'g',
)
