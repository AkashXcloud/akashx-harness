...
import { RemoteMock, ok, openStream } from '@akashx/akx-remote-mock'
...
const globals = globalThis as { __AKX_TRANSPORT__?: unknown; EventSource?: unknown; ResizeObserver?: unknown }
...
describe('TestClient (jsdom)', () => {
  it('boots the whole web-app roster, connects, mounts, and disposes with nothing unmatched', async () => {
    ...
    expect(globals.__AKX_TRANSPORT__).toBeDefined()
  ...
  it('boots overlapping clients one at a time onto their own mocks and keeps the shared globals until the last dispose', async () => {
    ...
    await a.reload('@akashx/akx-client-connection')
  ...
  it('creates no mount element when the roster cannot be loaded', async () => {
    const roster = ClientRoster.of([{ name: '@akashx/akx-client-test-runtime-missing', inject: [], immediately: true }])
    const before = document.body.childElementCount
  ...
  it('reports the log when the connection never becomes ready', async () => {
    ...
    const roster = webApp.closure(['@akashx/akx-api-workspace-controller'])
