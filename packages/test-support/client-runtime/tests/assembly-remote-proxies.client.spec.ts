...
import { RemoteMock, frames, ok, openStream } from '@akashx/akx-remote-mock'
...
const API_ROSTER = webApp.closure(['@akashx/akx-api-remotes'])
...
describe('remote proxies over a booted client', () => {
  ...
  it('drops the api-remotes row and answers ctx.remote.<ns>.<method> with the mock value, logging positional args', async () => {
    ...
    expect([...client.ctx.loader.entries()].map(entry => entry.options.name)).not.toContain('@akashx/akx-api-remotes')
