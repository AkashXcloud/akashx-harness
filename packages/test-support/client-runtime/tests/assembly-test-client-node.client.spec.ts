...
import type { ClientTransportHooks } from '@akashx/akx-client-connection/client'
import { RemoteMock } from '@akashx/akx-remote-mock'
...
const API_ROSTER = webApp.closure(['@akashx/akx-api-gateway'])
const TYPERT_ONLY = webApp.pick(['@akashx/akx-typert-registry'])
const globals = globalThis as { __AKX_TRANSPORT__?: ClientTransportHooks }
...
describe('TestClient (node environment)', () => {
  ...
  it('releases the shared globals even when the plugin tree fails to dispose, and still rethrows', async () => {
    ...
    expect(globals.__AKX_TRANSPORT__).toBeUndefined()
  ...
  it('rethrows a row that fails to apply, releases the globals, and lets the next boot take its turn', async () => {
    ...
    await expect(TestClient.start({ roster: TYPERT_ONLY, provide: { '@akashx/akx-typert-registry': failing } }, RemoteMock.create()))
  ...
  it('refuses a provide entry for the api-remotes row, whose services are the proxies', async () => {
    const roster = webApp.closure(['@akashx/akx-api-remotes'])
    await expect(TestClient.start({ roster, provide: { '@akashx/akx-api-remotes': { apply() {} } } }, RemoteMock.create()))
      .rejects.toThrow('@akashx/akx-api-remotes cannot be provided; its remote.<ns> services are the tier\'s proxies')
    expect(globals.__AKX_TRANSPORT__).toBeUndefined()
  })
