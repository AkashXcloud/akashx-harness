...
const test = createClientTest({ roster: webApp.closure(['@akashx/akx-api-gateway']) })
...
afterAll(async () => {
  ...
  expect('__AKX_TRANSPORT__' in globalThis).toBe(false)
})
...
const missingConnection = createClientTest({ roster: webApp.pick(['@akashx/akx-typert-registry']) })
