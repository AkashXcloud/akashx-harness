 ...
 * `RemoteMock` installed as the Connection carrier through `__AKX_TRANSPORT__.rpc`.
 * @module @akashx/akx-client-test-runtime/src/assembly/test-client
 */
import { Context, type Plugin } from '@akashx/cordis'
import type { Entry } from '@akashx/cordis-plugin-loader'
import { tearDownEntryFiber } from '@akashx/akx-client-hmr/client'
import type { ClientTransportHooks, ConnectionHandle } from '@akashx/akx-client-connection/client'
import { bootClient } from '@akashx/akx-client-web/src/boot-client.ts'
import { mountClient } from '@akashx/akx-client-web/src/mount.ts'
...
class SharedGlobals {
  ...
  install(transport: ClientTransportHooks): void {
    transportGlobal.__AKX_TRANSPORT__ = transport
  }
  ...
  acquire(transport: ClientTransportHooks): () => void {
    if (this.holders === 0) {
      this.previousTransport = transportGlobal.__AKX_TRANSPORT__
    ...
    return () => {
      ...
      if (this.previousTransport === undefined) delete transportGlobal.__AKX_TRANSPORT__
      else transportGlobal.__AKX_TRANSPORT__ = this.previousTransport
...
export class TestClient {
   ...
   * and wait for the connection. The `@akashx/akx-api-remotes` row is
