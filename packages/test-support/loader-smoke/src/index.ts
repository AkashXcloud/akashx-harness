 ...
 * zero-build dev path, resolving `@akashx/akx-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @akashx/akx-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'AKX_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.AKX_EXAMPLE_MODE`.
...
export function resolveExampleLaunch(options: ExampleLaunchOptions): ExampleLaunch {
  ...
  // A smoke launches a real `akx` against local fixtures, so it must not inherit the machine's
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated AKX homes. */
