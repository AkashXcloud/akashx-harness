 ...
 * @module @akashx/akx-llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@akashx/cordis'
import type {} from '@akashx/akx-compaction'
import type {} from '@akashx/akx-llm-api-extensions'
import { SESSION_FORMAT_VERSION, SessionLogOffset, type SessionEvent } from '@akashx/akx-session'
import type { SessionLogOffset as SessionLogOffsetType } from '@akashx/akx-session'
import {
  SessionFormatUnsupportedMigrationError,
  sessionFormatCatalog,
} from '@akashx/akx-session-format-catalog'
import type {
  ContentBlock,
  GenerateOptions,
...
import { LlmAdapter, LlmError, ReasoningEffortId, expandAssistantStream, requestImageHandleText, resolveRetryPolicy } from '@akashx/akx-llm'
...
function normalizeProjectedHeader(header: Readonly<Record<string, unknown>>): Record<string, unknown> {
  ...
  if (typeof header['cwd'] === 'string' && /^\{\{cwd\}\}(?:\/|$)/.test(header['cwd'])) {
    normalized['cwd'] = header['cwd'].replace('{{cwd}}', '/akx-snapshot-cwd')
  }
  return normalized
}
...
export function installLlmReplay(ctx: Context, config: ReplayConfig): ReplayHandle {
  ...
  const replay = (options: GenerateOptions): AsyncIterable<StreamChunk> => {
    ...
    return (async function* () {
      ...
      if (options.provider === 'akashx-official' && providerAccepted(resolved)) {
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.AKX_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $AKX_SNAPSHOT_FILE)')
  }
  validateConfiguredModels(config.providers)
  const overrideFile = config.overrideFile ?? process.env.AKX_SNAPSHOT_OVERRIDE
  const childEnv = process.env.AKX_SNAPSHOT_CHILD_FILES
