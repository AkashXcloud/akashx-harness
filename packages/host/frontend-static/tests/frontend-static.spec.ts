...
import * as Connection from '@akashx/akx-client-connection'
import LocalCredentials from '@akashx/akx-credentials-local'
...
async function loadComposition(): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'akx-frontend-static-'))
  ...
  await writeFile(configPath, [
    ...
    "    host: '127.0.0.1'",
    '    port: 0',
    "- name: '@akashx/akx-client-connection'",
    '- id: frontend',
    "  name: '@akashx/akx-host-frontend-static'",
    '  config:',
    `    distIndex: '${distIndex}'`,
    '',
  ].join('\n'))
  ...
  const modules = new Map<string, unknown>([
    ['@akashx/akx-credentials-local', LocalCredentials],
    ['@akashx/akx-host-webserver', HttpServer],
    ['@akashx/akx-client-connection', Connection],
    ['@akashx/akx-host-frontend-static', FrontendStatic],
  ])
...
describe('real Loader composition', () => {
  it('serves explicit index entries and files while preserving HTTP error semantics', { timeout: 60_000 }, async () => {
    ...
    expect(await request(port, '/')).toMatchObject({
      ...
      body: 'akx web authentication required; reopen the URL printed by akx web.\n',
    })
