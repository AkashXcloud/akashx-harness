...
import type { NativeCommandRunner } from '@akashx/akx-native-command'
import {
  createLaunchEnvironmentSnapshot, AKX_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentLayerInput,
...
async function boot(layers: readonly LaunchEnvironmentLayerInput[] = []): Promise<string> {
  ...
  root = await mkdtemp(join(tmpdir(), 'akx-open-in-app-loader-'))
  ...
  context = new Context()
  context.provide(AKX_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot(layers))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@akashx/akx-host-open-in-app', OpenInApp],
  ])
...
describe('open-in-app host routes (real Loader composition)', () => {
  ...
  it('serves the resolved catalog, one cached icon, and launches from the same resolution', async () => {
    ...
    const home = await mkdtemp(join(tmpdir(), 'akx-open-in-app-home-'))
  ...
  it('removes all three routes when the plugin row is disposed (HMR safety)', async () => {
    ...
    const entry = [...(context as Context).loader.entries()]
      .find(candidate => candidate.options.name === '@akashx/akx-host-open-in-app')
