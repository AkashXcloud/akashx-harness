...
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId } from '@akashx/akx-session'
...
import SandboxPolicyService, { setSandboxMode } from '@akashx/akx-sandbox-policy'
import SessionProjectionRegistry from '@akashx/akx-session-projection'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@akashx/akx-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@akashx/akx-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@akashx/akx-terminal-bash'
import { ENCODING_PREAMBLE } from '@akashx/akx-pwsh-local'
import * as ptyLocal from '@akashx/akx-terminal-bash'
import type { ResolvedConfig } from '@akashx/akx-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@akashx/akx-terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'akx> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        PROMPT_COMMAND: 'printf "\\033]133;D;%s\\007" "$?"; PS1=\'akx> \'',
        AKX_SHELL: '1', AKX_SESSION_ID: 'agent', AKX_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo akx> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo akx> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', AKX_SHELL: '1', AKX_SESSION_ID: 'agent', AKX_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'akx> ' : "function prompt { 'akx> ' }\n",
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'akx> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
