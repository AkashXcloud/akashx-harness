...
import { normalizeTerminalText, TerminalSanitizer } from '@akashx/akx-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07dsh> ')).toEqual({ text: 'akx> ', prompt: true, promptTail: 'akx> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('akx> ')).toEqual({ text: 'akx> ', prompt: false, promptTail: 'akx> ' })
  })
