...
import type { TerminalSendOperation } from '@akashx/akx-terminal'
...
async function harness(
  ...
  const root = mkdtempSync(join(tmpdir(), 'akx-pty-local-'))
  roots.push(root)
...
describe.skipIf(process.platform === 'win32')('terminal-bash real shell', () => {
  it('persists cwd and environment across sends, scrubs secrets, and closes', async () => {
    const previous = process.env.AKX_TEST_SECRET
    process.env.AKX_TEST_SECRET = 'must-not-leak'
    try {
      const { ctx, root, agent } = await harness('danger-full-access')
      const created = await ctx.terminals.spawn(agent, { type: 'shell', name: 'main', cwd: root })
      expect(created.motd).toContain('akx> ')
      ...
      if (previous === undefined) delete process.env.AKX_TEST_SECRET
      else process.env.AKX_TEST_SECRET = previous
    }
  }, 10_000)
  ...
  it('restores the controlled prompt after an in-shell PS1 override', async () => {
    ...
    expect(result.viewport).toContain('healed=[akx> ]')
  ...
  it('quiesces a disowned same-session descendant after the shell exits naturally', async () => {
    ...
    try {
      const background = ctx.terminals.startSend(agent, created.sessionId, {
        text: `sh -c 'trap "" TERM; printf "%s" "$$" > "$1"; sleep 60' akx "${pidFile}" & disown`,
        submit: true,
      })
...
describe.skipIf(!hasPwsh)('terminal-bash pwsh real shell', () => {
  it.each([false, true])('bootstraps a persistent pwsh, persists state, and scrubs secrets (hold command: %s)', async (holdCommand) => {
    ...
    try {
      ...
      const command = "Write-Output ('keep={0} cwd={1} secret={2}END' -f $env:KEEP, (Get-Location).Path, $env:AKX_TEST_SECRET)"
