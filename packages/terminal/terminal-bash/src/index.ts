 ...
 * @module @akashx/akx-terminal-bash
 */

import { Context } from '@akashx/cordis'
import type { Agent } from '@akashx/akx-agent'
import type { Session, SessionEvent } from '@akashx/akx-session'
import { TerminalBackendCleanupError } from '@akashx/akx-terminal'
import type { TerminalBackend, TerminalBackendSpawnSpec, TerminalSendOperation } from '@akashx/akx-terminal'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from '@akashx/akx-subprocess'
import type { SandboxExecutionPolicy } from '@akashx/akx-sandbox'
...
function childEnvironment(spec: TerminalBackendSpawnSpec, dialect: ShellDialect): Record<string, string> {
  ...
  const common = {
    ...
    GIT_PAGER: 'cat',
    AKX_SHELL: '1',
    AKX_SESSION_ID: spec.owner.id,
    AKX_PTY_SESSION_ID: spec.sessionId,
  }
  if (dialect === 'pwsh') {
