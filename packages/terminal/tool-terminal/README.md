...
The tools need `ctx.terminals` — a backend must be mounted — and the system-prompt service for the guidance section. Background sends additionally require the jobs service and its model-facing controller (`@akashx/akx-tool-jobs`).
...
The generated [configuration catalog](../../../docs/config-catalog.md#akashx-akx-tool-terminal) and [tool catalog](../../../docs/tool-catalog.md#akashx-akx-tool-terminal) are the exhaustive sources for config fields and schemas.
...
- [Tool catalog](../../../docs/tool-catalog.md#akashx-akx-tool-terminal) — the six generated schemas and result shapes.
...
The six generated schemas are listed in the [`akx-tool-terminal` catalog section](../../../docs/tool-catalog.md#akashx-akx-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- **Background mode requires the jobs surface** — `run_in_background` needs both `@akashx/akx-jobs` and its model-facing controller (`@akashx/akx-tool-jobs`); without them the argument is rejected.
