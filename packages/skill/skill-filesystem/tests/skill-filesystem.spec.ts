...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@akashx/akx-fs'
...
async function tempDir(name: string): Promise<string> {
  const dir = await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `akx-${name}-`)))
  tempDirs.push(dir)
...
describe('akx-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.akx/skills'), 'same', 'user akx skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.akx/skills'), 'same', 'project akx skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.akx/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project akx skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-akx')
    ...
    await writeSkill(join(noGit, '.akx/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.akx/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.akx/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.akx/skills')
  ...
  it.each([false, true])('publishes regular-file paths for linked skills while retaining their resource roots (filesystem service: %s)', async (withFileSystem) => {
    ...
    await mkdir(join(home, '.akx/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.akx/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.akx/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.akx/skills/broken-link'))
    await symlink('/dev/null', join(home, '.akx/skills/device-link'))
    ...
    const fiber = ctx.plugin(SkillFileSystem, { akxHome: join(home, '.akx'), agentsHome: join(home, '.agents'), watch: false })
    await fiber
    ...
    try {
      ...
      for (const name of ['linked-dir', 'linked-flat']) {
        ...
        expect(loaded?.resourceBase).toEqual({ kind: 'directory', path: name === 'linked-dir' ? join(home, '.akx/skills', name) : join(home, '.akx/skills') })
        expect((await lstat(path)).isFile()).toBe(true)
      }
      await writeFile(join(external, 'replacement.md'), '---\nname: linked-flat\ndescription: Replacement\n---\n\nReplacement body.\n')
      await rm(join(home, '.akx/skills/linked-flat.md'))
      await symlink(join(external, 'replacement.md'), join(home, '.akx/skills/linked-flat.md'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { akxHome: join(home, '.akx'), agentsHome: join(home, '.agents'), watch: false })
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.akx/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.akx/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousAkxHome = process.env.AKX_HOME
    const previousAgentsHome = process.env.AKX_AGENTS_HOME
    const previousBundledSkillDir = process.env.AKX_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.AKX_HOME = join(envHome, '.akx')
      process.env.AKX_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.AKX_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.akx/skills'), 'env-skill', 'Env skill')
      ...
      process.env.AKX_HOME = join(envHome, 'empty-akx')
      delete process.env.AKX_BUNDLED_SKILL_DIR
      process.env.AKX_AGENTS_HOME = join(envHome, 'empty-agents')
      const empty = new Context()
      await empty.plugin(SkillRegistry)
      SkillFileSystem.apply(empty, { watch: false })
      expect(await empty.skills.list()).toEqual([])

      delete process.env.AKX_AGENTS_HOME
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        signal: new AbortController().signal,
        invalidate() {},
      }, { akxHome: join(envHome, 'empty-akx') }).name).toBe('filesystem')
    } finally {
      if (previousAkxHome === undefined) {
        delete process.env.AKX_HOME
      } else {
        process.env.AKX_HOME = previousAkxHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.AKX_AGENTS_HOME
      } else {
        process.env.AKX_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.AKX_BUNDLED_SKILL_DIR
      } else {
        process.env.AKX_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
