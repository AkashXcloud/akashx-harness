...
import { createUserMessage, ToolCallId, type Message } from '@akashx/akx-llm'
...
import AgentRegistry, { agentEvents, type Agent, type PreStepDecision } from '@akashx/akx-agent'
...
describe('akx-tool-skill', () => {
  ...
  it('resumes from the durable entries of the latest visible catalog', async () => {
    ...
    session.append('user/message', createUserMessage({
      ...
      source: { kind: 'plugin', plugin: 'akx-tool-skill' },
    }), { surfaceOp: 'append' })
  ...
  it('loads a skill for the calling agent cwd', async () => {
    ...
    await writeSkill(join(project, '.akx/skills'), 'project-skill', 'Project skill', 'Project instructions.')
    const ctx = await setup(home)
    ...
    expect(result.value).toEqual({
      name: 'project-skill',
      provider: 'filesystem',
      resourceBase: { kind: 'directory', path: join(project, '.akx/skills/project-skill') },
    ...
    expect(block.text).toBe([
      ...
      '<skill_resources>',
      `Base directory for this skill: ${join(project, '.akx/skills/project-skill')}`,
  ...
  it('returns isError for unknown, invalid, and model-disabled skills', async () => {
    ...
    await writeSkill(join(home, '.akx/skills'), 'hidden-skill', 'Hidden skill', 'Hidden instructions.')
    await writeFile(join(home, '.akx/skills/hidden-skill/SKILL.md'), '---\nname: hidden-skill\ndescription: Hidden skill\ndisable-model-invocation: true\n---\n\nHidden instructions.\n')
    const ctx = await setup(home)
    ctx.skills.register({
      name: 'model-only-skill',
      description: 'Model-only skill',
