...
import { type GenerateOptions, LlmAdapter, ReasoningEffortId, type LlmResolvedModelInfo, type StreamChunk } from '@akashx/akx-llm'
...
export async function makeBridgeHarness(options: {
  ...
  const persistenceRoot = options.persistenceRoot ?? await mkdtemp(join(tmpdir(), 'akx-acp-test-'))
  ...
  const clientApp = createAcpClientApp({ name: 'akx-acp-test-client' })
