...
describe('TeamAction', () => {
  ...
  it('keeps a successful task mutation newer than an in-flight refresh', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Complete/u }))
    expect(await screen.findByRole('button', { name: /Reopen/u })).toBeTruthy()

    stale.resolve({ ok: true, value: view })
    await Promise.resolve()
    expect(screen.getByRole('button', { name: /Reopen/u })).toBeTruthy()
  ...
  it('keeps a created task newer than an in-flight refresh', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'New task' } })
    fireEvent.change(screen.getByPlaceholderText('Task description'), { target: { value: 'Details' } })
  ...
  it('keeps task and create failures newer than an in-flight refresh', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: en.refresh }))
    fireEvent.click(screen.getByRole('button', { name: /New task/u }))
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Rejected task' } })
    fireEvent.change(screen.getByPlaceholderText('Task description'), { target: { value: 'Rejected details' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  ...
  it('tracks simultaneous create and task mutations independently', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /New task/u }))
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Concurrent task' } })
    fireEvent.change(screen.getByPlaceholderText('Task description'), { target: { value: 'Concurrent details' } })
    const save = screen.getByRole<HTMLButtonElement>('button', { name: 'Save' })
    fireEvent.click(save)
    await waitFor(() => { expect(save.disabled).toBe(true) })

    const complete = screen.getByRole<HTMLButtonElement>('button', { name: /Complete/u })
  ...
  it('reloads derived fields for every task after a mutation', async () => {
    ...
    fireEvent.click(screen.getAllByRole('button', { name: /Complete/u })[0]!)
  ...
  it('creates a task from normalized blocker and write-scope lists', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: ' New task ' } })
    fireEvent.change(screen.getByPlaceholderText('Task description'), { target: { value: ' Details ' } })
    fireEvent.change(screen.getByPlaceholderText(/Blocked byTo-dos/u), { target: { value: 'task-1, task-1' } })
    fireEvent.change(screen.getByPlaceholderText(/Write scopes/u), { target: { value: 'src/a, src/b' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
    await waitFor(() => {
  ...
  it('assigns, edits, completes, reopens, and deletes with contiguous CAS revisions', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Updated runtime' } })
    fireEvent.change(screen.getByPlaceholderText('Task description'), { target: { value: 'Updated details' } })
    fireEvent.change(screen.getByPlaceholderText(/Blocked byTo-dos/u), { target: { value: 'task-0' } })
    fireEvent.change(screen.getByPlaceholderText(/Write scopes/u), { target: { value: 'src/runtime' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
    ...
    fireEvent.click(screen.getByRole('button', { name: /Complete/u }))
    fireEvent.click(await screen.findByRole('button', { name: /Reopen/u }))
    await waitFor(() => {
  ...
  it('keeps reload failures visible after task and dependency conflicts', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /Edit/u }))
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Edited' } })
    fireEvent.change(screen.getByPlaceholderText(en.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  ...
  it('renders roster/task state variants and contains navigation, refresh, and close actions', async () => {
    ...
    expect(screen.getByText(en.ready)).toBeTruthy()
  ...
  it('shows load and create failures and ignores a create result after a session switch', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /New task/u }))
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Task' } })
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /New task/u }))
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Late task' } })
    fireEvent.change(screen.getByPlaceholderText('Task description'), { target: { value: 'Late description' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  ...
  it('contains edit and dependency failures and supports form cancellation and unassignment', async () => {
    ...
    expect(screen.queryByPlaceholderText('Task subject')).toBeNull()
    ...
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Saved edit' } })
    fireEvent.change(screen.getByPlaceholderText(en.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  ...
  it('skips the dependency mutation when an edit keeps the same blockers', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /Edit/u }))
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Same dependencies' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  ...
  it('reloads a dependency conflict and ignores dependency settlement after a session switch', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /Edit/u }))
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Conflict edit' } })
    fireEvent.change(screen.getByPlaceholderText(en.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
    expect(await screen.findByText(en.conflict)).toBeTruthy()
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /Edit/u }))
    fireEvent.change(screen.getByPlaceholderText('Task subject'), { target: { value: 'Late edit' } })
    fireEvent.change(screen.getByPlaceholderText(en.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
