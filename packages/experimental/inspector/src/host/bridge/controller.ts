...
export async function startInspector(options: InspectorOptions = {}): Promise<InspectorHandle> {
  ...
  const clientProtocol = `akx-inspector-v${String(INSPECTOR_PROTOCOL_VERSION)}-${randomBytes(32).toString('base64url')}`
  ...
  lifecycle.markRunning((error) => {
    try {
      ...
      console.error('akx inspector: Host source cleanup after Worker failure failed', closeError)
    }
    void fetchObserver?.stop().catch((stopError: unknown) => {
      console.error('akx inspector: fetch cleanup after Worker failure failed', stopError)
    })
    console.error('akx inspector: Worker stopped unexpectedly', error)
  })
