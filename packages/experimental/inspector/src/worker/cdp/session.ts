...
export class CdpSession implements NetworkSink {
  ...
  constructor(
    ...
    this.unsubscribeSources = sources.subscribeStatus(() => {
      if (this.diagnosticsEnabled) this.sendEvent('AkxInspector.sourcesChanged', { sources: this.sources.describe() })
    })
  }
  ...
  receive(value: unknown): void {
    ...
    try {
      ...
      } else if (request.method === 'AkxInspector.enable') {
        this.diagnosticsEnabled = true
        result = { sources: this.sources.describe() }
      } else if (request.method === 'AkxInspector.disable') {
        this.diagnosticsEnabled = false
        result = {}
      } else if (request.method === 'AkxInspector.getSources') {
        result = { sources: this.sources.describe() }
      } else if (request.method === 'AkxInspector.getCordisTree') {
