...
const bootstrap: InspectorClientBootstrap = {
  ...
  protocol: 'akx-inspector-v0-token',
  maxQueuedRecords: 16,
...
describe('experimental Inspector Client plugin', () => {
  ...
  afterEach(() => {
    ...
    delete globalThis.__AKX_INSPECTOR__
    Reflect.deleteProperty(globalThis, '__AKX_BOOT__')
  })

  it('provides ctx.inspector and sends observations after the Worker accepts the source', async () => {
    globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket
    globalThis.__AKX_INSPECTOR__ = bootstrap
    ...
    expect(JSON.parse(socket.sent[1]!) as unknown).toMatchObject({
      t: 'source/replace',
      records: [{ topic: '@akashx/cordis/tree', payload: { schemaVersion: 0, truncated: false } }],
    })
  ...
  it('rotates a copied session identity while its original page remains live', async () => {
    ...
    try {
      ...
      sessionStorage.setItem('akx.experimental-inspector.client-source-id.v0', first.sourceId)
  ...
  it('does not report queue loss again after a replacement absorbs it', async () => {
    globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket
    globalThis.__AKX_INSPECTOR__ = { ...bootstrap, maxQueuedRecords: 1 }
    const ctx = new Context()
    const fiber = ctx.plugin({ apply })
    await fiber.await()
    const socket = FakeWebSocket.sockets[0]!
  ...
  it('discovers and serves its built Client bundle through the source protocol', async () => {
    globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket
    globalThis.__AKX_INSPECTOR__ = bootstrap
    Reflect.set(globalThis, '__AKX_BOOT__', {
      rev: 'graph',
      entries: [{
        id: '@akashx/akx-experimental-inspector',
        url: '/plugins/@akashx/akx-experimental-inspector/client.js?rev=bundle-rev',
        rev: 'bundle-rev',
      }],
    })
    ...
    await vi.waitFor(() => {
      ...
      expect(script?.url).toContain('/plugins/@akashx/akx-experimental-inspector/client.js?rev=bundle-rev')
      expect(script?.sourceMapUrl)
        .toContain('/plugins/@akashx/akx-experimental-inspector/client.js.map?rev=bundle-rev')
