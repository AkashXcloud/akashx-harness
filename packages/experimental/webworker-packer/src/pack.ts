 ...
 * @module @akashx/akx-experimental-webworker-packer/src/pack
 */
...
} from '@akashx/akx-experimental-webworker-runtime'
import picomatch from 'picomatch'
import yaml from 'js-yaml'
import { entryListSchema } from '@akashx/cordis-plugin-include'
import { REPLACED_EXTERNAL_PACKAGES } from '@akashx/akx-experimental-webworker-runtime/src/node/external_packages/replaced-externals.ts'
import { MODULE_PROXIES, MODULE_PROXY_PREFIXES } from '@akashx/akx-experimental-webworker-runtime/src/module-proxies.ts'
...
export { DEFAULT_ROOT } from '@akashx/akx-experimental-webworker-runtime'
...
function sweepImage(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    try {
      ...
      const external = importer.startsWith('node_modules/') && !importer.startsWith('node_modules/@akashx/')
...
function materialize(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    for (const field of ['dependencies', 'peerDependencies'] as const) {
      ...
      // (@akashx/cordis is a peerDependency of every harness package),
