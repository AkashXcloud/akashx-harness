 ...
 * assumptions about pnpm workspaces or the `akx` CLI.
 * @module @akashx/akx-experimental-webworker-packer/src/repository
 */
import { execFileSync } from 'node:child_process'
import { existsSync, mkdtempSync, readFileSync, readdirSync, rmSync } from 'node:fs'
import { tmpdir } from 'node:os'
import { join, relative } from 'node:path'
import { AKX_HOME_ENV } from '@akashx/akx-home-paths'
...
/** Composition entry point package: the `akx` CLI, run from source. */
const CLI_PACKAGE = 'apps/cli'

/** Composition entry point: the `akx` CLI, run from source. */
 ...
 * layers only, so the image is the shipped profile: the machine's `$AKX_HOME`
...
export function composeProfile(repoRoot: string, profile: string): string {
  const home = mkdtempSync(join(tmpdir(), 'akx-pack-home-'))
  try {
    return execFileSync(
      process.execPath,
      ['--import', 'tsx/esm', join(repoRoot, CLI_ENTRY), '--profile', profile, '--dump-default-config'],
      { cwd: repoRoot, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024, env: { ...process.env, [AKX_HOME_ENV]: home } },
    )
  } finally {
 ...
 * (`akx.configTrees` in its package.json): `path` is relative to the CLI
...
export function configTrees(repoRoot: string): ConfigTree[] {
  ...
  const manifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as {
    akx?: { configTrees?: unknown }
  }
  const declared = manifest.akx?.configTrees
  if (declared === undefined) return []
  if (!Array.isArray(declared)) {
    throw new Error(`vfs image: ${CLI_PACKAGE} akx.configTrees must be an array`)
  }
  const mounts = new Set<string>()
  return declared.map((entry, index) => {
    const tree = entry as Partial<ConfigTreeDeclaration> | null
    const at = `${CLI_PACKAGE} akx.configTrees[${String(index)}]`
