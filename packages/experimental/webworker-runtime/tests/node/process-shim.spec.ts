...
describe('process shim', () => {
  it('publishes cwd, env, and version zero for the loader probe', () => {
    const shim = installProcessGlobal({ cwd: '/akx', env: { AKX_HOME: '/akx/home' } })
    expect(shim.cwd()).toBe('/akx')
    expect(shim.env.AKX_HOME).toBe('/akx/home')
    expect(shim.title).toBe('akx-webworker')
  ...
  it('answers getBuiltinModule from the module proxies and undefined otherwise', () => {
    ...
    const shim = installProcessGlobal({ cwd: '/akx', env: {} })
