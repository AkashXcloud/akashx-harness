...
import { spawn, spawnSync } from '@akashx/akx-experimental-webworker-runtime/src/node/builtin_modules/implemented/child_process.ts'
import {
  LAUNCHER_FAILURE_EXIT, grantArgs, launcherPath, probe,
} from '@akashx/node-addon-system/landlock-run'
import { processAlive, signalProcess } from '@akashx/akx-experimental-webworker-runtime/src/node/process-table.ts'
import { hostFileSystem } from '@akashx/akx-experimental-webworker-runtime/src/shell/fs-access.ts'
import {
  LANDLOCK_EXECUTABLE, landlockFileSystem, parseLandlockArguments,
} from '@akashx/akx-experimental-webworker-runtime/src/shell/process/landlock.ts'
import { spawnSubprocess } from '@akashx/akx-subprocess-local/src/spawn.ts'

vi.mock('node:child_process', async () =>
  await import('@akashx/akx-experimental-webworker-runtime/src/node/builtin_modules/implemented/child_process.ts'))

const WORKSPACE = '/akx/workspace'
const HOME = '/akx/home'
const TMP = '/akx/tmp'
...
it('keeps the native Landlock package API and CLI failure contract', async () => {
  ...
  const missingGrant = spawn(launcherPath(), ['--rw', '/akx/missing', '--', 'touch', `${WORKSPACE}/never`], { cwd: WORKSPACE })
  expect(await collect(missingGrant)).toEqual({
    stdout: '',
    stderr: 'landlock-run: cannot open rule path: /akx/missing: No such file or directory\n',
    code: 125,
  })
