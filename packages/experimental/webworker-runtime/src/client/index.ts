 ...
 * @module @akashx/akx-experimental-webworker-runtime/client
 */
import { IMAGE_FILE_NAME } from '../image-layout.ts'
import type { ClientFileUploadHooks } from '@akashx/akx-client-file-upload/types'
...
interface ClientFileUploadGlobal {
  __AKX_FILE_UPLOAD__?: ClientFileUploadHooks
}
...
interface BootReadyGlobal {
  __AKX_BOOT_READY__?: PromiseWithResolvers<void>
}

function bootReadyGate(): PromiseWithResolvers<void> {
  return (globalThis as BootReadyGlobal).__AKX_BOOT_READY__ ??= Promise.withResolvers<void>()
}
 ...
 * bundles, `__AKX_BOOT__`, the theme bootstrap — in table order. The
 * boot-readiness deferred (`__AKX_BOOT_READY__`) is installed before the
...
export async function connectWorkerHost(worker: Worker, options?: WorkerHostConnectOptions): Promise<WorkerHostConnection> {
  ...
  try {
    ...
    ;(globalThis as ClientTransportGlobal).__AKX_TRANSPORT__ = {
    ...
    ;(globalThis as ClientFileUploadGlobal).__AKX_FILE_UPLOAD__ = {
      fetch: (input, init) => tunnel.fetch(input, init),
    }
