...
import { AKX_TMP } from '../../storage/paths.ts'
...
function vfsPath(path: string, cwd: string): string {
  ...
  if (absolute === '/tmp') return AKX_TMP
  if (absolute.startsWith('/tmp/')) return `${AKX_TMP}${absolute.slice('/tmp'.length)}`
  return absolute
}
...
/** Throw the denial dialect consumed by `akx-bash-sandbox`. */
