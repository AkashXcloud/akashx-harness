 ...
 * @module @akashx/akx-experimental-webworker-runtime/src/worker-host
 */
import { setActiveModuleLoader, WorkerModuleLoader, type StaticModuleFactory } from './module-system/module-loader.ts'
import type { TypertGateway } from '@akashx/akx-api-gateway'
import type { HostConnectionHandle } from '@akashx/akx-client-connection'
...
export interface WorkerHostOptions {
  ...
  /** Environment for the process shim; `AKX_HOME` defaults to `<root>/home`. */
...
export function createWorkerHost(options: WorkerHostOptions): WorkerHost {
  ...
  const start = async (): Promise<void> => {
    try {
      ...
      installProcessGlobal({ cwd: root, env: { AKX_HOME: home, HOME: home, ...options.env } })
      ...
      const appBoot = require('@akashx/akx-app-boot') as {
        boot(
          binName: string,
          configPath: string,
          patches: unknown[],
      ...
      const cmdline = require('@akashx/akx-cmdline') as {
      ...
      const ctx = await appBoot.boot('akx-webworker', configPath, patches, (hostCtx) => {
...
export function installLogSink(ctx: HostContext, require: (specifier: string) => unknown): void {
  const { Logger } = require('@akashx/cordis') as { Logger: LogRenderer }
...
function bootPatches(
  ...
  if (configPath.endsWith('.json')) {
    ...
    const include = loader.load(loader.resolve('@akashx/cordis-plugin-include', root)) as { entryListSchema: unknown }
