 ...
 * @module @akashx/akx-experimental-webworker-runtime/src/node/globals/process
 */
...
export interface ProcessShimOptions {
  ...
  readonly cwd: string
  /** Environment the tree reads; `AKX_HOME` belongs here. */
...
export function installProcessGlobal(options: ProcessShimOptions): ProcessShim {
  ...
  const shim: ProcessShim = {
    env: { ...options.env },
    argv: [...(options.argv ?? ['node', 'akx-webworker'])],
    execArgv: [],
    title: 'akx-webworker',
    platform: 'linux',
    arch: 'x64',
