...
interface KoffiType {
  readonly __akxKoffiType: string
...
const token = (label: string, size: number, alignment = Math.min(size, 8) || 1): KoffiType =>
  ({ __akxKoffiType: label, size, alignment })
...
const typeOf = (target: unknown): KoffiType => {
  ...
  if (descriptor?.__akxKoffiType === undefined) {
...
const describe = (target: unknown): string =>
  typeof target === 'string' ? target : (target as KoffiType | undefined)?.__akxKoffiType ?? 'anonymous'
...
function array(target: unknown, length: number): KoffiType {
  ...
  return token(`array(${element.__akxKoffiType}, ${String(length)})`, element.size * length, element.alignment)
}
