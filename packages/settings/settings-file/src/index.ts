 ...
 * leaf-level diff.
 * @module @akashx/akx-settings-file
 */
...
import { SettingsProvider, type SettingsNamespace } from '@akashx/akx-settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveAkxHome(config.akxHome), 'settings.yaml'))
