...
describe('real Loader composition', () => {
  it('registers on a real WebServer and dispatches a signed request', { timeout: 60_000 }, async () => {
    root = await mkdtemp(join(tmpdir(), 'akx-webhook-github-loader-'))
    ...
    await writeFile(configPath, [
      ...
      "    host: '127.0.0.1'",
      '    port: 0',
      "- name: '@akashx/akx-webhook-github'",
      '  config:',
      '    source: loader',
      '    path: /github',
      '    secretEnv: AKX_GITHUB_WEBHOOK_SECRET',
      '    maxBodyBytes: 1024',
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@akashx/akx-webhook-github', GitHubAdapter],
    ])
