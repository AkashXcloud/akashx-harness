...
```ts
import { resolveAkxHome, akxHomePath, akxCachePath } from '@akashx/akx-home-paths'

const home = resolveAkxHome()                // configured path, else $AKX_HOME, else ~/.akx
const settings = akxHomePath('settings')     // join one child onto the resolved home
const cache = akxCachePath('models')         // $AKX_HOME/cache/models, default ~/.akx/cache/models
```

An explicit configured path has the highest precedence, then `$AKX_HOME`, then the default `~/.akx`. An empty or whitespace-only `$AKX_HOME` is treated as unset, so a blank override never resolves the home to the current working directory.

`akxCachePath(...segments)` derives paths from the resolved home's `cache` directory. With no segments it returns the cache directory itself. Pass an initial options object, `akxCachePath({ akxHome: home }, ...segments)`, to use an explicit configured home with the same precedence and tilde expansion. It returns an absolute path without creating directories.

### Displaying a home

For user-facing paths, render the root symbolically rather than as a machine path: the default home displays as `~/.akx` and any configured home displays as `$AKX_HOME`. The display form never leaks an absolute machine path.
...
`resolveAkxHome` reads the explicit override, then `$AKX_HOME`, then falls back to the operating-system home joined with `.akx`. The chosen value is tilde-expanded and normalized to an absolute path; `akxHomePath` joins child segments with Node's platform path rules. `akxHomeDisplay` compares the resolved path against the default root and returns the symbolic label, so a configured home never leaks its absolute path.
...
- [Shell environment](../../shell/shell-env/README.md) — how `AKX_HOME` reaches model shell calls.
