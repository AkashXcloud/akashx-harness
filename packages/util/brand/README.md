...
```ts
import { brandString, type Branded } from '@akashx/akx-brand'
...
```ts
import { brandNumber, type BrandedNumber } from '@akashx/akx-brand'
...
Brand values that cross package boundaries and could plausibly be confused — `ToolCallId` in `akx-llm`, the shared agent/session `SessionId` in `akx-session`, `JobId` in `akx-jobs`, and `SessionSeq` versus `SessionLogOffset` in `akx-session`. Values that stay local or cannot be confused do not need this abstraction.
...
Keeping these helpers in their own package means `akx-jobs` can brand `JobId` without importing an unrelated capability package, while each capability still owns the meaning and validation of its concrete ids.
