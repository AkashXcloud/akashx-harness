...
const ALPHA = credentialRef('AKX_REVIEW_ALPHA')
const BETA = credentialRef('AKX_REVIEW_BETA')
const INNER = credentialRef('AKX_REVIEW_INNER')
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'akx-cred-review-'))
...
describe('document editor', () => {
  it('leaves a sibling multi-line value untouched while patching one entry', async () => {
    ...
    expect(await readFile(path, 'utf8'))
      .toBe(`version: 1\nrefs:\n  AKX_REVIEW_WRAPPED: |-\n    line1\n    line2\n  ${ALPHA}: b\n`)
    expect(await ctx.credentials.resolve(credentialRef('AKX_REVIEW_WRAPPED')))
      .toEqual({ value: 'line1\nline2', source: 'file' })
  })
