...
import { credentialKey, credentialKeyScope, credentialRef, parseCredentialKey } from '@akashx/akx-credentials'
import type { CredentialKey, CredentialRecord } from '@akashx/akx-credentials'
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'akx-cred-records-'))
...
describe('record storage', () => {
  ...
  it('keeps references and records in one document without either disturbing the other', async () => {
    ...
    await ctx.credentials.set(credentialRef('AKX_RECORDS_KEY'), 'sk-live')
    await put(ctx, CODEX, { kind: 'grant', payload: { token: 't' } })

    const text = await readFile(path, 'utf8')
    expect(text).toBe(
      'version: 1\nrefs:\n  AKX_RECORDS_KEY: sk-live\nrecords:\n'
      + '  llm-pi-ai/openai-codex:\n    kind: grant\n    payload:\n      token: t\n',
    )
    expect(await ctx.credentials.resolve(credentialRef('AKX_RECORDS_KEY'))).toEqual({ value: 'sk-live', source: 'file' })
  })
