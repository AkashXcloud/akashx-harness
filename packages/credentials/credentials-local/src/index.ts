/**
 * File-backed credentials provider over `$AKX_HOME/.credentials.yaml`, layered
 ...
 * > $AKX_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `AKASHX_API_KEY=… akx`, a CI
 ...
 * @module @akashx/akx-credentials-local
 */
...
import { canonicalizeWatchPath, resolveAkxHome } from '@akashx/akx-home-paths'
import { launchEnvironmentOf } from '@akashx/akx-launch-environment'
import { CredentialProvider, credentialRef, parseCredentialKey } from '@akashx/akx-credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
  ...
  CredentialRef,
  ResolvedCredential,
} from '@akashx/akx-credentials'
import type { LaunchEnvironmentEntry } from '@akashx/akx-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveAkxHome(config.akxHome), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `akx-atomic-write`, this is a
...
/** File-backed credentials provider (`$AKX_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start akx from instead',
      )
    }
  }
