 ...
 *
 * @module @akashx/akx-api-settings-controller/src/credentials.ts
 */

import { Context } from '@akashx/cordis'
import { credentialRef } from '@akashx/akx-credentials'
import type { CredentialProvider } from '@akashx/akx-credentials'
  ...
  private provider(): CredentialProvider {
    ...
    if (credentials === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'credentials service is absent: this deployment does not mount a credential provider (e.g. @akashx/akx-credentials-local) in its composition',
        {},
      )
    }
    return credentials
