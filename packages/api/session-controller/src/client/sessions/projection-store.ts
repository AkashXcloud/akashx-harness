...
import type { SessionSeqCursor } from '@akashx/akx-session/types'
...
// (`/types`, zero imports), never the package root: the root's akx-agent →
// akx-session chain would drag the host `Context.sessions` merge into the
...
export type { SessionProjectionMap } from '@akashx/akx-session-projection/types'
