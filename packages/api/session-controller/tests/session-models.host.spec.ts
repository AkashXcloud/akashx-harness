...
import LlmRuntime, { LlmAdapter, ReasoningEffortId } from '@akashx/akx-llm'
import type {
...
async function harness(logged?: {
  ...
  ctx.llm.registerAdapter(['akashx-official'], new CatalogAdapter('AkashX', [
    { provider: 'akashx-official', id: 'akashx-chat', name: 'AkashX Chat' },
    { provider: 'akashx-official', id: 'akashx-reasoner', name: 'AkashX Reasoner', description: 'Reasoning model' },
  ], REASONING))
...
describe('Web session model selection', () => {
  it('validates an ordered image batch before persisting any member', async () => {
    ...
    const remote = createSessionTestRemote(ctx, {
      defaultModelSelection: () => ({ provider: 'akashx-official', model: 'akashx-chat' }),
      cwd: '/tmp',
    })
  ...
  it('groups successful providers and leaves an unlisted current selection out of the catalog', async () => {
    ...
    const remote = createSessionTestRemote(ctx, { defaultModelSelection: () => ({ provider: 'akashx-official', model: 'akashx-chat' }), cwd: '/tmp' })
    ...
    expect(catalog.groups).toEqual([{
      ...
      name: 'AkashX',
      models: [
        { id: 'akashx-chat', name: 'AkashX Chat', reasoning: REASONING },
        {
          id: 'akashx-reasoner',
          name: 'AkashX Reasoner',
  ...
  it('accepts an advisory-unlisted model, rejects an unavailable provider, and switches only after the next assembly', async () => {
    ...
    expect(currentSelection(ctx, sessionId))
      .toEqual({ provider: 'akashx-official', model: 'akashx-chat' })
    ...
    expect((await ctx.systemPrompt.assemble()).variables)
      .toMatchObject({ provider: 'akashx-official', model: 'private-preview' })
    await expect(agentEvents(ctx, agent).waterfall(
    ...
    expect(unsupported).toMatchObject({
      ...
      error: {
        ...
        message: 'provider "akashx-official" model "private-preview" does not support reasoning effort "medium"',
      },
    })
    ...
    expect(currentSelection(ctx, sessionId))
      .toEqual({ provider: 'akashx-official', model: 'private-preview', reasoningEffort: 'max' })
    await ctx.fiber.dispose()
  })

  it('reads the Agent default live for a session whose log names no selection', async () => {
    const { ctx, sessionId } = await harness()
    let stored = { provider: 'akashx-official', model: 'akashx-chat' }
    createSessionTestRemote(ctx, {
    ...
    stored = { provider: 'akashx-official', model: 'akashx-reasoner' }
    expect(currentSelection(ctx, sessionId))
      .toEqual({ provider: 'akashx-official', model: 'akashx-reasoner' })
    await ctx.fiber.dispose()
  })
  ...
  it('saves an accepted selection as the default and survives a storage failure', async () => {
    ...
    expectValue(await remote.selectModel(request({
      sessionId, provider: 'akashx-official', model: 'akashx-reasoner', reasoningEffort: 'max',
    })))
    expect(saved).toEqual([
      { provider: 'akashx-official', model: 'akashx-reasoner', reasoningEffort: 'max' },
    ])
    ...
    const stillAccepted = expectValue(await remote.selectModel(request({
      sessionId, provider: 'akashx-official', model: 'akashx-chat',
    })))
    expect(stillAccepted.selected).toEqual({ provider: 'akashx-official', model: 'akashx-chat', reasoningEffort: 'high' })
    expect(currentSelection(ctx, sessionId))
      .toEqual({ provider: 'akashx-official', model: 'akashx-chat', reasoningEffort: 'high' })
    await ctx.fiber.dispose()
  })
  ...
  it('refuses a prompt no adapter can route, and reports it on the directory', async () => {
    ...
    expectValue(await remote.selectModel(request({
      sessionId, provider: 'akashx-official', model: 'unlisted-but-served',
    })))
    const catalog = await buildModelCatalog(ctx)
