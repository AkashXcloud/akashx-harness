...
import { SESSION_FORMAT_VERSION, SessionLogOffset, SessionSeq } from '@akashx/akx-session'
...
import { subagentIdentityProjectionDefinition } from '@akashx/akx-subagent/src/projection.ts'
import TypertRegistry from '@akashx/akx-typert-registry'
import { createUserMessage, MessageId } from '@akashx/akx-llm'
import { snapshotSubagentDescriptor } from '@akashx/akx-subagent'
import { createInboxStub } from '@akashx/akx-agent-loop-testkit'
import type { Agent, Inbox } from '@akashx/akx-agent'
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@akashx/akx-session').Session
