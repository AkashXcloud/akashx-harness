 ...
 * `akx-resource://file/session/<sessionId>/<path>`, carries an absolute path or one
 ...
 * An `absolute` address, `akx-resource://file/absolute/<path>`, carries no
...
import { parseFileAddress } from '@akashx/akx-util-workspace-path'
...
function unsupportedAddress(address: string): RemoteError<'workspace-file/unsupported-address'> {
  return new RemoteError(
    ...
    `${address} is not a akx-resource://file/session/<sessionId>/<path> or akx-resource://file/absolute/<path> address`,
    { address },
  )
}
...
function unknownWorkspace(address: string): RemoteError<'workspace-file/unknown-workspace'> {
  return new RemoteError(
    ...
    `${address} requires a akx-resource://file/session/<sessionId>/<path> address`,
    { address },
  )
}
