 ...
 * harness home resolved by {@link resolveAkxHome} (`$AKX_HOME` > `~/.akx`),
 ...
 * machine: every process sharing one `$AKX_HOME` reports the same id, and
 ...
 *
 * @module @akashx/akx-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `AKX_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveAkxHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
