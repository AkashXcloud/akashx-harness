---
description: "Anonymous per-harness-home identity for users and maintainers tracing how telemetry, feedback acknowledgement, and AkashX provider requests correlate records."
...
When you want the records your installation sends out to be recognizable as coming from the same harness home — telemetry, feedback, and AkashX requests all carry one shared id — this package is what provides it. There is nothing to install or configure: the id appears automatically, and the shipped feedback, telemetry, and AkashX features already use it. Do not use it to identify a user or to join records across different homes; it is anonymous and home-scoped.
...
- **AkashX requests** — every provider request carries the `x-akx-harness-user-id` header, so usage can be attributed per installation.

### Observing and resetting the id

The id lives in `$AKX_HOME/.anonymous-user-id` (`$AKX_HOME` defaults to `~/.akx`) as a plain UUID text file. Delete that file to get a fresh id at the next launch; the running process keeps its current id until it exits. Separate harness homes keep separate ids, and no machine or account detail ever goes into the value.

### Using it in your own package

When you build a feature that should share the installation's anonymous id, import the value once and reuse it — telemetry, feedback, and AkashX already use the same id, so your records line up with theirs:
...
- [akx-home-paths](../../util/home-paths/README.md) — owns `$AKX_HOME` and `~/.akx` resolution.
- [akx-session-telemetry-otel](../../session/session-telemetry-otel/README.md) — reports the id as the OTel Resource `user.id`.
- [akx-command-feedback](../../feedback/command-feedback/README.md) — embeds the id in the feedback acknowledgement.
- [akx-llm-akx](../../llm/llm-akx/README.md) — sends `x-akx-harness-user-id` on provider requests.
...
None, as the shared identifier reaches AkashX only as model-hidden HTTP metadata and registers nothing model-facing.
...
- **No cross-home identity** — different `$AKX_HOME` values cannot be correlated.
- **Configured AkashX gateways receive the id** — `akx-llm-akx` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
