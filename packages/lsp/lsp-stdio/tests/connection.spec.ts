...
import { LspConnection } from '@akashx/akx-lsp-stdio'
import type { ConnectionWriter } from '@akashx/akx-lsp-stdio/src/connection.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit AKX_* env entries to the child', async () => {
    // A configured AKX_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'AKX_LSP_TEST_FACT', AKX_LSP_TEST_FACT: 'managed' })
