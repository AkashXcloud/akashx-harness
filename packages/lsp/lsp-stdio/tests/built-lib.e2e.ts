...
/**
 * Keyless built-artifact smoke: plain Node imports `@akashx/akx-lsp` and
 * `@akashx/akx-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@akashx/akx-lsp')
      const LspLocal = await import('@akashx/akx-lsp-stdio')
      const { default: LocalFileSystem } = await import('@akashx/akx-fs-local')
