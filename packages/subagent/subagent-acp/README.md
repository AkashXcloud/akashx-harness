...
The generated [configuration catalog](../../../docs/config-catalog.md#akashx-akx-subagent-acp) is the exhaustive source for every accepted field and its JSDoc.

A AkashX Harness child uses the product launcher and an explicit absolute `AKX_HOME`. The isolated home prevents a nested runtime from discovering the launching person's profiles or credentials; the generic ACP provider does not impose this requirement on non-AKX agents.

```yaml
- id: subagent-acp
  name: '@akashx/akx-subagent-acp'
  config:
    providerName: acp
    command: akx
    ...
    permission: reject
    env:
      AKX_HOME: /absolute/path/to/isolated-child-home
...
- [akx-subprocess seam](../../subprocess/subprocess/README.md) — the process-spawn and teardown machinery behind each run.
- [Generated configuration catalog](../../../docs/config-catalog.md#akashx-akx-subagent-acp) — every accepted config field and its source declaration.
