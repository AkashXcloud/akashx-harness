 ...
 * quiescent disposal.
 *
 * @module @akashx/akx-subagent-acp/run
 */
...
import type { SubprocessHandle, SubprocessOutcome, SubprocessSpawnSpec } from '@akashx/akx-subprocess'
...
export interface AcpRunSpec {
   ...
   * `AKASHX_API_KEY`). Merged on top of the subprocess seam's scrubbed
   ...
   * Explicit `AKX_*` entries are deployment-owned facts for the child harness
   * (e.g. `AKX_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and AKX_* names
  ...
  const clientApp = createAcpClientApp({ name: 'akx-harness-subagent-acp' })
