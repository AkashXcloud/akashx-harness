...
/**
 * With-key cross-process boundary proof: the backend spawns the real akx ACP profile, speaks ACP over
 * stdio, and returns its real model answer. This is the out-of-process counterpart to in-process
 * spawn coverage and self-skips without `AKASHX_API_KEY`.
 */

// The real ACP profile: akx plus the example's live AkashX patch.
...
// How to launch the child ACP profile (src via tsx / lib via plain node, per AKX_EXAMPLE_MODE).
...
function resolveChildLaunch(akxHome: string) {
  return resolveExampleLaunch({
    ...
    env: {
      ...process.env.AKASHX_API_KEY !== undefined ? { AKASHX_API_KEY: process.env.AKASHX_API_KEY } : {},
      ...process.env.AKASHX_BASE_URL !== undefined ? { AKASHX_BASE_URL: process.env.AKASHX_BASE_URL } : {},
      AKX_HOME: akxHome,
...
describe.skipIf(!process.env.AKASHX_API_KEY)('ACP backend with-key e2e (drive our own acp-agent)', () => {
  it('drives the real acp-agent example process to answer a prompt', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'akx-subagent-acp-e2e-'))
    const childLaunch = resolveChildLaunch(join(workdir, '.akx-child'))
    ctx = new Context()
