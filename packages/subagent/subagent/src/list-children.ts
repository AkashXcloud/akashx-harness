 ...
 *
 * @module @akashx/akx-subagent
 */

import type { Context } from '@akashx/cordis'
import { SessionLogOffset } from '@akashx/akx-session'
import type { Session, SessionHeader, SessionId } from '@akashx/akx-session'
import type { SessionProjectionRegistry } from '@akashx/akx-session-projection'
import type { SessionProjectionCache } from '@akashx/akx-session-projection-cache'
import type { SessionObservation, SessionQueryEngine } from '@akashx/akx-session-query'
...
async function prepareListing(
  ...
  if (projections === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionProjections registry (load @akashx/akx-session-projection)',
  ...
  if (sessions === undefined) {
    throw new SubagentError(
      'listing subagents requires the session store (load @akashx/akx-session)',
  ...
  if (query === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionQuery service (load @akashx/akx-session-query)',
