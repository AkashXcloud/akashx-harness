...
describe.skipIf(!process.env.AKASHX_API_KEY)('spawn backend with-key smoke', () => {
  it('a parent delegates to a child that writes a file on disk', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'akx-subagent-spawn-e2e-'))
    ctx = await spawnHarness(workdir)
    const parent = await ctx.agentLoop.create(SessionId('e2e-parent'), { provider: 'akashx-official', model: 'akashx-v4-flash' })
