...
```sh
akx plugin --profile <name> add @akashx/akx-subagent-codex
akx plugin --profile <name> remove @akashx/akx-subagent-codex
akx --profile <name>
```
...
- [Generated configuration catalog](../../../docs/config-catalog.md#akashx-akx-subagent-codex) — every accepted config field and its source declaration.
...
- **Compatibility is pinned by development evidence** — upgrading from the verified 0.153.4 protocol baseline requires regenerating upstream schema evidence and rerunning handshake, answer-selection, approval, cancellation, keyless real-product, and credentialed AkashX nonce tests.
- **No human approval path** — known unattended approval requests are denied and unknown server requests fail closed; the three Profile modes never create a AKX interaction channel or per-call allow policy.
