...
/** One running test-only Responses-to-AkashX bridge. */
export interface AkashXResponsesBridge {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on('data', (chunk: string) => {
      ...
      if (Buffer.byteLength(body) > MAX_REQUEST_BYTES) {
        request.destroy(new Error('AkashX bridge request exceeded its byte limit'))
      }
    })
...
function deepSeekBaseUrl(): string {
  ...
  if (configured !== OFFICIAL_AKASHX_BASE_URL) {
    throw new Error('Codex AkashX e2e requires the official AkashX base URL')
  }
  return configured
}

async function completeWithAkashX(
  ...
  if (!response.ok) {
    ...
    throw new Error(`AkashX bridge upstream returned HTTP ${response.status}`)
  }
  ...
  if (typeof content !== 'string' || content.trim().length === 0) {
    throw new Error('AkashX bridge upstream returned no text')
  }
  return content
}
 ...
 */
export async function startAkashXResponsesBridge(
  nonce: string,
): Promise<AkashXResponsesBridge> {
  let seenRequests = 0
  ...
  const server = createServer((request, response) => {
    ...
    void (async () => {
      ...
      if (
        ...
        throw new Error('Codex AkashX bridge received no bearer credential')
      }
      const body = JSON.parse(await readRequest(request)) as Record<string, unknown>
      const task = taskText(body)
      if (!task.includes(nonce)) {
        throw new Error('Codex AkashX bridge request omitted the expected nonce')
      }
      const text = await completeWithAkashX(authorization, task)
      completedRequests += 1
      response.writeHead(200, {
        'content-type': 'text/event-stream',
        'cache-control': 'no-cache',
        connection: 'keep-alive',
        'x-request-id': 'req_akashx_e2e',
      })
      ...
      response.end(JSON.stringify({ error: { message: 'AkashX bridge request failed' } }))
    })
  })
  ...
  if (address === null || typeof address === 'string') {
    throw new Error('AkashX bridge did not acquire a TCP port')
  }
  return {
