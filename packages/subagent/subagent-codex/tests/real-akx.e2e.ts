...
import {
  startAkashXResponsesBridge,
  type AkashXResponsesBridge,
} from './akx-responses-bridge.ts'
...
const bridges: AkashXResponsesBridge[] = []
...
describe.skipIf(!process.env.AKASHX_API_KEY)(
  'Codex provider with real AkashX API',
  () => {
    it('returns one unique nonce through the production provider and real Codex', async () => {
      ...
      const root = mkdtempSync(join(tmpdir(), 'akx-codex-akashx-e2e-'))
      roots.push(root)
      const workspace = join(root, 'workspace')
      const codexHome = join(root, 'codex-home')
      mkdirSync(workspace)
      mkdirSync(codexHome)
      const nonce = `AKX_CODEX_AKASHX_${randomUUID()}`
      const bridge = await startAkashXResponsesBridge(nonce)
      bridges.push(bridge)
      writeFileSync(join(codexHome, 'config.toml'), [
        'model = "akashx-v4-flash"',
        'model_provider = "akashx-e2e"',
        'approval_policy = "never"',
        'sandbox_mode = "read-only"',
        'disable_response_storage = true',
        'check_for_update_on_startup = false',
        '',
        '[model_providers.akashx-e2e]',
        'name = "AkashX E2E bridge"',
        `base_url = "${bridge.baseUrl}"`,
        'env_key = "AKASHX_API_KEY"',
        'wire_api = "responses"',
