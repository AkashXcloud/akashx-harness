...
const configPath = fileURLToPath(new URL('akx-sdk.patch.yml', fixtureDir))
...
async function childLaunch(failure = false): Promise<{
  ...
  const childHome = await mkdtemp(join(tmpdir(), 'akx-sdk-subagent-home-'))
  ...
  return {
    ...
    env: {
      ...
      ...(failure ? { AKX_TEST_CHILD_FAILURE: '1' } : {}),
    },
  }
}
...
describe('SDK subagent routing and diagnostics through the production profile', () => {
  it('runs the selected child route in the parent session workspace', async () => {
    ...
    try {
      const { stderr } = await runLoaderSmoke({
        label: 'akx-sdk-subagent cwd composition smoke',
        tempDirPrefix: 'akx-sdk-subagent-cwd-e2e-',
        binScript: driver,
        libBinScript: driver,
        configPath,
        ...
        env: {
          ...child.env,
          AKX_TEST_CHILD_DEFAULT_ROUTE: '1',
          AKX_TEST_PARENT_MODEL_RECORD: '.parent-model-routes',
        },
        inspect: async (cwd) => {
  ...
  it('presents the child error diagnostic separately from partial output', async () => {
    ...
    try {
      const { stderr } = await runLoaderSmoke({
        label: 'akx-sdk-subagent diagnostic composition smoke',
        tempDirPrefix: 'akx-sdk-subagent-diagnostic-e2e-',
        binScript: driver,
        libBinScript: driver,
        configPath,
        tsconfigPath: repoTsconfig,
      ...
      expect(toolResultText(events)).toBe(
        ...
        + 'Diagnostic: Subagent failure (provider: AKX SDK; stage: session-run; category: child-error)\n'
