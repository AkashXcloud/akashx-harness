/**
 * Fresh-process SDK subagent client. Drives one child AkashX Harness
 * runtime over stdio JSON-RPC through `@akashx/akx-sdk-client` and owns
 ...
 *
 * @module @akashx/akx-subagent-akx-sdk/run
 */
...
import type { SessionEvent, SessionId, TurnEndReason } from '@akashx/akx-session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@akashx/akx-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@akashx/akx-subagent'
...
export interface SdkRunSpec {
  /** Explicit akx CLI module; omission resolves the SDK client's same-version dependency. */
  akxBin?: string
  /** Named child profile. */
  profile: string
   ...
   * runtime's own `AKASHX_API_KEY`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `AKX_*` fact survives while ambient namesakes never leak.
   */
...
class SdkRunFailure extends Error {
  constructor(readonly facts: SdkFailureFacts, cause: unknown) {
    super(`subagent-akx-sdk: ${failureDiagnostic(facts)}`, { cause })
    this.name = 'SdkRunFailure'
  }
}

/** Runtime constructor seam replaced only by package-local fake-runtime tests. */
export const internals: { createHarness(options: AkashXHarnessOptions): AkashXHarness } = {
  createHarness: options => new AkashXHarness(options),
}
 ...
 * @returns an Error whose message contains only the fixed AKX SDK failure line.
 */
...
export async function startSdkRun(request: SubagentStartRequest, spec: SdkRunSpec): Promise<SubagentRun> {
  ...
  const harness = internals.createHarness({
    ...spec.akxBin === undefined ? {} : { akxBin: spec.akxBin },
    profile: spec.profile,
    patches: spec.patches,
    akxHome: spec.akxHome,
    processCwd: spec.cwd,
