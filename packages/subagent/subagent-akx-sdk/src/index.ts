/**
 * Out-of-process SDK subagent backend. Each child is a complete AkashX
 ...
 * @module @akashx/akx-subagent-akx-sdk
 */
...
import type { AgentOptions } from '@akashx/akx-agent'
import type { SubagentCapabilities, SubagentProvider, SubagentStartRequest } from '@akashx/akx-subagent'
import { assertPositiveFinite, NO_START_CAPABILITIES, resolveChildCwd, validateConfiguredCwd } from '@akashx/akx-subagent'
import {
...
export const name = 'subagent-akx-sdk'
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('akx-sdk'),
  akxBin: z.string(),
  profile: z.string().default('sdk'),
  patches: z.array(z.string()).default([]),
  akxHome: z.string().required(),
  cwd: z.string(),
  provider: z.string().default('akashx-official'),
  model: z.string().default('akashx-v4-flash'),
...
type ResolvedConfig = Required<Omit<Config, 'cwd' | 'maxTokens' | 'akxBin'>> & Pick<Config, 'cwd' | 'maxTokens' | 'akxBin'>
...
function resolveConfiguredFile(field: string, value: string): string {
  ...
  throw new TypeError(`subagent-akx-sdk ${field} must name an existing file: ${path}`)
}

/** AKX SDK can apply Agent route options while the other start features remain child-owned. */
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    ...
    let cwd: string
    try {
      cwd = resolveChildCwd('subagent-akx-sdk', this.config.cwd, request.parent.session.header.cwd)
    } catch (error: unknown) {
      const failure = sdkConfigurationFailure(error)
      this.ctx.logger.warn(`subagent-akx-sdk "${this.name}": child start failed: %o`, error)
      throw failure
    }
    const route = resolveSdkRoute(this.config, request.agentOptions)
    const spec: SdkRunSpec = {
      ...this.config.akxBin === undefined ? {} : { akxBin: this.config.akxBin },
      profile: this.config.profile,
      patches: this.config.patches,
      akxHome: this.config.akxHome,
      cwd,
      ...route,
      env: this.config.env,
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-akx-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-akx-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-akx-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-akx-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-akx-sdk maxTokens must be a positive safe integer')
  }
  if (!isAbsolute(resolved.akxHome)) throw new TypeError('subagent-akx-sdk akxHome must be an absolute path')
  const launchPaths: ResolvedConfig = {
    ...resolved,
    patches: resolved.patches.map((path, index) => resolveConfiguredFile(`patches[${String(index)}]`, path)),
    ...resolved.akxBin === undefined ? {} : { akxBin: resolveConfiguredFile('akxBin', resolved.akxBin) },
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-akx-sdk', resolved.cwd)
