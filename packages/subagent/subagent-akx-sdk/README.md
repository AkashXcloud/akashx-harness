...
Mount this provider when a delegation should run as a complete Harness runtime in its own process. The common path is explicit: mount the seam, mount this provider, configure `akxBin` when needed, and select the child `profile` and ordered `patches`.
...
The provider advertises `agentOptions: true`, with `outputSchema`/`depthLimit`/`toolFilter`/`persona` false, and `inheritsParentContext: false`. Its immutable `agentRouteDefaults` publish the configured provider/model baseline to `akx-tool-subagent` before model overrides and exact-route preflight; `start()` independently applies the same configuration defaults for direct callers and `maxTokens`. Agent route values cross the SDK wire as an explicit whitelist; the child remains a fresh runtime in another process, and the only value derived from the parent agent itself is the workspace cwd. `akx-tool-subagent` deployments over this provider set `maxDepth: 'provider-managed'` — the child harness owns its own recursion budget.

### Configuration

| Field | Default | Meaning |
|---|---|---|
| `providerName` | `akx-sdk` | Registry name on `ctx.subagents` |
| `akxBin` | SDK dependency | Explicit akx CLI module, resolved and checked at plugin load; omission uses the SDK dependency |
| `profile` | `sdk` | Named child profile |
| `patches` | `[]` | Ordered per-launch profile patch files, resolved and checked at plugin load |
| `akxHome` | required | Absolute isolated Harness home for every nested child process |
| `cwd` | parent session cwd | Working-directory override for the child process and its SDK session |
| `provider` | `akashx-official` | Provider route sent in the child's `initialize` |
| `model` | `akashx-v4-flash` | Model sent in the child's `initialize` |
...
The generated [configuration catalog](../../../docs/config-catalog.md#akashx-akx-subagent-akx-sdk) is the exhaustive source for every accepted field and its JSDoc.
...
```yaml
- id: subagent-akx-sdk
  name: '@akashx/akx-subagent-akx-sdk'
  config:
    providerName: akx-sdk
    profile: sdk
    patches: ['./profiles/research-child.cordis.yml']
    akxHome: !!js akxHomePath('children')
    maxTokens: 49152
    env:
      AKASHX_API_KEY: !!js process.env.AKASHX_API_KEY
- id: tool-subagent
  name: '@akashx/akx-tool-subagent'
  config: { provider: akx-sdk, toolName: subagent, maxDepth: 'provider-managed' }
```
...
- [akx-subagent seam](../subagent/README.md) — the registry and start API this provider registers on.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#akashx-akx-subagent-akx-sdk) — every accepted config field and its source declaration.
