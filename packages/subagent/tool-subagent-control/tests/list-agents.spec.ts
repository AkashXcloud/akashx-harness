...
import type { SubagentListEntry } from '@akashx/akx-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'akx-tool-list-agents-'))
  roots.push(root)
...
describe('akx-tool-subagent-control/list-agents', () => {
