 ...
 *
 * @module @akashx/akx-agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@akashx/cordis'
...
import type { SessionEvent, SessionId, SessionLogOffset } from '@akashx/akx-session'
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in akx-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `akx-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `akx-agent-loop` package.
 */
 ...
 * (`@akashx/akx-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@akashx/akx-agent#Agent',
