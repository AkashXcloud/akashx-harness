 ...
 * execution pipeline.
 * @module @akashx/akx-tools
 */

import { Context, Service } from '@akashx/cordis'
import z from '@akashx/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@akashx/akx-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@akashx/akx-scope'
import type { ToolCallId, ContentBlock, ToolSchema } from '@akashx/akx-llm'
...
import { assertNever, deepFreeze, snapshotJsonValue, type JsonValue } from '@akashx/akx-util-values'
import type { ToolProviderResult } from '@akashx/akx-system-prompt'
import type { CodeRuntime } from '@akashx/akx-code-runtime'
 ...
 * instead of deriving them: the seam's `akx-code-runtime` README pair, its
...
// lives in its own UI-facing module; re-export it so `@akashx/akx-tools`
...
declare module '@akashx/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@akashx/akx-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@akashx/akx-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@akashx/akx-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@akashx/akx-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@akashx/akx-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `akx-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`akx-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`akx-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @akashx/akx-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`akx-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `akx-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('akx.tool.execution') as ToolExecutionToken
}
