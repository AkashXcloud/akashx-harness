 ...
 * @module @akashx/akx-tools/src/ptc
 */

import { brandString } from '@akashx/akx-brand'
import { createUserMessage, HarnessError } from '@akashx/akx-llm'
import type { ContentBlock, ToolCallId } from '@akashx/akx-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@akashx/akx-code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`akx-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
