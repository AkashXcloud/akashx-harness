...
import { createScope, scopeTarget } from '@akashx/akx-scope'
import { createSystemMessage, createUserMessage, ToolCallId, createMessage, createToolResultMessage, freezeMessage } from '@akashx/akx-llm'
import SessionStore, { SessionId, SessionSeq, TOOL_NOT_STARTED } from '@akashx/akx-session'
...
describe('session-log invariants', () => {
  ...
  it('requires a system/message to name the open step', async () => {
    ...
    session.append('turn/start', { turn: 1 })
    const message = createSystemMessage('You are terse.', '@akashx/akx-system-prompt')
