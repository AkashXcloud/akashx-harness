...
export class WebRuntime extends Service {
   ...
   * `$AKX_WEB_SEARCH_PROVIDER` / `$AKX_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.AKX_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.AKX_WEB_FETCH_PROVIDER
  }
