...
import { AkashXSearchProvider } from '../src/provider.ts'
describe('akashx search egress', () => {
  it('goes through the proxy', async () => {
    const p = new AkashXSearchProvider(() => ({ apiKey: 'probe', baseURL: 'http://dsk-probe.invalid', model: 'm', apiVersion: '2023-06-01', maxTokens: 16, maxUses: 1 }))
