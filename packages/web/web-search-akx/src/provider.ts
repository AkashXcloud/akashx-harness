/**
 * AkashX search through an Anthropic-compatible Messages model call with the native
 ...
 * @module @akashx/akx-web-search-akx/provider
 */

import { WebError } from '@akashx/akx-web'
import type {
  WebSearchProvider,
  WebSearchRequest,
  WebSearchResult,
...
import type {} from '@akashx/akx-session'
import type {
  AnthropicError,
  AnthropicResponse,
  ContentBlock,
...
export const AKASHX_PROVIDER_ID = 'akashx-official'

/**
 * Default endpoint: AkashX's Anthropic-compatible API, `/v1` included
 * (`/messages` is appended). This is NOT the chat-completions base
 * (`https://api.akashx.com`) `@akashx/akx-llm-akx` uses, so this
 * provider does NOT reuse `$AKASHX_BASE_URL` — only the API key is shared.
 */
export const AKASHX_DEFAULT_BASE_URL = 'https://api.akashx.com/anthropic/v1'

/** Default Anthropic-format model name (aligned with the repo's AkashX model vocabulary). */
export const AKASHX_DEFAULT_MODEL = 'akashx-v4-flash'

/** Default `anthropic-version` header value. */
export const AKASHX_DEFAULT_API_VERSION = '2023-06-01'

/** Default upper bound on generated tokens for the Messages request. */
export const AKASHX_DEFAULT_MAX_TOKENS = 4096

/** Default maximum `web_search` server-tool uses per request. */
export const AKASHX_DEFAULT_MAX_USES = 5

/** Attribution header sent on every request. Bump with the package version. */
const USER_AGENT = 'akx-harness/0.0.1'

/**
 * Exact secret-free AkashX Messages request recorded immediately before one
 * auxiliary search dispatch.
 */
export interface AkashXSearchLlmRequest {
...
export interface AkashXSearchProviderOptions {
  /** Literal AkashX API key; when present it wins over {@link resolveApiKey}. */
  apiKey?: string
  /** Resolve the current AkashX API key for one search operation. */
   ...
   */
  recordRequest?: (request: AkashXSearchLlmRequest) => void
}
...
/**
 * Map a AkashX Anthropic Messages response to a normalized search result. Walks
...
export function mapAnthropicResponse(response: AnthropicResponse): WebSearchResult {
  ...
  if (resultBlocks.length === 0) {
    throw new WebError(
      'AkashX returned no web_search_tool_result blocks; the request may not have triggered native web search',
      'WEB_PROVIDER_ERROR',
    )
  }
...
/**
 * The AkashX-backed search provider. HTTP redirects fail as `WEB_PROVIDER_ERROR`;
 * failures after dispatch name the endpoint and tell the model how the user can configure it.
 */
export class AkashXSearchProvider implements WebSearchProvider {
  readonly id = AKASHX_PROVIDER_ID
   ...
   */
  constructor(private readonly resolveOptions: () => AkashXSearchProviderOptions) {}
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    const body: AkashXSearchLlmRequest['body'] = {
      model: options.model,
    ...
    try {
      response = await fetch(endpoint, {
        ...
        redirect: 'error',
        headers: {
          // Official AkashX expects `x-api-key`; an Anthropic-compatible proxy
      ...
      throw searchEndpointError(
        endpoint,
        `AkashX search request failed: ${String(error)}`,
        error,
      )
    }
    ...
    if (!response.ok) {
      ...
      let message = `AkashX API error (HTTP ${status})`
      try {
    ...
    try {
      ...
      const message = error instanceof WebError
        ? error.message
        : `AkashX returned an unprocessable response body: ${String(error)}`
   ...
   */
  private async apiKey(options: AkashXSearchProviderOptions, signal?: AbortSignal): Promise<string> {
    throwIfSearchAborted(signal)
    ...
    try {
      ...
      if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal, error)
      throw new WebError(
        `AkashX search credential resolution failed: ${String(error)}`,
        'WEB_PROVIDER_ERROR',
        { cause: error },
      )
    }
    if (resolved !== undefined && resolved.length > 0) return resolved
    const ref = options.apiKeyEnv ?? 'AKASHX_API_KEY'
    throw new WebError(
      `AkashX search has no API key for "${ref}"; store it through the credentials service`
      + ' (the web Models page writes it), export it in the launching environment, or set a literal'
      + ' "apiKey" in the web-search-akx config',
...
function searchAborted(signal?: AbortSignal, fallback?: unknown): WebError {
  return new WebError('AkashX search aborted', 'WEB_ABORTED', {
...
/** True for AkashX request limits that can be sent to the Messages API. */
