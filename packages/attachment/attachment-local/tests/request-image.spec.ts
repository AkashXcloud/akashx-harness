...
async function home(): Promise<string> {
  const akxHome = await mkdtemp(join(tmpdir(), 'akx-request-image-'))
  homes.push(akxHome)
  return akxHome
}

async function store(): Promise<LocalAttachmentStore> {
  return new LocalAttachmentStore(new Context(), { akxHome: await home() })
}
...
describe('local request-image cache', () => {
  it('rebuilds a cleared cache without moving or losing durable attachments', async () => {
    ...
    vi.stubEnv('AKX_HOME', fallbackHome)
    try {
      const akxHome = await home()
      const attachments = new LocalAttachmentStore(new Context(), { akxHome })
      ...
      const cacheRoot = join(akxHome, 'cache')
      const path = join(cacheRoot, 'attachments', 'request-images', hash.slice(0, 2), hash)

      expect(attachments.root).toBe(join(akxHome, 'attachments', 'v1'))
      ...
      const reopened = new LocalAttachmentStore(new Context(), { akxHome })
  ...
  it('regenerates invalid, oversized, incompatible, or mismatched cached variants', async () => {
    ...
    const initial = await attachments.readImageRequest(attachment, policy)
    const hash = String(initial.variantId).slice('sha256:'.length)
    const path = join(akxHome, 'cache', 'attachments', 'request-images', hash.slice(0, 2), hash)
