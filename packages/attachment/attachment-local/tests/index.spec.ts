...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const akxHome = await mkdtemp(join(tmpdir(), 'akx-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { akxHome })
      ...
      await rm(akxHome, { recursive: true, force: true })
    }
  })

  it('commits a fully prepared image batch in input order', async () => {
    const akxHome = await mkdtemp(join(tmpdir(), 'akx-attachment-batch-success-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { akxHome })
  ...
  it.each([3, 4] as const)('admits a 16-bit %s-channel PNG as an 8-bit normalized object', async (channels) => {
    const akxHome = await mkdtemp(join(tmpdir(), 'akx-attachment-16-bit-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { akxHome })
  ...
  it('prepares every batch member before any write', async () => {
    const akxHome = await mkdtemp(join(tmpdir(), 'akx-attachment-batch-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { akxHome })
  ...
  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const akxHome = await mkdtemp(join(tmpdir(), 'akx-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { akxHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { akxHome, maxImageBytes: 1 })
