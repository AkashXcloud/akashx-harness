...
The limit is set where the tool is configured. For example, `akx-tool-web`'s `fetchTimeoutMs`/`searchTimeoutMs` settings (default 30,000 ms) put the limit on `web_fetch` and `web_search`. Tools without a limit — the shipped `bash`, `read`, `write`, and `edit` — are never cut off. The generated [configuration catalog](../../../docs/config-catalog.md#akashxakx-tool-web) lists the tool settings that produce limits.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#akashxakx-tool-web) — `akx-tool-web`'s `fetchTimeoutMs`/`searchTimeoutMs` budgets the policy enforces.
