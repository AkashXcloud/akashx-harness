...
import * as CommandInvariant from '@akashx/akx-commands/invariant'
import InvariantRegistry, { InvariantError } from '@akashx/akx-invariants'
import SessionStore, { SessionId, SessionSeq, type Session } from '@akashx/akx-session'
import { CommandId } from '@akashx/akx-commands'
...
describe('command lifecycle invariants', () => {
  ...
  it.each([-1, 1.5, 1])('rejects invalid or non-prior sourceEventSeq %s', async (sourceEventSeq) => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@akashx/akx-commands',
    }))
  })
