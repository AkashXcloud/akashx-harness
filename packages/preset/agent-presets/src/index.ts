 ...
 * @module @akashx/akx-agent-presets
 */
...
import { bindScopeParent, createScope, scopeOf, type Scope, type ScopeKey, type ScopeParentBinding } from '@akashx/akx-scope'
...
import type SettingsService from '@akashx/akx-settings'
import type { SettingsScope } from '@akashx/akx-settings'
...
export class AgentPresets extends TypertRemoteService {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = [
      ...
      ...config.roots,
      ...config.includeUserRoot ? [{ path: akxHomePath(USER_PRESET_DIR), trust: 'user' } satisfies PresetRoot] : [],
    ]
  ...
  async remoteExportList(): Promise<AgentPresetRoster> {
    ...
    return {
      presets: presets.map(preset => ({
        ...
        ...preset.hidden === undefined ? {} : { hidden: preset.hidden },
      })),
   ...
   * scope key. The binding is akx-scope's only re-link capability; holding it
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `akx-skill-filesystem` watches its
