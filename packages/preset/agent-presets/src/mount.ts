 ...
 * @module @akashx/akx-agent-presets/mount
 */

import { pathToFileURL } from 'node:url'
import { Context, type Fiber } from '@akashx/cordis'
import { Include } from '@akashx/cordis-plugin-include'
import type { EntryTree } from '@akashx/cordis-plugin-loader'
import { scopeOf, scopeParentOf, type ScopeKey } from '@akashx/akx-scope'
...
class PresetTree extends Include {
   ...
   * dependencies, so every `@akashx/akx-*` row would fail to import. The
 ...
 * invariant companion's service listener, and `akx-invariants` is a
