...
使用 `akx-storage` 持久保存类型化应用数据，而不将其加入会话历史。将它与受支持的存储介质和领域配置一同挂载后，调用方即可通过公共 `ctx.storageDomain` API 访问记录。工作区记录、会话伴随数据或其他必须在重启后保留且不应成为会话事件的应用状态适合使用它。它仅供宿主代码使用，对模型没有可见影响；无需此类数据的组合可以省略它。
...
```yaml
- name: '@akashx/akx-storage'
- name: '@akashx/akx-storage-json'
  config:
    root: /var/lib/akx/data
- name: '@akashx/akx-storage-domain'
  config:
    backend: json
```

这些行加载后，`json` 后端注册自身、`domain` 数据形式挂载；诸如 `akx-workspace` 之类的消费方随后在已路由后端上打开自己的领域，并通过 `ctx.storageDomain` 读写记录。多个后端可以并排保持挂载；哪个后端服务哪个领域由领域数据形式的配置决定，绝非枢纽的全局选择。
...
- `form-not-mounted`——消费方在 `akx-storage-domain` 加载前读取 `ctx.storage.domain`；请把领域行放在消费方之前。
