...
import Storage, { storageBackendServiceKey } from '@akashx/akx-storage'
import type { KvUnitDescriptor } from '@akashx/akx-storage'
...
async function freshDbPath(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'akx-storage-sqlite-'))
  dirs.push(dir)
