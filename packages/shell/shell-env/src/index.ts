 ...
 * trusted, per-execution `AKX_*` variables consumed by the model-facing shell
 * tools (`akx-tool-bash`, `akx-tool-pwsh`). Built-in shell facts are owned by
 * the registry itself while plugins can register additional, enumerable facts
 * with effect-scoped disposal.
 *
 * @module @akashx/akx-shell-env
 */

import { Service, type Context } from '@akashx/cordis'
import z from '@akashx/schemastery'
import { AKX_ENV_PREFIX } from '@akashx/akx-shell'
import type { AkxEnvironment, AkxEnvironmentKey } from '@akashx/akx-shell'
import { AKX_HOME_ENV, resolveAkxHome } from '@akashx/akx-home-paths'
...
/** Model-visible metadata for one managed `AKX_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `AKX_*` keys this contributor may return. */
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `AKX_*` environment variable name. */
  key: AkxEnvironmentKey
}

const AKX_SHELL_KEY = `${AKX_ENV_PREFIX}SHELL` as const
const AKX_SESSION_ID_KEY = `${AKX_ENV_PREFIX}SESSION_ID` as const
const RESERVED_BASH_ENV_KEYS = new Set<AkxEnvironmentKey>([
  AKX_HOME_ENV,
  AKX_SHELL_KEY,
  AKX_SESSION_ID_KEY,
])
...
/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `AKX_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `AKX_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  private readonly keyOwners = new Map<AkxEnvironmentKey, string>()
  private readonly akxHome: string
  ...
  constructor(ctx: Context, config: Config = {}) {
    super(ctx, 'shellEnv')
    this.akxHome = resolveAkxHome(config.akxHome)
  }
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      const variables = Object.entries(contributor.variables) as [AkxEnvironmentKey, BashEnvVariable][]
      for (const [key, variable] of variables) {
        if (!key.startsWith(AKX_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(AKX_ENV_PREFIX.length))) {
  ...
  collect(execution: ToolExecution): AkxEnvironment {
    const values: Record<AkxEnvironmentKey, string> = {
      [AKX_HOME_ENV]: this.akxHome,
      [AKX_SHELL_KEY]: '1',
    }
    if (execution.agent !== undefined) {
      values[AKX_SESSION_ID_KEY] = execution.agent.session.header.id
    }
    ...
    for (const contributor of [...this.contributors.values()].sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      for (const [rawKey, value] of Object.entries(resolved)) {
        const key = rawKey as AkxEnvironmentKey
