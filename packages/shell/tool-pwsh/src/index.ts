 ...
 * `@akashx/akx-pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `akx-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `AKX_*` environment through the
 ...
 * exit-status parse from `@akashx/akx-shell`.
 *
 * @module @akashx/akx-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@akashx/cordis'
import z from '@akashx/schemastery'
import { defineTool, TOOL_ABORTED } from '@akashx/akx-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@akashx/akx-tools'
import { HarnessError } from '@akashx/akx-llm'
import type { Agent } from '@akashx/akx-agent'
import type {} from '@akashx/akx-jobs'
import type {} from '@akashx/akx-shell-env'
import type {} from '@akashx/akx-user-approval'
import type { SandboxExecutionPolicy, SandboxMode } from '@akashx/akx-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@akashx/akx-sandbox'
import type { SandboxPolicyService } from '@akashx/akx-sandbox-policy'
import type { ShellRunResult } from '@akashx/akx-shell'
import { parseExitStatus } from '@akashx/akx-shell'
import { processOutcome } from './background.ts'
import { renderPwshProcessRead, renderPwshResult } from './render.ts'
import type { RenderablePwshResult } from './render.ts'

declare module '@akashx/akx-jobs' {
...
/* jscpd:ignore-start -- minimal mirror of akx-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:AKX_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror akx-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of akx-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of akx-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of akx-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors akx-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with akx-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors akx-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        akxEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @akashx/akx-jobs and @akashx/akx-tool-jobs')
        }
