/**
 * Integration tests: the REAL `@akashx/akx-pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@akashx/akx-tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@akashx/akx-pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'akx-tool-pwsh-'))
