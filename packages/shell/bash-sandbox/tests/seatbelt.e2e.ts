...
import { seatbeltProfileArgs } from '@akashx/akx-sandbox-local/src/profiles.ts'
 ...
 * confinement is covered by `@akashx/akx-sandbox-local`. Skips off macOS or when
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'akx-seatbelt-e2e-'))
  tempDirs.push(dir)
  return dir
}
...
describe.skipIf(!seatbeltUsable)('bash-sandbox: real Seatbelt confinement through ctx.shell', () => {
  ...
  it('evaluates BASH_ENV only after Seatbelt confines the inner Bash', async () => {
    ...
    await writeFile(hook, [
      'printf hook > "$AKX_BASH_ENV_INSIDE"',
      'printf escaped > "$AKX_BASH_ENV_OUTSIDE"',
      '',
    ].join('\n'))
    const bash = await sandboxedBash(workdir, 'workspace-write')
    ...
    await bash.run(bash.resolve({
      ...
      env: { BASH_ENV: hook },
      akxEnv: {
        AKX_BASH_ENV_INSIDE: insideProbe,
        AKX_BASH_ENV_OUTSIDE: outsideProbe,
      },
    }))
