...
import type { ShellRunResult, CollectedOutput } from '@akashx/akx-shell'
import SessionProjectionRegistry from '@akashx/akx-session-projection'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@akashx/akx-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@akashx/akx-sandbox'
import { SandboxPolicyService } from '@akashx/akx-sandbox-policy'
import { SandboxBashExecutor } from '@akashx/akx-bash-sandbox'
...
import type { Config } from '@akashx/akx-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'akx-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'AKX_WRAP=1', 'bash', '-c', 'printf "%s" "$AKX_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$AKX_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'akx-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.AKX_ORDER_FILE, "runner\\n");',
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'akx-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const deniedRoot = mkdtempSync(join(tmpdir(), 'akx-sandbox-denied-'))
    try {
