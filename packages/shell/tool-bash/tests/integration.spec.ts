...
import { SessionId, type SessionEvent } from '@akashx/akx-session'
 ...
 */
async function harness(adapter: MockAdapter, sessionRoot?: string, akxHome?: string) {
...
describe('bash tool through the agent loop', () => {
  it('first-turn bash receives session identity in a scrubbed AKX_* namespace', async () => {
    const root = mkdtempSync(join(tmpdir(), 'akx-bash-session-env-'))
    dirs.push(root)
    const akxHome = join(root, 'akx-home')
    vi.stubEnv('AKX_STALE_PARENT', 'stale')
    const adapter = new MockAdapter([
      toolCallResponse('call-1', 'bash', {
        command: 'printf \'%s\\n%s\\n%s\\n%s\\n\' "$AKX_HOME" "$AKX_SHELL" "$AKX_SESSION_ID" "${AKX_STALE_PARENT-unset}"',
        description: 'inspect session environment',
      }),
      textResponse('Session environment inspected.'),
    ])
    const ctx = await harness(adapter, root, akxHome)
    ...
    expect(resultText(result)).toBe(`${akxHome}\n1\nsession-env-id\nunset\n`)
  ...
  it('background: start ack → completion wakes the idle agent → job_output collects it', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'akx-bg-'))
    dirs.push(dir)
