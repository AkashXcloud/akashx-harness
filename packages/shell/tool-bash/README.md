...
Load this plugin in any composition where the agent should run bash commands: it registers the `bash` tool once an executor provider and the `akx-shell-env` registry are mounted, and stays pending until the `tools`, `shell`, `systemPrompt`, and `shellEnv` services exist.
...
```yaml
- name: '@akashx/akx-bash-local'
- name: '@akashx/akx-shell-env'
- name: '@akashx/akx-tool-bash'

# Optional: background jobs
- name: '@akashx/akx-jobs-local'
- name: '@akashx/akx-tool-jobs'
```
...
- **Request from named args only.** The tool never exposes `stdin`, `env`, or `stdoutMaxBytes`; it builds each request from command/workdir/timeout/signal fields plus the registry-collected `akxEnv`, so model-supplied keys cannot replace managed values.
