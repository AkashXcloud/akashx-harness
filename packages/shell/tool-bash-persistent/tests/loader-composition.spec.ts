...
suite('persistent Bash through a real cordis.yml Loader composition', () => {
  it('preserves cwd and environment across calls', async () => {
    root = await mkdtemp(join(tmpdir(), 'akx-persistent-bash-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@akashx/akx-tool-bash-persistent', ToolBashPersistent],
    ])
    ...
    expect(observed).not.toContain('AKX_PERSISTENT_BASH')
    ...
    expect(multiline).not.toContain('AKX_PERSISTENT_BASH')
