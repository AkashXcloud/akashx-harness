 ...
 * @module @akashx/akx-tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSessionId } from '@akashx/akx-terminal'
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__AKX_PERSISTENT_BASH_START_${nonce}__`,
    end: `__AKX_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __akx_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__akx_persistent_bash_status"`
}
