 ...
 *
 * @module @akashx/akx-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors akx-bash-local call-for-call by
   design (see this package's README), so the two import the same seam surface */
import { Context } from '@akashx/cordis'
import z from '@akashx/schemastery'
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@akashx/akx-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@akashx/akx-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@akashx/akx-subprocess'
import type {} from '@akashx/akx-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@akashx/akx-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of akx-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `akx-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.akxEnv },
    }
  }
   ...
   * `akx-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@akashx/akx-pwsh-sandbox`.
