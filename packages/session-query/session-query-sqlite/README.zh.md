...
当你想对既往会话进行带排序与分页的全文召回时选择它。它与 `akx-session-query` 和会话服务一起使用；持久化后端可选但建议挂载，这样重启后持久化历史仍可搜索。不要把 `path` 指向 session-persistence 数据库——本包拥有独立的派生索引。
...
- [akx-session-query](../session-query/README.zh.md)——服务定义：本后端继承的精确读取、过滤与追踪。
- [akx-tool-session-query](../tool-session-query/README.zh.md)——调用这些搜索方法的面向模型消费方。
