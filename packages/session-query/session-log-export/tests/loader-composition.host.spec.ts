...
import * as SessionLogDownload from '@akashx/akx-session-log-export'
...
describe('session-log-download real Loader composition', () => {
  it('discovers and executes /export through the assembled command plane', async () => {
    root = await mkdtemp(join(tmpdir(), 'akx-session-export-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@akashx/akx-session-log-export', SessionLogDownload],
    ])
    ...
    expect(context.commands.list(agent)).toContainEqual({
      definitionId: '@akashx/akx-session-log-export',
