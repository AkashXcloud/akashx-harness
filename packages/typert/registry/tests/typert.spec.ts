...
} from '@akashx/akx-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@akashx/akx-tools', 'ToolInput')).toBe('@akashx/akx-tools#ToolInput')
    expect(typertPackageKey('@akashx/akx-tools', 'host')).toBe('@akashx/akx-tools#host')
    expect(ctx.typert.get('@akashx/akx-tools#ToolInput')).toMatchObject({
      package: '@akashx/akx-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@akashx/akx-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@akashx/akx-tools', 'host')).toMatchObject({
      key: '@akashx/akx-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@akashx/akx-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@akashx/akx-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@akashx/akx-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@akashx/akx-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@akashx/akx-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@akashx/akx-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@akashx/akx-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@akashx/akx-tools#ToolInput')).not.toBe(projected)
  })
