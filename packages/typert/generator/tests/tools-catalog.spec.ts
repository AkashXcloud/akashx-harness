...
import type { TypertContribution } from '@akashx/akx-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@akashx/akx-tool-cordis/src/api-catalog.ts'
...
describe('model-driven akx-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@akashx/akx-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('akx-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@akashx/akx-tools')
    ...
    const record = ctx.typert.getPackage('@akashx/akx-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@akashx/akx-tools', 'host')).toBeUndefined()
  })
})
