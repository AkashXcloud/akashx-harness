...
import type { CodeFileType, FileType } from '@akashx/akx-client-ui-primitives'
...
const TYPE_COLORS: Readonly<Record<TraditionalFileType, string>> = {
  code: 'var(--dsw-static-akashx-500)',
  excel: 'var(--dsw-static-green-500)',
  folder: 'var(--dsw-static-amber-400)',
  html: 'var(--dsw-static-akashx-500)',
  image: 'var(--akx-file-type-violet)',
  markdown: 'var(--dsw-static-akashx-500)',
  other: 'var(--dsw-static-neutral-bluish-300)',
  pdf: 'var(--dsw-static-red-600)',
  ppt: 'var(--dsw-static-amber-500)',
  video: 'var(--akx-file-type-violet)',
  word: 'var(--dsw-static-akashx-450)',
}
...
describe('FileTypeIcon.module.css', () => {
  it.each(Object.entries(TYPE_COLORS) as [TraditionalFileType, string][])(
    ...
    (type, color) => {
      ...
      expect(rule).toContain(`--akx-file-type-default-color: ${color}`)
    },
  )

  it('keeps one caller override and the supplied violet in named variables', () => {
    expect(css).toContain('color: var(--akx-file-type-icon-color, var(--akx-file-type-default-color))')
    expect(css).toContain('--akx-file-type-violet: rgb(139, 118, 246)')
  })
})
