 ...
 * Everything else under @akashx/* is either a module-table entry
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.AKX_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.AKX_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.AKX_BUILD_FACE must be host or client, received ${String(value)}`)
}
...
interface WorkspaceManifest {
  ...
  readonly akx?: { readonly client?: { readonly external?: unknown } }
}
...
/**
 * Module-table specifiers one `akx.client` declaration requests. Matching is
 ...
 * @param declaration - the package's `akx.client` object.
...
export function requestedExternals(
  ...
  return new Set(optionalStringArray(subject, 'akx.client.external', declaration.external) ?? [])
}

/**
 * Module-table specifiers one package requests. The shell baseline is implicit
 * for every dynamic bundle; `akx.client.external` only adds package-specific
...
function clientExternals(id: string): ReadonlySet<string> {
  ...
  const externals = new Set([
    ...
    ...requestedExternals(id, workspaceManifest(id).akx?.client ?? {}),
  ])
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @akashx value import is a build error — a
...
const STATIC_LINKED_PLUGIN = 'akx-static-linked-external'
