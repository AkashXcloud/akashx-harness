...
describe('FeedbackDialog', () => {
  ...
  it('shows every category, the detail box with the hint, and an enabled Submit for an empty draft', () => {
    ...
    const dialog = ui.getByRole('dialog', { name: en['dialog.title'] })
    expect(dialog).toBeTruthy()
    const chips = ui.getByRole('group', { name: en['dialog.categories'] }).querySelectorAll('button')
    expect([...chips].map(chip => chip.textContent)).toEqual(
      FEEDBACK_CATEGORIES.map(category => en[`category.${category}`]),
    )
    expect(ui.getByLabelText(en['dialog.detail']).getAttribute('placeholder')).toBe(en['dialog.hint'])
    expect(ui.getByRole('button', { name: commonEn.submit }).hasAttribute('disabled')).toBe(false)
  })
  ...
  it('forwards typing, submits, and closes through the injected verbs', () => {
    ...
    fireEvent.change(ui.getByLabelText(en['dialog.detail']), { target: { value: 'draft more' } })
    expect(ui.edit).toHaveBeenCalledWith({ text: 'draft more' })

    fireEvent.click(ui.getByRole('button', { name: commonEn.submit }))
    expect(ui.submit).toHaveBeenCalledTimes(1)

    fireEvent.click(ui.getByRole('button', { name: commonEn.close }))
  ...
  it('disables Submit and the chips while a submission is in flight', () => {
    ...
    expect(ui.getByRole('button', { name: commonEn.submitting }).hasAttribute('disabled')).toBe(true)
    const chips = ui.getByRole('group', { name: en['dialog.categories'] }).querySelectorAll('button')
  ...
  it('renders submission failures as toasts outside the dialog', () => {
    ...
    expect(conflictToast.textContent).toBe(en['error.conflict'])
    expect(conflict.getByRole('dialog').contains(conflictToast)).toBe(false)
    cleanup()

    const oversized = mount({ failure: 'note-too-large' })
    expect(oversized.getByRole('alert').textContent).toBe(en['error.noteTooLarge'])
    cleanup()

    const other = mount({ failure: 'session-not-found' })
    expect(other.getByRole('alert').textContent).toBe(en['error.generic'])
  })
  ...
  it('shows the acknowledgement toast and retires it after the fade', () => {
    ...
    try {
      ...
      expect(ui.getByRole('alert').textContent).toBe(en['toast.recorded'])
