...
import { createSnapshotStore, type ObservableSnapshot, type SnapshotStore } from '@akashx/akx-client-store'
import type {
  ResourceOpenContext,
  ResourceProtocol,
  ResourceProvider,
...
interface ResourceRecord {
  ...
  /** The address's protocol key (`akx-resource://` host); absent when the address is not a resource address. */
...
/**
 * The one URL scheme resource addresses use: `akx-resource://<type>/…`, where
 * the host names the protocol. Other schemes (`sidebar://…`) are navigation
 * addresses and name no resource.
 */
export const RESOURCE_SCHEME = 'akx-resource'

/**
 * The protocol key of one address: the host of a `akx-resource://` URL, as the
