...
import type { RemoteFailure, RemoteResult } from '@akashx/akx-typert-protocol'
...
describe('protocolOf', () => {
  it('reads the akx-resource host, lower-cased, and reports none for any other address', () => {
    expect(protocolOf('akx-resource://file/session/s1/home/ys/b.txt')).toBe('file')
    expect(protocolOf('AKX-RESOURCE://File/session/s1/a')).toBe('file')
    expect(protocolOf('akx-resource://chat/node/1')).toBe('chat')
    ...
    expect(protocolOf('akx-resource:///no-host')).toBeUndefined()
...
describe('ResourceRegistry stream generations', () => {
  ...
  it('streams another protocol as plain numbers', async () => {
    ...
    const source = b.registry.source('akx-resource://counter/one')
    const unsubscribe = source.subscribe(() => {})
