...
import { bindSnapshotSelector, SlotTestRuntime, stubSettingsScope } from '@akashx/akx-client-test-runtime'
...
import { EMPTY_CHAT_SNAPSHOT } from '@akashx/akx-client-ui-chat/client'
import type {
...
} from '@akashx/akx-client-ui-conversation/src/client/skeleton/ConversationSession.tsx'
import { createConversationStore } from '@akashx/akx-client-ui-conversation/src/client/stores.ts'
import { en as conversationEn } from '@akashx/akx-client-ui-conversation/src/client/locales.ts'
import { apply as localeApply, inject as localeInject } from '@akashx/akx-client-locale/client'
import { apply, inject } from '@akashx/akx-client-ui-trajectory/client'
import { apply as nodeApply } from '@akashx/akx-client-ui-trajectory'
...
describe('plugin registration', () => {
  ...
  it('shares one browser-wide duration preference across session injections', async () => {
    ...
    expect(localStorage.getItem('akx.trajectory.duration')).toBe('true')
    expect(localStorage.getItem(`akx.trajectory.duration.${SID}`)).toBeNull()
  })
...
describe('tab switching in ConversationRoot', () => {
  it('renders two tabs, defaults to chat, and switches to the trajectory ledger', async () => {
    ...
    expect(screen.getByRole('row', { name: /User/u })).toBeTruthy()
  ...
  it('opens a local record inspector and switches payload tabs without opening chat details', async () => {
    ...
    fireEvent.keyDown(screen.getByRole('row', { name: /Tools/u }), { key: 'Enter' })
  ...
  it('dragging the overview focuses overlapping records without filtering the ledger', async () => {
    ...
    expect(screen.getByRole('row', { name: /User/u }).getAttribute('data-timeline-focus'))
      .toBe('outside')
...
describe('TrajectoryView state', () => {
  it('reveals resident history one bounded page at a time', async () => {
    ...
    fireEvent.click(screen.getAllByRole('button', { name: 'Load earlier history' }).at(-1)!)
  ...
  it('keeps resident request numbering and cumulative usage outside the layout page', async () => {
    ...
    await waitFor(() => { expect(screen.getByRole('button', { name: 'Request #1' })).toBeTruthy() })
    await waitFor(() => { expect(screen.getByRole('button', { name: 'Request #11' })).toBeTruthy() })
    fireEvent.click(screen.getByRole('button', { name: 'Request #11' }))
    fireEvent.click(screen.getByRole('tab', { name: 'Usage' }))
    expect(screen.getByText('Session cumulative').closest('section')?.textContent).toContain('11 tok')
  })
