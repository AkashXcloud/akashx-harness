 ...
 * not built (`pnpm --filter @akashx/akx-client-ui-trajectory bundle`).
 */
import { readFileSync } from 'node:fs'
import { resolve } from 'node:path'
import { Context } from '@akashx/cordis'
import { stubSettingsScope } from '@akashx/akx-client-test-runtime'
import { afterEach, describe, expect, it } from 'vitest'
import { UiConversation } from '@akashx/akx-client-ui-conversation/client'
import { SlotRegistry } from '@akashx/akx-client-ui-renderer/client'

const PLUGIN_ID = '@akashx/akx-client-ui-trajectory'
...
describe('tsdown client artifact', () => {
  ...
  async function loadArtifact() {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@akashx/akx-client-store', await import('@akashx/akx-client-store')],
      ['@akashx/akx-client-ui-conversation/client', await import('@akashx/akx-client-ui-conversation/client')],
      ['@akashx/akx-client-ui-primitives', await import('@akashx/akx-client-ui-primitives')],
    ])
  ...
  it.skipIf(code === undefined)('mounted as an object plugin, apply registers the view tab on the real ring', async () => {
    ...
    const locale = await import('@akashx/akx-client-locale/client')
