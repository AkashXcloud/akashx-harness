...
const FIXTURE_SYSTEM_PROMPT = 'You are the AkashX Harness fixture assistant. Answer concisely and call a tool when one is needed.'
...
function buildAlphaLog(): SessionEvent[] {
  ...
  for (let turn = 0; turn < 60; turn++) {
    ...
    const userSeq = push({
      ...
      data: userMessage(text(turn === 59 ? USER_MARKDOWN_LITERAL : `Question ${turn}：fixture history message, for paging and rendering verification.`)),
    })
    if (turn === 0) {
      push({
        type: 'session/title',
    ...
    if (withReasoning) blocks.push({ type: 'reasoning', text: `Reasoning process  ${turn}：This is collapsible reasoning content.` })
    blocks.push({ type: 'text', text: turn === 59 ? MARKDOWN_FIXTURE : `Answer ${turn}：This is the fixture-generated history reply body.` })
    if (withTool) {
  ...
  const fixtureTodos = [
    { content: 'Clarify requirements', status: 'completed' },
    { content: 'Implement the fixture sample', status: 'in_progress' },
    { content: 'Run the background build', status: 'in_progress' },
    { content: 'Browser verification', status: 'pending' },
  ]
  ...
  push({ type: 'user/message', surfaceOp: 'append', data: userMessage(text('Question 72：List all one hundred entries in full.')) })
  push({ type: 'step/start', data: { turn: 72, step: 0 } })
  push({
    type: 'assistant/message',
    surfaceOp: 'append',
    data: { turn: 72, step: 0, message: assistantMessage(text('Entry 1：First.Entry 2：Second.Entry 3：This entry was cut off halfway through ')) },
  })
  ...
  push({
    type: 'user/message',
    surfaceOp: 'append',
    data: userMessage([{ type: 'image', attachment: FIXTURE_IMAGE_REF }, ...text('history user image')]),
  })
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixtureQuestions: readonly FixtureQuestionItem[] = [
    {
      id: 'harness-profile',
      header: 'Preference',
      question: 'Which kind of Agent/Harness candidate are you hiring for now?',
      options: [
        { label: 'Engineering delivery (Recommended)', description: 'Values hands-on work on runtimes, tool executors, sandboxes, traces, and production debugging.' },
        { label: 'Research potential', description: 'Values Agent understanding, evaluation design, and long-term growth.' },
        { label: 'Balanced', description: 'Requires both engineering skill and Agent understanding, but screens harder.' },
      ],
    },
    {
      id: 'work-mode',
      header: 'Approach',
      question: 'Which working style should the candidate show first?',
      options: [
        { label: 'Prototype first (Recommended)', description: 'Validate the key assumption with something runnable as soon as possible.' },
        { label: 'Full design first', description: 'Settle the boundaries, protocol, and risks before implementing.' },
      ],
    },
    {
      id: 'signals',
      ...
      detail: 'Pick for the current hiring goal; skipping means no preference.',
      multiSelect: true,
      options: [
        { label: 'System design' },
        { label: 'Code quality' },
        { label: 'Agent product judgement' },
      ],
    },
  ]
  ...
  const commandRemotes = {
    ...
    execute(id: SessionId, line: string, attachments: readonly unknown[] = []): RpcResult<CommandExecution | undefined> {
      ...
      const outcomes: Record<string, string> = {
        compact: 'fixture: compacted (simulated)',
        echo: args.trim(),
        plan: args.trim() === 'off'
  ...
  const workspaceFileLines = (path: string): string[] => {
    const name = path.slice(path.lastIndexOf('/') + 1)
    const head = [`# ${name}`, '', 'Sample text in fixture mode, for verifying the sidebar text preview.', 'A real build reads the same-named file from the Workspace.']
    return path.includes('demo') || path.includes('huge')
      ? [...head, ...Array.from({ length: 12_000 }, (_, index) => `line ${index + 5}: Long-text sample for verifying paging and scrolling.`)]
      : head
  }
  ...
  const timingHooks = {
    ...
    beginModelRetry(id: string): void {
      ...
      pushAssistant(sessionId, { type: 'text-delta', index: 0, text: 'Half-finished reply that should be withdrawn' })
    },
    ...
    scheduleModelRetry(id: string, retry = 1, delayMs = 450): void {
      ...
      if (!scenario.stepStarted) {
        ...
        pushAssistant(sessionId, { type: 'block-start', index: 0, blockType: 'text' })
        pushAssistant(sessionId, { type: 'text-delta', index: 0, text: `line ${String(retry)}  retracted reply` })
    ...
    completeModelRetry(id: string): void {
      ...
      if (scenario === undefined) throw new Error(`fixture: no model retry scenario for ${id}`)
      retryScenarios.delete(sessionId)
      const completed = 'Complete reply after the retry'
      beginAssistant(sessionId, scenario.turn, 1)
      pushAssistant(sessionId, { type: 'block-start', index: 0, blockType: 'text' })
      ...
      startReply(
        ...
        userText === 'render markdown'
          ...
          : userText === 'report model'
            ...
            : `Echo: ${userText}。This is the fixture streaming reply, for verifying typewriter growth and the settle transition.`,
      )
      return sessionOk({ accepted: true as const })
    },
    attachment: (request) => {
  ...
  const approvalInvocation = (): FixtureRemoteEventInvocationFrame => ({
    ...
    request: {
      ...
      reason: 'Persistent fixture approval (answer approve or decline and it disappears)',
    },
  })
    ...
    call(channel, endpoint, payload, signal) {
      ...
      switch (endpoint) {
        ...
        case 'skills/list': {
          ...
          return sessionOk({
            skills: [
              { name: 'fixture-demo', description: 'fixture skill sample', whenToUse: 'Rendering verification of the UI catalog only', modelInvocable: true },
