...
import type { CredentialProvider, CredentialRecord } from '@akashx/akx-credentials'
import type {
...
export class BrowserAuth {
  ...
  authorizeIndex(req: ConnectionIndexRequest, res: ConnectionIndexResponse): boolean {
    ...
    const url = new URL(req.url ?? '/', 'http://akx.invalid')
  ...
  private writeUnauthorized(req: ConnectionIndexRequest, res: ConnectionIndexResponse): void {
    ...
    res.end(req.method === 'HEAD'
      ? undefined
      : `${process.env.AKX_CLI_NAME ?? 'akx'} web authentication required; reopen the URL printed by ${process.env.AKX_CLI_NAME ?? 'akx'} web.\n`)
  }
}
