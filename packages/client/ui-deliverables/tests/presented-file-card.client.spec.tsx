...
it('localizes reveal failures and accurately reports a directory-only action', () => {
  const p = props()
  const view = render(<PresentedFileCard {...p} phase="revealError" t={makeTranslate(en)} />)
  expect(view.getByText(en['presented.revealError'])).toBeTruthy()
...
it.each([en, en])('distinguishes directory-only progress and errors in each locale', (dictionary) => {
