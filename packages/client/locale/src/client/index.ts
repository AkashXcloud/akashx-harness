...
export type { Translate, TranslateNS } from '@akashx/akx-client-ui-slots'
...
/**
 * The locale the UI opens in: English is the only shipped locale, so it serves
 * both as the default when the browser names no registered language (and for
 * non-browser runs) and as the dictionary consulted after the active locale
 * misses a key.
 */
...
export class LocaleRuntime {
   ...
   * different browser sharing the same AKX home. Only the render notification
