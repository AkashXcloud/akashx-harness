...
import type { TerminalBlockLabels, TerminalBlockProps } from '@akashx/akx-client-ui-primitives'
import type { TranslateNS } from '@akashx/akx-client-ui-slots'
import { resolveWorkspacePath } from '@akashx/akx-util-workspace-path'
import { hasSpillNotice } from '@akashx/akx-spill-policy/notice'
...
function shellCall(name: string, args: Record<string, unknown>): ShellCall | null {
  ...
  if (description === undefined) {
    // Standard akx-tool-bash and akx-tool-pwsh schemas require `description`;
...
/**
 * Parse the marker literals owned by `@akashx/akx-shell/render` without
