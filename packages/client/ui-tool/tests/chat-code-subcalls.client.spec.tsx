...
import { en as conversationEn, NS as CONVERSATION_NS } from '@akashx/akx-client-ui-conversation/src/client/locales.ts'
import { apply as applyChat, inject as injectChat } from '@akashx/akx-client-ui-chat/client'
...
async function bench(snapshot: ChatSnapshot) {
  ...
  locale.register(CONVERSATION_NS, { en: conversationEn })
...
describe('run_code sub-calls through the real chat machinery', () => {
  ...
  it('a file sub-row click opens the file in the Sidebar; bash sub-rows open nothing', async () => {
    ...
    await vi.waitFor(() => {
      expect(b.sidebarRight.openResource).toHaveBeenCalledWith('akx-resource://file/session/s1/notes/demo.txt')
    })
