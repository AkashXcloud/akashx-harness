...
describe('ToolRow', () => {
  ...
  it('the expanded card gutter-labels each section it carries (IN / OUT)', () => {
    ...
    expect(both.getByText('Input')).toBeTruthy()
    expect(both.getByText('Output')).toBeTruthy()
    ...
    expect(inputOnly.getByText('Input')).toBeTruthy()
    expect(inputOnly.queryByText('Output')).toBeNull()
    cleanup()
    const outputOnly = render(<ToolRow {...rowProps} bodyRaw={null} output="only out" />)
    fireEvent.click(outputOnly.getByRole('button'))
    expect(outputOnly.queryByText('Input')).toBeNull()
    expect(outputOnly.getByText('Output')).toBeTruthy()
