...
import type { SessionReferenceMentionCandidate } from '@akashx/akx-session-reference/types'
...
async function bench(
  ...
  sessions: RemoteLookup<SessionReferenceMentionCandidate> = vi.fn(() => Promise.resolve({
    ...
    value: [{
      ...
      mention: '@[Research](akx-session:InNvdXJjZSI)',
    }],
  })),
  ...
  class RemoteService extends Service {
    readonly $host = { home: HOME, isLoopback: true, isPrivileged: true }
...
describe('apply', () => {
  it('declares its services and releases the @ reference registration on disposal', async () => {
    ...
    class RemoteService extends Service {
      readonly $host = { home: undefined, isLoopback: false, isPrivileged: true }
  ...
  it('labels a session without a cwd and still dates it', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: false,
        createdAt: CREATED_AT,
        mention: '@[same](akx-session:InNhbWUi)',
      }],
    }))
    const { source } = await bench(files, sessions)
  ...
  it('falls back to the candidate createdAt for a session the Host list does not carry', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: CREATED_AT,
        mention: '@[Unlisted run](akx-session:InVubGlzdGVkIg)',
      }],
    }))
  ...
  it('reads a session opened moments ago as the present, not a zero distance', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: NOW - 1_000,
        mention: '@[Just now](akx-session:Imp1c3Qtbm93Ig)',
      }],
    }))
  ...
  it('dates a session in the current workspace without repeating that workspace', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        sameWorkspace: true,
        createdAt: CREATED_AT,
        mention: '@[Sibling run](akx-session:InNpYmxpbmdyIg)',
      }],
    }))
    const { source } = await bench(files, sessions)
...
describe('pick and codec', () => {
  ...
  it('inserts sessions as atomic chips whose clipboard and model forms are canonical mentions', async () => {
    ...
    const mention = '@[Research](akx-session:InNvdXJjZSI)'
...
describe('reference preview', () => {
  it('opens plain and quoted file references without treating folders or sessions as files', async () => {
    ...
    expect(openResource).toHaveBeenNthCalledWith(1, 'akx-resource://file/session/target/notes/readme.md')
    expect(openResource).toHaveBeenNthCalledWith(2, 'akx-resource://file/session/target/docs/a%20b.md')
    expect(source.openReference?.(session, { ref: '@docs/', appearance: 'folder' })).toBe(false)
    expect(source.openReference?.(session, { ref: '@[Research](akx-session:abc)', appearance: 'session' })).toBe(false)
