...
describe('WorkflowRunPanel', () => {
  ...
  it('folds each normal completion once and opens a new same-key activity cycle', () => {
    ...
    const view = render(<WorkflowRunPanel {...panelProps(running)} />)
    const runningPhase = screen.getByRole('button', { name: /Unphased/u })
    ...
    const phaseHeader = screen.getByRole('button', { name: /Unphased/u })
    expect(phaseHeader.getAttribute('aria-expanded')).toBe('false')
    ...
    fireEvent.click(screen.getByRole('button', { name: /Unphased/u }))
    const runHeader = screen.getByRole('button', { name: /^audit/ })
    fireEvent.click(runHeader)
    ...
    expect(screen.getByRole('button', { name: /Unphased/u }).getAttribute('aria-expanded')).toBe('true')
    ...
    expect(screen.getByRole('button', { name: /^audit/ }).getAttribute('aria-expanded')).toBe('true')
    expect(screen.getByRole('button', { name: /Unphased/u }).getAttribute('aria-expanded')).toBe('false')
    fireEvent.click(screen.getByRole('button', { name: /Unphased/u }))
  ...
  it('refolds a phase when a complete activity cycle arrives as one clean update', () => {
    ...
    expect(runHeader.getAttribute('aria-expanded')).toBe('false')
    expect(screen.queryByRole('button', { name: /Unphased/u })).toBeNull()
  })
  ...
  it('keeps clean sibling phases independent and preserves empty versus absent names', () => {
    ...
    const runHeader = screen.getByRole('button', { name: /^audit/ })
    expect(runHeader.getAttribute('aria-expanded')).toBe('true')
    const cleanPhase = screen.getByRole('button', { name: /Empty phase name/u })
    expect(cleanPhase.getAttribute('aria-expanded')).toBe('false')
    const activePhase = screen.getByRole('button', { name: /Unphased/u })
    ...
    fireEvent.click(runHeader)
    fireEvent.click(runHeader)
    expect(screen.getByRole('button', { name: /Empty phase name/u }).getAttribute('aria-expanded')).toBe('true')
    expect(screen.getByRole('button', { name: /Unphased/u }).getAttribute('aria-expanded')).toBe('false')
    fireEvent.click(screen.getByRole('button', { name: /Empty phase name/u }))
    expect(screen.queryByText('Empty member name')).toBeNull()
  })
  ...
  it('defers normal completion collapse until focused member content loses focus', () => {
    ...
    expect(runHeader.getAttribute('aria-expanded')).toBe('false')
    fireEvent.click(runHeader)
    const completedPhase = screen.getByRole('button', { name: /Unphased/u })
