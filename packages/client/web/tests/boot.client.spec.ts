...
import type {
  ClientBundleRegistration, ClientModuleCreateOptions, ClientModuleLoaderTarget, AkxWindow,
  WebBootEntry,
} from '@akashx/akx-client-modules/client'
...
const win = globalThis as AkxWindow
const transportGlobal = globalThis as {
  __AKX_TRANSPORT__?: { loadBundle(url: string): Promise<void> }
}
...
afterEach(() => {
  ...
  delete transportGlobal.__AKX_TRANSPORT__
...
describe('bootstrap failure rendering', () => {
  ...
  it('renders a malformed boot manifest', async () => {
    await expectBootFailure(() => {
      installFacade()
      delete win.__AKX_BOOT__
    }, 'window.__AKX_BOOT__ is missing or not an object')
  })
...
describe('plugin activation', () => {
  it('prefetches a parser-loaded immediate row through the injected bundle transport', async () => {
    ...
    transportGlobal.__AKX_TRANSPORT__ = {
