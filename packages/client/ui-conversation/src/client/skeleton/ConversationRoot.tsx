...
const WIDTH_PREF_KEY = 'akx.conversation.contentWidth'
...
function WidthHandle(props: {
  ...
  const onPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    e.currentTarget.style.setProperty('--akx-width-handle-pointer-y', `${e.clientY - box.top}px`)
...
export function ConversationRoot({
  ...
  renderSlot, renderSlotChain, selectWorkspace, usePanes, t,
}: ConversationRootProps) {
  const isPane = usePanes(state => state.panes !== undefined)
  ...
  // scroll body: the seat's height as --akx-composer-height, so controls clear
  // the composer as it grows, and the scrollport's own height as
  // --akx-conversation-viewport-height, so a control can sit in the band the
  ...
  const seatResizeRef = useCallback((seat: HTMLDivElement | null): void => {
    ...
    seatObserver.current = new ResizeObserver(() => {
      scroller.style.setProperty('--akx-composer-height', `${seat.offsetHeight}px`)
      scroller.style.setProperty(
        '--akx-conversation-viewport-height',
  ...
  // Publishes the column's live width as --akx-conversation-column-width so
  ...
  const publishWidths = useCallback((root: HTMLDivElement): void => {
    ...
    root.style.setProperty('--akx-conversation-column-width', `${column}px`)
    const preference = readWidthPreference()
    if (preference === null) {
      root.style.removeProperty('--akx-chat-user-width')
    } else {
      root.style.setProperty('--akx-chat-user-width', `${resolveContentWidth(column, preference)}px`)
    }
  }, [])
  ...
  const onHandleDrag = useCallback((width: number): void => {
    ...
    root.style.setProperty('--akx-chat-user-width', `${clamped}px`)
  }, [])
