  ...
  it('renders a localized label as the title with the name as its alias, an icon component, and the description', () => {
    ...
    mount(openState({
      groups: [{
        ...
        status: 'ready',
        items: [
          { name: 'plan', label: 'plan', description: 'Enter or leave plan mode', icon: Glyph, section: 'Add' },
          { name: 'file', label: 'File', section: 'Add' },
        ],
      }],
    }))
    const options = screen.getAllByRole('option')
    expect(options.map(o => o.textContent)).toEqual(['planplanEnter or leave plan mode', 'File'])
    ...
    const { onPick } = mount(openState({
      groups: [{
        ...
        items: [
          ...
          { name: 'Session · Research', section: 'Sessions' },
        ],
      }],
