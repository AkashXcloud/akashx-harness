...
import type { CommandResult } from '@akashx/akx-commands/types'
import { CommandDefinitionId } from '@akashx/akx-commands/brand'
import { createScope, scopeOf } from '@akashx/akx-api-session-controller/client'
import type { SessionId } from '@akashx/akx-session/types'
import { RemoteError, TestRemote } from '@akashx/akx-client-test-runtime'
import { IconGoalOutline16 } from '@akashx/akx-client-ui-primitives'
import type { ClientSessionContext, ConsumeTokenRequest, InputTriggerPick, InputTriggerSource, SubmitAttachment } from '@akashx/akx-client-ui-input-trigger/client'
...
import { en, type CommandKey } from '../src/client/locales.ts'
...
describe('candidates', () => {
  ...
  it('localizes canonical built-in and contribution descriptions on every candidate request', async () => {
    let locale = 'en'
    const commands: CommandDescriptor[] = [
      { definitionId: CommandDefinitionId('@akashx/akx-command-compact'), name: 'compact', description: 'Compact older conversation history' },
  ...
  describe('menu presentation (design doc for #3567)', () => {
    ...
    const SHIPPED: CommandDescriptor[] = [
      { definitionId: CommandDefinitionId('@akashx/akx-command-compact'), name: 'compact', description: 'Compact older conversation history' },
      { definitionId: CommandDefinitionId('@akashx/akx-session-log-export'), name: 'export', description: 'Download this Session log as a ZIP archive' },
      { definitionId: CommandDefinitionId('@akashx/akx-command-feedback'), name: 'feedback', description: 'Record feedback about this session', input: { hint: '<text>' } },
      { definitionId: CommandDefinitionId('@akashx/akx-command-goal'), name: 'goal', description: 'Set or view the goal for a long-running task', input: { hint: '<objective>', attachments: true } },
      { definitionId: CommandDefinitionId('@akashx/akx-permission-presets'), name: 'permission', description: 'Switch the permission preset (sandbox mode + approval policy)', input: { hint: '<preset>' } },
      { definitionId: CommandDefinitionId('@akashx/akx-plan-mode'), name: 'plan', description: 'Enter or leave plan mode', input: { hint: '[off|message]', attachments: true } },
    ...
    it('an empty query lists Add then Commands in usage order; built-in rows carry their localized face, other rows their own text', async () => {
      ...
      expect(rows[6]).toMatchObject({ name: 'model', label: 'Model', description: 'Select the model for this conversation', icon: Glyph })
    ...
    it.each([['en', en], ['en', en]] as const)('description edits preserve menu claims and bilingual parsing under %s', async (_locale, dictionary) => {
      ...
      for (const name of ['goal', 'plan', 'feedback'] as const) {
        ...
        for (const spelling of [en[`token.${name}`], en[`token.${name}`]]) {
    ...
    it('a typed query ranks rows flat by name or label and drops the section headings', async () => {
      ...
      command.register(modelContribution())
      const names = async (query: string) => (await source.candidates(proj('s1'), req(query))).map(c => c.name)
      await expect(names('Model')).resolves.toEqual(['model'])
