...
import { renderIndexInjections, type WebServer, type WebRoute } from '@akashx/akx-host-webserver'
...
const MODULES_ID = '@akashx/akx-client-modules'
const UI_RENDERER_ID = '@akashx/akx-client-ui-renderer'
...
function writePackage(
  packageName: string,
  metadata: Record<string, unknown> = { akx: { client: { platform: 'web' } } },
): string {
  root ??= realpathSync(mkdtempSync(join(tmpdir(), 'akx-client-modules-')))
...
function writeBuiltPackage(packageName: string, client: Record<string, unknown>): void {
  const clientPath = writePackage(packageName, { akx: { client: { platform: 'web', ...client } } })
...
describe('HTML bootstrap facade', () => {
  it('precedes blocking preloads and the boot graph, then becomes the live registration target', async () => {
    ...
    const graphAt = html.indexOf('globalThis["__AKX_BOOT__"] = ')
...
describe('client bundle activation', () => {
  ...
  it('serves the source map beside a registered client bundle', async () => {
    ...
    const shellResponse = service.fetchBundle(new Request(`akx-app://app${batch.url}`))
...
describe('shared module declarations', () => {
  ...
  it('rejects a non-array external', () => {
    ...
    expect(() => construct([packageName]))
      .toThrow(`client-modules: ${packageName} akx.client.external must be a string array`)
  })
})
...
describe('module graph order', () => {
  ...
  it('leaves a request no row answers to the static assembly channel', () => {
    expect(ids(orderByModuleGraph([
      entry('consumer', { external: ['@akashx/cordis'] }),
      entry('other'),
