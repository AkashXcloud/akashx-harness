/**
 * Node half of the client module system (`akx.client` dual-face package): scans
 * the host Loader's entries for packages declaring `akx.client`, composes the
 * `window.__AKX_BOOT__` entry graph (wire single source: {@link WebBootEntry}
 ...
 * @module @akashx/akx-client-modules
 */
...
import type { Entry } from '@akashx/cordis-plugin-loader'
import type { IndexInjection } from '@akashx/akx-host-webserver'
import { exactPackageSpecifier, parseAkxClient, stripClientSuffix } from './client/manifest.ts'
...
function comboSectionMap(record: WebPluginRecord): Record<string, unknown> {
  ...
  const base = new URL(`/plugins/${record.entry.id}/client.js.map`, 'http://akx.invalid')
    ...
    for (const name of entry.external ?? []) {
      ...
      if (dependency === entry) {
        throw new Error(
          ...
          + '— a row must not declare its own package in akx.client.external',
        )
      }
...
const CLIENT_MODULES_ID = '@akashx/akx-client-modules'
...
window.__ModuleLoader__={
  ...
  rows.push({ kind: 'global', name: '__AKX_BOOT__', value: graph })
  return rows
}

/**
 * The web plugin table service: incremental `akx.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  private resolveMeta(loaderName: string, baseUrl: string): ResolvedPkgMeta | null {
    ...
    const akx = pkg.akx
    const decl = parseAkxClient(
      packageName,
      akx !== null && typeof akx === 'object' ? (akx as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${packageName} declares akx.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
