...
import Schema from '@akashx/schemastery'
import { bindSnapshotSelector, RemoteError } from '@akashx/akx-client-test-runtime'
import type {
...
import {
  AkashXModelsEditor, formatCapacity, modelDrafts, parseCapacity, validateAkashXModels,
} from '../src/client/AkashXModelsEditor.tsx'
...
const AKASHX_TARGET = { provider: 'akashx-official', displayName: 'AkashX' }
const deepSeekCopy = (template: string): string => providerCopy(template, AKASHX_TARGET)
...
const AkashXConfig = Schema.object({
  apiKeyEnv: Schema.string().role('credential-ref'),
...
const DEFAULT_AKASHX_MODELS = [
  ...
  { id: 'akashx-v4-pro', name: 'AkashX-V4-Pro', contextWindow: 1_000_000 },
]
...
function wireNamespaces(): SettingsNamespaceView[] {
  return [
    {
      ns: 'llm-akx',
      schema: JSON.parse(JSON.stringify(AkashXConfig.toJSON())) as JsonValue,
      value: {
        apiKeyEnv: 'AKASHX_API_KEY',
        baseURL: 'https://base',
        defaultContextWindow: 1_000_000,
        maxTokens: 256_000,
        models: DEFAULT_AKASHX_MODELS,
      },
      base: { defaultContextWindow: 1_000_000, maxTokens: 256_000, models: DEFAULT_AKASHX_MODELS },
...
const REFUSALS: { [Code in RefusalCode]: (message: string) => RemoteError<Code> } = {
  'credential/rejected': message => new RemoteError('credential/rejected', message, { ref: 'AKASHX_API_KEY' }),
 ...
 * anywhere, so the whole-section AkashX route owns the first-run setup card.
 */
...
/**
 * Mount and open the AkashX editor. The shared fixture already has a usable
 * openai route, so AkashX is an ordinary row whose card opens through Edit
 * rather than by itself.
 */
async function mountAkashXCard(overrides: Parameters<typeof scriptedFace>[0] = {}) {
...
describe('ModelsSection', () => {
  ...
  it('shows a configuration diagnostic inside the first-run setup card', async () => {
    ...
    scripted.face.llm.listConfigurableProviders.mockResolvedValue(remoteOk([
      { provider: 'akashx-official', displayName: 'AkashX', settingsNs: 'llm-akx', settingsPath: [], error: failure },
    ]))
    scripted.face.credentials.describe.mockResolvedValue(remoteOk({
      AKASHX_API_KEY: { configured: false, writable: true },
    }))
    await mountFace(scripted)
  ...
  it('dispatches the provider-card seat per rendered row, keyed by the owning namespace', async () => {
    ...
    expect(cards).toContainEqual(['akashx-official', true, false, 'llm-akx'])
  ...
  it('dispatches the provider-card seat inside the first-run setup card', async () => {
    ...
    expect(cardSeatCalls(renderSlot)).toContainEqual(['akashx-official', true, false, 'llm-akx'])
  })
  ...
  it('renders the unkeyed whole-section provider as an open setup card in the first-run posture', async () => {
    await mountFirstRun()
    // Nothing is reachable yet, and AkashX has no configured credential and
    // no stored apiKey → setup card.
    expect(screen.getByText('AkashX')).toBeTruthy()
    expect(screen.getByLabelText(en.keyInput)).toBeTruthy()
  ...
  it('leaves the unkeyed provider a plain row once another provider is usable', async () => {
    ...
    expect(missing.closest('li')?.textContent).toContain('AkashX')
  ...
  it('decides setup need from the joined credential state and the first-run posture', () => {
    const entry = { provider: 'p', displayName: 'p', settingsNs: 'llm-akx', settingsPath: [], active: true }
  ...
  it('uses one stable provider identity in action copy', () => {
    const target = { provider: 'akashx-official', displayName: 'AkashX' }
    expect(providerTargetLabel(target)).toBe('AkashX (akashx-official)')
    expect(providerCopy(en.deleteTitle, target)).toBe('Delete AkashX (akashx-official)?')
  ...
  it('stores a typed key write-only from the setup card without touching settings', async () => {
    ...
    await waitFor(() => { expect(set).toHaveBeenCalledWith('AKASHX_API_KEY', 'sk-live') })
    ...
    expect((await screen.findByRole('status')).textContent).toBe(
      providerCopy(en.savedProvider, { provider: 'akashx-official', displayName: 'AkashX' }),
    )
  ...
  it('reuses the provider editor as a required credential-only onboarding form', async () => {
    ...
    expect(set).toHaveBeenCalledWith('AKASHX_API_KEY', 'sk-onboarding')
    expect(mutate).not.toHaveBeenCalled()
  ...
  it('applies customized akashx fields as path ops', async () => {
    const { mutate } = await mountAkashXCard({
    ...
    // The akashx placeholder is pinned to the public endpoint, not the
    // effective value (which may reflect a launch-environment override).
    expect(baseURL.placeholder).toBe('https://api.akashx.com')
  ...
  it('materializes inherited models and adds an arbitrary AkashX id', async () => {
    const { mutate } = await mountAkashXCard({
      mutate: vi.fn(() => Promise.resolve(remoteOk(wireNamespaces()[0]))),
    })
    fireEvent.click(screen.getByText(en.customized))
    ...
    expect(screen.getAllByLabelText(new RegExp(en.modelId)).map(input => (input as HTMLInputElement).value))
      .toEqual(['akashx-v4-flash', 'akashx-v4-pro'])
  ...
  it('rejects duplicate AkashX model ids before writing', async () => {
    const { mutate } = await mountAkashXCard()
    fireEvent.click(screen.getByText(en.customized))
    fireEvent.click(screen.getByText(en.addModel))
    const ids = screen.getAllByLabelText(new RegExp(en.modelId))
    fireEvent.change(ids[2] as HTMLInputElement, { target: { value: 'akashx-v4-flash' } })
    fireEvent.click(screen.getByText(en.apply))
  ...
  it('validates every adapter-owned model catalog invariant', () => {
    ...
    expect(validateAkashXModels([{}])).toEqual({ index: 0, key: 'modelIdRequired' })
    expect(validateAkashXModels([{ id: 'same' }, { id: 'same' }]))
      .toEqual({ index: 1, key: 'modelIdDuplicate' })
    expect(validateAkashXModels([{ id: 'model', name: '' }]))
      .toEqual({ index: 0, key: 'modelNameInvalid' })
    expect(validateAkashXModels([{ id: 'model', contextWindow: null }]))
      .toEqual({ index: 0, key: 'modelContextInvalid' })
    expect(validateAkashXModels([{ id: 'model', contextWindow: 1.5 }]))
      .toEqual({ index: 0, key: 'modelContextInvalid' })
    expect(validateAkashXModels([{ id: 'model', contextWindow: 0 }]))
      .toEqual({ index: 0, key: 'modelContextInvalid' })
    expect(validateAkashXModels([{ id: 'model', contextWindow: 1 }])).toBeUndefined()
    expect(validateAkashXModels([{ id: 'model', maxTokens: null }]))
      .toEqual({ index: 0, key: 'modelMaxTokensInvalid' })
    expect(validateAkashXModels([{ id: 'model', maxTokens: 1.5 }]))
      .toEqual({ index: 0, key: 'modelMaxTokensInvalid' })
    expect(validateAkashXModels([{ id: 'model', maxTokens: 0 }]))
      .toEqual({ index: 0, key: 'modelMaxTokensInvalid' })
    expect(validateAkashXModels([{ id: 'model', maxTokens: 8192 }])).toBeUndefined()
  })
  ...
  it('accepts a suffixed context window and stores the plain count', async () => {
    ...
    expect(mutate.mock.calls[0]).toEqual([
      ...
      [{
        ...
        path: ['models'],
        value: [
          { ...DEFAULT_AKASHX_MODELS[0], contextWindow: 1_000_000 },
          { ...DEFAULT_AKASHX_MODELS[1], contextWindow: 256_000 },
        ],
      }],
      0,
    ])
  ...
  it.each([
    ...
    expect(screen.getByText(en.modelsInherited)).toBeTruthy()
    expect(screen.getAllByLabelText(new RegExp(en.modelId)).map(input => (input as HTMLInputElement).value))
      .toEqual(base === undefined ? ['akashx-v4-flash', 'akashx-v4-pro'] : ['pinned-by-deployment'])
  })
  ...
  it('edits an output cap per model and carries its text across a removal', async () => {
    ...
    expect(mutate.mock.calls[0]).toEqual([
      ...
      [{
        op: 'set',
        path: ['models'],
        value: [{ ...DEFAULT_AKASHX_MODELS[1], maxTokens: 64_000 }],
      }],
      0,
    ])
  })
  ...
  it('settles a pasted id and refuses whitespace that would never match', async () => {
    ...
    fireEvent.change(ids[0] as HTMLInputElement, { target: { value: '  akashx-v4-flash  ' } })
    fireEvent.blur(ids[0] as HTMLInputElement)
    expect((ids[0] as HTMLInputElement).value).toBe('akashx-v4-flash')
    ...
    expect(validateAkashXModels([{ id: '   ' }])).toEqual({ index: 0, key: 'modelIdRequired' })
    expect(validateAkashXModels([{ id: 'model' }, { id: 'model ' }]))
      .toEqual({ index: 1, key: 'modelIdDuplicate' })
  })

  it('renders malformed draft fallbacks without inventing catalog values', () => {
    render(<AkashXModelsEditor
      models={[{}]}
      overridden={false}
  ...
  it('can empty and reset the model override, then clear optional fields without dropping hidden data', async () => {
    ...
    expect(mutate.mock.calls[0]).toEqual([
      ...
      [{
        ...
        path: ['models'],
        value: [
          { id: 'akashx-v4-flash', description: 'Preserved hidden detail' },
          DEFAULT_AKASHX_MODELS[1],
        ],
      }],
      0,
    ])
  ...
  it('pins the akashx placeholder and clears typed input back to inherited', async () => {
    const { face } = scriptedFace()
  ...
  it('tells the user to reopen when another writer moved the namespace first', async () => {
    ...
    const { set } = await mountAkashXCard({
  ...
  it('surfaces a shadowed credential write on the card', async () => {
    await mountFirstRun({
      set: vi.fn(() => Promise.resolve(remoteFail('credentials: AKASHX_API_KEY is shadowed by the read-only environment'))),
    })
    const key = screen.getByLabelText<HTMLInputElement>(en.keyInput)
    fireEvent.change(key, { target: { value: 'sk-live' } })
    fireEvent.click(screen.getByText(en.apply))
  ...
  it('collapses the setup card on cancel without disturbing another open card', async () => {
    ...
    // …and AkashX collapsed to an ordinary row carrying the missing-key dot.
    expect(screen.getAllByLabelText(en.keyInput)).toHaveLength(1)
    expect(screen.getAllByRole('img', { name: en.credentialMissing })
      .some(dot => dot.closest('li')?.textContent?.includes('AkashX') === true)).toBe(true)
  ...
  it('loads on first render of an idle controller', async () => {
    ...
    render(<ModelsSection
      ...
      renderSlot={() => null}
    />)
    await screen.findByText('AkashX')
  })
...
describe('apiKeyFailure', () => {
  ...
  it.each([
    ['a pasted environment line', 'AKASHX_API_KEY=sk-abc'],
