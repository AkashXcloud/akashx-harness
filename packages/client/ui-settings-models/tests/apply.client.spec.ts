...
async function bench(isLoopback = true, mock = RemoteMock.create().load(remoteDefaultResponses), services: object = {}) {
  ...
  remote.$host = { home: undefined, isLoopback, isPrivileged: isLoopback }
...
describe('ui-settings-models apply', () => {
  ...
  it('the label thunk follows the active locale without re-registration', async () => {
    ...
    b.locale.setLocale('en')
    expect(resolveSlotLabel(b.slots.entries('settings.section')[0]!.options.label)).toBe('Model')
    expect(injected().t('deleteTitle')).toBe('删除 {provider}？')
  })
  ...
  it('registers the en/en nav dictionaries and disposes everything with the fiber', async () => {
    const b = await bench()
    declare(b.slots)
    const fiber = b.ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    expect(b.locale.bind('settings.models')('nav')).toBe('Model')
    await fiber.dispose()
    expect(b.slots.entries('settings.section')).toHaveLength(0)
    expect(b.slots.entries('settings.onboarding')).toHaveLength(0)
