 ...
 * both families, AkashX's id/name/context-window model catalog, and the
...
import {
  AkashXModelsEditor, modelDrafts, validateAkashXModels,
} from './AkashXModelsEditor.tsx'
...
type EditorLayout = 'akashx' | 'pi-ai' | 'unknown'

/** The public AkashX endpoint shown as the akashx base-URL placeholder. */
const AKASHX_PUBLIC_BASE_URL = 'https://api.akashx.com'
...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  ...
  const modelFailure = validateAkashXModels(schema.getPath(draft, ['models']))
  ...
  const applyOnce = async (): Promise<string | undefined> => {
    ...
    if (props.credentialOnly !== true) {
      ...
      const failure = validateAkashXModels(schema.getPath(next, ['models']))
   ...
   */
  const curatedFields = (family: 'akashx' | 'pi-ai'): ReactNode => {
    // What a hand-declared route names for itself and nothing else can supply.
    // A whole-section `llm-akx` profile is a composition fact with no
          ...
          <div className={styles['customizedBody']}>
            ...
            <div className={styles['field']}>
              ...
              <input
                ...
                placeholder={family === 'akashx'
                  ? AKASHX_PUBLIC_BASE_URL
            ...
            {/* Both families edit the same rows through the same contract; only
                the extras differ — AkashX's inherited capacities, pi-ai's
