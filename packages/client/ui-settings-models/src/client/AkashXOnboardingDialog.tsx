/**
 * Official-AkashX first-run step. Readiness comes from the same
 ...
 * official AkashX route. The step reuses that page's credential editor in
...
import type { InjectFace, PropsRuntime } from '@akashx/akx-client-ui-slots'
...
import styles from './AkashXOnboardingDialog.module.css'

/** Registration-side dependencies of {@link AkashXOnboardingDialog}. */
export interface AkashXOnboardingInjected {
  hooks: {
...
export type AkashXOnboardingDialogProps =
  PropsRuntime<'settings.onboarding'> & InjectFace<AkashXOnboardingInjected>

/* v8 ignore next 3 -- closed-union defaults only defend future source widening */
function assertNever(_value: never): never {
  throw new Error('unexpected AkashX onboarding state')
}

/**
 * Prompt a first-run user for the official AkashX credential while no
 * provider can serve requests and that credential is writable.
 * @param props - settings-shell owner state and Models feature dependencies.
 * @returns the onboarding modal or null when onboarding needs no intervention.
 */
export function AkashXOnboardingDialog(props: AkashXOnboardingDialogProps): ReactNode {
  ...
  const namespace = state.namespaces.get('llm-akx')
