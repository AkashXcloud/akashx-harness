/**
 * Curated editor for the direct AkashX adapter's advisory model catalog.
...
export type AkashXModelDraft = Record<string, unknown>
 ...
 * (rejected by {@link validateAkashXModels} before any write).
 */
...
export interface AkashXModelsValidationFailure {
...
export function modelDrafts(value: unknown): AkashXModelDraft[] {
  if (!Array.isArray(value)) return []
  return value.map(entry =>
    typeof entry === 'object' && entry !== null && !Array.isArray(entry)
      ? entry as AkashXModelDraft
      : {})
}
 ...
 */
export function validateAkashXModels(value: unknown): AkashXModelsValidationFailure | undefined {
...
/** Props of {@link AkashXModelsEditor}. */
export interface AkashXModelsEditorProps {
  /** Effective rows: inherited until the parent materializes an override. */
  models: readonly AkashXModelDraft[]
  ...
  onChange: (models: AkashXModelDraft[]) => void
  /** Remove the user-owned array and return to inheritance. */
  onReset: () => void
}

/**
 * Render the direct AkashX adapter's model catalog: id and display name on
 * each row, capacities behind the row's own disclosure.
 * @param props - effective rows plus the array-level override actions.
 * @returns the catalog editor.
 */
export function AkashXModelsEditor(props: AkashXModelsEditorProps): ReactNode {
  ...
  const capacityText = (model: AkashXModelDraft, index: number, field: CapacityField): string => {
