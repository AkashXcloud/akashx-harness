...
import { SidebarRightTabRegistry } from '@akashx/akx-client-ui-sidebar-right/src/client/tab-registry.ts'
...
describe('basenameOf', () => {
  it('decodes the last segment, so an escaped name reads as itself', () => {
    expect(basenameOf('akx-resource://file/session/s-1/work/notes/a%20b%23c.md')).toBe('a b#c.md')
  })

  it('falls back to the whole address when there is no last segment', () => {
    expect(basenameOf('akx-resource://file/session/s-1/')).toBe('akx-resource://file/session/s-1/')
  })

  it('keeps a malformed percent escape as it is rather than refusing the address', () => {
    expect(basenameOf('akx-resource://file/session/s-1/work/%E0%A4%A')).toBe('%E0%A4%A')
  })
})
...
describe('textDefinition', () => {
  it('is the fallback claimant of every Session file address, titled by basename', () => {
    ...
    expect(definition.patterns).toEqual(['akx-resource://file/**'])
    expect(definition.priority).toBe('fallback')
    expect(definition.title('akx-resource://file/session/s-1/work/README.md')).toBe('README.md')
    // Reads require an addressed Session, even when the file path is absolute.
    expect(definition.canOpen?.('akx-resource://file/session/s-1/work/README.md')).toBe(true)
    expect(definition.canOpen?.(sessionFileAddress('s-1', '/home/me/README.md'))).toBe(true)
    expect(definition.canOpen?.('akx-resource://file/absolute/home/me/README.md')).toBe(false)
    expect(definition.canOpen?.('akx-resource://file/shared/team/README.md')).toBe(false)
    expect(definition.canOpen?.('akx-resource://file/session')).toBe(false)
  })
})
...
describe('text type in the registry', () => {
  ...
  it('claims files of any extension, depth, and dot-prefix with relative or absolute Session paths', () => {
    ...
    for (const address of [
      'akx-resource://file/session/s-1/a.md',
      'akx-resource://file/session/s-1/deep/er/path/x.py',
      'akx-resource://file/session/s-1/w/.env',
      'akx-resource://file/session/s-1/w/Makefile',
  ...
  it.each([
    'akx-resource://file/shared/team/notes.md',
    'akx-resource://file/absolute/home/me/notes.md',
    'akx-resource://file/absolute/C:/w/notes.md',
  ...
  it('yields an address to a narrower type at the extension band, and keeps the rest', () => {
    ...
    expect(tabs.claim('akx-resource://file/session/s-1/w/logo.png').kind).toBe('image')
    expect(tabs.claim('akx-resource://file/session/s-1/w/logo.md').kind).toBe(TEXTPREVIEW_KIND)
    // Still listed for the picture: a caller naming the kind may open it as text.
    expect(tabs.candidates('akx-resource://file/session/s-1/w/logo.png').map(type => type.kind)).toEqual(['image', TEXTPREVIEW_KIND])
  })
