...
describe('HTML registration', () => {
  ...
  it('registers its dictionary and matching keyed body, and removes all contributions on disposal', async () => {
    ...
    expect(dictionaries.get('documentHtml')).toEqual({ en })
    ...
    await injected?.readRelated('akx-resource://file/session/explicit-session/sub/index.html', '../app.js', signal)
    ...
    expect(() => injected?.readRelated('akx-resource://file/absolute/workspace/index.html', './app.js', signal))
