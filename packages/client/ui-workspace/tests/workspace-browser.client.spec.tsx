...
function mount(overrides: Partial<WorkspaceBrowserProps> = {}) {
  ...
  const props: WorkspaceBrowserProps = {
    ...
    useHostInfo: selector => selector({ home: undefined, isLoopback: true, isPrivileged: true }),
...
describe('WorkspaceBrowser', () => {
  ...
  it('workspace hover card shows a POSIX home descendant as ~', () => {
    ...
    try {
      mount({
        ...
        useHostInfo: selector => selector({ home: '/home/u', isLoopback: true, isPrivileged: true }),
      })
  ...
  it('anchors collapsed drags before hidden rows so the source stays visible', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Show less' }))
    const collapsedBlank = screen.getByText('New Session').closest('[role="treeitem"]') as HTMLElement
  ...
  it('renders a fork child as a top-level row without a session twist', () => {
    ...
    expect(screen.queryByRole('button', { name: /Expand|Show less/u })).toBeNull()
