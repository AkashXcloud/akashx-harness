...
import type { SettingsPathOpView } from '@akashx/akx-api-remotes/client'
import { RemoteError, stubSettingsScope, type StubSettingsScope } from '@akashx/akx-client-test-runtime'
...
} from '@akashx/akx-client-ui-settings/src/client/settings-mirror.ts'
...
function credentialsApi(configured: boolean) {
  const describe = vi.fn(() => Promise.resolve({
    ok: true as const,
    value: { AKASHX_API_KEY: { configured, writable: true } },
  }))
...
describe('WebSearchCardController', () => {
  ...
  it('writes the staged key through the credentials domain, never the settings section', async () => {
    ...
    credentials.describe.mockImplementation(() => Promise.resolve({
      ok: true as const,
      value: { AKASHX_API_KEY: { configured: true, writable: true } },
    }))
    face.save()
    await vi.waitFor(() => { expect(credentials.set).toHaveBeenCalled() })

    expect(credentials.set).toHaveBeenCalledWith('AKASHX_API_KEY', 'ds-secret')
    expect(host.set).not.toHaveBeenCalled()
    await vi.waitFor(() => {
  ...
  it('re-reads when the Host reports the watched reference changed', async () => {
    ...
    controller.refreshCredential('AKASHX_API_KEY')
  ...
  it('keeps the card usable when the credential read is refused', async () => {
    ...
    const refusal = () => Promise.resolve({
      ok: false as const,
      error: new RemoteError('credential/rejected', 'offline', { ref: 'AKASHX_API_KEY' }),
    })
...
describe('ConfigurablePluginsTabController', () => {
  ...
  it('never dispatches a card whose namespace this deployment does not serve', async () => {
    ...
    const controller = new ConfigurablePluginsTabController(settings.mirror, () => ledger('bash', 'web-search-akx'))
