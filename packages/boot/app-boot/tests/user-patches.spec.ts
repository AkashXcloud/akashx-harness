/**
 * User patch-layer behavior of `akx-app-boot`: the optional patch-list loader
...
const tmp = (): string => {
  const dir = mkdtempSync(join(tmpdir(), 'akx-user-patches-'))
  tempRoots.push(dir)
  return dir
}
...
describe('loadOptionalPatches', () => {
  ...
  it('parses a patch list and preserves !!js expressions as loader expression nodes', () => {
    ...
    writeFileSync(join(dir, PROFILE_PATCH_FILENAME), [
      ...
      '  config:',
      '    model: !!js process.env.AKX_SPEC_MODEL',
      '- insert:',
      '    - id: llm',
    ...
    expect(patches?.[0]).toMatchObject({
      id: 'agent-loop',
      config: { model: { __jsExpr: 'process.env.AKX_SPEC_MODEL' } },
    })
  ...
  it.each([
    ...
    writeFileSync(patchPath, JSON.stringify([
      ...
      { insert: [
        ...
        { id: 'bare', name: '@akashx/akx-system-prompt' },
    ...
    expect(patches[1]?.insert?.map(entry => entry.name)).toEqual([
      pluginUrl, pluginUrl, '@akashx/akx-system-prompt', 'cordis:group',
    ])
...
describe('boot with user patches', () => {
  it('applies id-targeted overrides, inserts, and interpolates !!js from the environment', async () => {
    ...
    writeFileSync(join(userDir, PROFILE_PATCH_FILENAME), [
      ...
      '  name: ./noop.mjs',
      '  config:',
      '    value: !!js process.env.AKX_APP_BOOT_USER_SPEC',
      '- insert:',
      '    - id: user-extra',
      '      name: ./noop.mjs',
      '',
    ].join('\n'))
    process.env['AKX_APP_BOOT_USER_SPEC'] = 'user-value'
    ...
    try {
      ...
      await ctx.fiber.dispose()
      delete process.env['AKX_APP_BOOT_USER_SPEC']
    }
  })
