 ...
 * `akx --profile` launcher family.
 *
 * A profile is a directory under `$AKX_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `akx.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"akx": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `akx.profile.bundles` order over
 ...
 * first from the akx installation (the launcher's own package), then from the
 * profile directory. Pnpm-managed entries in the profile's `node_modules`
 * resolve first. Akx-owned links add packages carried only by selected
 * bundles, while `$AKX_HOME/profiles/node_modules` supplies the installation
 ...
 * @module @akashx/akx-app-boot/profile
 */
...
import { withFileLock } from '@akashx/akx-atomic-write'
import type { EntryOptions } from '@akashx/cordis-plugin-loader'
import { applyEntryPatches, type PatchOptions } from '@akashx/cordis-plugin-include'
import { resolveAkxHome } from '@akashx/akx-home-paths'
import type { AkxPackageManifest, ProfilePatchReload } from '@akashx/akx-package-manifest'
...
const PROFILE_MODULE_FALLBACK_DIR = '.akx-module-fallback'
...
export type ProfileManifest = Partial<AkxPackageManifest>

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `akx.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `akx.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`akx --profile <name>`).
 * @param home - the Harness home; defaults to {@link resolveAkxHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveAkxHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`akx: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, ProfileTemplate> = {
  acp: {
    bundles: ['@akashx/akx-base', '@akashx/akx-acp-app'],
    patchReload: 'startup',
  },
  web: {
  ...
  headless: {
    bundles: ['@akashx/akx-base', '@akashx/akx-headless'],
    patchReload: 'startup',
  },
  sdk: {
    bundles: ['@akashx/akx-base', '@akashx/akx-sdk-app'],
    patchReload: 'startup',
  },
  'sdk-minimal': {
    bundles: ['@akashx/akx-sdk-minimal'],
    patchReload: 'startup',
  },
}
...
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@akashx/akx-base', '@akashx/akx-web-app', '@akashx/akx-headless'],
}

/** The bundle list a `akx plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@akashx/akx-base']
...
const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this akx profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `akx.profile.bundles` layer list.
...
export function initProfile(
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `akx-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      akx: { profile: { bundles: [...bundles], patchReload } },
    }
...
/** Ensure `link` is a symlink to `target`, replacing a wrong link or a akx-managed packaged proxy. */
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      ...
      if (existing?.akx?.moduleFallback?.targets === undefined) {
        throw new Error(`akx: ${link} exists and is not a symlink or akx-managed module proxy; remove it so akx can manage the installation fallback`)
      }
...
interface ModuleProxyManifest {
  ...
  akx: { moduleFallback: { targets: Record<string, string> } }
}

interface ModuleProxyRecord {
  version?: unknown
  akx?: { moduleFallback?: { targets?: unknown } }
}
...
function packageEntryFromPackage(
  ...
  try {
    ...
    throw new Error(`akx: cannot resolve ESM export ${specifier} from installed package ${packageName}`, { cause: error })
  }
  for (const candidate of candidates ?? []) {
    ...
    if (!target.startsWith('./') || /^\.\.(?:[\\/]|$)/u.test(relativeEntry)) {
      throw new Error(`akx: installed package ${packageName} export ${subpath} resolves outside its package: ${target}`)
    }
...
function packageProxySource(
  ...
  if (typeof manifest.version !== 'string' || manifest.version.length === 0) {
    throw new Error(`akx: installed package ${packageName} must declare a non-empty version`)
  }
  ...
  if (declared === undefined) {
    ...
    try {
      ...
      throw new Error(`akx: installed package ${packageName} main entry is missing at ${entry}`, { cause: error })
    }
  }
...
function ensureModuleProxy(
  ...
  if (stat !== undefined) {
    ...
    if (existing?.akx?.moduleFallback?.targets === undefined) {
      throw new Error(`akx: ${link} exists and is not a akx-managed module proxy; remove it so akx can manage the installation fallback`)
    }
    if (existing.version === version
      && JSON.stringify(existing.akx.moduleFallback.targets) === JSON.stringify(targets)
...
function resolveModuleFallbackEntries(
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (akx-subprocess,
    // akx-compaction, ...) are peers of their implementations, never plain
...
function moduleFallbackEntryCurrent(modulesDir: string, entry: ModuleFallbackEntry): boolean {
  ...
  try {
    ...
    return existing?.version === entry.version
      && JSON.stringify(existing.akx?.moduleFallback?.targets) === JSON.stringify(entry.targets)
...
export interface ProfileModuleFallbackOptions {
  /** Absolute package.json path of the running akx installation. */
  installAnchor: string
  /** Loaded profile whose selected bundles may carry profile-local plugins. */
  profile?: Profile
  /** Harness home; defaults to {@link resolveAkxHome}. */
  home?: string
}

/**
 * Maintain module fallbacks for one profile launch. The shared
 * `$AKX_HOME/profiles/node_modules` mirrors the akx installation dependency
...
export async function healProfilesModuleFallback(options: ProfileModuleFallbackOptions): Promise<void> {
  const { installAnchor, profile, home = resolveAkxHome() } = options
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.akx?.profile?.bundles
  ...
  const needsReloadDefault = manifest.akx?.profile?.patchReload === undefined && isCurrentTuple
  ...
  const normalized: ProfileManifest = {
    ...
    akx: {
      ...manifest.akx,
      profile: {
        ...manifest.akx?.profile,
        bundles: [...template.bundles],
        patchReload: manifest.akx?.profile?.patchReload ?? template.patchReload,
      },
    },
  }
 ...
 * `@akashx/akx-base` (and every other in-box bundle) always comes from
 * the same installation as the running akx, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `akx.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the akx app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the akx installation or ${profileDir}; `
    + `run 'akx plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}
 ...
 * @param installAnchor - absolute path of the owning akx app's package.json.
...
export function loadProfileDirectory(
  ...
  const bundles = manifest.akx?.profile?.bundles ?? []
  const rawPatchReload: unknown = manifest.akx?.profile?.patchReload
  if (rawPatchReload !== undefined && rawPatchReload !== 'live' && rawPatchReload !== 'startup') {
    throw new Error(
      `${binName}: profile manifest ${join(dir, 'package.json')} akx.profile.patchReload must be "live" or "startup"`,
    )
  }
  ...
  const layers = bundles.map((packageName): ProfileLayer => {
    ...
    const declared = bundleManifest.akx?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no akx.bundle in its package.json`)
    }
...
/**
 * Load a profile: resolve every `akx.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `akx.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the akx app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveAkxHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveAkxHome(),
  options: { userLayer?: boolean } = {},
): Profile {
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'akx plugin --profile ${name} add <package>'`,
      )
    }
