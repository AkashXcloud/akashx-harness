/**
 * Shared boot glue for `akx` profiles, including the CLI packaged by the Python runtime wheel: load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the Harness home (`~/.akx`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @akashx/akx-app-boot
 */
...
import { Context, type FiberState } from '@akashx/cordis'
import Loader, { type Entry, type EntryOptions } from '@akashx/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@akashx/cordis-plugin-include'
import Group from '@akashx/cordis-plugin-group'
import { akxHomePath, resolveAkxHome } from '@akashx/akx-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@akashx/akx-launch-environment'
import type {} from '@akashx/cordis-plugin-hmr'
import type {} from '@akashx/akx-system-prompt'

declare module '@akashx/cordis' {
  interface Context {
    /** Harness-home path resolver available to Loader `!!js` config expressions. */
    akxHomePath?: typeof akxHomePath
  }
}
 ...
 * @param snapshotMode - the bin's `$AKX_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_NAMES = new Set([
  ...
  'AKASHX_BASE_URL', 'AKASHX_SEARCH_BASE_URL',
...
const BOOTSTRAP_PREFIXES = ['AKX_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
 ...
 * them; the home file is the user's own, and `AKX_HOME` is itself bootstrap-only, so no `.env` can
 ...
 * directory. The self-referential `akx-tool-cordis` toolset reads and edits this
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    text: `The AkashX Harness implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend AKX itself.`,
  })
}
