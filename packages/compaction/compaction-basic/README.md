...
Mount this package to get automatic conversation condensation in a composition that already provides an LLM, session storage, and token measurement. The shipped `akx` base enables it by default; mount it explicitly to control when condensation starts.
...
- name: '@akashx/akx-command-compact'
```
...
All settings are optional. The defaults start condensing at 80% of the routed model's context window and keep the newest 16% verbatim; the table below is the complete policy surface, and the generated [configuration catalog](../../../docs/config-catalog.md#akashx-akx-compaction-basic) is the exhaustive source.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#akashx-akx-compaction-basic) — every accepted config field and its source declaration.
...
- **Overflow classification is adapter-maintained** — provider wording can change; both AkashX adapters normalize recognized context-limit failures to `CONTEXT_WINDOW_EXCEEDED`.
