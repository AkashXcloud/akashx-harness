...
import * as commandCompact from '@akashx/akx-command-compact'
...
describe('command-compact real Loader composition', () => {
  it('discovers and executes /compact through the assembled command plane', async () => {
    root = await mkdtemp(join(tmpdir(), 'akx-command-compact-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@akashx/akx-command-compact', commandCompact],
    ])
    ...
    expect(context.commands.list(agent)).toContainEqual({
      definitionId: '@akashx/akx-command-compact',
      name: 'compact',
