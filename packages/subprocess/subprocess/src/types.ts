 ...
 * @module akx-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for AkashX Harness-managed child environment facts. */
export const AKX_ENV_PREFIX = 'AKX_' as const

/** One environment key inside the managed {@link AKX_ENV_PREFIX} namespace. */
export type AkxEnvironmentKey = `${typeof AKX_ENV_PREFIX}${string}`

/** Trusted AkashX Harness variables for one child-process execution. */
export type AkxEnvironment = Readonly<Record<AkxEnvironmentKey, string>>
 ...
 * to `TerminalSignal` in `@akashx/akx-terminal` without a cross-seam dependency;
