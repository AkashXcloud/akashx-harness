...
import { scrubbedParentEnv, SubprocessRuntime } from '@akashx/akx-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and AKX_ names (case-insensitively) but keeps PATH', () => {
    process.env.AKX_SCRUB_PROBE = 'stale'
    process.env.akx_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.AKX_SCRUB_PROBE).toBeUndefined()
      expect(env.akx_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.AKX_SCRUB_PROBE
      delete process.env.akx_scrub_probe_lower
