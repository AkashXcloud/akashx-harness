...
import type { SubprocessSpawnSpec, SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from '@akashx/akx-subprocess'
import { childEnv } from '../src/spawn.ts'

function mockWin32ForIsolatedRuntime(): void {
  vi.doMock('@akashx/akx-win32-process', () => ({
...
function unmockWin32ForIsolatedRuntime(): void {
  vi.doUnmock('@akashx/akx-win32-process')
}
...
describe('LocalSubprocessRuntime', () => {
  ...
  it('resolves absolute and PATH executables and honors lookup cancellation', async () => {
    ...
    await expect(ctx.subprocess.resolveExecutable('node_modules/.bin/server'))
      .rejects.toThrow('is a relative path')
    await expect(ctx.subprocess.resolveExecutable('akx-command-that-does-not-exist', { PATH: '' }))
      .rejects.toThrow('was not found on PATH')
    await expect(ctx.subprocess.resolveExecutable('/akx-absolute-command-that-does-not-exist'))
  ...
  it('rechecks native prerequisites for every eligible spawn and prepares storage before launch', async () => {
    ...
    const prepareManagedProcessBinding = vi.fn(() => ({ spillDir: '/tmp/akx-test-spill' }))
  ...
  it('disposal tolerates a handle whose spawn already failed', async () => {
    const ctx = new Context()
    const fiber = await ctx.plugin(LocalSubprocessRuntime)
    const handle = ctx.subprocess.spawn(spec('true', { cwd: '/nonexistent-dir-akx-subprocess-test' }))
