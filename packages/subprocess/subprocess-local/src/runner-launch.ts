...
export const SUBPROCESS_RUNNER_ENV = 'AKX_SUBPROCESS_RUNNER' as const
...
export function spawnRunnerInvocation(): RunnerInvocation {
  ...
  if (extname(fileURLToPath(import.meta.url)) !== '.ts') {
    return [process.execPath, fileURLToPath(import.meta.resolve('@akashx/akx-subprocess-local/runner'))]
  }
  return [
    process.execPath,
    '--import',
