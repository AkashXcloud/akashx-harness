...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'akx_state=$1',
  'mapfile -d \'\' -t akx_env < "$akx_state/environment"',
  'mapfile -d \'\' -t akx_argv < "$akx_state/argv"',
  'akx_output_marker=$(<"$akx_state/output-marker")',
  'rm -f -- "$akx_state/environment" "$akx_state/argv" "$akx_state/output-marker" "$akx_state/runner.bash"',
  'if (( ${#akx_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$akx_output_marker"',
  'exec env -i -- "${akx_env[@]}" "${akx_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`akx-e2b-bootstrap:${randomUUID()}`)
