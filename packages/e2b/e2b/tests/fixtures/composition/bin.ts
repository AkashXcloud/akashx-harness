...
import { boot } from '@akashx/akx-app-boot'
import { type Agent } from '@akashx/akx-agent'
import { Session, SessionId } from '@akashx/akx-session'
import type {} from '@akashx/akx-fs-e2b'
import type {} from '@akashx/akx-bash-local'
import type {} from '@akashx/akx-lsp-stdio'
import type {} from '@akashx/akx-terminal-bash'
...
try {
  ...
  const outputDrainHandle = ctx.subprocess.spawn({
    argv: ['bash', '-c', "bash -c 'exec -a akx-output-drain-descendant sleep 30' & printf 'leader-done\\n'"],
    cwd: process.cwd(),
  ...
  const outputDrainClean = !outputDrainProcesses.some(processInfo =>
    JSON.stringify([processInfo.cmd, processInfo.args]).includes('akx-output-drain-descendant'),
  )
  ...
  const sleeping = ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "printf 'AKX_SLEEP_%s\\n' READY; sleep 30",
    submit: true,
  })
  let sleepReadyOutput = ''
  const sleepReadyDeadline = Date.now() + 5_000
  while (!sleepReadyOutput.includes('AKX_SLEEP_READY\n')) {
    sleepReadyOutput += sleeping.readOutput().delta
    if (sleepReadyOutput.includes('AKX_SLEEP_READY\n')) break
  ...
  const stubborn = await ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "bash -c 'trap \"\" TERM; exec sleep 30' & printf 'AKX_STUBBORN_PID=%s\\n' \"$!\"",
    submit: true,
  }).done
  const stubbornMatch = /AKX_STUBBORN_PID=([1-9][0-9]*)/.exec(stubborn.viewport)
