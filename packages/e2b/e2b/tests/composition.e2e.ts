...
import { runLoaderSmoke } from '@akashx/akx-loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
  SandboxNotFoundError,
} from '@akashx/akx-e2b'
import TerminalSessionService, { TerminalSessionId } from '@akashx/akx-terminal'
import { BashTerminalBackend } from '@akashx/akx-terminal-bash'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', AKX_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/akx-e2b-bootstrap-profile-leak'
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'akx_leak=0',
          'for akx_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$akx_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$akx_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then akx_leak=1; fi',
          'done',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> AKX=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$AKX_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> AKX=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'akx-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.akx-e2b/terminals/')
