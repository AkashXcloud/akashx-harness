/**
 * End-to-end tests for akx-mcp-client. Exercises the REAL MCP protocol against:
...
import { apply } from '@akashx/akx-mcp-client/src/index.ts'
import { publicToolName } from '@akashx/akx-mcp-client/src/tools.ts'
...
async function mountImageRegistry(akxHome: string): Promise<Context> {
  const ctx = await mountRegistry()
  await ctx.plugin(LocalAttachmentStore, { akxHome })
