 ...
 * @module @akashx/akx-jobs-local
 */

import { Context } from '@akashx/cordis'
import z from '@akashx/schemastery'
import type { Agent } from '@akashx/akx-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@akashx/akx-scope'
import type { ScopeLayer } from '@akashx/akx-scope'
import { deadline, timeoutOf } from '@akashx/akx-timeout'
import { JobRegistry, JobId } from '@akashx/akx-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@akashx/akx-jobs'
 ...
 * `@akashx/akx-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @akashx/akx-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @akashx/akx-agent)')
    }
