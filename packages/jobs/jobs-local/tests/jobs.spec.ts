...
import type { ScopeKey } from '@akashx/akx-scope'
import { JobId } from '@akashx/akx-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@akashx/akx-jobs'
import LocalJobRegistry, { type Config as JobsConfig } from '@akashx/akx-jobs-local'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @akashx/akx-tool-jobs in its composition)')
  })
