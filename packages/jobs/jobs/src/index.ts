 ...
 * process-local registry lives in `@akashx/akx-jobs-local`.
 * @module @akashx/akx-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@akashx/akx-jobs is the abstract job registry seam; load an implementation such as @akashx/akx-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
