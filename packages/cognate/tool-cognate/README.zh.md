...
`dsh-tool-cognate` 注册 `run_sql` 和 `render_chart`。`run_sql` 将单条语句发送到 `ctx.cognate`；服务负责 SQL 策略、取消、Provider 选择和结果限制。`render_chart` 只验证参数中已有的表格数据，不打开数据库连接。
...
生成的 [`run_sql` 和 `render_chart` Schema](../../../docs/tool-catalog.zh.md#deepseek-aidsh-tool-cognate) 提供 SQL 参数或图表元数据和已有行。`render_chart` 不能执行查询或访问 Provider。
...
- 图表以结构化展示元数据返回，专用 Client 图表渲染器延期。
- 工具不提供独立的知识搜索 schema；生产文档检索使用已配置 Provider 提供的 AkashXDB 操作。

### 开发备注
<a id="dev-note"></a>

除延期的 Client 图表渲染器和 Provider 提供的知识操作外，本包没有待维护者决定的开放事项。
