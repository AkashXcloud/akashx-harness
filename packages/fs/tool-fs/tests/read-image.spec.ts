...
import { CodeRuntime } from '@akashx/akx-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@akashx/akx-code-runtime'
import { ToolCallId, LlmAdapter, LlmRuntime } from '@akashx/akx-llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, Message, StreamChunk } from '@akashx/akx-llm'
import SystemPrompt from '@akashx/akx-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@akashx/akx-tools'
import type { Config as ToolConfig } from '@akashx/akx-tools'
import LocalFileSystem from '@akashx/akx-fs-local'
import * as FsPolicy from '@akashx/akx-fs-observation-policy'
import LocalAttachmentStore from '@akashx/akx-attachment-local'
import { AttachmentError, AttachmentId, AttachmentStore } from '@akashx/akx-attachment'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'akx-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'akx-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { akxHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { akxHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { akxHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
