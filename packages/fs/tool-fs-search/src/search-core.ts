 ...
 *
 * @module @akashx/akx-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@akashx/akx-output-retention'
import type { RetainedItems } from '@akashx/akx-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@akashx/akx-subprocess'
 ...
 * `@akashx/akx-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`akx-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@akashx/akx-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@akashx/akx-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@akashx/akx-tool-fs-search/presentation} `grepSearchMeta`)
