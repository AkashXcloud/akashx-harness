...
describe('ripgrep resolution', () => {
  it('uses the native sidecar beside the current executable', async () => {
    ...
    process.execPath = '/runtime/akx'
    existsSync.mockReturnValue(true)
    const sidecar = '/runtime/akx-rg'
    const { resolveRgPath } = await import('@akashx/akx-tool-fs-search')
  ...
  it('uses a conventional executable name for the Windows ripgrep sidecar', async () => {
    ...
    process.execPath = 'C:\\runtime\\akx-harness-sdk-runtime-win-x64.exe'
    existsSync.mockReturnValue(true)
    const sidecar = 'C:\\runtime\\akx-harness-sdk-runtime-win-x64-rg.exe'
    const { resolveRgPath } = await import('@akashx/akx-tool-fs-search')
