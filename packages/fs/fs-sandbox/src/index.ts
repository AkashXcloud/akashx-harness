 ...
 * `@akashx/akx-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * (`@akashx/akx-bash-sandbox`). The residual
 ...
 * `FS_SANDBOX_DENIED`.
 *
 * @module @akashx/akx-fs-sandbox
 */

import { Context } from '@akashx/cordis'
import { LocalFileSystem } from '@akashx/akx-fs-local'
import type { Config as LocalConfig } from '@akashx/akx-fs-local'
import { FsError } from '@akashx/akx-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@akashx/akx-fs'
import { writableRoots } from '@akashx/akx-sandbox'
import type { SandboxExecutionPolicy, SandboxMode } from '@akashx/akx-sandbox'
import type {} from '@akashx/akx-sandbox-policy'
 ...
 * INSTEAD OF `akx-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `akx-tool-fs` resolves
