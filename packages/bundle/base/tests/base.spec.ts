/**
 * The bundle's substance is its patch file: the `akx.bundle.patch` manifest
...
describe('akx-base bundle', () => {
  it('declares a parseable patch list through the akx.bundle.patch manifest field', () => {
    ...
    expect(rows.find(row => row.id === 'session-telemetry-otel')?.config?.['mode']).toEqual({
      __jsExpr: "process.env.AKX_TELEMETRY_MODE || 'FEEDBACK_ONLY'",
    })
    ...
    expect(manifest.dependencies).not.toHaveProperty('@akashx/akx-subagent-claude-code')
    expect(manifest.dependencies).toHaveProperty('@akashx/akx-web-fetch-http')
  })
