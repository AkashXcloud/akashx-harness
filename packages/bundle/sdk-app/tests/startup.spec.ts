...
import { internals, provideCmdline } from '@akashx/akx-cmdline'
...
describe('SDK app startup', () => {
  ...
  it('prints app help without publishing readiness or binding stdin', () => {
    ...
    expect(out()).toContain('akx --profile sdk')
  ...
  it('renders the selected SDK profile name in help', () => {
    ...
    expect(out()).toContain('Usage: akx --profile sdk-minimal')
    expect(out()).toContain('akx --profile sdk-minimal')
  })
})
