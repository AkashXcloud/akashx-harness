...
afterEach(async () => {
  ...
  vi.unstubAllEnvs()
  Reflect.deleteProperty(globalThis, '__akxWebAppApply')
  Reflect.deleteProperty(globalThis, '__akxWebServer')
  Reflect.deleteProperty(globalThis, '__akxConnection')
})

describe('web app browser startup', () => {
  it('opens the canonical URL only after the complete page is reachable', async () => {
    const root = mkdtempSync(join(tmpdir(), 'akx-web-browser-open-'))
    tempRoots.push(root)
    ...
    writeFileSync(webserverModule, 'export default globalThis.__akxWebServer\n')
    writeFileSync(connectionModule, [
      "export const inject = ['webServer']",
      "export const apply = ctx => ctx.provide('connection', globalThis.__akxConnection)",
      '',
    ].join('\n'))
    writeFileSync(webAppModule, [
      "export const name = 'fixture-web-app'",
      "export const inject = ['webServer']",
      'export const apply = (ctx, config) => globalThis.__akxWebAppApply(ctx, config)',
      '',
    ].join('\n'))
    ...
    const globals = globalThis as unknown as {
      __akxWebAppApply: typeof apply
      __akxWebServer: typeof WebServer
      __akxConnection: {
    ...
    globals.__akxWebAppApply = apply
    globals.__akxWebServer = WebServer
    globals.__akxConnection = {
