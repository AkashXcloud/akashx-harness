...
import { AssistantStreamAccumulator, createUserMessage, createSystemMessage, ToolCallId, createMessage } from '@akashx/akx-llm'
import type { ContentBlock, Message, TokenUsage } from '@akashx/akx-llm'
import SessionStore, { Session, SessionId, SessionSeq, canonicalHeader } from '@akashx/akx-session'
import type { EpochHeader, SessionEvent, SessionSeq as SessionSeqType } from '@akashx/akx-session'
import SessionProjectionRegistry from '@akashx/akx-session-projection'
import TokenMeter from '@akashx/akx-token-meter'
import type { TokenMeasurement, TokenMeterConfig } from '@akashx/akx-token-meter'
...
const SYSTEM_PLUGIN = '@akashx/akx-system-prompt'
...
describe('TokenMeter pricing', () => {
  ...
  it('prices tools, the system node, and the surface when no reusable usage exists', () => {
    ...
    appendHeader(session, header('akashx-v4-flash', { tools: [READ_TOOL] }))
    const result = service.measure(session)
...
describe('replay anchors and surface folds', () => {
  ...
  it('uses disjoint provider usage and signed durable-output rewrites', () => {
    ...
    appendSuccessfulCall(session, header('akashx-v4-flash'), {
      providerText: 'short',
  ...
  it('invalidates usage for any canonical envelope change or explicit override', () => {
    ...
    const anchoredHeader = header('akashx-v4-flash')
    ...
    expect(service.measure(session, header('akashx-v4-pro')).baseline.kind)
      .toBe('estimated')
  ...
  it('folds the latest full header snapshot into the effective envelope', () => {
    ...
    appendHeader(session, header('akashx-v4-flash'))
    session.append('request/header', {
      header: header('akashx-v4-pro'),
      reason: 'change',
    })
  ...
  it('replays seeded append and replace operations with signed deltas', () => {
    ...
    appendSuccessfulCall(original, header('akashx-v4-flash'), {
      usage: USAGE,
...
describe('malformed replay and listener lifecycle', () => {
  ...
  it('rejects an assistant without its step boundary transactionally', () => {
    ...
    session.append('assistant/message', {
      ...
      message: createMessage({
        ...
        source: {
          kind: 'model',
          ...{ provider: 'mock', model: 'akashx-v4-flash' },
        },
      }),
    }, { surfaceOp: 'append' })
  ...
  it('clears completed step boundaries and rejects overlapping or late step events', () => {
    ...
    appendHeader(late, header('akashx-v4-flash'))
