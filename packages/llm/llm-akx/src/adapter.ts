/**
 * `AkashXAdapter`: fetch + SSE against a AkashX (OpenAI-compatible)
 * chat-completions endpoint, emitting harness StreamChunks. The adapter is
 * transport-only: connection facts arrive through a thunk resolved once per
 * operation and the bearer token through a per-request resolver, so the
 * registering plugin owns validation, layering, and credential policy.
 *
 * @module akx-llm-akx/adapter
 */

import { attributionHeaders, contentHasImage, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, LlmAdapter, LlmError, offloadedImageText, offloadRequestImagesWithPolicy, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId } from '@akashx/akx-llm'
import type {
  ContentBlock,
  GenerateOptions,
...
import type { CredentialRef } from '@akashx/akx-credentials'
import { deadline, idleWatchdog, timeoutOf } from '@akashx/akx-timeout'
import type { AnonymousUserId } from '@akashx/akx-anonymous-user-id'
import type {
  AkashXLlmApiExtensionRequest,
  AkashXLlmApiJson,
  PreparedAkashXLlmApiExtensions,
} from '@akashx/akx-llm-api-extensions'
...
import { AkashXFileStore } from './file-store.ts'
import type { AkashXFilePolicy } from './file-store.ts'
import type { AkashXFileId } from './file-id.ts'
 ...
 */
export interface AkashXConnectionOptions {
  ...
  models: readonly AkashXCatalogModel[]
  ...
  filePolicy: AkashXFilePolicy
  /** Provider-owned model-request retry policy, already resolved. */
  retryPolicy: ResolvedRetryPolicy
}

/** Constructor options for {@link AkashXAdapter}: the operation-local resolution hooks the plugin owns. */
export interface AkashXAdapterOptions {
  /** Current validated connection facts; called once per operation. */
  options: () => AkashXConnectionOptions
  /**
   ...
   */
  resolveApiKey: (connection: AkashXConnectionOptions) => Promise<string>
  ...
  resolveFiles?: () => AkashXFileStore
  /** Prepare the official API's plugin-contributed top-level fields for one exact wire request. */
  prepareExtensions: (request: AkashXLlmApiExtensionRequest) => Promise<PreparedAkashXLlmApiExtensions>
}
...
const FILES_API_TIMEOUT_CODE = 'AKASHX_FILES_API_TIMEOUT'
...
class FileResolutionFailure extends Error {
  constructor(cause: unknown) {
    super('AkashX Files API could not resolve a request image.', { cause })
...
function detailNamesFileId(detail: string, fileId: AkashXFileId): boolean {
...
function normalizedImageDiagnostic(
  ...
  if (target !== undefined) {
    return `AkashX rejected normalized image ${normalizedImageFacts(target)}: ${providerMessage}. `
  ...
  return `AkashX rejected a normalized request image: ${providerMessage}. Candidate images: `
    + `${candidates.map(normalizedImageFacts).join('; ')}. `
    + 'The provider rejected bytes already normalized by the harness; PNG, JPEG, WebP, and GIF remain supported input formats.'
}

function modelInfo(provider: string, model: AkashXCatalogModel): LlmModelInfo {
  return {
    provider,
    id: model.id,
...
function requestId(headers: Headers): ReturnType<typeof ProviderRequestId> | undefined {
  const value = headers.get('x-request-id') ?? headers.get('x-akashx-request-id')
 ...
 */
export class AkashXAdapter extends LlmAdapter {
  private readonly files: AkashXFileStore

  constructor(private readonly config: AkashXAdapterOptions) {
    super()
    this.files = config.resolveFiles?.() ?? new AkashXFileStore()
  }

  override providerInfo(provider: string): LlmProviderInfo {
    return { id: provider, name: 'AkashX' }
  }
  ...
  private modelInfoFor(
    connection: AkashXConnectionOptions,
    provider: string,
    model: string,
  ): LlmResolvedModelInfo {
  ...
  private async * streamWithConnection(
    ...
    if (hasImages) {
      ...
      if (model?.inputModalities?.includes('image') !== true) {
        throw new LlmError(
          `AkashX model "${options.model}" does not accept image input.`,
          'UNSUPPORTED_CONTENT',
        )
      }
      ...
      if (attachments === undefined) {
        throw new LlmError(
          'AkashX image conversion requires the durable attachment service.',
          'UNSUPPORTED_CONTENT',
        )
      }
    }
    ...
    try {
      ...
      if (timeoutOf(watchdog.signal, STREAM_IDLE_TIMEOUT_CODE) !== undefined) {
        throw new LlmError(
          `AkashX stream idle timeout after ${connection.streamIdleTimeoutMs}ms`,
          'TIMEOUT',
          { cause: error },
        )
      }
      if (options.signal?.aborted) {
        throw new LlmError('AkashX request aborted by caller', 'ABORTED', { cause: error })
      }
      if (error instanceof LlmError) throw error
      throw new LlmError(`AkashX API stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    } finally {
      consumer.abort('AkashX stream consumer stopped')
  ...
  private async * request(
    ...
    const headers = {
      ...
      ...attributionHeaders(),
      'x-akx-harness-user-id': String(userId),
      ...options.sessionId !== undefined
        ? { 'x-akx-harness-session-id': String(options.sessionId) }
        : {},
      ...options.purpose === 'compaction'
        ? { 'x-akx-harness-compact': '1' }
        : {},
    }
    ...
    while (true) {
          ...
          body = await serializeRequestWithImages(requestOptions, {
            representation: {
              ...
              resolveFileId: async (version, _block, location) => {
                ...
                let resolved: Awaited<ReturnType<AkashXFileStore['ensureUploaded']>>
                try {
      ...
      let extensions: PreparedAkashXLlmApiExtensions
      try {
        extensions = await this.config.prepareExtensions({
          body: body as unknown as Readonly<Record<string, AkashXLlmApiJson>>,
          signal,
          ...options.sessionId === undefined ? {} : { sessionId: String(options.sessionId) },
          ...options.purpose === undefined ? {} : { purpose: options.purpose },
        })
      } catch (error) {
        throw new LlmError('AkashX request extension preparation failed', 'REQUEST_EXTENSION', { cause: error })
      }
      for (const field of Object.keys(extensions.fields)) {
        if (Object.hasOwn(body, field)) {
          throw new LlmError(`AkashX request extension field ${JSON.stringify(field)} collides with the base request`, 'REQUEST_EXTENSION')
        }
      }
      ...
      try {
        ...
        if (signal.aborted) throw error
        throw new LlmError(
          `AkashX API request to ${connection.baseURL} failed`,
          'TRANSPORT',
          { cause: error },
        )
      }
      ...
      if (!response.ok) {
        let message = `AkashX API error (HTTP ${response.status})`
        ...
        throw new LlmError(message, httpErrorCode(response.status, providerError), {
          cause: new Error(rawResponse.length > 0 ? rawResponse : `AkashX HTTP ${response.status}`),
      ...
      try {
        ...
        throw new LlmError('AkashX request extension acceptance failed', 'REQUEST_EXTENSION', { cause: error })
      }
      if (!response.body) {
        throw new LlmError('AkashX API returned no response body', 'EMPTY_RESPONSE')
      }
