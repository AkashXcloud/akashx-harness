/**
 * AkashX vision-token accounting: the provider's published image-token
 * calculator (api-docs.akashx.com, Token & Token Usage) ported verbatim in
 ...
 *
 * @module akx-llm-akx/image-tokens
 */
...
function safeResize(height: number, width: number, paddedHeight: number, paddedWidth: number): GridResize {
  ...
  if (solved.numTokens > MAX_IMAGE_TOKENS) {
    throw new Error(`akashx image tokens: no grid fits the token budget for ${width}x${height}`)
  }
  return solved
}
...
/**
 * Vision tokens AkashX charges for one request image of the given
 * dimensions.
...
export function deepSeekImageTokens(width: number, height: number): number {
  ...
  throw new Error(`akashx image tokens: resize did not converge for ${width}x${height}`)
}
