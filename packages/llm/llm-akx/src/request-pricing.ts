/**
 * Provider-side request-image pricing for AkashX routes: reproduces the
 ...
 *
 * @module akx-llm-akx/request-pricing
 */

import { offloadedImageText, offloadedImagePrefixCount, requestImageHandleText, textOnlyImageText } from '@akashx/akx-llm'
import type { ImageAttachmentAccessResolver, LlmImageRequestPrice, LlmImageRequestPricing } from '@akashx/akx-llm'
import { requestImageDimensions } from '@akashx/akx-attachment'
import type { ImageAttachmentRef, ImageRequestPolicy } from '@akashx/akx-attachment'
...
/**
 * Resolve the request-image budgets owned by one AkashX model route.
 * @param model - Advertised model route and its optional image overrides.
 * @returns Complete pixel and encoded-byte budgets.
 * @internal
 */
export function resolveRequestImagePolicy(model: AkashXCatalogModel): ImageRequestPolicy {
...
/**
 * Build the request-image pricing for one AkashX route from a validated
