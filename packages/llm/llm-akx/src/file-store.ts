/** AkashX Files API upload reuse, invalidation, and quota recovery. @module akx-llm-akx/file-store */

import type { RequestImageAttachment } from '@akashx/akx-attachment'
import { LlmError } from '@akashx/akx-llm'
import { AkashXFilesClient, isFilesQuotaError } from './files-api.ts'
import type { AkashXFileId } from './file-id.ts'
import { deepSeekFileScope, AkashXUploadIndex } from './upload-index.ts'
import type { AkashXUploadRecord } from './upload-index.ts'

/** AkashX chat accepts at most 32 MiB per image even when it is referenced by file id. */
export const MAX_CHAT_IMAGE_BYTES = 32 * 1024 * 1024
const OWNED_FILE_PREFIX = 'akx-'

/** Resolved file-store policy from the plugin configuration. */
export interface AkashXFilePolicy {
...
export interface AkashXFileConnection {
  baseURL: string
  apiKey: string
}

/** Result of one file-id resolution. */
export interface AkashXFileReference {
  record: AkashXUploadRecord
  uploaded: boolean
}

interface FileStoreOptions {
  index?: AkashXUploadIndex
  now?: () => number
  fetch?: typeof fetch
}
...
interface SharedUpload {
  ...
  promise: Promise<AkashXFileReference>
  settled: boolean
  waiters: number
}
...
function abortReason(signal: AbortSignal): Error {
  ...
  return reason instanceof Error
    ? reason
    : new Error('AkashX file upload cancelled with a non-Error reason.', { cause: reason })
}

function uploadFailure(error: unknown): Error {
  return error instanceof Error
    ? error
    : new Error('AkashX file upload failed with a non-Error reason.', { cause: error })
}

function waitForUpload(operation: SharedUpload, signal: AbortSignal | undefined): Promise<AkashXFileReference> {
  ...
  return new Promise<AkashXFileReference>((resolve, reject) => {
    const abort = (): void => {
...
/** User-scoped durable file-id reuse for the AkashX route. */
export class AkashXFileStore {
  private readonly index: AkashXUploadIndex
  ...
  constructor(options: FileStoreOptions = {}) {
    this.index = options.index ?? new AkashXUploadIndex()
    this.now = options.now ?? Date.now
    this.fetchImpl = options.fetch
  }

  private client(connection: AkashXFileConnection): AkashXFilesClient {
    return new AkashXFilesClient({
  ...
  ensureUploaded(
    ...
    connection: AkashXFileConnection,
    policy: AkashXFilePolicy,
    signal?: AbortSignal,
  ): Promise<AkashXFileReference> {
    signal?.throwIfAborted()
  ...
  private async ensureUploadedOnce(
    ...
    if (version.bytes > MAX_CHAT_IMAGE_BYTES) {
      throw new LlmError('AkashX chat image exceeds the 32 MiB per-image limit.', 'INVALID_REQUEST')
    }
    const scope = deepSeekFileScope(connection.baseURL, connection.apiKey)
    const now = this.now()
    ...
    const upload = async (): Promise<AkashXUploadRecord> => {
      ...
      if (remote.bytes !== version.data.byteLength) {
        throw new LlmError('AkashX Files API upload response does not match the submitted image.', 'INVALID_RESPONSE')
      }
      return {
        scope,
    ...
    let candidate: AkashXUploadRecord
    try {
  ...
  async reclaimOldestOwned(
    ...
    const client = this.client(connection)
    let after: AkashXFileId | undefined
    const owned: AkashXFileId[] = []
   ...
   */
  async releaseAll(connection: AkashXFileConnection, signal?: AbortSignal): Promise<number> {
    let total = 0
    for (;;) {
