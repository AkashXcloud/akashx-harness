/**
 * Register a {@link AkashXAdapter} for the `akashx-official` provider route on
 * `ctx.llm`, with connection facts resolved per request instead of frozen at
 * load: the plugin layers its `cordis.yml` entry config under the optional
 * `llm-akx` user-settings section (`ctx.settings`) and resolves the API
 * key through the optional credential seam (`ctx.credentials`), so a changed
 * base URL, catalog, or key reaches the very next request without restarting
 * anything, while an in-flight stream keeps the facts it started with. The
 * one registration-captured fact — the retry policy — re-registers the route
 * in place when it changes.
 * @module @akashx/akx-llm-akx
 */

import type { Context } from '@akashx/cordis'
import z from '@akashx/schemastery'
import { assertUsableApiKey, LlmError, resolveImageAttachmentAccess, resolveRetryPolicy, RetryPolicySchema } from '@akashx/akx-llm'
import type { ModelModality, RetryPolicyConfig } from '@akashx/akx-llm'
import type {} from '@akashx/akx-fs'
import { credentialRef } from '@akashx/akx-credentials'
import { launchEnvironmentOf, type LaunchEnvironmentSnapshot } from '@akashx/akx-launch-environment'
...
import type { AkashXCatalogModel, AkashXConnectionOptions } from './adapter.ts'
import {
...
export type { AkashXAdapterOptions, AkashXCatalogModel, AkashXConnectionOptions } from './adapter.ts'
export {
  DEFAULT_LOW_DETAIL_IMAGE_PIXEL_BUDGET,
  DEFAULT_MAX_IMAGES_PER_REQUEST,
  DEFAULT_MAX_REQUEST_FILES_BYTES,
...
export { AkashXFileStore, MAX_CHAT_IMAGE_BYTES } from './file-store.ts'
export type { AkashXFileConnection, AkashXFilePolicy, AkashXFileReference } from './file-store.ts'
export { AkashXFilesClient, MAX_FILE_EXPIRY_SECONDS, MAX_FILE_UPLOAD_BYTES, MAX_STORED_FILE_BYTES, MAX_STORED_FILE_COUNT, MIN_FILE_EXPIRY_SECONDS } from './files-api.ts'
export type { AkashXFileObject, AkashXFilePage } from './files-api.ts'
export { AkashXFileId } from './file-id.ts'
export type { AkashXFileId as AkashXFileIdType } from './file-id.ts'
export { AkashXUploadIndex, deepSeekFileScope } from './upload-index.ts'
export type { AkashXUploadRecord } from './upload-index.ts'
export type { RequestDefaults } from './serialize.ts'
export type * from './types.ts'

export const name = 'llm-akx'
export const inject = ['llm']

const NS = 'llm-akx'
const DEFAULT_API_KEY_ENV = 'AKASHX_API_KEY'
/** The single provider route this plugin owns. */
const PROVIDER = 'akashx-official'

const DEFAULT_MODELS: AkashXCatalogModel[] = [
  {
    id: 'akashx-flash',
  ...
  {
    id: 'akashx-v4-flash-vision-exp',
    name: 'AkashX-V4-Flash-Vision-Exp',
    contextWindow: DEFAULT_CONTEXT_WINDOW,
    inputModalities: ['text', 'image'],
    imagePixelBudget: DEFAULT_REQUEST_IMAGE_PIXEL_BUDGET,
    imageMaxBytes: DEFAULT_REQUEST_IMAGE_MAX_BYTES,
...
const catalogModel: z<AkashXCatalogModel> = z.object({
...
/** Public API default; the internal endpoint comes from $AKASHX_BASE_URL. */
export const PUBLIC_BASE_URL = 'https://api.akashx.com'

/** Environment variable naming this provider's endpoint, honored only from trusted layers. */
const BASE_URL_ENV = 'AKASHX_BASE_URL'
 ...
 * newer key.
 */
export type ResolvedAkashXOptions = AkashXConnectionOptions

/** Resolve, validate, and detach the advisory model catalog. */
function resolveModels(models: readonly AkashXCatalogModel[] | undefined): AkashXCatalogModel[] {
  const seen = new Set<string>()
  return (models ?? DEFAULT_MODELS).map((model) => {
    if (Object.hasOwn(model, 'imageDetail')) {
      throw new Error('llm-akx: catalog model imageDetail is no longer supported; use imagePixelBudget')
    }
    if (model.id.length === 0) throw new Error('llm-akx: catalog model ids must be non-empty')
    if (model.name !== undefined && model.name.length === 0) {
      throw new Error(`llm-akx: catalog model "${model.id}" has an empty name`)
    }
    if (model.contextWindow !== undefined
      && (!Number.isInteger(model.contextWindow) || model.contextWindow <= 0)) {
      throw new Error(
        `llm-akx: catalog model "${model.id}" contextWindow must be a positive integer`,
      )
    }
    if (model.maxTokens !== undefined
      && (!Number.isInteger(model.maxTokens) || model.maxTokens <= 0)) {
      throw new Error(
        `llm-akx: catalog model "${model.id}" maxTokens must be a positive integer`,
      )
    }
    const inputModalities = model.inputModalities ?? ['text']
    if (inputModalities.length === 0) {
      throw new Error(`llm-akx: catalog model "${model.id}" inputModalities must not be empty`)
    }
    if (inputModalities.some(modality => !MODEL_MODALITIES.includes(modality))) {
      throw new Error(
        `llm-akx: catalog model "${model.id}" inputModalities must contain only "text" and "image"`,
      )
    }
    if (new Set(inputModalities).size !== inputModalities.length) {
      throw new Error(`llm-akx: catalog model "${model.id}" inputModalities must not contain duplicates`)
    }
    const hasImage = inputModalities.includes('image')
    if (!hasImage && (model.imagePixelBudget !== undefined || model.imageMaxBytes !== undefined)) {
      throw new Error(`llm-akx: text-only catalog model "${model.id}" cannot declare image request limits`)
    }
    if (model.imagePixelBudget !== undefined
      ...
      throw new Error(`llm-akx: catalog model "${model.id}" imagePixelBudget must be "low" or a positive safe integer`)
    }
    if (model.imageMaxBytes !== undefined
      && (!Number.isSafeInteger(model.imageMaxBytes) || model.imageMaxBytes <= 0)) {
      throw new Error(`llm-akx: catalog model "${model.id}" imageMaxBytes must be a positive safe integer`)
    }
    ...
    if (systemPromptUpdate !== undefined && systemPromptUpdate !== 'in-history') {
      throw new Error(`llm-akx: catalog model "${model.id}" systemPromptUpdate must be "in-history" when present`)
    }
    if (seen.has(model.id)) throw new Error(`llm-akx: duplicate catalog model "${model.id}"`)
    seen.add(model.id)
    return {
      id: model.id,
 ...
 */
export function resolveAdapterOptions(config: Config, environment?: LaunchEnvironmentSnapshot): ResolvedAkashXOptions {
  if (config.thinking === 'disabled'
    && config.reasoningEffort !== undefined
    && config.reasoningEffort !== 'off') {
    throw new Error('llm-akx: only reasoningEffort "off" can be configured when thinking is disabled')
  }
  if (config.defaultContextWindow !== undefined
    && (!Number.isInteger(config.defaultContextWindow) || config.defaultContextWindow <= 0)) {
    throw new Error('llm-akx: defaultContextWindow must be a positive integer')
  }
  if (config.maxTokens !== undefined
    && (!Number.isSafeInteger(config.maxTokens) || config.maxTokens <= 0)) {
    throw new Error('llm-akx: maxTokens must be a positive safe integer')
  }
  ...
  if (!Number.isFinite(streamIdleTimeoutMs)
    ...
    throw new Error(
      `llm-akx: streamIdleTimeoutMs must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`,
    )
  }
  ...
  if (!Number.isSafeInteger(maxRequestFilesBytes) || maxRequestFilesBytes <= 0) {
    throw new Error('llm-akx: maxRequestFilesBytes must be a positive safe integer')
  }
  ...
  if (!Number.isSafeInteger(maxInlineRequestImageBytes) || maxInlineRequestImageBytes <= 0) {
    throw new Error('llm-akx: maxInlineRequestImageBytes must be a positive safe integer')
  }
  ...
  if (!Number.isSafeInteger(maxImagesPerRequest) || maxImagesPerRequest <= 0) {
    throw new Error('llm-akx: maxImagesPerRequest must be a positive safe integer')
  }
  ...
  if (!Number.isSafeInteger(imageOffloadByteQuantum) || imageOffloadByteQuantum <= 0) {
    throw new Error('llm-akx: imageOffloadByteQuantum must be a positive safe integer')
  }
  if (imageOffloadByteQuantum > maxRequestFilesBytes) {
    throw new Error('llm-akx: imageOffloadByteQuantum must not exceed maxRequestFilesBytes')
  }
  ...
  if (!Number.isSafeInteger(inlineImageOffloadByteQuantum) || inlineImageOffloadByteQuantum <= 0) {
    throw new Error('llm-akx: inlineImageOffloadByteQuantum must be a positive safe integer')
  }
  if (inlineImageOffloadByteQuantum > maxInlineRequestImageBytes) {
    throw new Error('llm-akx: inlineImageOffloadByteQuantum must not exceed maxInlineRequestImageBytes')
  }
  ...
  if (!Number.isSafeInteger(imageOffloadCountQuantum) || imageOffloadCountQuantum <= 0) {
    throw new Error('llm-akx: imageOffloadCountQuantum must be a positive safe integer')
  }
  if (imageOffloadCountQuantum > maxImagesPerRequest) {
    throw new Error('llm-akx: imageOffloadCountQuantum must not exceed maxImagesPerRequest')
  }
  ...
  if (!Number.isFinite(filesApiTimeoutMs)
    ...
    throw new Error(
      `llm-akx: filesApiTimeoutMs must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`,
    )
  }
  ...
  if (!Number.isSafeInteger(fileExpiresAfterSeconds)
    ...
    throw new Error('llm-akx: fileExpiresAfterSeconds must be an integer from 3600 through 2592000')
  }
  ...
  if (!Number.isSafeInteger(fileRefreshMarginSeconds)
    ...
    throw new Error('llm-akx: fileRefreshMarginSeconds must be a non-negative integer below fileExpiresAfterSeconds')
  }
  ...
  if (!Number.isSafeInteger(fileQuotaCleanupBatch)
    ...
    throw new Error('llm-akx: fileQuotaCleanupBatch must be an integer from 1 through 1000')
  }
  return {
    ...
    retryPolicy: resolveRetryPolicy(config.retryPolicy, 'llm-akx: retryPolicy'),
  }
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  let lastGood: ResolvedAkashXOptions | undefined
  const options = (): ResolvedAkashXOptions => {
    const raw = current()
    ...
    try {
      ...
      lastRaw = raw
      ctx.logger.error('llm-akx: keeping the last good configuration after an invalid settings section')
  ...
  const resolveApiKey = async (connection: ResolvedAkashXOptions): Promise<string> => {
    ...
    if (credentials !== undefined) {
      ...
      if (hit !== undefined) return assertUsableApiKey(hit.value, 'llm-akx', ref)
    } else {
      // Without the seam there is no managed store to rank against, so the
      // environment is the whole credential plane.
      const ambient = launchEnvironmentOf(ctx).get(ref)
      if (ambient !== undefined && ambient.value.length > 0) {
        return assertUsableApiKey(ambient.value, 'llm-akx', ref)
      }
    }
    throw new LlmError(
      `llm-akx: no API key for provider route "${PROVIDER}"; store ${ref} through the credentials`
  ...
  const adapter = new AkashXAdapter({
    ...
    prepareExtensions: (request) => {
      const extensions = ctx.get('akashxLlmApiExtensions')
      return extensions?.prepare(request)
        ?? Promise.resolve({ fields: {}, accept: () => Promise.resolve() })
    },
  })
  ctx.llm.registerConfigurableProviders([
    { provider: PROVIDER, displayName: 'AkashX', settingsNs: NS, settingsPath: [] },
  ])
