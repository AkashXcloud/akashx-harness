/** Durable AkashX attachment-to-file-id index. @module akx-llm-akx/upload-index */

import { createHash } from 'node:crypto'
import { readFile, mkdir } from 'node:fs/promises'
import { dirname, join } from 'node:path'
import { withFileLock, writeFileAtomic } from '@akashx/akx-atomic-write'
import { resolveAkxHome } from '@akashx/akx-home-paths'
import { ImageVariantId } from '@akashx/akx-attachment'
import type { AttachmentId, ImageVariantId as ImageVariantIdType } from '@akashx/akx-attachment'
import { AkashXFileId, AkashXFileScope } from './file-id.ts'
import type { AkashXFileId as AkashXFileIdType, AkashXFileScope as AkashXFileScopeType } from './file-id.ts'

/** One durable remote upload mapping. Unix times are milliseconds. */
export interface AkashXUploadRecord {
  scope: AkashXFileScopeType
 ...
 */
export function deepSeekFileScope(baseURL: string, apiKey: string): AkashXFileScopeType {
  const digest = createHash('sha256')
    .update(baseURL.replace(/\/+$/u, ''))
    .update('\0')
    .update(apiKey)
    .digest('hex')
  return AkashXFileScope(digest)
}
...
function parseRecord(value: unknown): AkashXUploadRecord {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-akx: upload index contains a non-object record')
  }
  ...
  if (typeof record.scope !== 'string' || !/^[0-9a-f]{64}$/u.test(record.scope)
    ...
    throw new InvalidUploadIndexError('llm-akx: upload index contains an invalid record')
  }
  return {
    scope: AkashXFileScope(record.scope),
    attachmentId: record.attachmentId as AttachmentId,
    variantId: ImageVariantId(record.variantId),
    fileId: AkashXFileId(record.fileId),
...
function parseIndex(text: string): StoredIndex {
  ...
  try {
    ...
    throw new InvalidUploadIndexError('llm-akx: upload index is not valid JSON', { cause: error })
  }
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-akx: upload index is not an object')
  }
  const index = value as { formatVersion?: unknown; records?: unknown }
  if (index.formatVersion !== 3 || !Array.isArray(index.records)) {
    throw new InvalidUploadIndexError('llm-akx: unsupported upload index format')
  }
  const records = index.records.map(parseRecord)
  const keys = new Set<string>()
  for (const record of records) {
    const key = `${record.scope}\0${record.variantId}`
    if (keys.has(key)) throw new InvalidUploadIndexError('llm-akx: upload index contains duplicate mappings')
    keys.add(key)
  }
  return { formatVersion: 3, records }
}

function reusable(record: AkashXUploadRecord, now: number, refreshMarginMs: number): boolean {
  return record.expiresAt - now > refreshMarginMs
}

/** Atomic local index shared by every AkashX session in this AKX home. */
export class AkashXUploadIndex {
  /** Absolute owner-private JSON index path. */
  readonly path: string

  /**
   * @param path - explicit test path; omission uses `AKX_HOME/llm-akx/files-v3.json`.
   */
  constructor(path = join(resolveAkxHome(), 'llm-akx', 'files-v3.json')) {
    this.path = path
  }
  ...
  ): Promise<AkashXUploadRecord | undefined> {
   ...
   * @param scope - endpoint/API-key namespace.
   */
  async clear(scope: AkashXFileScopeType): Promise<void> {
    await mkdir(dirname(this.path), { recursive: true, mode: 0o700 })
    await withFileLock(this.path, async () => {
      const index = await this.load()
