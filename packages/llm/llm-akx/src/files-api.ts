/** OpenAI-compatible AkashX Files API transport. @module akx-llm-akx/files-api */

import { attributionHeaders, LlmError } from '@akashx/akx-llm'
import type { ImageMediaType } from '@akashx/akx-attachment'
import { AkashXFileId } from './file-id.ts'
import type { AkashXFileId as AkashXFileIdType } from './file-id.ts'
...
export interface AkashXFileObject {
  id: AkashXFileIdType
  bytes: number
  createdAt: number
  filename: string
...
export interface AkashXFilePage {
  data: AkashXFileObject[]
  firstId?: AkashXFileIdType
  lastId?: AkashXFileIdType
  hasMore: boolean
}

/** Files API operation failure with its HTTP status retained for recovery policy. */
export class AkashXFilesError extends LlmError {
 ...
 */
export function isFilesQuotaError(error: unknown): error is AkashXFilesError {
  return error instanceof AkashXFilesError
...
function invalidResponse(operation: string): LlmError {
  return new LlmError(`AkashX Files API returned an invalid ${operation} response.`, 'INVALID_RESPONSE')
}

function parseFileObject(value: unknown, operation: string): AkashXFileObject {
...
export class AkashXFilesClient {
  ...
  private async request(path: string, init: RequestInit, signal?: AbortSignal): Promise<Response> {
    ...
    try {
      ...
      throw new LlmError(`AkashX Files API request to ${this.baseURL} failed`, 'TRANSPORT', { cause: error })
    }
    ...
    throw new AkashXFilesError(
      message ?? `AkashX Files API error (HTTP ${response.status})`,
      response.status,
      detail,
    )
  }
  ...
  async upload(input: {
    ...
    signal?: AbortSignal
  }): Promise<AkashXFileObject & { expiresAt: number }> {
    if (input.data.byteLength > MAX_FILE_UPLOAD_BYTES) {
      throw new LlmError('AkashX Files API upload exceeds 128 MiB.', 'INVALID_REQUEST')
    }
    if (!Number.isSafeInteger(input.expiresAfterSeconds)
      ...
      throw new LlmError('AkashX file expiry must be between 3600 and 2592000 seconds.', 'INVALID_REQUEST')
    }
   ...
   */
  async list(options: {
    after?: AkashXFileIdType
    limit?: number
    order?: 'asc' | 'desc'
    signal?: AbortSignal
  } = {}): Promise<AkashXFilePage> {
    ...
    return {
      ...
      ...typeof wire.first_id === 'string' ? { firstId: AkashXFileId(wire.first_id) } : {},
      ...typeof wire.last_id === 'string' ? { lastId: AkashXFileId(wire.last_id) } : {},
   ...
   */
  async retrieve(fileId: AkashXFileIdType, signal?: AbortSignal): Promise<AkashXFileObject> {
   ...
   * @param signal - request cancellation.
   */
  async delete(fileId: AkashXFileIdType, signal?: AbortSignal): Promise<void> {
