...
import type { ImageAttachmentRef, RequestImageAttachment } from '@akashx/akx-attachment'
import { AkashXFileStore, MAX_CHAT_IMAGE_BYTES } from '../src/file-store.ts'
import { AkashXFileId } from '../src/file-id.ts'
import { deepSeekFileScope, AkashXUploadIndex } from '../src/upload-index.ts'
...
const CONNECTION = { baseURL: 'https://api.akashx.com', apiKey: 'key' }
...
describe('AkashXFileStore', () => {
  it('singleflights the first upload and reuses the durable mapping across store instances', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'akx-file-store-'))
    roots.push(dir)
    const index = new AkashXUploadIndex(join(dir, 'index.json'))
    const remote = uploadFetch()
    const first = new AkashXFileStore({ index, now: () => NOW, fetch: remote.fetchImpl })
    ...
    const resumed = new AkashXFileStore({ index, now: () => NOW, fetch: remote.fetchImpl })
  ...
  it('keeps a shared upload alive while another waiter remains', async () => {
    ...
    const store = new AkashXFileStore({ index, now: () => NOW, fetch: fetchImpl })
  ...
  it('normalizes a non-Error cancellation reason', async () => {
    ...
    const store = new AkashXFileStore({
      index: new AkashXUploadIndex(join(dir, 'index.json')),
      now: () => NOW,
      fetch: fetchImpl,
    })
    const controller = new AbortController()
    ...
    await expect(upload).rejects.toMatchObject({
      message: 'AkashX file upload cancelled with a non-Error reason.',
      cause: 'cancelled',
    })
  })
  ...
  it('starts a fresh upload while the cancelled transport is settling', async () => {
    ...
    const fetchImpl = vi.fn((_url: string | URL | Request, init?: RequestInit) => {
      ...
      return Promise.resolve(new Response(JSON.stringify({
        ...
        filename: 'akx-retry.png', purpose: 'user_data',
        expires_at: NOW / 1_000 + POLICY.expiresAfterSeconds,
      }), { status: 200 }))
    }) as typeof fetch
    const store = new AkashXFileStore({
  ...
  it('rejects a request version above the chat per-image limit before transport', async () => {
    ...
    const store = new AkashXFileStore({ now: () => NOW, fetch: fetchImpl })
  ...
  it('rejects an upload response whose byte count differs from the request version', async () => {
    ...
    const fetchImpl = vi.fn(() => Promise.resolve(new Response(JSON.stringify({
      ...
      filename: 'akx-wrong.png', purpose: 'user_data',
      expires_at: NOW / 1_000 + POLICY.expiresAfterSeconds,
  ...
  it.each([
    ...
    const remote = uploadFetch()
    const store = new AkashXFileStore({
      index: new AkashXUploadIndex(join(dir, `${extension}.json`)),
      now: () => NOW,
  ...
  it('normalizes a non-Error failure from the durable upload index', async () => {
    ...
    const store = new AkashXFileStore({ index, now: () => NOW, fetch: vi.fn() as typeof fetch })

    await expect(store.ensureUploaded(VERSION, CONNECTION, POLICY)).rejects.toMatchObject({
      message: 'AkashX file upload failed with a non-Error reason.',
  ...
  it('reuses local expires_at above the refresh margin and uploads again at the margin', async () => {
    ...
    const store = new AkashXFileStore({ index, now: () => now, fetch: remote.fetchImpl })
  ...
  it('removes a losing upload and keeps the winning durable mapping when duplicate cleanup fails', async () => {
    ...
    vi.spyOn(index, 'commit').mockResolvedValue({
      ...
      record: {
        ...
        fileId: AkashXFileId('file-api-winner'),
        bytes: 3,
        createdAt: NOW,
    ...
    const fetchImpl = vi.fn((input: string | URL | Request, init?: RequestInit) => {
      if (init?.method === 'POST') {
        ...
        return Promise.resolve(new Response(JSON.stringify({
          ...
          filename: 'akx-recovered.png', purpose: 'user_data',
          expires_at: NOW / 1_000 + POLICY.expiresAfterSeconds,
        }), { status: 200 }))
      }
      if (init?.method === 'DELETE') {
      ...
      return Promise.resolve(new Response(JSON.stringify({
        ...
        data: [{
          ...
          filename: 'akx-old.png', purpose: 'user_data',
        }],
  ...
  it('stops pagination when a page omits or repeats its cursor', async () => {
    ...
    for (const mode of ['missing', 'repeated'] as const) {
      ...
      const store = new AkashXFileStore({
        index: new AkashXUploadIndex(join(dir, `${mode}.json`)),
        now: () => NOW,
        fetch: fetchImpl,
      })
