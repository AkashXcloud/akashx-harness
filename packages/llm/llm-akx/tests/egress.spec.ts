...
import * as LlmAkashX from '../src/index.ts'

let home: string
beforeAll(() => {
  home = mkdtempSync(join(tmpdir(), 'akx-akashx-egress-'))
  vi.stubEnv('AKX_HOME', home)
  vi.stubEnv('AKASHX_API_KEY', 'probe-key')
})
afterAll(() => {
  vi.unstubAllEnvs()
...
async function streamOnce(): Promise<void> {
  ...
  await ctx.plugin(LlmAkashX, { baseURL: 'http://akashx-probe.invalid/v1', models: [{ id: 'm' }] })
  for await (const _chunk of ctx.llm.stream({ provider: 'akashx-official', model: 'm', messages: [] })) {
    // The endpoint never answers; the proxy record is the assertion.
  }
}

describe('llm-akx egress', () => {
  it('sends the chat-completions request through the proxy', async () => {
    const observed = await observe(streamOnce)
    expect(observed.join('|')).toContain('akashx-probe.invalid')
  })
})
