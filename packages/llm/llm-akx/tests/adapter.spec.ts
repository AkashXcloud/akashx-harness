...
import type { AttachmentStore, ImageAttachmentRef, RequestImageAttachment } from '@akashx/akx-attachment'
import { createLaunchEnvironmentSnapshot } from '@akashx/akx-launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@akashx/akx-anonymous-user-id'
import { SessionId } from '@akashx/akx-session'
import AkashXLlmApiExtensionRegistry from '@akashx/akx-llm-api-extensions'
import type { PreparedAkashXLlmApiExtensions } from '@akashx/akx-llm-api-extensions'
import * as LlmAkashX from '@akashx/akx-llm-akx'
import { AkashXAdapter, resolveAdapterOptions } from '@akashx/akx-llm-akx'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'akx-llm-akx-'))
  vi.stubEnv('AKX_HOME', testHome)
})
...
async function harness(baseURL: string, config: object = {}) {
  ...
  vi.stubEnv('AKASHX_API_KEY', 'test-key')
  const ctx = new Context()
  await ctx.plugin(LlmRuntime)
  await ctx.plugin(AkashXLlmApiExtensionRegistry)
  await ctx.plugin(LlmAkashX, { baseURL, ...config })
  return ctx
}

/** Direct adapter over the plugin's real resolve step, with a static key. */
function noExtensions(): Promise<PreparedAkashXLlmApiExtensions> {
  return Promise.resolve({ fields: {}, accept: () => Promise.resolve() })
}

/** Direct adapter over the plugin's real resolve step, with a static key. */
function adapterOf(
  config: Partial<LlmAkashX.Config> & { apiKey?: string } = {},
  attachments?: AttachmentStore,
  files?: LlmAkashX.AkashXFileStore,
): AkashXAdapter {
  const { apiKey, ...rest } = config
  return new AkashXAdapter({
...
function fileStoreOf(
  implementation: (...args: Parameters<LlmAkashX.AkashXFileStore['ensureUploaded']>) => ReturnType<LlmAkashX.AkashXFileStore['ensureUploaded']>,
) {
  const ensureUploaded = vi.fn(implementation)
  const invalidate = vi.fn(() => Promise.resolve())
  return {
    store: { ensureUploaded, invalidate } as unknown as LlmAkashX.AkashXFileStore,
    ensureUploaded,
    invalidate,
  }
}

function fileReference(fileId: string): Awaited<ReturnType<LlmAkashX.AkashXFileStore['ensureUploaded']>> {
  return {
    record: { fileId: LlmAkashX.AkashXFileId(fileId) },
    uploaded: true,
  } as Awaited<ReturnType<LlmAkashX.AkashXFileStore['ensureUploaded']>>
}
...
describe('request image policy', () => {
  ...
  it('answers image request pricing from the current connection snapshot', () => {
    ...
    const priced = adapter.imageRequestPricing('akashx-official', 'vision')?.priceImages([imageRef])
    expect(priced).toHaveLength(1)
    expect(priced?.[0]!.visualTokens).toBeGreaterThan(0)
    const textOnly = adapter.imageRequestPricing('akashx-official', 'unlisted')?.priceImages([imageRef])
  ...
  it('prices descriptor text through the serializer\'s access resolution', () => {
    ...
    const adapter = new AkashXAdapter({
...
describe('AkashXAdapter against a mock server', () => {
  it('merges prepared extension fields and accepts them once after HTTP 2xx', async () => {
    ...
    await drain(adapter.stream({ provider: 'akashx-official', model: 'm', messages: [], sessionId: SessionId('s') }))
    expect(server.requests[0]).toMatchObject({ akx_test: { version: 1 } })
  ...
  it('fails before fetch on extension preparation or base-field collision', async () => {
    ...
    const failed = new AkashXAdapter({
      ...base,
      prepareExtensions: () => Promise.reject(new Error('metadata unavailable')),
    })
    await expect(drain(failed.stream({ provider: 'akashx-official', model: 'm', messages: [] })))
      .rejects.toMatchObject({ code: 'REQUEST_EXTENSION' })

    const collision = new AkashXAdapter({
      ...base,
      prepareExtensions: (() => Promise.resolve({ fields: { model: 'replacement' }, accept: () => Promise.resolve() })) as never,
    })
    await expect(drain(collision.stream({ provider: 'akashx-official', model: 'm', messages: [] })))
      .rejects.toMatchObject({ code: 'REQUEST_EXTENSION' })
  ...
  it('does not accept extensions on non-2xx and does accept before a later stream failure', async () => {
    ...
    const adapter = new AkashXAdapter({
      ...
      resolveApiKey: () => Promise.resolve('k'),
      resolveUserId: () => TEST_USER_ID,
      prepareExtensions: () => Promise.resolve({ fields: { akx_test: 1 }, accept: async () => { accept() } }) as never,
    })
    const request = { provider: 'akashx-official', model: 'm', messages: [] }
  ...
  it('reports a post-2xx extension acceptance failure without relabelling it as transport', async () => {
    ...
    await expect(drain(adapter.stream({ provider: 'akashx-official', model: 'm', messages: [] })))
  ...
  it('streams a text generation end to end through the assembler', async () => {
    ...
    // App attribution and AkashX request identity are independent wire facts.
    expect(server.headers[0]?.['user-agent']).toBe(userAgent())
    expect(server.headers[0]?.['x-akx-harness-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-akx-harness-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-akx-harness-compact')
  })
  ...
  it('uploads a durable image once and sends only its Files API id to the vision model', async () => {
    ...
    expect(server.fileRequests).toEqual([{
      method: 'POST',
      path: '/files',
      filename: `akx-${'a'.repeat(16)}-${'b'.repeat(8)}.png`,
      bytes: 3,
    }])
  ...
  it('falls back to one all-base64 request when Files API resolution fails', async () => {
    ...
    const adapter = adapterOf({
      baseURL: server.url,
      models: [{ id: 'akashx-v4-flash-vision-exp', inputModalities: ['text', 'image'] }],
    }, attachments, files.store)

    await drain(adapter.stream({
      provider: 'akashx-official',
      model: 'akashx-v4-flash-vision-exp',
      messages: [createUserMessage({
        content: [
          { type: 'image', attachment: imageRef },
  ...
  it('projects nested tool-result images with route-owned request budgets', async () => {
    ...
    await drain(adapter.stream({ provider: 'akashx-official', model: 'vision-low', messages: [nested] }))
    await drain(adapter.stream({ provider: 'akashx-official', model: 'vision-custom', messages: [nested] }))
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    await drain(adapter.stream({ provider: 'akashx-official', model: 'akashx-v4-flash-vision-exp', messages }))
    await drain(adapter.stream({
      provider: 'akashx-official',
      model: 'akashx-v4-flash-vision-exp',
      messages,
    ...
    expect(server.headers[1]?.['x-akx-harness-compact']).toBe('1')
  })
  ...
  it.each(['akashx-v4-flash', 'unlisted-pass-through'])(
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-akx-harness-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-akx-harness-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-akx-harness-compact']).toBe('1')
  })
  ...
  it('uses the configured maxTokens default and preserves an explicit request cap', async () => {
    ...
    await assemble(ctx, { model: 'akashx-v4-flash', messages: [] })
    await assemble(ctx, { model: 'akashx-v4-flash', messages: [], maxTokens: 8_192 })
  ...
  it('publishes only off and omits the wire effort when thinking is disabled', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('akashx-official', 'akashx-v4-flash'))
  ...
  it.each([
    ...
    const ctx = await harness(server.url)
    const result = await assemble(ctx,{ model: 'akashx-v4-flash', messages: [] })
  ...
  it('parses a future Retry-After HTTP date and the AkashX request-id fallback', async () => {
    ...
    try {
      const server = await mockServer([{
        ...
        headers: {
          ...
          'x-akashx-request-id': 'akashx-503',
        },
      }])
      const ctx = await harness(server.url)
      const result = await assemble(ctx, { model: 'akashx-v4-flash', messages: [] })
      expect(result.finish).toEqual({
        ...
        failure: {
          ...
          requestId: ProviderRequestId('akashx-503'),
        },
      })
    } finally {
      dateNow.mockRestore()
  ...
  it('reports a transport failure with the endpoint in the message', async () => {
    ...
    expect(result.finish).toMatchObject({
      ...
      failure: {
        code: 'TRANSPORT',
        message: 'AkashX API request to http://127.0.0.1:1 failed',
      },
    })
  })
  ...
  it('throws EMPTY_RESPONSE when the response has no body', async () => {
    ...
    try {
      const iterate = async (): Promise<void> => {
        for await (const _chunk of adapter.stream({ provider: 'akashx-official', model: 'm', messages: [] })) { /* drain */ }
      }
  ...
  it('classifies an abrupt body close as TRANSPORT', async () => {
    ...
    expect(result.finish.failure.message).toMatch(/^AkashX API stream from .* failed$/)
  })
  ...
  it('renders a non-Error transport rejection without losing its cause', async () => {
    ...
    try {
      ...
      await expect(drain()).rejects.toMatchObject({
        message: 'AkashX API request to https://example.invalid failed',
        code: 'TRANSPORT',
        cause: 'offline',
      })
    } finally {
  ...
  it('keeps an idle provider read alive through SSE comments', async () => {
    ...
    try {
      const chunks: string[] = []
      const drain = (async () => {
        for await (const chunk of adapter.stream({ provider: 'akashx-official', model: 'm', messages: [] })) {
...
describe('plugin registration and config', () => {
  it('keeps wire helpers off the package root', () => {
    for (const helper of [
      ...
      'mapUsage',
      'translate',
    ]) expect(LlmAkashX).not.toHaveProperty(helper)
  })

  it('registers the akashx provider and unregisters on dispose (HMR safety)', async () => {
    const server = await mockServer([])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const fiber = await ctx.plugin(LlmAkashX, {
      baseURL: server.url,
    })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'akashx-official', name: 'AkashX' }])
  ...
  it('registers retryPolicy from the provider config', async () => {
    ...
    expect(ctx.llm.providerRetryPolicy('akashx-official')).toEqual({
      mode: 'always',
      initialDelayMs: 25,
      maxDelayMs: 100,
      jitterRatio: 0.2,
  ...
  it('owns the akashx provider and advertises the default models', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmAkashX, { baseURL: 'http://127.0.0.1:1' })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'akashx-official', name: 'AkashX' }])
    await expect(ctx.llm.listModels('akashx-official')).resolves.toEqual([
      { provider: 'akashx-official', id: 'akashx-flash', name: 'AkashX-V41-Flash', inputModalities: ['text', 'image'] },
      {
        provider: 'akashx-official',
        id: 'akashx-v4-flash',
      ...
      { provider: 'akashx-official', id: 'akashx-v4-flash-vision-exp', name: 'AkashX-V4-Flash-Vision-Exp', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('akashx-official', 'akashx-flash'))
      .resolves.toMatchObject({
        provider: 'akashx-official',
        id: 'akashx-flash',
        name: 'AkashX-V41-Flash',
        inputModalities: ['text', 'image'],
  ...
  it.each([
    { model: 'akashx-v4-flash', inputModalities: ['text'] },
    { model: 'akashx-v4-pro', inputModalities: ['text'] },
    { model: 'akashx-v4-flash-vision-exp', inputModalities: ['text', 'image'] },
  ])('keeps $model available with its V4 capabilities', async ({ model, inputModalities }) => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmAkashX, { baseURL: 'http://127.0.0.1:1' })
    const info = await ctx.llm.resolveModelInfo('akashx-official', model)
  ...
  it.each(['off', 'low', 'max'] as const)('uses the configured %s reasoning default', async (effort) => {
    ...
    await expect(ctx.llm.resolveModelInfo('akashx-official', 'unlisted-pass-through'))
      .resolves.toMatchObject({
        reasoning: {
          efforts: [
            { id: ReasoningEffortId('off'), name: 'Off', description: 'Use for simple tasks that do not need reasoning.' },
  ...
  it.each(['low', 'high', 'max'] as const)(
    ...
    async (reasoningEffort) => {
      const ctx = new Context()
      await ctx.plugin(LlmRuntime)
      await expect(ctx.plugin(LlmAkashX, {
        baseURL: 'http://127.0.0.1:1',
        thinking: 'disabled',
        reasoningEffort,
  ...
  it('accepts disabled thinking with off at the resolver boundary', async () => {
    ...
    await expect(adapter.resolveModel('akashx-official', 'pass-through')).resolves.toMatchObject({
      reasoning: {
        efforts: [{
          id: ReasoningEffortId('off'),
          name: 'Off',
  ...
  it('uses the default model catalog when apply is called directly', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    LlmAkashX.apply(ctx, { baseURL: 'http://127.0.0.1:1' })
    await expect(ctx.llm.listModels('akashx-official')).resolves.toEqual([
      { provider: 'akashx-official', id: 'akashx-flash', name: 'AkashX-V41-Flash', inputModalities: ['text', 'image'] },
      {
        provider: 'akashx-official',
  ...
  it('defaults an adapter-supplied catalog entry to text input', async () => {
    ...
    await expect(adapter.listModels('akashx-official')).resolves.toEqual([{
      provider: 'akashx-official',
      id: 'adapter-model',
      name: 'adapter-model',
      inputModalities: ['text'],
  ...
  it('advertises configured models without restricting arbitrary request ids', async () => {
    ...
    await expect(ctx.llm.listModels('akashx-official')).resolves.toEqual([
      { provider: 'akashx-official', id: 'private-fast', name: 'private-fast', inputModalities: ['text'] },
      { provider: 'akashx-official', id: 'private-reasoner', name: 'Private Reasoner', description: 'Higher reasoning budget', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('akashx-official', 'private-fast'))
      .resolves.toMatchObject({ context: { contextWindow: 32_000 } })
    await expect(ctx.llm.resolveModelInfo('akashx-official', 'private-reasoner'))
      .resolves.toMatchObject({
        name: 'Private Reasoner',
        description: 'Higher reasoning budget',
        inputModalities: ['text', 'image'],
      })
    await expect(ctx.llm.resolveModelInfo('akashx-official', 'arbitrary-unlisted'))
      .resolves.toMatchObject({
        context: { contextWindow: 1_000_000 },
        defaultMaxTokens: 256_000,
      })
  ...
  it('uses exact model capacity before the adapter-wide default', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('akashx-official', 'inherits-default'))
      .resolves.toMatchObject({ context: { contextWindow: 256_000 } })
    await expect(ctx.llm.resolveModelInfo('akashx-official', 'exact-override'))
  ...
  const invalidModels: Array<[LlmAkashX.AkashXCatalogModel[], RegExp]> = [
    ...
    [[{
      id: 'm',
      inputModalities: ['audio'] as unknown as NonNullable<LlmAkashX.AkashXCatalogModel['inputModalities']>,
    }], /expected "text" \| "image"/],
  ...
  const invalidProgrammaticModalities: Array<[LlmAkashX.AkashXCatalogModel[], RegExp]> = [
  ...
  it('surfaces a catalog model\'s in-history system prompt update mode and rejects any other mode', async () => {
    ...
    await expect(adapter.resolveModel('akashx-official', 'capable'))
      .resolves.toMatchObject({ systemPromptUpdate: 'in-history' })
    await expect(adapter.resolveModel('akashx-official', 'plain'))
      .resolves.not.toHaveProperty('systemPromptUpdate')
    await expect(adapter.resolveModel('akashx-official', 'not-in-catalog'))
      .resolves.not.toHaveProperty('systemPromptUpdate')
  ...
  it('prefers a model\'s own output cap over the profile default', async () => {
    ...
    await expect(adapter.resolveModel('akashx-official', 'capped'))
      .resolves.toMatchObject({ defaultMaxTokens: 512 })
    await expect(adapter.resolveModel('akashx-official', 'uncapped'))
  ...
  it('falls back to AKASHX_API_KEY and AKASHX_BASE_URL env vars', async () => {
    vi.stubEnv('AKASHX_API_KEY', 'env-key')
    vi.stubEnv('AKASHX_BASE_URL', 'http://127.0.0.1:1')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmAkashX, {})
    expect(ctx.llm.listProviders()).toEqual([{ id: 'akashx-official', name: 'AkashX' }])
  ...
  it('loads keyless, keeps the catalog browsable, and fails the request actionably', async () => {
    ...
    expect(ctx.llm.listProviders()).toEqual([{ id: 'akashx-official', name: 'AkashX' }])
    await expect(ctx.llm.listModels('akashx-official')).resolves.toHaveLength(4)
    const first = await assemble(ctx, { model: 'akashx-v4-flash', messages: [] })
    expect(first.finish).toMatchObject({ kind: 'error', failure: { code: 'MISSING_CREDENTIAL' } })
    // The guidance leads with the managed credential store.
    const second = await assemble(ctx, { model: 'akashx-v4-flash', messages: [] })
    ...
    expect(second.finish.failure.message)
      .toMatch(/store AKASHX_API_KEY through the credentials service.*export AKASHX_API_KEY/s)
  })
  ...
  it('reads the ambient variable when no credentials seam is mounted', async () => {
    ...
    vi.stubEnv('AKASHX_API_KEY', 'ambient-key')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmAkashX, { baseURL: server.url })
    await assemble(ctx, { model: 'akashx-v4-flash', messages: [] })
  ...
  it('prefers explicit config over env for key and base URL', async () => {
    vi.stubEnv('AKASHX_API_KEY', 'env-key')
    vi.stubEnv('AKASHX_BASE_URL', 'http://env-host:1')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
  ...
  it('uses AKASHX_BASE_URL when config omits baseURL', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    vi.stubEnv('AKASHX_BASE_URL', server.url)
    vi.stubEnv('AKASHX_API_KEY', 'test-key')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmAkashX, {})
  ...
  it('takes AKASHX_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.akx/.env', values: { AKASHX_BASE_URL: 'https://user.example' } },
    ])
    ...
    const project = createLaunchEnvironmentSnapshot([
      { source: 'project-env', path: '/work/.env', values: { AKASHX_BASE_URL: 'https://project.example' } },
    ])
    ...
    const shell = createLaunchEnvironmentSnapshot([
      { source: 'process', values: { AKASHX_BASE_URL: 'https://stale.example' } },
    ])
  ...
  it('defaults to the public base URL without config or env', async () => {
    vi.stubEnv('AKASHX_API_KEY', 'k')
    vi.stubEnv('AKASHX_BASE_URL', undefined)
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    // Registration succeeds; no call is made (would hit api.akashx.com).
    await ctx.plugin(LlmAkashX, {})
    expect(ctx.llm.listProviders()).toEqual([{ id: 'akashx-official', name: 'AkashX' }])
  })
  ...
  it('adapter is constructible directly for embedding over the shared resolver', async () => {
    const adapter = adapterOf()
    expect(adapter).toBeInstanceOf(AkashXAdapter)
    ...
    await expect(adapter.listModels('akashx-official')).resolves.toHaveLength(4)
  })
  ...
  it('resolves connection facts and the credential exactly once per stream call', async () => {
    ...
    const adapter = new AkashXAdapter({ options, resolveApiKey, resolveUserId, prepareExtensions: noExtensions })
