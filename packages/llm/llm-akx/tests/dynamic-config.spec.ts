...
import LlmRuntime, { createUserMessage, INVALID_CREDENTIAL_CODE } from '@akashx/akx-llm'
...
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'akx-llm-dynamic-'))
 ...
 * llm-akx over one temp harness home. `watch: false` keeps every change
...
async function boot(dir: string, config: object): Promise<Harness> {
  ...
  await ctx.plugin(LlmAkashX, config)
  return { ctx, settingsFiber }
}

function prompt(ctx: Context) {
  return assemble(ctx, { model: 'akashx-v4-flash', messages: [] })
}
...
describe('request-level dynamic configuration', () => {
  ...
  it('advertises a live settings catalog without re-registration', async () => {
    ...
    await expect(ctx.llm.listModels('akashx-official')).resolves.toEqual([
      { provider: 'akashx-official', id: 'settings-model', name: 'From Settings', inputModalities: ['text', 'image'] },
    ])
  })
  ...
  it('applies changed request file limits to the next request', async () => {
    ...
    await assemble(ctx, { model: 'akashx-flash', messages })
  ...
  it('re-registers the route in place when the captured retry policy changes, without an empty-registry window', async () => {
    ...
    expect(observed).toEqual([['akashx-official']])
  })
  ...
  it('keeps the last good options when a settings snapshot fails beyond-schema validation', async () => {
    ...
    await expect(ctx.llm.listModels('akashx-official')).resolves.toEqual([
      { provider: 'akashx-official', id: 'recovered', name: 'recovered', inputModalities: ['text'] },
    ])
  })
  ...
  it('keeps the whole last-good snapshot when a rejected one changed the URL', async () => {
    ...
    vi.stubEnv('AKASHX_API_KEY', 'good-key')
