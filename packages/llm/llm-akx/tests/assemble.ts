...
import { BlockAssembler } from '@akashx/akx-llm'
import type { Context } from '@akashx/cordis'
import type { FinishReason, GenerateOptions, Message, TokenUsage } from '@akashx/akx-llm'
...
export async function assemble(ctx: Context, options: Omit<GenerateOptions, 'provider'> & { provider?: string }): Promise<AssembledResult> {
  ...
  const request = { provider: 'akashx-official', ...options }
