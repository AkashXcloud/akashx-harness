...
import { userAgent } from '@akashx/akx-llm'
...
describe('AkashXFilesClient', () => {
  it('uploads multipart bytes with the required purpose and explicit expiry', async () => {
    ...
    const client = new AkashXFilesClient({ baseURL: 'https://api.akashx.com/', apiKey: 'key', fetch: fetchImpl })
  ...
  it('validates list, retrieve, and delete responses', async () => {
    ...
    await expect(client.list({ after: AkashXFileId('file-api-before'), limit: 20, order: 'desc' })).resolves.toMatchObject({
      data: [{ id: 'file-api-one' }], firstId: 'file-api-one', lastId: 'file-api-one', hasMore: false,
    })
    await expect(client.retrieve(AkashXFileId('file-api-one'))).resolves.toMatchObject({ id: 'file-api-one' })
    await expect(client.delete(AkashXFileId('file-api-one'))).resolves.toBeUndefined()
  })
  ...
  it.each([
    ...
    const client = new AkashXFilesClient({
      baseURL: 'https://api.akashx.com',
      apiKey: 'key',
      fetch: vi.fn(() => Promise.resolve(new Response('not-json', { status }))),
    })
    await expect(client.retrieve(AkashXFileId('missing'))).rejects.toMatchObject({
      name: 'AkashXFilesError',
      code,
      detail: '',
    })
  })
  ...
  it.each([
    ...
    const error = await client.retrieve(AkashXFileId('missing')).catch((caught: unknown) => caught)
    expect(error).toBeInstanceOf(AkashXFilesError)
    expect(error).toMatchObject({ message: 'AkashX Files API error (HTTP 400)', detail: '' })
  })
  ...
  it('wraps transport failures but preserves an aborted request reason', async () => {
    ...
    await expect(client.retrieve(AkashXFileId('one'))).rejects.toMatchObject({
      code: 'TRANSPORT',
      cause: transport,
    })
    ...
    await expect(client.retrieve(AkashXFileId('one'), controller.signal)).rejects.toBe(transport)
  })
  ...
  it.each([
    ...
    await expect(client.retrieve(AkashXFileId('one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
  ...
  it('accepts a list without cursors and uses the global fetch default', async () => {
    ...
    try {
      const client = new AkashXFilesClient({ baseURL: 'https://api.akashx.com///', apiKey: 'key' })
  ...
  it.each([
    ...
    await expect(client.delete(AkashXFileId('file-api-one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
})
