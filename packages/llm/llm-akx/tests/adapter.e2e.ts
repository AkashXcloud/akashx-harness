...
import LlmRuntime, { createUserMessage, ToolCallId, ReasoningEffortId, createMessage, createSystemMessage } from '@akashx/akx-llm'
import type { Message, ToolSchema } from '@akashx/akx-llm'
import AttachmentStore, { AttachmentId, ImageVariantId } from '@akashx/akx-attachment'
import LocalAttachments from '@akashx/akx-attachment-local'
import type {
...
import * as PluginPackageInventoryAkashX from '@akashx/akx-plugin-package-inventory'
import * as SessionLogAkashX from '@akashx/akx-session-log-akx'
import * as LlmAkashX from '@akashx/akx-llm-akx'
import type { Config } from '@akashx/akx-llm-akx'
 ...
 * entirely without $AKASHX_API_KEY; the pre-release vision smoke additionally
 * requires $AKX_VISION_E2E=1, and the Flash image/system-update smoke
 * requires $AKX_FLASH_E2E=1 (see vitest.e2e.config.ts).
 */

const FLASH = 'akashx-v4-flash'
const VISION = 'akashx-v4-flash-vision-exp'
const VISION_E2E_ENABLED = process.env.AKX_VISION_E2E === '1'
/** A model whose endpoint reads the latest `system` message at any position; unset skips the in-history smoke. */
const IN_HISTORY_MODEL = process.env.AKASHX_IN_HISTORY_MODEL
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'akx-e2e-user-id-'))
  vi.stubEnv('AKX_HOME', identityHome)
})
...
describe.skipIf(!process.env.AKASHX_API_KEY)('llm-akx e2e (real API)', () => {
  it.skipIf(process.env.AKX_FLASH_E2E !== '1')('akashx-flash accepts images and retains system updates', async () => {
    const ctx = new Context()
    contexts.push(ctx)
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LocalAttachments)
    await ctx.plugin(LlmAkashX, { maxTokens: 4096 })
    const model = 'akashx-flash'
    await expect(ctx.llm.resolveModelInfo('akashx-official', model)).resolves.toMatchObject({
  ...
  it.skipIf(!VISION_E2E_ENABLED)('uses the built-in official route to upload, reference, and delete one image', async () => {
    const key = process.env.AKASHX_API_KEY
    if (key === undefined) throw new Error('e2e ran without AKASHX_API_KEY')
    const baseURL = process.env.AKASHX_BASE_URL ?? LlmAkashX.PUBLIC_BASE_URL
    const ctx = await harness(VISION, { baseURL })
    await ctx.plugin(E2eAttachmentStore)
    const attachments = ctx.attachments as E2eAttachmentStore
    let uploadedFile: LlmAkashX.AkashXFileIdType | undefined
    ...
    const observedFetch: typeof fetch = async (input, init) => {
      ...
      if (method === 'POST' && url.pathname.endsWith('/files') && response.ok) {
        ...
        if (typeof value.id === 'string') uploadedFile = LlmAkashX.AkashXFileId(value.id)
      }
      return response
    }
    vi.stubGlobal('fetch', observedFetch)
    const files = new LlmAkashX.AkashXFilesClient({ baseURL, apiKey: key })
    ...
    try {
      ...
      expect(
        result.finish.kind,
        `AkashX vision result: ${JSON.stringify(result.finish)}`,
      ).toBe('stop')
  ...
  it('accepts the session-log and plugin-package request extension fields', async () => {
    ...
    await ctx.plugin(SessionLogAkashX, { enabled: true })
    await ctx.plugin(PluginPackageInventoryAkashX)
    await ctx.plugin(LlmAkashX, { thinking: 'disabled' })
  ...
  it(
    ...
    async () => {
      ...
      expect(
        first.finish.kind,
        `AkashX Flash tool-call turn finished as ${JSON.stringify(first.finish)}`,
      ).toBe('tool-calls')
      ...
      expect(
        second.finish.kind,
        `AkashX Flash tool-result turn finished as ${JSON.stringify(second.finish)}`,
      ).toBe('stop')
  ...
  it.skipIf(IN_HISTORY_MODEL === undefined)(
    ...
    async () => {
      ...
      await expect(ctx.llm.resolveModelInfo('akashx-official', model))
