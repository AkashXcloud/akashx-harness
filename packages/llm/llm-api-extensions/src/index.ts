/**
 * AkashX LLM API extension registry: plugins own independent top-level request
 * fields while the official adapter performs one preparation and acceptance transaction.
 * @module @akashx/akx-llm-api-extensions
 */

import { Context, Service } from '@akashx/cordis'
import type {
  AkashXLlmApiExtensionMap,
  AkashXLlmApiExtensionProvider,
...
declare module '@akashx/cordis' {
  interface Context {
    akashxLlmApiExtensions: AkashXLlmApiExtensionRegistry
  }
}

interface ErasedProvider {
  prepare(request: AkashXLlmApiExtensionRequest):
    | { readonly value: AkashXLlmApiJson; accept?(): void | Promise<void> }
    | undefined
    | Promise<{ readonly value: AkashXLlmApiJson; accept?(): void | Promise<void> } | undefined>
}

/** Recursively freeze a fresh structured clone. */
function freezeJson<T extends AkashXLlmApiJson>(value: T): T {
...
async function acceptAll(callbacks: readonly (() => void | Promise<void>)[]): Promise<void> {
  ...
  if (failures.length > 1) throw new AggregateError(failures, 'AkashX LLM API extension acceptance failed')
}
...
export class AkashXLlmApiExtensionRegistry extends Service {
  private readonly providers = new Map<string, ErasedProvider>()

  constructor(ctx: Context) {
    super(ctx, 'akashxLlmApiExtensions')
  }
   ...
   */
  register<K extends keyof AkashXLlmApiExtensionMap>(
    field: K,
    provider: AkashXLlmApiExtensionProvider<AkashXLlmApiExtensionMap[K]>,
  ): () => Promise<void> {
    const fieldName = field as string
    if (fieldName.length === 0 || fieldName.trim() !== fieldName) {
      throw new Error('llm-api-extensions: field must be a non-blank trimmed string')
    }
    const providers = this.providers
    const erased = provider as ErasedProvider
    const dispose = this.ctx.effect(() => {
      if (providers.has(fieldName)) {
        throw new Error(`llm-api-extensions: field ${JSON.stringify(fieldName)} is already registered`)
      }
      providers.set(fieldName, erased)
      return () => {
        providers.delete(fieldName)
      }
    }, `akashxLlmApiExtensions.register(${JSON.stringify(fieldName)})`)
    return dispose
  }
  ...
  async prepare(request: AkashXLlmApiExtensionRequest): Promise<PreparedAkashXLlmApiExtensions> {
    ...
    const fields: Record<string, AkashXLlmApiJson> = Object.create(null) as Record<string, AkashXLlmApiJson>
...
export default AkashXLlmApiExtensionRegistry
