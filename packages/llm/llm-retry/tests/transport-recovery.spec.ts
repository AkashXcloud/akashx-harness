import { createUserMessage, expandAssistantStream } from '@akashx/akx-llm'
...
import type { MockLlmBehavior, MockLlmServer } from '@akashx/akx-llm-mock-server'
...
async function harness(
  ...
  vi.stubEnv('AKASHX_API_KEY', 'mock-key')
...
describe('bounded retry through the real AkashX HTTP/SSE adapter', () => {
  ...
  it('stops after the configured transport retry budget is exhausted', async () => {
    ...
    if (end?.type === 'turn/end' && end.data.reason.kind === 'error') {
      expect(end.data.reason.error.message).toContain('AkashX API request to')
    }
  })
})
