/**
 * Active Loader-backed plugin package inventory for official AkashX requests.
 ...
 * @module @akashx/akx-plugin-package-inventory
 */
...
import type { Entry, EntryTree } from '@akashx/cordis-plugin-loader'
...
import type { AkashXPluginPackageIdentity, AkashXPluginPackageInventoryExtension } from './types.ts'
import type {} from './types.ts'

export type * from './types.ts'

/** Cordis plugin name. */
export const name = 'plugin-package-inventory'
/** Services required to locate host/requesting-agent entries and contribute the field. */
export const inject = ['agents', 'akashxLlmApiExtensions', 'loader']
...
function identityFromManifest(path: string, allowAnonymous: boolean): AkashXPluginPackageIdentity | undefined {
  const manifest = JSON.parse(readFileSync(path, 'utf8')) as PackageManifest
  if (allowAnonymous && manifest.name === undefined) return undefined
  if (typeof manifest.name !== 'string' || manifest.name.length === 0
    || typeof manifest.version !== 'string' || manifest.version.length === 0) {
    throw new Error(`plugin-package-inventory: ${path} must declare non-empty name and version`)
  }
...
class PackageIdentityResolver {
  ...
  private readonly cache = new Map<string, AkashXPluginPackageIdentity | undefined>()
  ...
  resolve({ entry, bareBaseUrl }: ActiveEntry): AkashXPluginPackageIdentity | undefined {
...
async function collectActivePluginPackages(
  ...
  hostBaseUrl: string,
  sessionId?: string,
): Promise<AkashXPluginPackageIdentity[]> {
  ...
  if (sessionId !== undefined && ctx.get('agentPresets') !== undefined) {
    ...
    if (agent !== undefined) {
      ...
      const { standingMountFor } = await import('@akashx/akx-agent-presets')
  ...
  const unique = new Map<string, AkashXPluginPackageIdentity>()
...
/**
 * Register the complete `akx_plugin_packages` request contribution when enabled.
 * @param ctx - plugin context carrying Loader provenance and the AkashX request-extension registry.
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.akashxLlmApiExtensions.register('akx_plugin_packages', {
    prepare: async (request) => {
      const value: AkashXPluginPackageInventoryExtension = {
        version: 1,
