...
import { resolveRetryPolicy, RetryPolicySchema } from '@akashx/akx-llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@akashx/akx-llm'
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with akx-llm-retry',
    )
  }
}
