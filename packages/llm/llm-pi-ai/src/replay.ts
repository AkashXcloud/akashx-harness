 ...
 *
 * @module akx-llm-pi-ai/replay
 */

import { LlmError } from '@akashx/akx-llm'
import type { Message, ModelMessageSource, ReplayEnvelope } from '@akashx/akx-llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    api: 'akx-foreign',
    provider: source?.provider ?? 'akx-foreign',
    model: source?.model ?? 'akx-foreign',
    usage: emptyPiUsage(),
