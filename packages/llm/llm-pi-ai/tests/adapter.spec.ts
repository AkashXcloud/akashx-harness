...
import { AttachmentId, AttachmentStore, ImageVariantId } from '@akashx/akx-attachment'
import type {
...
import LlmRuntime, { createUserMessage, CONTEXT_WINDOW_EXCEEDED_CODE, LlmError, ReasoningEffortId, userAgent } from '@akashx/akx-llm'
...
const HOST_IMAGE_PATH = '/host/.akx/attachments/objects/aa/object'
const MODEL_IMAGE_PATH = '/model/.akx/attachments/objects/aa/object'
...
async function harness(baseURL: string, overrides: Record<string, unknown> = {}): Promise<Context> {
  ...
  await ctx.plugin(LlmPiAi, {
    providers: { akashx: { apiKeyEnv: 'PI_TEST_KEY', baseURL, ...overrides } },
  })
  return ctx
}
...
describe('PiAiAdapter provider routing', () => {
  ...
  it('keeps prepared model metadata and dispatch on one profile snapshot', async () => {
    ...
    let providers: Record<string, LlmPiAi.PiAiProviderProfile> = {
      akashx: { apiKeyEnv: 'PI_TEST_KEY', baseURL: first.url },
    }
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    ...
    const prepared = await ctx.llm.prepareCall({ provider: 'deepseek', model: 'akashx-v4-flash' })
    providers = { akashx: { apiKeyEnv: 'PI_TEST_KEY', baseURL: second.url } }
  ...
  it('forwards common stream options and profile reasoning', async () => {
    ...
    expect(server.requests[0]).not.toHaveProperty('akx_session_log')
    expect(server.requests[0]).not.toHaveProperty('akx_plugin_packages')
  })
  ...
  it('preserves omitted profile options when constructing the adapter directly', async () => {
    ...
    ctx.llm.registerAdapter(['deepseek'], adapterOf({
      akashx: { apiKeyEnv: 'PI_TEST_KEY', baseURL: server.url },
    }))
  ...
  it('reports unsupported stop sequences rather than silently ignoring them', async () => {
    ...
    const ctx = await harness(server.url)
    const result = await assemble(ctx, { model: 'akashx-v4-flash', messages: [], stop: ['END'] })
  ...
  it('uses the resolved catalog context window for usage-based overflow detection', async () => {
    const model = getBuiltinModels('deepseek').find(candidate => candidate.id === 'akashx-v4-flash')
    if (model === undefined) throw new Error('akashx-v4-flash missing from pi-ai test catalog')
    const events = [
...
describe('provider profile lifecycle', () => {
  ...
  it('exposes pi-ai model thinking levels verbatim without inventing a provider default', async () => {
    ...
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmPiAi, {
      providers: { akashx: {}, openai: {} },
    })

    await expect(ctx.llm.resolveModelInfo('deepseek', 'akashx-v4-flash'))
  ...
  it('uses a supported profile reasoning value as the model default and rejects an unsupported one', async () => {
    ...
    await supported.plugin(LlmPiAi, {
      providers: { akashx: { reasoning: 'max' } },
    })
    await expect(supported.llm.resolveModelInfo('deepseek', 'akashx-v4-flash'))
    ...
    await unsupported.plugin(LlmPiAi, {
      providers: { akashx: { reasoning: 'medium' } },
    })
    const described = await unsupported.llm.resolveModelInfo('deepseek', 'akashx-v4-flash')
    ...
    await disabled.plugin(LlmPiAi, {
      providers: { akashx: { reasoning: 'off' } },
    })
    await expect(disabled.llm.resolveModelInfo('deepseek', 'akashx-v4-flash'))
  ...
  it('rejects unsupported or unresolved image input before provider I/O', async () => {
    const adapter = adapterOf({ openai: {}, akashx: {} })
...
describe('abort wiring', () => {
  it('preserves an unknown pre-dispatch adapter Error exactly', async () => {
    ...
    const adapter = adapterOf({ akashx: {} })
  ...
  it('honors a pre-aborted caller signal', async () => {
    ...
    const controller = new AbortController()
    controller.abort('already stopped')
    const result = await assemble(ctx, { model: 'akashx-v4-flash', messages: [], signal: controller.signal })
  ...
  it('forwards an abort that arrives while provider streaming is active', async () => {
    ...
    const resultPromise = assemble(ctx, {
      model: 'akashx-v4-flash', messages: [], signal: controller.signal,
    })
  ...
  it('aborts upstream when a consumer stops early', async () => {
    const server = await mockServer([{ events: textEvents, delayMs: 30 }])
    const ctx = await harness(server.url)
    for await (const chunk of ctx.llm.stream({ provider: 'deepseek', model: 'akashx-v4-flash', messages: [] })) {
