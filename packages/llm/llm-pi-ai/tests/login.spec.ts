...
import type { AuthorizationInteraction, AuthorizationNotice, AuthorizationPrompt } from '@akashx/akx-authorization'
import LocalCredentialProvider from '@akashx/akx-credentials-local'
import type { CredentialKey } from '@akashx/akx-credentials'
...
async function harness(): Promise<Context> {
  const dir = await mkdtemp(join(tmpdir(), 'akx-pi-login-'))
  dirs.push(dir)
