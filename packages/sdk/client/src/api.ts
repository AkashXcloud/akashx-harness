/**
 * High-level run API over {@link HarnessClient}: `AkashXHarness` owns one
 ...
 *
 * @module @akashx/akx-sdk-client/api
 */

import { randomUUID } from 'node:crypto'
import { resolve } from 'node:path'
import type { SessionEvent, TurnEndReason } from '@akashx/akx-session'
...
import type { ContentBlock, AkashXHarnessOptions, HarnessNotification, RunResult, SdkPromptContentBlock } from './types.ts'

/**
 * Reusable SDK for running AkashX Harness agent turns in a runtime
 ...
 * child is reaped.
 */
export class AkashXHarness implements AsyncDisposable {
  private clientInstance: HarnessClient
  private readonly createClient: () => HarnessClient
  private readonly cwd: string
  private readonly provider: string
  private readonly model: string
  private readonly reasoningEffort: AkashXHarnessOptions['reasoningEffort']
  private readonly maxTokens: number | undefined
  private initialized: Promise<void> | undefined
  private closed = false

  /** @param options - akx launch configuration plus the session route, effort, and output cap. */
  constructor(options?: AkashXHarnessOptions)
  constructor(options: AkashXHarnessOptions = {}, clientFactory?: () => HarnessClient) {
    ...
    this.provider = options.provider ?? 'akashx-official'
    this.model = options.model ?? 'akashx-v4-flash'
      ...
      try {
        ...
        try {
          ...
          throw new AggregateError(
            [error, cleanupError],
            'AkashX Harness initialization and cleanup failed',
          )
        }
...
export function createProcessAkashXHarness(
  runtime: RuntimeProcessOptions,
  options: AkashXHarnessOptions = {},
): AkashXHarness {
  const Constructor = AkashXHarness as unknown as new (
    publicOptions: AkashXHarnessOptions,
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: AkashXHarness, readonly id: string) {}
