/**
 * Low-level JSON-RPC client for a AkashX Harness SDK runtime subprocess.
 * {@link HarnessClient} owns the child process: it spawns the runtime, speaks
 * the `@akashx/akx-sdk-protocol` wire over the child's stdio, fans
 ...
 * spawns directly rather than through the `akx-subprocess` service — the
 * seam's documented exception for SDK-managed transports.
 *
 * @module @akashx/akx-sdk-client/client
 */
...
import { resolveAkxLaunch, type RuntimeProcessOptions } from './launch.ts'
...
/**
 * JSON-RPC client for the AkashX Harness SDK runtime over subprocess stdio.
 *
...
export class HarnessClient {
  /** Original public akx launch and timeout options for this client. */
  ...
  /** @param options - akx profile, patch, home, process, environment, and timeout options. */
  constructor(options?: HarnessClientOptions)
  constructor(options: HarnessClientOptions = {}, runtime?: RuntimeProcessOptions) {
    this.options = options
    this.runtime = runtime ?? resolveAkxLaunch(options)
  }
   ...
   */
  start(): void {
    if (this.closeTask !== undefined) throw new TransportClosedError('AkashX Harness runtime client is closed')
    ...
    child.once('error', (error) => {
      ...
      this.failSubscriptions(this.closedError('AkashX Harness runtime failed to start'))
    })
    ...
    child.once('exit', (code) => {
      ...
      settled.exited = true
      maybeSettle()
      this.failSubscriptions(this.closedError('AkashX Harness runtime exited'))
    })
    child.once('close', () => {
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    ...
    if (this.exitCode !== undefined || this.spawnError !== undefined) {
      ...
      throw this.closedError('AkashX Harness runtime is not running')
    }
    const transport = this.transport
    /* v8 ignore next -- start() either sets the transport or throws */
    if (transport === undefined) throw new TransportClosedError('AkashX Harness runtime is not running')
  ...
  subscribe(filter?: NotificationFilter): NotificationSubscription {
    ...
    if (this.closeTask !== undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      subscription.fail(this.closedError('AkashX Harness runtime closed'))
      return subscription
    }
  ...
  private async performClose(): Promise<void> {
    ...
    this.transport?.close()
    this.failSubscriptions(this.closedError('AkashX Harness runtime closed'))
  }
