---
description: "The TypeScript SDK client for callers that spawn a AkashX Harness runtime subprocess and drive agent turns over stdio JSON-RPC: the AkashXHarness run API and the lower-level HarnessClient."
kind: "package-library"
---

# @akashx/akx-sdk-client

English | [中文](README.zh.md)

## Summary

`akx-sdk-client` lets TypeScript programs start and drive a complete AkashX Harness runtime over stdio JSON-RPC. Use `AkashXHarness` to open sessions, send text or image prompts, collect event and notification streams, and obtain the last committed assistant response when the runtime becomes idle; use `HarnessClient` for direct protocol requests and subscriptions. Callers may provide `akxBin`; otherwise the client resolves the same-version `@akashx/akx` executable. The client owns the subprocess across runs, exposes typed transport and protocol failures, and reaps it on `close()` or `await using`. It is suitable when the caller can choose the runtime profile and launch settings.
...
Use this client when TypeScript code must drive a complete Harness runtime from another process and you can name the runtime executable explicitly. The common path is minimal: construct a `AkashXHarness` with a launch spec, run prompts, and close it so the child process is always reaped.

### Running agent turns with AkashXHarness

```ts
import { AkashXHarness } from '@akashx/akx-sdk-client'
import { ReasoningEffortId } from '@akashx/akx-llm'

await using harness = new AkashXHarness({
  profile: 'sdk',
...
The client is two layers over one wire: `AkashXHarness` (owned runs) over `HarnessClient` (the protocol client), mirroring the Python SDK's layering. It runs outside any harness context, so it spawns the runtime directly rather than through the `akx-subprocess` service — the seam's documented exception for SDK-managed transports — and its teardown ladder lives in this package. The runtime notifies for every session in its context; session-tree scoping is a client-side filter over `subagent.started` lineage edges.

### Source map

| File | Role |
|---|---|
| [`src/api.ts`](src/api.ts) | `AkashXHarness` + `HarnessSession`: owned runs, receipt-to-idle collection, `finalResponse` |
...
- [SDK subagent backend](../../subagent/subagent-akx-sdk/README.md) — a harness-internal consumer of this client.
- [SDK application bundle](../../bundle/sdk-app/README.md) — the `akx --profile sdk` runtime application this client launches.
...
- **No bundled-runtime resolution** — the client resolves the same-version `@akashx/akx` package (or a caller-provided `akxBin`); packaged-executable discovery stays Python-side until a TypeScript distribution consumer exists.
