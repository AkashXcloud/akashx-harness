/**
 * Named wire types for the AkashX Harness SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@akashx/akx-sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `akx-harness-sdk-runtime`.
 *
 * @module @akashx/akx-sdk-protocol/types
 */

import type { ContentBlock, ReasoningEffortId } from '@akashx/akx-llm'
import type { SessionEvent } from '@akashx/akx-session'
import type { SubagentStopReason } from '@akashx/akx-subagent'
...
export interface InitializeResult {
  /** Wire-stable server identity (`akx-harness-sdk-runtime`) and version. */
