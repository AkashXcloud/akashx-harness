...
import SandboxPolicyService, { SANDBOX_MODES, setSandboxMode } from '@akashx/akx-sandbox-policy'
import SessionProjectionRegistry from '@akashx/akx-session-projection'
import SystemPrompt, { renderContextSnapshot, renderPrompt } from '@akashx/akx-system-prompt'
...
describe('SandboxPolicyService', () => {
  ...
  it.skipIf(process.platform === 'win32')('resolves a symlink-sensitive session cwd with POSIX component semantics', async () => {
    const root = mkdtempSync(join(tmpdir(), 'akx-policy-cwd-'))
    try {
...
describe('sandbox:policy request context', () => {
  ...
  it.each(['read-only', 'workspace-write', 'danger-full-access'] as const)('renders the exact %s policy without a capability inventory', async (mode) => {
    ...
    const expected = {
      'read-only': 'Current AKX file policy: read-only. Any available operation enforced by the AKX file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.',
      'workspace-write': `Current AKX file policy: workspace-write. Any available operation enforced by the AKX file sandbox may modify files under the session workspace: ${JSON.stringify(workspaceRoot)}. Some platform temporary areas may also be writable.`,
      'danger-full-access': 'Current AKX file policy: danger-full-access. The AKX file sandbox does not restrict file modifications by available operations.',
    } as const
  ...
  it('reflects the latest durable switch on the next assembly and stays byte-stable otherwise', async () => {
    ...
    expect(danger).toBe('Current AKX file policy: danger-full-access. The AKX file sandbox does not restrict file modifications by available operations.')
    expect(await policyContext(ctx, active)).toBe(danger)

    setSandboxMode(active, 'workspace-write')
    expect(await policyContext(ctx, active)).toBe(`Current AKX file policy: workspace-write. Any available operation enforced by the AKX file sandbox may modify files under the session workspace: ${JSON.stringify(resolve('/projects/current'))}. Some platform temporary areas may also be writable.`)
  })
