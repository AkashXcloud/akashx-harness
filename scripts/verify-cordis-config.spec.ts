...
describe('verify-cordis-config metadata expressions', () => {
  it('accepts a disabled !!js expression', () => {
    const problems = metadataExpressionErrors(
      { id: 'tool-bash', name: '@akashx/akx-tool-bash', disabled: { __jsExpr: "process.platform === 'win32'" } },
      '[0]',
    )
...
describe('workspace Bundle discovery and product dependency closures', () => {
  it('discovers a Bundle outside packages/bundle from its manifest declaration', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'akx-bundle-discovery-'))
    try {
  ...
  it('allows a Bundle to mount itself but rejects an undeclared plugin package', () => {
    ...
    const self = { file, name: '@akashx/akx-subagent-example' }
    ...
    expect(bundlePluginDependencyErrors(manifestPath, manifest, [
      self,
      { file, name: '@akashx/akx-missing-plugin' },
    ])).toEqual([
      `${file}: @akashx/akx-missing-plugin must be declared in ${manifestPath} dependencies`,
    ])
  })
})
...
describe('package-owned Loader test dependency closures', () => {
  it('requires package test configs to declare each named plugin they load', () => {
    ...
    const manifest = {
      name: '@akashx/akx-owner',
      dependencies: {},
      devDependencies: {
        '@akashx/akx-declared': 'workspace:^',
      },
    }
    expect(packageTestPluginDependencyErrors(manifestPath, manifest, [
      { file, name: '@akashx/akx-owner' },
      { file, name: '@akashx/akx-declared' },
      { file, name: '@akashx/akx-missing' },
    ])).toEqual([
      `${file}: @akashx/akx-missing must be declared in ${manifestPath} dependencies or devDependencies`,
    ])
  })

  it('requires executable package test fixtures to declare their bare imports', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'akx-package-test-entrypoint-'))
    try {
      ...
      writeFileSync(join(driverDir, 'driver.ts'), [
        "import '@akashx/akx-owner'",
        "import '@akashx/akx-declared'",
        "import '@akashx/akx-missing'",
      ].join('\n'))
      writeFileSync(join(driverDir, 'cordis.yml'), '[]\n')
      writeFileSync(join(driverDir, 'fixture.mjs'), "import '@akashx/akx-declared'\n")
      const unrelatedDir = join(packageDir, 'tests/fixtures/unrelated')
      mkdirSync(unrelatedDir, { recursive: true })
      writeFileSync(join(unrelatedDir, 'driver.ts'), "import '@akashx/akx-unrelated'\n")

      expect(packageTestFixtureDependencyErrors(fixture)).toEqual([
        'packages/example/owner/tests/fixtures/loader/driver.ts: '
        + '@akashx/akx-missing must be declared in '
  ...
  it('fails loud when package-owned Loader fixtures disappear from the scan', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'akx-empty-package-test-entrypoint-'))
    try {
      expect(packageTestFixtureDependencyErrors(fixture)).toEqual([
