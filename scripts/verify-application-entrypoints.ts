/**
 * Enforce akx profiles as the only supported Node application launcher.
...
interface DemoPolicy {
  readonly kind: 'akx-direct' | 'akx-wrapper'
...
const MANIFEST_BIN_ALLOWLIST = new Map<string, ManifestBin>([
  ['apps/cli/package.json', { akx: 'lib/bin.js', akashx: 'lib/akashx.js' }],
  ['packages/experimental/webworker-packer/package.json', { 'akx-pack-vfs-image': './bin.js' }],
])
...
const EXECUTABLE_SOURCE_ALLOWLIST = new Map<string, string>([
  ['apps/cli/src/bin.ts', 'supported akx application launcher'],
  ['apps/cli/src/akashx.ts', 'supported AkashX application launcher'],
  ...
  ['packages/subagent/subagent-akx-sdk/tests/fixtures/loader/driver.ts', 'test-only subprocess driver'],
...
/** Root demos are application wrappers and therefore must visibly select akx. */
const ROOT_DEMO_POLICIES = new Map<string, DemoPolicy>([
  ['demo:ptc', { kind: 'akx-wrapper', wrapper: 'scripts/demo-ptc.mjs' }],
  ['demo:inspector', { kind: 'akx-direct' }],
])
...
function manifestBinViolations(root: string): string[] {
  ...
  for (const rawPath of manifests) {
    ...
    if (expected === undefined) {
      failures.push(`${path}: package bin bypasses the akx launcher; applications use apps/cli profiles`)
      continue
    }
...
function referencesAkxCli(source: string): boolean {
...
function rootDemoViolations(root: string): string[] {
  ...
  for (const [name, commandValue] of Object.entries(manifest.scripts ?? {}).sort(([left], [right]) => left.localeCompare(right))) {
    ...
    if (policy === undefined) {
      failures.push(`package.json scripts.${name}: demo launcher has no explicit akx or in-process classification`)
      continue
    }
    if (policy.kind === 'akx-direct') {
      if (!referencesAkxCli(command)) failures.push(`package.json scripts.${name}: application demo must launch apps/cli/src/bin.ts`)
    ...
    if (!referencesAkxCli(source)) failures.push(`${wrapper}: application demo wrapper must launch apps/cli/src/bin.ts`)
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (failures.length > 0) {
    ...
    console.log('verify-application-entrypoints: akx is the only supported Node application launcher.')
  }
}
