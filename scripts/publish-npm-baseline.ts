...
const RELEASE_ENTRY_PACKAGE = '@akashx/akx'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"akx web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed akx web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed akx web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
...
class DetachedWorktree {
  ...
  static create(repositoryRoot: string, commit: string, runner: CommandRunner): DetachedWorktree {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'akx-npm-baseline-'))
...
class WorkspacePackageSet {
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!isVendored && !name.startsWith('@akashx/')) {
        throw new Error(`${manifestPath} must name an @akashx package`)
      }
      if (name === '@akashx/akx-root') {
...
/** Installs one complete bundle outside the workspace and probes the shipped akx entry. */
class InstalledBundleSmoke {
  ...
  run(): void {
    const consumerRoot = mkdtempSync(join(tmpdir(), 'akx-npm-consumer-'))
    try {
      ...
      writeFileSync(resolve(consumerRoot, 'package.json'), `${JSON.stringify({
        name: 'akx-npm-baseline-consumer',
        version: '0.0.0',
        private: true,
        dependencies,
      }, null, 2)}\n`)
      ...
      const bin = resolve(consumerRoot, 'node_modules/@akashx/akx/lib/bin.js')
      assertPathWithin(consumerRoot, bin, 'installed akx bin')
      ...
      if (version !== this.bundle.manifest.version) {
        throw new Error(
          `installed akx --version returned ${JSON.stringify(version)}; `
          + `expected ${this.bundle.manifest.version}`,
        )
      }
      this.probeWeb(bin, consumerRoot, environment)
      console.log('publish-npm-baseline: installed akx entry and Web startup probes passed')
    } finally {
  ...
  private probeWeb(bin: string, consumerRoot: string, environment: NodeJS.ProcessEnv): void {
    if (process.platform === 'win32') {
      throw new Error('installed akx Web probe requires a POSIX host with python3')
    }
    const result = this.runner.result(
      'python3',
    ...
    if (result.status !== 0) {
      throw commandFailure('python3', ['installed-akx-web-probe'], result)
    }
  }
}
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@akashx/') || name === '@akashx/akx-root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.AKX_HOME = resolve(consumerRoot, '.akx')
  environment.AKX_AGENTS_HOME = resolve(consumerRoot, '.agents')
  environment.AKX_TELEMETRY_DISABLED = '1'
  environment.AKASHX_API_KEY = 'keyless-installed-web-no-call'
