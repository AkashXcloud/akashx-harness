...
describe('Vitest jsdom compatibility', () => {
  it('provides isolated browser storage instead of Node process storage', () => {
    ...
    localStorage.setItem('akx-vitest-storage-probe', 'available')

    expect(localStorage.getItem('akx-vitest-storage-probe')).toBe('available')
    localStorage.removeItem('akx-vitest-storage-probe')
  })
})
