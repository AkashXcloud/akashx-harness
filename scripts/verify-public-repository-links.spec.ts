...
describe('repository link policy', () => {
  it('rejects encoded and case-varied references to the unavailable repository', () => {
    const unavailableOwner = ['akashx', 'ai'].join('-')
    const unavailableName = ['akashx', 'harness', 'sdk'].join('-')
    ...
    const source = [
      'https://github.com/akashx-ai/akx-harness',
  ...
  it('preserves frozen archived Agent Notes', () => {
    const unavailableRepository = ['akashx-ai', 'akx-harness-sdk'].join('/')
