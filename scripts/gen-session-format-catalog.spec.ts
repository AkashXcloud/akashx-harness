...
function fixture(edges: Array<[number, number]>): string {
  const root = mkdtempSync(join(tmpdir(), 'akx-session-format-catalog-'))
  ...
  for (const [from, to] of edges) {
    ...
    const name = `@akashx/akx-session-format-v${from}-to-v${to}`
    mkdirSync(dir, { recursive: true })
    catalogDependencies[name] = 'workspace:^'
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      name,
      akx: { sessionFormatMigration: {
        from, to, export: '.',
      ...
      dependencies: from === 0
        ? { '@akashx/akx-session-format': 'workspace:^' }
        : {
          '@akashx/akx-session-format': 'workspace:^',
          [`@akashx/akx-session-format-v${from - 1}-to-v${from}`]: 'workspace:^',
        },
    }))
  }
  ...
  writeFileSync(join(catalog, 'package.json'), JSON.stringify({
    ...
    peerDependencies: { '@akashx/akx-session': 'workspace:^' },
    devDependencies: { '@akashx/akx-session': 'workspace:^' },
  }))
  return root
}
...
function edgeManifest(root: string, from: number, to: number): {
  ...
  value: Record<string, unknown> & {
    ...
    akx: { sessionFormatMigration: Record<string, unknown> }
  }
} {
...
describe('session format catalog generator', () => {
  ...
  it('discovers one complete adjacent chain and renders direct imports', () => {
    ...
    expect(output).toContain("from '@akashx/akx-session-format-v0-to-v1'")
    ...
    expect(output).toContain("from '@akashx/akx-session'")
  ...
  it('refuses a later edge whose declared source codec does not continue the prior target', () => {
    ...
    manifest.value.akx.sessionFormatMigration['sourceCodec'] = 'UnrelatedV1Codec'
  ...
  it('requires every later edge to depend on the package that owns its source codec', () => {
    const root = fixture([[0, 1], [1, 2]])
    const manifest = edgeManifest(root, 1, 2)
    delete manifest.value.dependencies['@akashx/akx-session-format-v0-to-v1']
    writeFileSync(manifest.path, JSON.stringify(manifest.value))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/must depend on @akashx\/akx-session-format-v0-to-v1/)
  })

  it('requires the package name to identify its declared adjacent edge', () => {
    const root = fixture([[0, 1], [1, 2]])
    const manifest = edgeManifest(root, 1, 2)
    manifest.value['name'] = '@akashx/akx-session-format-other'
    writeFileSync(manifest.path, JSON.stringify(manifest.value))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/name must be @akashx\/akx-session-format-v1-to-v2/)
  })
  ...
  it('requires the catalog to share the installed Session package as a peer', () => {
    ...
    manifest.dependencies['@akashx/akx-session'] = 'workspace:^'
    delete manifest.peerDependencies['@akashx/akx-session']
    writeFileSync(path, JSON.stringify(manifest))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/must share @akashx\/akx-session through peer \+ dev dependencies/)
  })
})
