 ...
 * stays valid, the consolidated `akx-doc` skill carries no stale copied
...
/**
 * The kind system: each label maps to exactly one template in the akx-doc
...
const KIND_TEMPLATES: Readonly<Record<string, string>> = {
  'package-group': '.agents/skills/akx-doc/templates/package-group.md',
  'package-reference': '.agents/skills/akx-doc/templates/package-reference.md',
  'package-library': '.agents/skills/akx-doc/templates/package-library.md',
  'package-bundle': '.agents/skills/akx-doc/templates/package-bundle.md',
}
 ...
 * bundle (`akx.bundle.patch`). Each entry names why the package is a
...
/** Whether the package manifest declares `akx.bundle.patch`. */
function declaresBundle(dir: string): boolean {
  const manifest = resolve(root, dir, 'package.json')
  if (!existsSync(manifest)) return false
  const metadata = JSON.parse(readFileSync(manifest, 'utf8')) as { akx?: { bundle?: { patch?: string } } }
  return metadata.akx?.bundle?.patch !== undefined
}
...
function validateSessionFormatRelease(source: string, currentWriterVersion: number): SessionFormatRelease {
  ...
  if (typeof evidenceTag !== 'string'
    || !/^akx-v\d+\.\d+\.\d+(?:-[\dA-Za-z]+(?:[.-][\dA-Za-z]+)*)?(?:\+[\dA-Za-z]+(?:[.-][\dA-Za-z]+)*)?$/u.test(evidenceTag)) {
    throw new Error('evidenceTag must be a non-empty akx-v version tag without URL delimiters')
  }
  const repository = 'https://github.com/akx-harness/akx-harness'
  for (const link of [
...
describe('Session format release authority', () => {
  ...
  it('rejects empty, malformed, and URL-injecting evidence tags', () => {
    const { record, links } = sessionFormatReleaseFixture()
    for (const tag of [
      null, true, 1, '', ' ', 'akx-v', record.evidenceTag.replace('akx-v', 'v'),
      ...
      expect(() => validateSessionFormatRelease(source, record.latestReleasedVersion), String(tag)).toThrow('akx-v version tag')
    }
  })
...
describe('akx-doc skill consolidation', () => {
  it('carries no prototype-era language', () => {
    const files = [
      '.agents/skills/akx-doc/SKILL.md',
      '.agents/skills/akx-doc/references/metadata-links-i18n.md',
      '.agents/skills/akx-doc/references/structure-hierarchy.md',
      '.agents/skills/akx-doc/references/style.md',
      '.agents/skills/akx-doc/references/review.md',
      '.agents/skills/akx-doc/references/website-sync.md',
    ]
  ...
  it('copies no stale website sidebar or section-owner values', () => {
    const source = readFileSync(resolve(root, '.agents/skills/akx-doc/references/website-sync.md'), 'utf8')
  ...
  it('keeps the reference example linked from the skill', () => {
    const skill = readFileSync(resolve(root, '.agents/skills/akx-doc/SKILL.md'), 'utf8')
  ...
  it('defines controlled English as a precision-preserving review discipline', () => {
    const skill = readFileSync(resolve(root, '.agents/skills/akx-doc/SKILL.md'), 'utf8')
    const style = readFileSync(resolve(root, '.agents/skills/akx-doc/references/style.md'), 'utf8')
  ...
  it('maps every kind label to exactly one skill template that exists', () => {
    const templateFiles = globSync('.agents/skills/akx-doc/templates/*.md', { cwd: root }).map(path => path.split(sep).join('/')).sort()
