/** Application-entrypoint classification and akx-launch enforcement. */
...
function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'akx-application-entrypoints-'))
  cleanups.push(root)
  return root
}
...
describe('application entrypoints', () => {
  ...
  it('rejects a package-level application bin', () => {
    ...
    expect(applicationEntrypointViolations(root)).toEqual([
      'packages/example/app/package.json: package bin bypasses the akx launcher; applications use apps/cli profiles',
    ])
  })
  ...
  it('rejects a private Python application carrier outside akx', () => {
    const root = fixture()
  ...
  it('rejects a new root demo until its launch role is classified', () => {
    const root = fixture()
    write(root, 'package.json', JSON.stringify({ scripts: { 'demo:new-app': 'akx --profile new-app' } }))

    expect(applicationEntrypointViolations(root)).toEqual([
      'package.json scripts.demo:new-app: demo launcher has no explicit akx or in-process classification',
    ])
  })
})
