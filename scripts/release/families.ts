 ...
 * owns: `akx` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@akashx/akx-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@akashx/')) throw new Error(`${normalized} must name an @akashx package`)
...
class AkxFamily extends ReleaseFamily {
  readonly id = 'akx'
  readonly patterns = [
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`akx release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@akashx/akx', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@akashx/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new AkxFamily(), new VendorFamily()]
}
