 ...
 *
 * The akx family shares one version across its publishable members, private
...
/** Release types the akx family accepts besides an explicit version. */
const RELEASE_TYPES = ['major', 'minor', 'patch'] as const

/** The workspace root manifest, which carries the akx family's version. */
...
/** One private akx workspace whose version follows the publishable family. */
interface PrivateAkxVersion {
...
function nextSharedVersion(current: string, request: string): string {
  if (!RELEASE_TYPES.includes(request as typeof RELEASE_TYPES[number])) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(request)) {
      throw new Error(`usage: release:akx <major|minor|patch|x.y.z>, got ${request}`)
    }
    return request
  }
...
/**
 * Discover private package and application manifests that share the akx version
 * without joining its publish set.
 * @param root - repository root.
 * @returns Private workspace manifests sorted by path.
 */
function privateAkxVersions(root: string): PrivateAkxVersion[] {
...
/**
 * Plan the akx family's rewrite: one version for every publishable member,
 * private package, and the root.
 * @param family - the akx family.
 * @param root - repository root.
...
export function planShared(
  ...
  for (const entry of privateAkxVersions(root)) {
...
function main(): void {
  ...
  if (values.family === undefined) throw new Error('usage: bump.ts --family <akx|vendor> [version]')
  ...
  if (family.id === 'akx') {
    ...
    if (request === undefined) throw new Error('usage: release:akx <major|minor|patch|x.y.z>')
    if (values.prerelease !== undefined) {
      throw new Error('release:akx takes the prerelease in its version argument, as in 0.0.1-rc.1')
    }
