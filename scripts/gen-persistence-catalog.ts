...
const SESSION_PACKAGE = '@akashx/akx-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@akashx/akx-session/types'
 ...
 * top-level declaration (in `@akashx/akx-session`) and any declaration
 * merge inside a `declare module '@akashx/akx-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @akashx/akx-session/known-event-types',
    ' */',
    '',
    '/**',
