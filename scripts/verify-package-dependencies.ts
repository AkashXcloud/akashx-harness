...
export function discoverPackageDependencyScope(
  ...
  for (const name of include) {
    ...
    if (pkg !== undefined
      && (pkg.manifestPath.startsWith('packages/client/') || hasClientDeclaration(pkg.manifest.akx))) {
  ...
  for (const name of exclude) {
    ...
    } else if (pkg !== undefined && !hasClientDeclaration(pkg.manifest.akx)) {
      violations.push(`clientFaceExclude names ${name}, which declares no akx.client entry`)
    }
  }
  ...
  for (const pkg of packages) {
    ...
    const clientHost = (hasClientDeclaration(pkg.manifest.akx) || include.has(pkg.name)) && !exclude.has(pkg.name)
...
export function readPackageDependencyFacts(
  ...
  const inject = pkg.manifest.akx?.client?.inject ?? []
...
export function expectedPackageDependencies(
  ...
  for (const name of facts.clientInject) {
    if (facts.workspaceNames.has(name)) add(name, 'devDependencies', 'akx.client.inject')
  }
