...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@akashx/akx-client-test', ['lib/types/index.js'])
    const development = bundle({ env: {} }).find(config => config.platform === 'browser')
    const artifact = bundle({ env: { AKX_BUILD_FACE: 'client' } })
...
function purityResolveId(id = REQUESTING_PACKAGE): ResolveId {
  ...
  const gate = plugins.find(p => p.name === 'akx-client-bundle-purity')
...
function cssModulePlugin(): CssModulePlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'akx-css-modules-inline')
...
function sourceMapPlugin(): SourceMapPlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'akx-tsc-sourcemap')
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@akashx/akx-client-store')).toBeNull()
    expect(resolveId('@akashx/akx-client-ui-slots')).toBeNull()
    expect(resolveId('@akashx/akx-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@akashx/akx-client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@akashx/akx-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe libraries inline', () => {
    expect(resolveId('@akashx/akx-session/surface')).toBeNull()
    expect(resolveId('@akashx/akx-brand')).toBeNull()
    expect(resolveId('@akashx/akx-deque')).toBeNull()
    expect(resolveId('@akashx/akx-util-values')).toBeNull()
    expect(resolveId('@akashx/akx-token-meter/client')).toBeNull()
    expect(() => resolveId('@akashx/akx-token-meter')).toThrow(/purity/)
    expect(() => resolveId('@akashx/akx-token-meter/client/internal')).toThrow(/purity/)
    expect(resolveId('@akashx/akx-host-open-in-app/shared')).toBeNull()
    expect(() => resolveId('@akashx/akx-host-open-in-app')).toThrow(/purity/)
  })

  it('admits only the pure spill notice entry, not its Host policy', () => {
    expect(resolveId('@akashx/akx-spill-policy/notice')).toBeNull()
    expect(resolveId('@akashx/akx-output-retention')).toBeNull()
    expect(() => resolveId('@akashx/akx-spill-policy')).toThrow(/purity/)
    expect(() => resolveId('@akashx/akx-spill-policy/notice/internal')).toThrow(/purity/)
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@akashx/akx-goal/remote')).toBeNull()
    expect(() => resolveId('@akashx/akx-goal')).toThrow(/purity/)
    expect(() => resolveId('@akashx/akx-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@akashx/akx-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @akashx leak', () => {
    expect(() => resolveId('@akashx/akx-agent')).toThrow(/purity/)
    expect(() => resolveId('@akashx/akx-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@akashx/akx-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@akashx/akx-client-ui-session')).toThrow(/purity/)
    expect(() => resolveId('@akashx/akx-client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits package-specific requests only for the declaring bundle', () => {
    const requesting = purityResolveId('@akashx/akx-api-session-controller')
    expect(requesting('@akashx/akx-api-gateway/client')).toBeNull()
    expect(() => resolveId('@akashx/akx-api-gateway/client')).toThrow(/purity/)
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@akashx/akx-client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@akashx/akx-client-store')).toBe(true)
  })
})

describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@akashx/akx-client-fixture', {
      external: ['react', 'react/jsx-runtime', '@akashx/akx-client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@akashx/akx-client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@akashx/akx-client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@akashx/akx-client-fixture', { external: 'react' }))
      .toThrow(/akx\.client\.external must be a string array/)
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('chains emitted tsc maps when the production Client build consumes lib/types', async () => {
    const root = mkdtempSync(join(tmpdir(), 'akx-client-sourcemap-'))
    try {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@akashx/akx-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://akx.test/plugins/@akashx/akx-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@akashx/akx-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@akashx/akx-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://akx.test/plugins/@akashx/akx-client-connection/client.js.map')
