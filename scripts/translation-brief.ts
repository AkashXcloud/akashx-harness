 ...
 * consumes the briefing is `.agents/skills/akx-translate-docs/SKILL.md`.
 */
...
function maskCodeSpans(markdown: string, spans: MarkdownSpan[]): string {
  return replaceSpanTexts(markdown, spans, new Map(spans.map(span => [span.index, `AKX_TRANSLATION_CODE_${span.index}\n`])))
}
