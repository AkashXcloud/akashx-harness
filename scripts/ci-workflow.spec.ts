...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    for (const [jobName, job] of [['windows-build', windowsBuild], ['windows-coverage', windowsCoverage], ['windows-native-tests', windowsNativeTests], ['windows-observational', windowsObservational]] as const) {
      ...
      expect(job['runs-on'], `${jobName} runs-on must use the Windows failover switch`).toContain('AKX_CI_FAILOVER_WINDOWS')
      expect(job['runs-on'], `${jobName} runs-on must not use the Linux failover switch`).not.toContain('AKX_CI_FAILOVER_LINUX')
      expect(job['runs-on']).toContain('self-hosted')
      expect(job['runs-on']).toContain('akx-win-ci')
      expect(job['runs-on']).toContain('akx-windows-2025-16core')
    ...
    expect(windowsCoverage.env).toMatchObject({ AKX_COVERAGE_PARTITIONS: '4' })
    ...
    expect(serialWindows['runs-on']).toEqual(['self-hosted', 'akx-win-ci', 'windows'])
    ...
    expect(serialGate!.env).toMatchObject({ AKX_COVERAGE_TEST_TIMEOUT_MS: '90000' })
    ...
    expect(node24Bench.steps).toContainEqual({
      ...
      run: 'pnpm --filter @akashx/akx-benchmarks exec playwright install --with-deps chromium',
    })
    expect(JSON.stringify(node24Bench.steps)).not.toContain('AKX_CI_FAILOVER_LINUX')
    expect(node24Bench.steps).toContainEqual({
      name: 'Run performance benchmarks',
      env: { AKX_GATE_VERBOSE: '1' },
    ...
    // and the verdict job resolve their pool through AKX_CI_FAILOVER_LINUX,
    // never the Windows switch.
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers]] as const) {
      expect(typeof job['runs-on']).toBe('string')
      expect(job['runs-on'], `${jobName} runs-on must use the Linux failover switch`).toContain('AKX_CI_FAILOVER_LINUX')
      expect(job['runs-on'], `${jobName} runs-on must not use the Windows failover switch`).not.toContain('AKX_CI_FAILOVER_WINDOWS')
      expect(job['runs-on']).toContain('vm-backup')
      expect(job['runs-on']).toContain('blacksmith-16vcpu-ubuntu-2404')
    }
    expect(aggregate['runs-on']).toContain('AKX_CI_FAILOVER_LINUX')
    expect(aggregate['runs-on']).not.toContain('AKX_CI_FAILOVER_WINDOWS')
    ...
    for (const [name, selector, variable, pool, hosted] of [
      ['linux gates', selectors.linux, 'AKX_CI_FAILOVER_LINUX', ['self-hosted', 'linux', 'x64', 'vm-backup'], 'akx-ubuntu-24-04-16core'],
      ['linux aggregate', selectors.linuxAggregate, 'AKX_CI_FAILOVER_LINUX', ['self-hosted', 'linux', 'x64', 'vm-backup'], 'ubuntu-latest'],
      ['windows lanes', selectors.windows, 'AKX_CI_FAILOVER_WINDOWS', ['self-hosted', 'akx-win-ci', 'windows'], 'akx-windows-2025-16core'],
    ] as const) {
    ...
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers], ['node-compat', nodeCompat]] as const) {
      expect(job.env, `${jobName} must enable fail-fast`).toMatchObject({ AKX_GATE_FAIL_FAST: '1' })
    }
    ...
    expect(windowsBuild.env, 'windows-build must enable fail-fast').toMatchObject({ AKX_GATE_FAIL_FAST: '1' })
    expect(windowsCoverage.env, 'windows-coverage must enable fail-fast').toMatchObject({ AKX_GATE_FAIL_FAST: '1' })
    ...
    expect(windowsObservational.env).not.toMatchObject({ AKX_GATE_FAIL_FAST: '1' })
  })
  ...
  it('gates standalone keyless blacksmith jobs and benchmark tiers on the failover variables', () => {
    ...
    expect(expectedFilenames['runs-on']).toContain('AKX_CI_FAILOVER_LINUX')
    ...
    expect(sandbox['runs-on']).toContain('AKX_CI_FAILOVER_LINUX')
    ...
    for (const name of ['larger-runner-benchmark', 'consolidated-runner-benchmark'] as const) {
      ...
      expect(benchmark['runs-on']).toContain('AKX_CI_FAILOVER_LINUX')
      expect(benchmark['runs-on']).toContain('AKX_CI_FAILOVER_WINDOWS')
  ...
  it('requires release-shaped Python runtime validation on Linux and Windows x64', () => {
    ...
    expect(pythonRuntime).toMatchObject({
      ...
      secrets: {
        AKASHX_API_KEY_EXTERNAL: '${{ secrets.AKASHX_API_KEY_EXTERNAL }}',
      },
    })
...
describe('AkashX e2e workflow', () => {
  it('prepares bubblewrap from the pinned payload without a package transaction', () => {
    const workflow = loadWorkflow('.github/workflows/e2e.yml')
    const e2e = workflowJob(workflow, 'e2e')
    if (!Array.isArray(e2e.steps)) throw new TypeError('AkashX e2e workflow must define steps')
  ...
  it('bounds profile subprocess fan-out to the tested e2e default', () => {
    ...
    const step = e2e.steps.filter(isRecord).find(candidate => candidate.name === 'E2E tests (real AkashX API)')
    expect(step).toMatchObject({ env: { AKX_E2E_MAX_WORKERS: 4 } })
  })
})
  ...
  it('is manual-only and fails loud before running the focused live suite', () => {
    ...
    expect(e2b).toMatchObject({
      env: {
        ...
        AKX_E2E_MAX_WORKERS: '1',
        AKX_EXAMPLE_MODE: 'lib',
      },
    })
...
describe('Python release workflows', () => {
  it('keeps complete wheel validation separate from protected public publication', () => {
    ...
    expect(pythonCompatSteps).toContain('dist/akx_harness_sdk-$VERSION-py3-none-any.whl')
    expect(pythonCompatSteps).toContain('dist/akx_harness_runtime_bin-$VERSION-py3-none-manylinux_2_28_x86_64.whl')
  ...
  it('exposes the native wheel builder to the release caller with normalized versions', () => {
    ...
    expect(call.secrets).toMatchObject({
      AKASHX_API_KEY_EXTERNAL: { required: false },
    })
    ...
    expect(realApiPreflightPosix).toMatchObject({
      env: { AKASHX_API_KEY: '${{ secrets.AKASHX_API_KEY_EXTERNAL }}' },
    })
    ...
    expect(installedRealApiPosix).toMatchObject({
      env: {
        AKASHX_API_KEY: '${{ secrets.AKASHX_API_KEY_EXTERNAL }}',
        AKASHX_BASE_URL: 'https://api.akashx.com',
      },
    })
    expect(JSON.stringify(installedRealApiPosix)).toContain('--scenario sdk-live')
    expect(JSON.stringify(installedRealApiPosix)).toContain('-u AKX_RUNTIME_MODE')
    ...
    expect(JSON.stringify(manylinuxSmoke)).toContain('-e AKX_TELEMETRY_DISABLED')
  })
...
describe('Issue lifecycle workflow', () => {
  ...
  it('uses a read-only Project token only for human pull request policy metadata', () => {
    ...
    expect(tokenStep).toMatchObject({
      ...
      with: {
        'client-id': '${{ vars.AKX_ISSUE_APP_CLIENT_ID }}',
        'private-key': '${{ secrets.AKX_ISSUE_APP_PRIVATE_KEY }}',
        owner: 'akx-harness',
        repositories: 'akx-harness',
...
describe('npm release workflows', () => {
  ...
  it('runs dependency policy and npm layout checks in the AKX release workflow', () => {
    ...
    if (!isRecord(workflow.on) || !Array.isArray(dependencies.steps)) {
      throw new TypeError('AKX release workflow must define triggers and dependency steps')
    }
...
describe('Documentation site publication', () => {
  it('keeps Pages deployment dispatch-only from a akx-v* tag', () => {
    ...
    // RELEASE_PUBLISH makes release:verify reject every ref that is not a akx-v*
    ...
    expect(verify).toMatchObject({
      ...
      run: 'pnpm run release:verify --family akx',
    })
