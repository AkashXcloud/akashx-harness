...
export function collectSessionFormatMigrations(
  ...
  for (const discovered of globSync('packages/session/session-format-v*-to-v*/package.json', { cwd: scanRoot }).sort()) {
    ...
    const metadata = manifest.akx?.sessionFormatMigration
    if (metadata === undefined) {
      throw new Error(`gen-session-format-catalog: ${rel} lacks akx.sessionFormatMigration`)
    }
    ...
    const expectedPackageName = `@akashx/akx-session-format-v${from}-to-v${to}`
  ...
  if (catalog.dependencies?.['@akashx/akx-session'] !== undefined
    || catalog.peerDependencies?.['@akashx/akx-session'] === undefined
    || catalog.devDependencies?.['@akashx/akx-session'] === undefined) {
    throw new Error(
      'gen-session-format-catalog: catalog must share @akashx/akx-session through peer + dev dependencies',
    )
  }
...
export function renderSessionFormatCatalog(
  ...
  return [
    ...
    ' */',
    '',
    "import { KNOWN_SESSION_EVENT_TYPES } from '@akashx/akx-session'",
