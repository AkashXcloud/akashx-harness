...
const CLIENT_BUILD_ENV_PREFIX = 'AKX_CLIENT_'

/** Non-public selector used by build orchestration to request a named client profile. */
export const CLIENT_BUILD_PROFILE_SELECTOR = 'AKX_BUILD_CLIENT_PROFILE'

/** Public client environment required by official AKX artifacts. */
...
const CLIENT_COMMIT_HASH_VARIABLE = 'AKX_CLIENT_COMMIT_HASH'

/** Public variable carrying the repository package version embedded in client artifacts. */
const CLIENT_VERSION_VARIABLE = 'AKX_CLIENT_VERSION'

/** Repository-relative path of the complete client build record. */
export const CLIENT_BUILD_RECORD_PATH = '.akx-build/client-build-environment.json'
...
export function repositoryClientBuildEnvironment(
  ...
  delete inherited.AKX_CLIENT_COMMIT_HASH
  delete inherited.AKX_CLIENT_GIT_DIRTY
  delete inherited.AKX_CLIENT_VERSION
  const dirty = repositoryGitDirty(root)
  return {
    ...inherited,
    AKX_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
    ...(dirty === true ? { AKX_CLIENT_GIT_DIRTY: 'true' } : {}),
    AKX_CLIENT_VERSION: repositoryVersion(root),
  }
}
...
export function officialClientBuildEnvironment(
  root: string,
  environment: NodeJS.ProcessEnv = process.env,
): Readonly<Record<`AKX_CLIENT_${string}`, string>> {
  return {
    AKX_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
    AKX_CLIENT_VERSION: repositoryVersion(root),
 ...
 * @returns defined `AKX_CLIENT_*` values only.
 */
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official') {
    ...
    return {
      AKX_CLIENT_COMMIT_HASH: commitHash,
      AKX_CLIENT_VERSION: version,
      ...OFFICIAL_CLIENT_BUILD_ENVIRONMENT,
    }
  }
...
export function assertClientBuildEnvironment(
  ...
  expected: Readonly<Record<`AKX_CLIENT_${string}`, string>>,
): void {
