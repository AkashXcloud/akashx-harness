...
describe('generated tsconfig package aliases', () => {
  it('maps each package to its own source directory', () => {
    ...
    const session = aliases.find(alias => alias.specifier === '@akashx/akx-session')
    expect(session).toEqual({
      specifier: '@akashx/akx-session',
    ...
    expect(aliases.some(alias => alias.specifier === '@akashx/akx-typert-protocol')).toBe(false)
  })

  it('yields to a hand-written alias and closes without a trailing comma', () => {
    const aliases = [
      { specifier: '@akashx/akx-a', source: './packages/g/a/src', hasInvariant: true },
      { specifier: '@akashx/akx-b', source: './packages/g/b/src', hasInvariant: false },
    ]
    const body = renderAliases(aliases, new Set(['@akashx/akx-a']))

    // The hand-written bare alias is skipped; its /invariant sibling is not.
    expect(body).toBe([
      '      "@akashx/akx-a/invariant": ["./packages/g/a/src/invariant.ts"]',
      '      "@akashx/akx-b": ["./packages/g/b/src"]',
    ].join(',\n'))
  ...
  it('names a package that no alias covers', () => {
    ...
    expect(uncoveredPackages(
      ['@akashx/akx-a', '@akashx/akx-b'],
      new Set(['@akashx/akx-a', '@akashx/akx-a/invariant']),
    )).toEqual(['@akashx/akx-b'])

    expect(uncoveredPackages(['@akashx/akx-a'], new Set(['@akashx/akx-a']))).toEqual([])
  })
  ...
  it('covers every workspace package in the committed config', () => {
    ...
    expect(names).toContain('@akashx/akx-typert-protocol')
  ...
  it('leaves no wildcard that probes every package group', () => {
    ...
    expect(config).not.toContain('"@akashx/akx-*":')
    expect(config).not.toContain('"@akashx/akx-*/invariant":')
  })
})
