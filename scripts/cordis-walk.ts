 ...
 * BLOCK — a file may legally hold several `declare module '@akashx/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@akashx/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@akashx/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@akashx/cordis'` Context merge
...
/**
 * Every event name a `declare module '@akashx/cordis'` Events merge declares in one
