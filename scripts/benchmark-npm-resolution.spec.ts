...
describe('npm resolution benchmark', () => {
  ...
  it('combines installed metadata with current publishable workspace fields', () => {
    const root = mkdtempSync(join(tmpdir(), 'akx-npm-registry-index-'))
    roots.push(root)
    ...
    writeJson(root, 'apps/cli/package.json', {
      name: '@akashx/akx',
      version: '0.1.0',
      dependencies: { '@akashx/akx-child': 'workspace:^', external: '^2.0.0' },
    ...
    expect(index.get('@akashx/akx')?.get('0.1.0')).toEqual({
      name: '@akashx/akx',
      version: '0.1.0',
      dependencies: { '@akashx/akx-child': '^0.1.0', external: '^2.0.0' },
    })
  })
  ...
  it('runs npm against the local registry without requesting an archive', async () => {
    const index: RegistryIndex = new Map([[
      '@akashx/akx',
      new Map([['0.1.0', { name: '@akashx/akx', version: '0.1.0' }]]),
    ]])
  ...
  it('returns npm placement for two aliased package versions without requesting archives', async () => {
    const index: RegistryIndex = new Map([[
      '@akashx/akx',
      new Map([
        ['0.1.0', { name: '@akashx/akx', version: '0.1.0' }],
        ['0.2.0', { name: '@akashx/akx', version: '0.2.0' }],
      ]),
    ]])

    const result = await resolveNpmPackageLock(index, {
      '@akashx/akx': '0.2.0',
      'akx-previous': 'npm:@akashx/akx@0.1.0',
    }, 10_000)

    expect(result.archiveRequests).toBe(0)
    expect(result.packageLock.packages['node_modules/@akashx/akx']?.version).toBe('0.2.0')
    expect(result.packageLock.packages['node_modules/akx-previous']).toMatchObject({
      name: '@akashx/akx',
      version: '0.1.0',
    })
  })
  ...
  it('isolates peer resolution from inherited npm configuration', async () => {
    const root = mkdtempSync(join(tmpdir(), 'akx-hostile-npm-config-'))
    roots.push(root)
    const userConfig = join(root, 'user.npmrc')
    writeFileSync(userConfig, '@akashx:registry=http://127.0.0.1:1/\nlegacy-peer-deps=true\nomit=peer\n')
    const previous = {
    ...
    try {
      const index: RegistryIndex = new Map([
        ['@akashx/akx', new Map([['0.1.0', {
          name: '@akashx/akx',
          version: '0.1.0',
          peerDependencies: { '@akashx/akx-peer': '1.0.0' },
        }]])],
        ['@akashx/akx-peer', new Map([['1.0.0', {
          name: '@akashx/akx-peer',
          version: '1.0.0',
        }]])],
      ])

      const result = await resolveNpmPackageLock(index, { '@akashx/akx': '0.1.0' }, 10_000)

      expect(result.archiveRequests).toBe(0)
      expect(result.packageLock.packages['node_modules/@akashx/akx-peer']?.version).toBe('1.0.0')
    } finally {
