/**
 * Rescope the vendored Cordis packages into the `@akashx` scope, and undo
 ...
 * `@akashx/akx-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@akashx/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@akashx/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@akashx/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@akashx/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@akashx/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@akashx/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@akashx/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@akashx/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@akashx/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@akashx/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@akashx/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@akashx\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@akashx/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@akashx/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@akashx/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@akashx/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@akashx/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@akashx/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@akashx` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@akashx\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@akashx/cordis']
    const dev = manifest.devDependencies?.['@akashx/cordis']

    if (!peer) errors.push(\`\${label}: @akashx/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @akashx/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @akashx/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@akashx/\')) {',
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@akashx/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@akashx/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@akashx/` as "another plugin package".
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@akashx/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@akashx\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@akashx/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
