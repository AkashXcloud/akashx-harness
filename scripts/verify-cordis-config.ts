 ...
 * silently changes composition. Shipped and test-only akx overlays resolve
...
// These overlays are consumed by the built akx app, so their bare specifiers
...
const CHOOSER_PACKAGE = '@akashx/akx-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@akashx/akx-host-directory-picker-native',
  '@akashx/akx-host-directory-picker-browse',
  '@akashx/akx-client-ui-directory-picker-browse',
  '@akashx/akx-client-ui-directory-picker-native',
]
const errors: string[] = []
 ...
 * `akx.client` block, and the node half of a surface plugin is an empty
 ...
 * `akx.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    ...
    const declaresClient = manifest.akx?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no akx.client, so its browser half is never served`
      : `${manifestPath}: declares akx.client but exports no "./client" entry to serve`]
  })
}
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if (value.name !== '@akashx/cordis-plugin-include') return
...
function validateAppResolution(): string[] {
  ...
  // akx app's own dependency surface — the profile module fallback mirrors it.
  ...
  for (const manifestPath of bundleManifests) {
    ...
    const patch = manifest.akx?.bundle?.patch
...
export function bundleManifestPaths(repoRoot: string = root): string[] {
  return globSync('packages/*/*/package.json', { cwd: repoRoot })
    .filter(path => typeof readManifest(path, repoRoot).akx?.bundle?.patch === 'string')
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `akx` source
