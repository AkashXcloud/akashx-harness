...
async function main(args: string[]): Promise<number> {
  ...
  const concurrencyOverride = process.env.AKX_GATE_CONCURRENCY
  const maxConcurrency = concurrencyFromEnv('AKX_GATE_CONCURRENCY', concurrencyDefault.workers)
  const concurrencySource = concurrencyOverride === undefined || concurrencyOverride === ''
    ? concurrencyDefault.source
    : '$AKX_GATE_CONCURRENCY'
  const failFast = flagEnabled('AKX_GATE_FAIL_FAST')
 ...
 * @param failFast - whether `AKX_GATE_FAIL_FAST` is enabled.
    ...
    case 'check-all':
      return [
        ...
        ...docSyncLeafGates({
          ...
          docTypecheckEnv: { AKX_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
...
function ciSharedStaticGates(): Gate[] {
  return [
    ...
    pnpmScript('akx-package-licenses', 'verify-akx-package-licenses', { label: 'AKX package licenses' }),
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('AKX_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
...
function nodeCompatSmokeGates(options: { cliSmoke?: boolean } = {}): Gate[] {
  const gates: Gate[] = [
    ...
    pnpmExec('akx-source-launch-smoke', [
      'vitest',
      'run',
      'apps/cli/tests/source-launch.compat.spec.ts',
    ], { label: 'akx source-launch smoke' }),
  ...
  if (options.cliSmoke) {
    gates.push(
      pnpmExec('cli-lazy-search-startup-smoke', [
        ...
        env: { AKX_REQUIRE_BUILT_CLI_SMOKE: '1' },
        needs: ['build:web'],
      }),
    )
  }
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
      env: { AKX_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
    }),
    pnpmScript('node-next-types', 'verify-node-next-types', {
      label: 'node-next types',
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[], after?: string[]): Gate {
  ...
  const workerRaw = process.env.AKX_WEB_SNAPSHOT_WORKERS
  if (workerRaw !== undefined && workerRaw !== '') {
    ...
    if (!Number.isSafeInteger(workers) || workers < 2 || String(workers) !== workerRaw) {
      throw new Error(`run-gates: AKX_WEB_SNAPSHOT_WORKERS must be an integer greater than 1, got ${JSON.stringify(workerRaw)}.`)
    }
    return pnpmScript('web-snapshot', 'test:web:ci', {
      label: 'web browser snapshot',
      displayCommand: `AKX_SNAPSHOT=replay AKX_WEB_SNAPSHOT_WORKERS=${workers} pnpm run test:web:ci`,
      env: { AKX_SNAPSHOT: 'replay' },
      ...order,
      streamOutput: true,
    })
  }
  return pnpmScript('web-snapshot', 'test:web:built', {
    label: 'web browser snapshot',
    displayCommand: 'AKX_SNAPSHOT=replay pnpm run test:web:built',
    env: { AKX_SNAPSHOT: 'replay' },
    ...order,
  })
}
...
function lintGate(options: { needs?: string[] } = {}): Gate {
  const raw = process.env.AKX_OXLINT_THREADS
  const script = 'lint:contracts-ready'
  return pnpmScript('lint', script, {
    ...raw === undefined || raw === ''
      ? {}
      : { displayCommand: `AKX_OXLINT_THREADS=${raw} pnpm run ${script}` },
...
//
// AKX_COVERAGE_MAX_WORKERS is the ordinary lane's worker budget, so the two
// parallel gates split it instead of each claiming it whole. When
// AKX_COVERAGE_PARTITIONS is set, its single-worker processes replace the
...
// total of one (the serial reference jobs) also set AKX_GATE_CONCURRENCY=1,
// which keeps the gates from overlapping at all.
// AKX_COVERAGE_TEST_TIMEOUT_MS raises Vitest's per-test, expect.poll, and hook
...
function coverageWorkerArgs(): { instrumented: string[]; exempt: string[] } {
  const [flag] = positiveIntArg('AKX_COVERAGE_MAX_WORKERS', '--maxWorkers')
...
function printResult(result: GateResult): void {
  const verbose = process.env.AKX_GATE_VERBOSE === '1'
