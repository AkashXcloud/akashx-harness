...
function createFixture(attributes = true): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'akx-translation-pairing-merge-'))
  fixtures.push(root)
  ...
  if (attributes) write(root, '.gitattributes', '*.i18n.yaml merge=akx-translation-pairing\n')
  return fixture
}
...
// The value matches AKX_COVERAGE_TEST_TIMEOUT_MS, which the Windows coverage
