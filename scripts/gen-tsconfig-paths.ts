 ...
 * `@akashx/akx-*` listed 49 candidate globs and `@akashx/akx-*\/invariant`
 ...
 * @returns Every directory whose manifest names a `@akashx/akx-` package and that carries `src`.
 */
 ...
 * `@akashx/akx-<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@akashx/akx-` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@akashx\/akx-[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@akashx\/[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys
