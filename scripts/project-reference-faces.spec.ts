...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'akx-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@akashx/akx-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@akashx/akx-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@akashx/akx-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@akashx/akx-client-consumer' })
