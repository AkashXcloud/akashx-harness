 ...
 * itself (`akx web`), so any process that rewrites `lib/client.js` files
 ...
 * `lib/client.js`, and `vite build` rewrites `apps/web/dist`, which `akx web`
...
/** Compile-shell workspace whose dist `akx web` serves. */
const SHELL_PACKAGE = '@akashx/akx-web-frontend'
 ...
 * whose package.json carries `akx.client` with platform "web" is a client
...
export function discoverPluginDirs(root = repoRoot): string[] {
  ...
  for (const manifestPath of globSync('packages/*/*/package.json', { cwd: root }).sort()) {
    const manifest = JSON.parse(readFileSync(join(root, manifestPath), 'utf8')) as {
      akx?: { client?: { platform?: unknown } }
    }
    if (manifest.akx?.client?.platform === 'web') dirs.push(dirname(manifestPath).split(sep).join('/'))
  }
  return dirs
}
 ...
 * client preset without declaring `akx.client` has no loader-delivered browser
...
export function discoverLibraryDirs(root = repoRoot): string[] {
  ...
  for (const configPath of globSync('packages/*/*/tsdown.config.ts', { cwd: root }).sort()) {
    ...
    if (manifest.akx?.client === undefined) dirs.push(dir)
  }
  return dirs
}
...
if (isMain) {
  ...
  for (const name of Object.keys(process.env)) {
    if (name === CLIENT_BUILD_PROFILE_SELECTOR || name.startsWith('AKX_CLIENT_')) {
      Reflect.deleteProperty(process.env, name)
    }
  }
  for (const [name, value] of Object.entries(buildEnvironment)) {
    if (name.startsWith('AKX_CLIENT_') && value !== undefined) process.env[name] = value
  }

  const pluginDirs = discoverPluginDirs()
  const libraryDirs = discoverLibraryDirs()
  if (pluginDirs.length === 0) {
    console.error('dev-web: no akx.client (platform "web") packages found under packages/')
    process.exit(1)
  }
  ...
  console.log(
    `dev-web: watching ${String(pluginDirs.length)} akx.client plugin packages`
