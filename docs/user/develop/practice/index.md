...
- **Service Definition** (`akx-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`akx-bash-local`) — executes commands on the local machine
- **Consumer** (`akx-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  akx-shell   │────▶│  akx-bash-local  │     │ akx-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@akashx/akx-my-cap'
...
```yaml
- name: '@akashx/akx-my-cap-local'
- name: '@akashx/akx-tool-my-cap'
```
