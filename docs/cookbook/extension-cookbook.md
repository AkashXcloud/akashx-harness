...
import type { PreToolDecision, ToolExecution } from '@akashx/akx-tools'
...
```ts
import type { Context } from '@akashx/cordis'
import { expandAssistantStream } from '@akashx/akx-llm'
...
|---|---|
| Hook system (user + project level) | listeners on `agent/session-start`, `agent/pre-step`, `agent/request`, `tools/pre-execute`, `tools/post-execute`, and `agent/turn-stopping`; the waterfalls return typed decisions, while `agent/turn-stopping` may steer another step; the `akx-hooks-claude-code` / `akx-hooks-codex` bridges map hook config files onto these extension points |
| `/goal` | `ctx.goals` owns durable state, `akx-goal-round-driver` schedules same-session rounds through the public `Agent`, and separate command/tool producers expose human/model control |
...
| Context compaction (auto + manual) | the `ctx.compaction` seam + `akx-compaction-basic`; automatic pressure runs on serial `agent/pre-step`, canonical overflow recovery runs on `agent/request-error`, and manual callers use the same compact service ([compaction Agent Note](../../.agents/notes/implemented/feature/2026-06-18-compaction-capability-seam.md)) |
...
| Built-in tools | `ctx.tools.register()`; schemas flow into the assembly automatically — the `akx-tool-*` families (bash, fs, web, subagent, todo) are the shipped examples |
...
| Subprocess sandbox (landlock / sandbox-exec) | use a `ctx.sandbox` backend through `akx-bash-sandbox`; use `tools/pre-execute` for capability-level denial |
...
| Plan mode | [`@akashx/akx-plan-mode`](../../packages/plan/plan-mode/README.md) — logged `plan/mode` state, the `plan:policy` guidance section, `/plan [message]` entry, `/plan off` direct exit, and the user-reviewed `exit_plan_mode` exit; enforcement stays on the independent sandbox/approval axes |
| Sub-agent delegation | the `ctx.subagents` provider registry (`akx-subagent-spawn-in-process`/`akx-subagent-fork-in-process`/`akx-subagent-acp`/`akx-subagent-codex`/`akx-subagent-claude-code`/`akx-subagent-akx-sdk`) + `akx-tool-subagent` exposing one configured provider to the model |
...
| Model adapters | `LlmAdapter` subclass via `registerAdapter` (`akx-llm-akx`, `akx-llm-pi-ai`) |
