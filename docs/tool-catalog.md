...
| --- | --- | --- | --- | --- | --- |
| `@deepseek-ai/dsh-tool-cognate` | `render_chart`, `run_sql` | `ctx.tools`, `ctx.systemPrompt`, `ctx.cognate` | `tool/call`, `tool/result` | - | run_sql uses the Cognate service policy and provider seam; render_chart validates existing tabular data and never executes SQL. |
