...
|---|---|---|
| HTTP field names | Lowercase kebab-case; HTTP matching remains case-insensitive | `user-agent`, `x-akx-harness-session-id` |
| AkashX request-body extension fields | Snake case with the reserved `akx_` prefix | `akx_plugin_packages`, `akx_session_log` |
| AKX-owned nested JSON members | Camel case | `afterSeq`, `throughSeq`, `sessionId` |
| Tagged values | Kebab-case strings; durable events use `domain/action` | `session-log-akx/delivery-accepted` |
...
The [`AkashXLlmApiExtensionRegistry`](../packages/llm/llm-api-extensions/README.md) reserves one provider per top-level extension name. Empty or whitespace-padded names, duplicate registrations, and collisions with the base AkashX request fail before HTTP dispatch.

## Request headers

| Header | Presence | Value |
|---|---|---|
| `user-agent` | Every provider HTTP request, including Files API operations | Application identity in `product/version (+url)` form; the default product is `akx-harness` |
| `x-akx-harness-user-id` | Every authorized chat-completion request | The stable anonymous UUID for the resolved Harness home |
| `x-akx-harness-session-id` | Chat-completion requests carrying a Session id | The exact request `sessionId` string |
| `x-akx-harness-compact` | Chat-completion requests whose purpose is `compaction` | The literal string `1` |

Credential failure happens before anonymous-user-id resolution, so an unauthorized request neither sends these headers nor creates the identity file. A direct request without a Session omits `x-akx-harness-session-id`. Session-title requests have no additional purpose header; the ordinary Session-id rule still applies when one carries a `sessionId`.
...
[`@akashx/akx-plugin-package-inventory`](../packages/llm/plugin-package-inventory/README.md) contributes the complete active Loader-backed plugin package inventory. The field is enabled by default.
...
| Member | Type | Meaning |
|---|---|---|
| `version` | `1` | Schema version for `akx_plugin_packages` |
...
| Member | Type | Meaning |
|---|---|---|
| `version` | `2` | Schema version for `akx_session_log` |
...
The request headers expose the Harness application version, one anonymous Harness-home identity, and an optional Session identity. `akx_plugin_packages` exposes active npm package names and versions. When enabled, `akx_session_log` may expose the Session working directory, system-prompt snapshots, user and Assistant content, embedded Assistant streams, failed-attempt output, tool arguments and results, compaction summaries, feedback, and plugin-owned events. Adapter API keys are not Session events and therefore do not enter the field. A gateway selected through `baseURL` receives the same values as the official endpoint.
