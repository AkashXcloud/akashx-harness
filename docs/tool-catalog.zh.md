...
| --- | --- | --- | --- | --- | --- |
| `@deepseek-ai/dsh-tool-cognate` | `render_chart`、`run_sql` | `ctx.tools`、`ctx.systemPrompt`、`ctx.cognate` | `tool/call`、`tool/result` | - | run_sql 使用 Cognate 服务策略和 Provider seam；render_chart 验证明确提供的表格数据且从不执行 SQL。 |
