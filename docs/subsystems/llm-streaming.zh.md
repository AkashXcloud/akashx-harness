...
[协议参考](../akx-llm-api-wire-extensions.zh.md)定义确切的请求标头、扩展事务、字段版本和接收方义务。随附组合会将 [`akx_session_log`](../../packages/session/session-log-akx/README.zh.md) 注册为无损增量权威日志后缀，并将 [`akx_plugin_packages`](../../packages/llm/plugin-package-inventory/README.zh.md) 注册为完整存活 Loader 包集合。这些字段仍位于模型消息之外，也不会进入 pi-ai 适配器路径。
