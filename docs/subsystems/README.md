...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `AKX_*` environment vocabulary |
...
| [cognate.md](cognate.md) | AkashXDB provider selection, SQL policy, semantic context, normalized results, and deterministic chart metadata |
...
| [client-modules.md](client-modules.md) | the web plugin table: `akx.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
...
| [client-resources.md](client-resources.md) | the client resource model: `akx-resource://<type>/…` addresses, protocol providers and `ResourceProtocolMap`, the `useResource` global hook and its states, pins and release |
