...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`akx-llm-mock-server` `pause()`, `akx-lsp-stdio`, `akx-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
