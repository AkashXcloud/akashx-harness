...
The existing opt-in [session-log-akashx contribution](../../../../packages/session/session-log-akashx/README.md) includes feedback in the ordinary `akx_session_log` suffix on a subsequent eligible request. It uses the existing AkashX destination selection and acceptance watermark. There is no separate `akx_feedback` uploader, feedback-triggered LLM request, or model-input field. The [explicit-feedback OTel decision](2026-09-05-nonofficial-feedback-otel.md) owns the independent feedback-triggered upload for all users and providers.
