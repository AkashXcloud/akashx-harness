...
[Runtime preparation](../../../../apps/desktop/scripts/prepare-akx.ts) materializes the production graph once at build time and ships it through `extraResources/akx`. The Electron shell stays in ASAR. A bundled upstream Node process runs the private Desktop Host from resources and loads enabled plugins from `$AKX_HOME/profiles/desktop`.
