...
- Persistence stays a plugin concern; the in-memory store ships in akx-session.
...
- Derivation cost grows with log length — compaction (akx-compaction) is the intended mitigation, not log mutation.
