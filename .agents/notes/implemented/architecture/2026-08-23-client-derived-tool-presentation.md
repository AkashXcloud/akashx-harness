...
The Client connection stops re-exporting `ToolCallView` and `ToolResultView` from `akx-tools/presentation` for Session consumers.
...
- Session history does not reference `akx-tools/presentation`, `ctx.tools`, `presenterScopeFor`, or `backscanArgs`.
...
Changes to this decision use `akx-pre-push-checks` to select commands for the final diff. Required evidence includes:
...
- `AKX_SNAPSHOT=replay pnpm run test:web`, without refreshing existing presentation goldens;
