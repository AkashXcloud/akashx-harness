...
- 颜色经由 `design-platform.css` 中专用的 `--dsw-alias-link` 别名（亮色 `akashx-500`，暗色 `akashx-400`），与 `state-business-primary` 解耦；链接以 `font-weight: 500` 呈现，默认无下划线，hover/focus 时为 3px offset 的 `underline dotted`。
