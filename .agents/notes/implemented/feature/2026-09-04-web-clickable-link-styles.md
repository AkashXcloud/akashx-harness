...
- Color comes through a dedicated `--dsw-alias-link` alias in `design-platform.css` (light `akashx-500`, dark `akashx-400`), decoupled from `state-business-primary`; links render at `font-weight: 500` with no underline at rest and `underline dotted` at 3px offset on hover/focus.
